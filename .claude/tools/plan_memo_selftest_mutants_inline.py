...
MUTANTS += [
    ...
    ("R17 #2 §6.6: raw HTML is a span of the one inline pass (drop the `<` arm: a tag is text and its brackets "
     ...
     [R17_SPAN, R17_ATTR_ID, INLINE_EXAMPLES,
      ...
      "raw-line rule"]),
    ("R17 #2 §6.6: an attribute value may be single- or double-quoted (drop the quoted arms: unquoted only)", HTML,
     '_ATTR_VALUE = r"(?:[^ \\t\\n\\"\'=<>`]+|\'[^\']*\'|\\"[^\\"]*\\")"',
     '_ATTR_VALUE = r"(?:[^ \\t\\n\\"\'=<>`]+)"',
     [R17_SPAN, R17_ATTR_ID, INLINE_EXAMPLES,
      "(html) a single-quoted attribute value `<span title='[x](absent.md)'>` is a tag: rc 0"]),
    ("R17 #2 §6.6: an HTML comment is a raw span (drop the comment arm)", HTML,
     ...
     ["(html) an HTML comment `<!-- [x](absent.md) -->` is raw: rc 0",
      ...
      INLINE_EXAMPLES]),
    ("R17 #2 §6.6: the 0.31 comment grammar admits `--` inside (re-inject 0.30's exclusion)", HTML,
     '!--.*?-->', '!--(?:(?!--).)*-->',
     ["(html) `<!-- Slice 9z -- owns it -->`: `--` inside a comment is allowed since 0.31 (§6.6: \"a string of "
      "characters not including the string -->\") -- the whole comment is masked",
      INLINE_EXAMPLES]),
    ("R17 #2 §6.6: a processing instruction is a raw span (drop the arm)", HTML,
     '_PI = r"\\?.*?\\?>"', '_PI = r"(?!)"',
    ...
    ("R17 #2 §6.6: a declaration needs an ASCII letter after `<!` (re-inject `<!` + anything)", HTML,
    ...
    ("R17 #2 §6.6: a CDATA section is a raw span (drop the arm)", HTML,
    ...
    ("R17 #2 §6.6: an attribute is preceded by at least one space, tab or line ending (re-inject optional whitespace: "
     "Example 622's `<a href='bar'title=title>` becomes a tag)", HTML,
     '_WS = r"(?:[ \\t]*\\n[ \\t]*|[ \\t]+)"', '_WS = r"(?:[ \\t]*\\n[ \\t]*|[ \\t]*)"',
...
MUTANTS += [
    ...
    ("R21 #1 §6.5: the email arm is an autolink too (drop it: the URI arm alone)", HTML,
     '_AUTOLINK = re.compile("<(?:%s|%s)" % (_URI_AUTOLINK, _EMAIL_AUTOLINK))',
     '_AUTOLINK = re.compile("<(?:%s)" % (_URI_AUTOLINK,))',
     [R21_EMAIL_ID, INLINE_EXAMPLES]),
    ("R21 #1 §6.5: a scheme is 2-32 characters (widen to 1: `<m:abc>` becomes an autolink)", HTML,
    ...
    ("R21 #1 §6.5: a space ends the absolute URI (admit it: `< https://foo.bar >` becomes an autolink)", HTML,
     '[^\\x00-\\x20\\x7f<>]*>"',
     '[^\\x00-\\x1f\\x7f<>]*>"',
