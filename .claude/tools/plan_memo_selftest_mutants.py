...
MUTANTS = [
    ...
    ("link: bare destination balances parentheses", LINKS,
     '        if c == "(":\n            depth += 1', '        if c == "(":\n            break',
     ["(link) bare destination with a balanced parenthesis pair"]),
    ("link: backslash escapes ASCII punctuation (destination, `<dest>`, title)", LINKS,
    ...
    ("link: only ASCII space ends a bare destination, not Unicode whitespace", LINKS,
    ...
    ("link: label content = a character that is not a space, tab, or line ending", LINKS,
    ...
    ("link: `[text]` followed by a link label is not a shortcut", LINKS,
    ...
    ("link: collapsed reference resolves the text as label", LINKS,
    ...
    ("population: the link walk is transitive", POPULATION,
     ...
     '            for f in (memo.linked_files() if len(self.memos) == 1 else []):',
