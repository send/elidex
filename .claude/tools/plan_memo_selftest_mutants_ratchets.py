...
MUTANTS += [
    ...
    ("ratchets: a stale sanction is judged PER MODULE (drop the module from the seen key -- a "
     ...
     [KIND_PARTNER]),
    ("ratchets: a sanction names the MODULE, not only the caller (key it on the caller alone -- "
     "an `a` in another module asks with this `a`'s sanction)", RATCHETS,
     "            if (mod, caller) in sites[name]:",
     "            if caller in {c for _m, c in sites[name]}:",
     [KIND_PARTNER]),
...
CRITERIA = (
    ...
    ("kind: a sanction names the module", "ratchets: a sanction names the MODULE"),
