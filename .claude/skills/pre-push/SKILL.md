...
- **Committed input.** Stages 4 and 5 read the committed `$BASE...HEAD` only — Stage 4 because it is handed a resolved range, `/elidex-review` because it always did. An uncommitted working tree is invisible to both, and invisibly so: the gate runs green over an empty or stale diff. Commit the branch's work **before Stage 4**, whether or not a stage produced edits. (Stage 3 `/simplify` reads the working tree and is unaffected.)
- **Fix → re-verify.** If *any* stage produces code edits (a `/simplify` rewrite, a `/code-review` fix, an accepted review finding, etc.) → re-run Stage 1 (fmt) then Stage 2 (ci), then **commit** per the rule above. Stage 1 is needed because Stage 3 (`/simplify`) is the pipeline's only auto-apply stage and its output may not be formatted; `cargo fmt --check` in Stage 2 ci will reject it otherwise. A fix applied at Stage 4 or 5 has additionally never been read by a correctness net — folding the old Stage 5 into Stage 4 removed a duplicate *skill*, but that invocation also occupied a *slot* after Stage 4's fixes landed. So when a Stage 4/5 fix **changes behaviour** (not formatting or prose) → re-run Stage 4 on the new range; repeat until a pass yields no behavioural edit. Later stages and the eventual push must see green, formatted, committed code. (This covers Stages 3/4/5 — no per-stage repeat below.)
