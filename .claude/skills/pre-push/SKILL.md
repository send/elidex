...
- Pure inert doc / non-code PR — no `**/*.rs` change **and** no change to review/enforcement behavior — → skip Stages 3–6; note the skip in the landing memo. Stages 1–2 still run.
  - A **review/enforcement-tooling edit** (`.claude/skills/**`, `.claude/tools/**` e.g. `webref`, `.claude/hooks/**`) is NOT inert — it alters how the gates behave. Inert *rule prose* → run **Stage 6 `/elidex-review`**; **executable gate code** (`.py` / `.sh` / `webref`, not covered by cargo-only `mise run ci`) → run the **full Stages 3–6**. Single source for this rule: `/elidex-review` "Skip OK".
