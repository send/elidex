...
- **Committed input.** Stage 4 is handed a resolved `$BASE...HEAD` range (below) and `/elidex-review` reads that range and nothing else, so an uncommitted working tree is invisible to both — and invisibly so: the gate runs green over an empty or stale diff. **Commit the branch's work before Stage 4**, whether or not a stage produced edits.
...
⚠ **Folding the old Stage 5 in here dropped a sample, and nothing here replaces it.** `/review` and `/code-review` are one command, so the fold lost no distinct reviewer — but the old Stage 5 also ran *after* Stage 4's fixes, so the diff was sampled twice, and this project has measured byte-identical re-runs producing findings the first pass missed (#515). What should stand in its place — a two-pass floor, where a re-run sits relative to the commit, how the loop terminates, whether `ultra` can take part at all — is a gate design decision of its own and is open: `#11-pre-push-second-sample`.
