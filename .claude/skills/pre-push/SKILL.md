...
### Stage 3 — `/simplify`
Invoke the `simplify` skill **on the resolved `$BASE...HEAD` range** — same resolution as Stage 4 below. With no target it reads the working tree, which the committed-input rule has just emptied; it would report a clean tree and review nothing. Quality-only pass — reuse / simplification / efficiency / altitude cleanups, read from the range and applied to the working tree. It does not hunt bugs (that's Stage 4); the two are complementary, not redundant. `/simplify` is the pipeline's only auto-apply stage (it mutates the working tree directly), so placing it here ensures both the ci re-verify (Stage 2 re-run) and the bug net (Stage 4) validate its output.
...
- ⚠ **Reaching the whole diff** (https://code.claude.com/docs/en/code-review.md):
  ...
  - **`ultra`**: **only the user can launch it.** A model-issued `/code-review ultra` never reaches the cloud pass — it falls back to a local max-effort review, and that fallback carries no target, so it reads `@{upstream}...HEAD` plus the working tree: on a branch pushed once, the unpushed commits, not the PR. Silently, and on exactly the PRs that asked for the deepest pass. So **ask the user to type `/code-review ultra`** themselves (they pass no target — it would read a single word as a base branch or PR number, and its own scope is already the branch against the default branch). If they decline, run `/code-review max <resolved range>` and say that is what ran.
- `/review` is an alias of `/code-review` since Claude Code v2.1.223 (same page), so the former Stage 5 is this stage.

Apply the fixes worth taking, then **run this stage again on the same resolved range**. Two passes are the floor, not a fix-triggered extra. The folded Stage 5 was a second sample of this same reviewer on the same input, and the project has measured byte-identical re-runs producing findings the first pass missed (#515) — folding the stages removed a duplicate *skill*, not a duplicate *sample*. The re-run also reads the fixes just applied, which nothing else in the gate does. Stop at the first pass past the floor that yields no behavioural edit (formatting and prose do not count). If a third pass still yields behavioural edits, stop and surface it: the churn is saying something about the change, not asking for another round.
