...
- `dry_verdict_match`: `Didn't find any major issues` — Codex's no-findings verdict, posted as a **plain PR issue-comment** (`Codex Review: Didn't find any major issues`), **not** a formal review. Keying head-staleness on `pulls/{n}/reviews` alone false-stalls every dry round (`#322`/`#337`). ⚠ **A dry-verdict comment on head is NOT by itself a dry round**: Codex can post a findings item and this verdict seconds apart (`#492` R1 — `## Review finding [IMP]` at `16:28:40Z`, the dry verdict at `16:28:48Z`), so treating the verdict as decisive discards the finding. It marks a dry round **only when no finding item accompanies it on that head**. See `external-{review,converge}/SKILL.md` Step 1 — `assessed_commit` is latest-wins, `dry_verdict` is not. (`memory/feedback_codex-dry-verdict-is-issue-comment.md`.)
