...
- **Enforcement path** — not yet a `preflight.py` gate (it does not parse §2, unlike §3 — a §2-parse hard-gate is the ideal future), but **enforced as a blocking finding**, not advisory: the `axes.md` **Axis 3 `[plan]` detect entry** "edge-dense plan §2 missing coupled-invariant enumeration → **IMP**" gives it a real detection path (the Step 2 agents read `axes.md`). IMP — not MIN — because this is a MUST: a MIN finding is deferable/judgment in this skill, which would let a stated MUST be acknowledged-and-skipped (toothless). This SKILL.md is the **authoring home** (single home for the requirement detail); Axis 3 carries the thin **detect pointer**. It lives in Axis 3 (which already owns the edge-dense *split* decision) as the orthogonal *enumeration* facet — same axis, so no MECE ownership split, and NOT a second edge-dense detector.
