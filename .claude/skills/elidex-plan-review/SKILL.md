...
- **Enforcement path** — not a `preflight.py` gate (it does not parse §2, unlike §3), but **not advisory-only either**: the `axes.md` **Axis 3 `[plan]` detect entry** "edge-dense plan §2 missing coupled-invariant enumeration → MIN" gives it a real detection path (the Step 2 agents read `axes.md`). This SKILL.md is the **authoring home** (single home for the requirement detail); Axis 3 carries the thin **detect pointer**. It lives in Axis 3 (which already owns the edge-dense *split* decision) as the orthogonal *enumeration* facet — same axis, so no MECE ownership split, and NOT a second edge-dense detector.
