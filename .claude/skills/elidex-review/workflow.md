...
4. **Prose-substantive entries (not token swaps) → surface, never auto-apply.** If a swept site needs *prose rewriting* — an AO was removed/renamed, or the claim is no longer spec-accurate — rather than a mechanical §-number / token swap, flag it `prose-substantive`, surface the OLD + proposed NEW prose for review, and do NOT apply it inline (especially when the sweep is delegated to a sub-agent). Layered technical claims get the layering wrong while sounding authoritative — TypedArray element endianness is genuinely subtle (a `DataView` access takes an explicit `littleEndian` argument; a *TypedArray* element access instead defaults to the surrounding agent's `[[LittleEndian]]` — host-defined — per ECMA-262 §25.1.3.16 GetValueFromBuffer / §25.1.3.18 SetValueInBuffer; it is NOT "mandated little-endian"), so a sub-agent's confident one-line rewrite of such a docstring is exactly the prose to surface for review rather than auto-apply (`feedback_r0-subagent-prose-claim-error-mode.md`, PR #231 R6 — whose own citation of this very example was itself imprecise, underscoring the point).
