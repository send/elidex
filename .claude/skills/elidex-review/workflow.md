...
- **2026-05-20** — `elidex-review` Step 1 diff range switched 2-dot → 3-dot (`git diff main...HEAD`) + staleness preflight added.  Triggered by an observer-refactor PR review: a sibling event-handler PR merged to `main` mid-session while the observer branch (forked from the previous `main`) was under review.  Plain 2-dot `main..HEAD` reported the 13 main-only event-handler files as phantom "deleted" files, contaminating the 5-agent run (wasted, had to detect + rebase + re-run).  3-dot uses `merge-base(main, HEAD)..HEAD` = the branch's own changes (matching the GitHub PR diff), immune to main advancing; it stays committed-only so the original working-tree-contamination guard is preserved.  The added `git merge-base --is-ancestor main HEAD` preflight emits a non-blocking "branch is N commits behind main — consider `git rebase main`" warning so review fidelity against current main is a conscious choice. (`elidex-plan-review` Step 1 takes a plan-memo path, not a diff, so it is unaffected.)
