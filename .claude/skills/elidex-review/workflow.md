...
- **/code-review と相補、重複は避ける**: cover axis 異なる (correctness bugs + 一般 PR 観点 vs design/project-context)。本 skill 群は elidex 専門 axis 限定。重複指摘は context-aware な本 skill finding 優先。(`/review` は Claude Code v2.1.223 以降 `/code-review` の alias。)
