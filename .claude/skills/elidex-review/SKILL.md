...
- Step 4 confirmation prompts user by `F<N>` IDs (`「全 Fix decision (F1..F<N>) で進めますか?」` lists the ID range; `「F<X> は accept-as-is」` references a single block).
...
- 「全 Fix decision (F1..F<N>) で進めますか?」 (= apply every block's *Proposed fix*)
