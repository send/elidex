...
[per-finding 表示、agent label + severity + 該当 file:line + summary + suggested fix]
...
⚠️ **MANDATORY before Step 4.**  Findings aggregation has a strong "smallest patch" bias — agents (especially Simplify-tier) surface symptoms with polish-level suggested fixes (`const X = "literal"` / rename / accept-as-is).  CLAUDE.md "ideal over pragmatic" demands structural-level fixes when available.

For each CRIT / IMP / ISSUE-tier finding, record this chain-of-thought BEFORE proposing the fix:
