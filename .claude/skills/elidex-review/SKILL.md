...
- **1000-line file growth awareness**: 本 PR の changed source files の現在行数を実 bash で取って (例: `git diff --name-only --diff-filter=AMR main -- 'crates/**/*.rs' | while IFS= read -r f; do wc -l "$f"; done` — filter は `A`dded / `M`odified / `R`enamed 全部含める (`R` 抜けると rename + content-add の PR が check をスルーする); `while read` ループは BSD/macOS の `xargs` が GNU 専用の `-r` flag を持たない問題 + `xargs` 抜きの stdin redirection `<` だと bash が file-redirect と誤解する問題、両方を回避する portable な形) 本 PR が source file を 1000 行超えに pushed したか OR 既 1000+ file に substantive add (>50 LoC) を入れたか確認。Surface as **MIN finding** (gate NOT) — author 判断で (a) 本 PR 内で split (明確 seam ありかつ split が design 改善になる場合) OR (b) 既存の periodic sweep tranche に委ねる (split shape に file 全体 + 周辺 PR の文脈必要な場合) を選ぶ。1000 行は magical threshold ではなく "focus 霧散の proxy signal"、author の design judgment 優先。Past tranches: MEMORY.md "1000-line file cleanup tranches 1-3" 参照。
...
5 agent 完了後、findings を集約して以下 format で出力。Format は 2 part に分かれ、間に **operative guidance** (Finding ID rule + Step 3 gate warning) を挟む。Operative guidance は code fence の外側に置く — fence 内に書くと literal output template の一部となり、skill 実行毎にメタ指示が user に露出してしまうため。

**Part 1 — Summary table + Findings list (output template):**
...
[per-finding 表示、`F<N>` ID + agent label + severity + 該当 file:line + summary + agent's *raw* suggested fix]
```

#### Finding ID rule (load-bearing for Step 3.5 / Step 4 cross-reference)

ALL findings are numbered sequentially `F1`, `F2`, …, `F<N>` **regardless of severity (CRIT/IMP/MIN/FP) and regardless of source agent** — single monotonic counter across the aggregated set, assigned in the printed Findings list order (severity-sorted as above).  The ID stays stable for the rest of the workflow:
- Step 3.5 emits a `Fix decision F<N>` block ONLY for fix-tier findings (`CRIT`/`IMP`/`MIN`).  Skipped IDs (`FP`) leave gaps — `F2` may go straight to `F4` if `F3` is `FP`, that's expected and signals "F3 ignored, see Findings line for reason".
- Step 3.5 subsumption references use the same IDs (`Subsumption check: F1 + F5`) so chains are unambiguous.
- Step 4 confirmation prompts user by **the concrete fix-tier ID list** (NOT a `F1..F<N>` range expression — that would imply contiguity and contradict the gap rule above when an `FP` precedes a fix-tier finding).  Example: with F1=IMP, F2=FP, F3=MIN the prompt says `「全 Fix decision (F1, F3) で進めますか?」`, not `「F1..F3」`.  `「F<X> は accept-as-is」` references a single block by ID.
...
**Part 2 — Recommendation + Pre-push gate (output template, continued):**
...
For each **fix-tier** finding — `CRIT`, `IMP`, and `MIN` (i.e. every severity except `FP`, which is already excluded from fix consideration) — produce a **structured decision record** (not hidden internal reasoning; the record IS the user-visible output below) using these four prompts as a checklist:
...
#### Step 3.5 output — MANDATORY user-visible block (one per fix-tier finding)

Emit this block into the response stream before Step 4's confirmation prompt.  Skipping it (e.g. keeping the decision as internal-only notes) is a workflow violation — Step 4 explicitly references the `Fix decision F<N>` IDs and cannot run without them.
...
- **Root-level alternative**: <structural fix, or "none — agent suggestion already structural">
- **Subsumption check**: <other F<M> this fix also resolves, or "none">
- **Polish-domination smell**: <triggered? — if yes, the alternative above is the proposed fix>
- **Proposed fix**: <chosen fix, one sentence>
```

The aggregate of these `Fix decision F<N>` blocks IS the philosophy-aligned fix proposal that Step 4 confirms with the user — agents reading this skill should not silently accept the smallest-patch agent suggestion at Step 3; that bias is exactly the workflow gate this section exists to enforce.
...
**Zero fix-tier findings short-circuit**: Step 3 が CRIT/IMP/MIN 全部 0 (= FP only か no findings at all) の clean review なら Step 3.5 block は emit 不要 (decisions to record が無い) — そのまま Step 4 へ進み、user に "0 fix-tier findings; FP <N> 件は ignore、push 可" を確認するだけで loop 終了。下記の Step 3.5 block referencing は ≥1 fix-tier finding がある場合の gate。

≥1 fix-tier finding がある場合、Step 3.5 で emit した **Fix decision F<N>** block 群を referenced して user に:
- 「全 Fix decision (concrete fix-tier ID list — 例: F1, F3, F4 when F2 is FP) で進めますか?」 (= apply every block's *Proposed fix*)
- 「特定 Fix decision F<X> は accept-as-is (= FP / 判断保留) でいいですか?」 (= override individual block)
- 「push にどう影響しますか? (CRIT 残してでも push したい場合 escalation)」

を確認。Auto-fix はしない (user 判断 driven)。≥1 fix-tier finding があるのに Step 3.5 block 無しで Step 4 へ入ったら gate を満たしていない — Step 3.5 へ戻る。
