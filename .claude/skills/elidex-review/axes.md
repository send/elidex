...
- `[plan]` **edge-dense plan-memo の §2 coupled-invariant enumeration 欠落** (`feedback_coupled-invariant-design-corner.md`、D-19): plan が edge-dense (≥3 intersecting invariant axes — 直上の entry で判定済) なのに §2 が「設計が同時に満たす invariant + 各 pair の交点」を bullet/matrix で列挙せず prose 一筆書き ("既存 seam 再利用") のまま → **MIN**。authoring 要件の home + 詳細は `elidex-plan-review/SKILL.md` Pre-condition #3 (single home); 本 entry はその **detect pointer** (= enforcement path、agent が axes.md を読むので surface される)。直上 entry が edge-dense の *split* 判定を所有するのに対し本 entry は同 axis 内の *enumeration* facet (ownership split なし、second edge-dense detector ではない)
