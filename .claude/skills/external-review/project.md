...
- `trigger`: `@codex review`, posted as a PR comment. **Never wait on Codex's auto-review.** The trigger condition, the clock origin and the rationale live in ONE place — `external-converge/project.md` § "Never wait on auto-review" (the canonical site) — and apply here unchanged. Do not restate them here: an independently-worded copy is what let this file drift into an *unconditional* trigger while the canonical rule was per-head, so a single-pass re-run on an already-triggered head requested a redundant review. Single-pass differs in exactly one respect: it fires the trigger and **stops** — no window loop, no self-scheduling; the caller re-runs the skill once the review lands.
...
- `dry_verdict_match`: `Didn't find any major issues` — Codex's no-findings verdict, posted as a **plain PR issue-comment**, not a formal review. Keying the head check on `pulls/{n}/reviews` alone false-reports "stale review" (`#322`/`#337`). ⚠ **A dry-verdict comment on head is NOT by itself a clean pass**: Codex can post a findings item and this verdict seconds apart (`#492` R1 — `## Review finding [IMP]` at `16:28:40Z`, the dry verdict at `16:28:48Z`), so treating the verdict as decisive discards the finding. It marks a clean pass **only when no finding item accompanies it on that head**. See SKILL.md Step 1 — `assessed_commit` is latest-wins, `dry_verdict` is not. (`memory/feedback_codex-dry-verdict-is-issue-comment.md`.)
