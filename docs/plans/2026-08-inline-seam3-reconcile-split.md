...
⚠ **A file may appear on both sides; a *clause* may not.** ⚠ The command below enumerates only
**file-keyed** rows, and one row is keyed on a value instead — so its output is a *subset*. On the
file-keyed rows it returns `MEMORY.md`, `project_line-box-decorated-inline-content.md` and
`project_open-defer-slots.md`; `active-lane-detail.md` is also on both sides, via the value-keyed
`508` row, and the command cannot see it:
...
filename. The `508` row is keyed on a value because its obligation spans four files; that is the
one row the command misses, and it is named above rather than left to be discovered.
