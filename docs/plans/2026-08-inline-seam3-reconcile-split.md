...
| …so the allowlist is unchanged | `grep -n 'inline/mod.rs' .claude/tools/layout-box-reader-allowlist.tsv` | one row (TSV `:73`), classified `type-def`, whose content line is `inline/mod.rs:29` — above the range |
...
* **Seams 1 and 2** of `#11-inline-fragmented-fn-decomposition` (the slot's entry locates them). The
  umbrella books them into the successor slot `#11-inline-fragmented-fn-seams-1-2`.
...
* **The eleven-parameter signature.** Reducing it is a design change (§5.3) and belongs with the
  ...
  ⚠ **Why a sink type cannot be introduced here.** Byte-identity governs the body after `) {`; the
  signature is **authored**, so that contract does not govern it. The obstruction is one level
  in: the two sink flags (`persist_flow`, `do_carrier`) are **consumed by the arms** `if persist_flow { … } else if do_carrier { … }`,
