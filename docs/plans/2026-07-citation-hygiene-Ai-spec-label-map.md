...
3. **K2 — stated as two claims, because only one of them is mechanised.** A-i discharges the two
   ...
   - **Closed part, absolute**: `bash .claude/tools/webref-generic-core-trip-wire.sh` → PASSED. It FAILS
     ...
     (green; K2 firing under `skills`, under `tools`, on the path A-i removed, and inside binary content;
     an empty scope, an unreadable file and an unsearchable directory all failing closed). The subject is
     ...
     rounds R70-R72 kept finding: **a green over something the wire did not read is a transcript too.**
