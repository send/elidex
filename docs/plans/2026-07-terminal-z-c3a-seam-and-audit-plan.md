...
reductions **on** these folds (§4 seeds).
...
|---|---|---|
| RO `contentRect` | border-box-local (origin = padding top/left, RO §3.3.1) | `BoxModel::content_rect_local()` |
...
|---|---|---|---|---|---|
| RESIZE OBSERVER §3.3.1 content rect | — | `top`=padding top / `left`=padding left / `w,h`=content size | `content_rect_local()` **relocation** (`LayoutBox` inherent → `BoxModel` default) — **arithmetic unchanged, no behavior change** | ✓ (the SVG-no-box sub-branch and the multicol width note are RO's reader policy, unchanged by a method move) | no |
...
- **trait-erased `&dyn BoxModel`** consumers (e.g. `render/builder/paint/mod.rs`) that read `LayoutBox` data
...
1. **RO** — frame border-box-local (axis 1), spec-zero (axis 3). Open: which fragment (RO §3.3.1 pins *width*
