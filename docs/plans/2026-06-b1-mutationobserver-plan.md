...
| DOM §4.3 "queue a mutation observer microtask" (`#queue-a-mutation-observer-compound-microtask`) | flag + schedule | full | new VM `queue_mutation_record` wrapper (production side) | ✓ | no |
...
5. **`Vm::deliver_mutation_records` (the embedder API conflating notify+callback) is retired for the VM;
   the MO tests are rewritten to drive real JS mutations and pump the microtask (§6, §7-AC).** This closes
   the B0-named **test-invisible gap** (current tests hand-build `SessionRecord`s and call
   `deliver_mutation_records` directly — none asserts "a JS mutation yields a record"). The notify half
   becomes `queue_mutation_record`; the callback half moves into the microtask drain.
...
### 2.2 Production side — `queue_mutation_record`

New VM-side wrapper (replaces the notify-half of `deliver_mutation_records`):
...
2. **`ConsumerDispatcher` fan-out preservation** ✓ — writes go through `EcsDom::set_attribute` /
   tree primitives unchanged; the 7 consumers (incl. CE) still fire. B1 adds **no** dispatcher consumer and
...
**no** `MutationRecord` (doc-comment `child_node/mutations.rs:6` + `document.rs:251` cite the unbuilt
`#11-tree-mutation-record-pipeline` — **which B1 implements for this family**). B1:
- The handler **captures the sibling context before the write** and, after applying through the chokepoint,
  **returns** a §4.3.2 "queue a tree mutation record" childList record (`addedNodes`/`removedNodes` +
...
- `replaceChild` = one Remove + one Insert at the same parent → **two** records (§4.2.3 replace emits a
  removal record then an insertion record; it is *not* a coalesced single record — only replace-*all* coalesces).
  B1 derives this from §4.2.3 at implementation; flagged here as the one non-1:1 case in B1's surface.
- The bridge return channel carries the record(s) to the host caller, which calls `queue_mutation_record`.
  `vm/host/` stays marshalling-only (mirrors the innerHTML pattern).
...
- **Transient registered observers** (§4.3 step 6.3 / `#transient-registered-observer`, for subtree tracking
  of removed nodes) — the current registry models only persistent `MutationObservedBy`. B1 does not add
  transient observers; named for B1.2 (it is coupled to the removal record path). Flagged, not silently dropped.
...
| **B1.2** | direct tree ops (ParentNode/ChildNode/insertAdjacent/select) + **replace-all loop-coalescing** (replaceChildren / textContent-on-Element) + Range mutations + **move-record** semantics + transient registered observers | §4.2.3 suppressObservers coalescing; move/shadow; live-range (Range) | after B1 |
...
1. **`crates/api/elidex-api-observers/src/mutation.rs`** — `notify` returns whether it enqueued (count or
   bool), so the VM can decide to schedule the microtask. No behavior change to the gating logic.
2. **`crates/script/elidex-js/src/vm/host/mutation_observer.rs`** — **split** `deliver_mutation_records`:
   keep `notify_one`; extract the callback-invocation block (`:442-469`) into a reusable
   `deliver_pending_mutation_records(vm)` the microtask calls. Add `VmInner::queue_mutation_record(record)` =
   `notify_one` + (if enqueued) set flag + push `Microtask::NotifyMutationObservers`.
3. **`crates/script/elidex-js/src/vm/vm_api.rs`** — public `Vm::queue_mutation_record` (host-facing, marshalling
   forward). **Retire** `Vm::deliver_mutation_records` for the VM (no production caller remains after step 5;
   tests rewritten). Confirm the field `mutation_observer_microtask_queued` (used by slotchange today) is the
   right flag to reuse.
4. **`crates/script/elidex-js/src/vm/natives_promise.rs`** — extend the `NotifyMutationObservers` arm (`:333`)
   with §4.3 steps 2–6 (call `deliver_pending_mutation_records`) **before** the slotchange step, preserving the
   clear-flag-first re-entrancy discipline.
5. **`crates/script/elidex-js/src/vm/host/dom_inner_html.rs`** — `:148`/`:362`: `deliver_mutation_records(&[rec])`
   → `queue_mutation_record(rec)`.
6. **`crates/script/elidex-js/src/vm/host/node_proto.rs`** (+ the `elidex-dom-api` child-node handler it bridges
   to) — single-node childList: capture siblings before the write, return a §4.3.2 tree mutation record from the
   handler, host forwards to `queue_mutation_record`. Reuse `apply_append_child`/`apply_insert_before` record
   shapes. `replaceChild` → two records (§3.2).
7. **Tests** — rewrite `crates/script/elidex-js/src/vm/tests/tests_mutation_observer/*` to **drive real JS
   mutations** (`appendChild`/`removeChild`/`insertBefore`/`replaceChild`/`innerHTML`) + **pump the microtask**
   and assert the delivered records — closing the test-invisible gap. Keep one synthetic-record unit test for
   the registry gating (or move it to `elidex-api-observers`).
...
1. **A JS mutation yields the correct record on the microtask checkpoint** (the B0 test-invisible gap closed):
   ...
   - `insertBefore` / `removeChild` analogues; `replaceChild` → two records (§3.2).
...
3. **No double-delivery / no regression** — innerHTML/outerHTML no longer self-deliver synchronously
   (`deliver_mutation_records` VM API gone); slotchange still fires from the same microtask; CE reactions
   unaffected (separate driver). Existing VM/slotchange suites pass.
...
- **A1 (PR #376, `webapi-compat-a1`)** edits `vm/init.rs` / `vm/mod.rs` / `vm/globals.rs` / `window.rs` /
  `engine.rs` / `elidex-dom-api/registry.rs`. **B1 touches none of those** (B1 = `mutation_observer.rs` /
  `vm_api.rs` / `natives_promise.rs` / `dom_inner_html.rs` / `node_proto.rs` / `elidex-api-observers`). The one
  shared file is `vm_api.rs` (A1 ctor region vs B1's `queue_mutation_record` near `:867`) — distinct regions,
  LOW risk. **Re-confirm at PR-open** (A1 may have merged).
- **media Slice3 (`elidex-mq-slice3`)** is CSS-cascade / shell-producer — no overlap.
- **`#11-tree-mutation-record-pipeline`** is implemented (for the single-node family) by B1 — update its
  references (`document.rs:251`, `content_tests.rs:464`) to reflect partial landing + the B1.2 remainder.
...
## §9. Open questions (for the 5-agent gate to falsify)

1. **Mechanism (§2) — production-returns-record + `queue_mutation_record` + microtask.** Is the
   "write-at-chokepoint / record-at-seam / deliver-at-microtask" split the faithful §4.1 resolution, or does a
   reviewer find a fourth structure that dominates? (Lens says this is the ideal; §2.5/§2.6 reject both poles.)
2. **Delivery locus — reuse `Microtask::NotifyMutationObservers`.** Confirm extending the existing arm (vs a
   new microtask kind) is correct — the spec puts MO callbacks + slotchange in one "notify mutation observers"
   algorithm, so reuse is spec-faithful, not a shortcut.
3. **Retiring `Vm::deliver_mutation_records`.** Is there any non-test, non-boa VM caller that must keep it?
   (Grep says no; the shell drives the **boa** runtime's version, not the VM's.) If a caller exists, keep it as
   a thin `notify-all + schedule` shim rather than the conflated notify+deliver.
4. **`replaceChild` = two records.** Confirm §4.2.3 emits a removal record then an insertion record (not a
   coalesced single) for `replaceChild` — derive at impl from the spec, not assumed here.
5. **Slice boundary.** Is "single-node childList + innerHTML/outerHTML" a coherent terminal slice, or should
   B1 absorb the direct tree ops (B1.2) to avoid an `appendChild`-observed / `el.append()`-silent asymmetry?
   (Lens: keep B1 narrow + mechanism-pure; the asymmetry is logged, not hidden — but the gate may prefer the
   full childList family in one PR. Flagged as the main scoping question.)
6. **`notify` return-for-scheduling.** Is threading "did-enqueue" out of the engine-independent registry the
   cleanest way to gate microtask scheduling, or should the VM check `has_pending_records()` delta instead?
   (Either keeps scheduling VM-side; pick the one with no extra walk.)
...
| registered observer / registered observer list | DOM §4.3 (§4.3.1) | `#registered-observer` |
