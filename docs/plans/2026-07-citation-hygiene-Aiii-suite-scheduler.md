...
no longer imports it. A-ii's suite stubs `preflight._resolve_citation` at suite level (A-ii §6), and after A-ii §4.2.6 `preflight` has no child to spawn.
→ `rederive suites`
...
| **Q7** | the same `tools` job region (Q6's reader) carries a `python-version:` matrix with a `"3.9"` leg **and a `uses: actions/setup-python` step whose `python-version:` input is `${{ matrix.python-version }}`, placed in `steps` *before* the `run:` step Q6 identifies** — a matrix no step consumes runs the runner's Python on every leg and measures nothing (Codex R48), and a `setup-python` that follows the driver step has the same effect: the suites already ran under the runner's interpreter (Codex R49); §12(6)'s instrument; a floor the job does not run is declared, not measured | — | **yes** — the job does not exist |
...
| **Q8** | the `tools` job is **present and ungated**: its region (Q6's reader) is non-empty and carries **no `needs:` line and no `if:` line at all** — the predicate the harness already states for `trip-wires` (`-Aiii.sh`, the "present AND ungated" block: `[ -n "$r" ] && ! grep -qE '^    [\"']?(needs|if)[\"']?:'`), restated in the test, which cannot source the harness; it is deliberately *not* `_gated`'s complement (`_gated` enumerates the approved `rust|config` outputs and admits `|| always()`, so its negation would read a job gated on a *new* `.claude` output as ungated — the very case this pin exists for). **Negative control**: the same predicate reads `check` (`ci.yml:51-52`: `needs: changes` + `if: needs.changes.outputs.rust == 'true' || github.event_name == 'push'`) as gated, or the pin proves nothing. A job someone later gates on a new `.claude` filter output would pass Q1–Q7 and skip the suites on every `crates/**`-only and Dependabot PR (Codex R49) | 2 | **yes** — the job does not exist |
...
**(3) The job fires on this slice's own file class — and on every other**: Q8 green (no `needs`/gating `if` in the region, the `check` job read as gated by the same predicate), and a `.claude/**`-only commit shows the `tools` job on
