...
- **Shared prerequisite of Slices 2 and 3** (an ordering fact, so it is stated once here rather than on
  ...
  **instance field → whichever of 2aa and 3 lands second**, **static field → whichever of 2ac and 3**,
  **static block → whichever of 2b and 3**. That slice integrates the home object *and* runs this
  bullet's regression for its own producer — `class B extends A { x = super.x }` for 2aa,
  `class B extends A { static x = super.x }` for 2ac, `class B extends A { static { this.y = super.x } }`
  for 2b.
...
for the reason its row states. Inside Slice 4 there is no chain: **4a** — the whole tag path,
GetTemplateObject and the lowering together — depends on **1b** (I-3 requires reusing the arg helper),
and **4c** is independent of it. Slice 4's own Deps cell is `—`. D runs after the connecting slices so it deletes only what
