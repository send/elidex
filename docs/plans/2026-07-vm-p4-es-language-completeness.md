...
coupled-invariant enumeration in its own memo. 3 stays terminal and carries its own; ⚠ **5 does not — it is an umbrella whose children are minted at its own start (its row), and the verdict that stood here would have let identity, fields and methods land as one PR without that derivation or a per-child plan-review**.
