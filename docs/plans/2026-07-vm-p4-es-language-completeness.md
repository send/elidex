...
for **3**, by the child that owns the representation rather than the umbrella. ⚠ **And a forced-collection regression asserts that the collection was *placed*, not
...
- **Umbrella row** — ships **no implementation PR**. It carries **(1) the split** — its own boundary
  ...
  **never lands**; it *retires* once every child it minted has landed, and a partial landing is recorded
  ...
  rows too** — the split is **27** §5 rows and **8** §8 slots. The narrower pipeline was what this rule
...
`vm/gc/collect.rs` 2074 (Slice 7b), `vm/gc/trace.rs` 1255 (**Slices 7a and 7b** — 7a's unconditional
...
claim it carried is false in the 1a half: dec. 13a gives the child of umbrella 1a that removes
...
`vm/dispatch_iter.rs:295-325` implements it as a drain-into-array, so 0bc repairs and consumes that
...
emits `Op::IteratorRest` for an array rest element and `vm/dispatch_iter.rs:295-325` implements it as
...
untouched, which is the opposite of what this paragraph exists to prevent. §7.2 pins the drain and the no-`return()` case together (§-numbers and
...
**Decision** (§9 decision 13, restated): the drain fix belongs to **the child of umbrella 1a that removes `op_array_spread`'s `return()`** (it is the reuse precondition), placed there by umbrella 1a's derivation, which mints it,
...
13. **§6.2a — the reused drain's `IteratorClose` divergence — RESOLVED as two units.** ⚠ **Round-8
    sequencing hazard**: 0bc owns [C39], whose rest form
    (`[a, ...rest] = it`) needs a drain-into-array and whose spec *requires* `IteratorClose` —
    while the child of umbrella 1a that removes `op_array_spread`'s `return()` **deletes**
    its `return()`. ⚠ **This clause read "`op_array_spread`/`spread_iter_loop` is the only in-tree
    ...
    `vm/dispatch_iter.rs:295-325` implements it as a drain-into-array. So 0bc **repairs and consumes
    ...
    `ArraySpread` is factored out rather than duplicated (*One issue, one way*). So either 0bc routes through it (and that child silently falsifies 0bc's [C39] claim) or 0bc
    hand-rolls a second drain (the I-3 failure). ⚠ *This clause opened "§5 lands 0bc before 1a"; the
    ...
    **[C19]/[C22]-only**, have 0bc **repair and consume `Op::IteratorRest`** — the rest drain that
    already exists (`compiler/stmt_destructure.rs:70`, `vm/dispatch_iter.rs:295-325`) — **with no
