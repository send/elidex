...
it). Each needs a cohesion verdict in its own
...
7. **`Function`/`eval` gate.** I-6 does *not* supply it (strict `eval` is core). The real question
   is a security/CSP policy decision — ⚠ **and it is one of two questions, not the whole of it**: the
