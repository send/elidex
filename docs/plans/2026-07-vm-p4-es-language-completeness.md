...
→ **10** rows (negative control, same pipeline over `GcRootsXX|mark_rootsXX` → **0**): §5 rows **3**,
**4a**, **C**, **S**, **7**, **7a**, **7b**, and §8 slot rows `#11-vm-arrow-lexical-new-target`,
...
- **Shared prerequisite of Slices 2b and M** (an ordering fact, so it is stated once here rather than on
  ...
  **§15.7.1** Static Semantics: Early Errors, production
  ...
  cite one. The fix is the parser early error, and ⚠ **"whichever comes first of 2b and the terminal child of umbrella M" is withdrawn: it orders nobody, which is the defect row K was minted to fix, and it is the only scheduling instruction this artifact had — 2b's `Deps` names only K, and umbrella M has no child to name yet.** So the artifact takes a **terminal prerequisite row of its own**, exactly as the class-name-ordering artifact became row **K** once the same wording was measured to order nobody: it lands before either consumer, both carry a `Deps` edge to it, and neither can retire while `class C { static { await 0 } }` is still accepted in module mode —
...
- **[1a]** = VM infrastructure, and ⚠ **1a is an UMBRELLA from this revision (§5), so this tag names a
  ...
  opcode is added and no emit path changes, so **the 1a half is observably a
  no-op for every call shape** — its only live consumer is `op_super_call_spread`, which keeps its
  current semantics. That is what makes each child a legal standalone PR under I-4.
