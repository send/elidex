...
no collection in between. ⚠ **The instances are enumerated by command, not curated — the list this clause first carried was
inspected, and the enumeration returns two rows it had dropped.**
...
`#11-vm-operand-rooting-by-construction`, `#11-vm-typed-array-family-layering-and-gate`. The two the
curated list omitted are the two that show why a list is the wrong shape: **4a**, whose cell already
owed a root field and a forced-collection regression and wrote that regression *without* the
sole-root half; and **C**, which had to write "so I-7 binds this row too" into its own cell — the
restatement this rule's last sentence forbids. Each row owes whatever the enumeration places on it,
re-run at that row's own parent HEAD; for **S**, whichever answer its plan-review gives about the
environment link being object-valued; for **3**, the child that owns the representation rather than
the umbrella. ⚠ **And a forced-collection regression asserts that the collection was *placed*, not
