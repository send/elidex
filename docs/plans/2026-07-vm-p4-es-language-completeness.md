...
each answered one completeness question and reported the answer as the column's. There are three
questions, not one:
...
- **(c) obligations from static semantics, early errors and parse context, and edges whose artifact
  is not a spec construct at all.** Reachable by neither, and **not claimed here**. These are derived
  by each slice's own mandatory `/elidex-plan-review` against that slice's parent HEAD — where this
  document already puts every claim it cannot bound.
...
- **Shared prerequisite of the 2a family** (an ordering fact, so it is stated once here rather than on
  ...
  **and accepts** the computed-name forms
  ...
  So the divergence is measured and the governing rule is not — **2a's mandatory plan-review resolves
  it**, and until it does, nothing here asserts one. Writing a replacement citation from recall is how
...
16. **§7.2's conformance-row outcome type — RESOLVED (crash-aware outcome), owner Slice 0cb.** The
    ...
    **1b**'s, §5 gives 1b `Deps: 1a`, and §15's ship order runs `… → 0b → 0c → 1a → 1b`, so hoisting
    ...
    edge**, and §5's own scope statement (c) already says the column does not hold every ordering
    fact. Was an unowned either/or in §7.2; ratified here so 0cb's author
