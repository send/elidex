...
- **Umbrella row** — ships **no implementation PR**. It carries **(1) the split** — its own boundary
  ...
  **never lands**; it *retires* once every child it minted has landed, and a partial landing is recorded
  ...
  both). The population is read from each row's **declaration** — all 62 §5 rows and all 47 §8 slot
  rows — and the count above is that reading's output rather than a grep's. The earlier control agreed
