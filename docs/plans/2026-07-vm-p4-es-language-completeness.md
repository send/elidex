...
| [C13] | ECMA-262 §13.3.7.1 Runtime Semantics: Evaluation (`super`) | `#sec-super-keyword-runtime-semantics-evaluation` | Slice 1a handler + 1b emit, the child of umbrella 3 that owns the super-property emit shape |
...
`eval`'s caller-scope injection is LegacySemantics. What a security/CSP policy decision (§9 decision 7)
settles is whether `eval` is *permitted* — the host-defined **§19.2.1.2** HostEnsureCanCompileStrings
hook the algorithms reach — and not whether the strict-mode algorithm is implemented; §5's
`Function`/`eval` pointer row carries that separation and the home of each half.
...
- **Umbrella row** — ships **no implementation PR**. It carries **(1) the split** — its own boundary
  ...
  **never lands**; it *retires* once every child it minted has landed, and a partial landing is recorded
  ...
  rows too** — the split is **26** §5 rows and **8** §8 slots. The narrower pipeline was what this rule
...
16. **§7.2's conformance-row outcome type — RESOLVED (crash-aware outcome), owner Slice 0cb.** The
    ...
    **1b**'s and §5 gives 1b `Deps: 1a`, so hoisting
    ...
    needed here. **1a landing first breaks nothing**: its acceptance is stated over its own artifacts —
    the existing suite plus edges 21/27/28/32 — and turns true whether or not the conformance table
