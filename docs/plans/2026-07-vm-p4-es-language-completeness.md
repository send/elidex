...
| **10b** | **`Proxy` exotic object** (ECMA-262 §10.5) — the trap table and the invariant checks each internal method must enforce. **Does not depend on 7b**, though the Deps cell named it until PR-B: the two rows share `vm/object_kind.rs`, and a shared file is not an ordering fact. 7b's deliverable is weak-key GC tracing; 10b holds a Proxy's target and handler strongly and consumes nothing 7b produces | `vm/object_kind.rs`, `vm/ops_property.rs` | `#11-vm-proxy-reflect` (10b) | T3 | **10a** |
...
`2aa`). 5 depends on **2ab** (`#x = 1` reuses the field-initializer mechanism,
...
returning zero for everything). `#11-vm-computed-compound-assignment`
