...
the row that closes an iterator on abrupt *per-item* resolution (**9d**) sat outside it with an empty
Deps cell while 7a / 9a / 9c carried the edge. `grep -c 'Depends on Pa'` over §5 is the derivation;
