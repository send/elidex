...
⚠ **Every derivation this document states is an ENTRY POINT plus a CLOSURE RULE — never a list of
clauses to walk.** A clause list is a hand-curated enumeration, which §13's *enumerate by command, never
by recall* forbids and which §5's demotion says cannot bound the surface it is written over; the two
rules already in force apply to a derivation's *input* exactly as they apply to a row's obligations.
Written as a list it fails in one direction only, silently: an output reachable from the surface but
defined in an unlisted clause is minted by nobody, and every child retires having followed the stated
...
RegExp constructor surface, **§20.4.2**/**§6.1.5.1** from `Symbol.dispose`. **So a derivation states its SEED and nothing more, and the closure machinery that stood here is
withdrawn.** Three formulations were written in three review rounds — a clause list, a prose
