...
no collection in between. Measured instances, each owed by the row named: **7a** (its cell's
`[[MapData]]`/`[[SetData]]` withdrawal — the same test applied to a new `ObjectKind` instead of to a
representation move), **the child of umbrella 3 that owns the `[[HomeObject]]` representation on the
closure**, **`#11-vm-arrow-lexical-new-target`** (§8 — lexical `new.target` on `FunctionObject`; Slice N is a pointer row, so the slot is the home), and **S** if the environment link it
carries is object-valued — the rule binds whichever answer that row's plan-review gives. No row
restates this rule; a row states only what it owes under it.
...
false — and **I-7 adds three more readers of that same file, with a fourth conditional**: the child of
umbrella 3 that carries the `[[HomeObject]]` representation, the slot
`#11-vm-arrow-lexical-new-target` (**not** Slice N — N is a pointer row, so the slot is the home), and
Slice **C**, whose row states that whichever representation its plan-review settles on forces a
marking site; **S** joins them if the environment link it carries turns out to be object-valued, which
is that row's plan-review to settle. The list is read off I-7's own instance enumeration rather than
restated, which is how it went stale once already — it named N after the obligation had moved to the
slot). Each needs a cohesion verdict in its own
...
| `#11-vm-async-generators` | needs a new async-iterator opcode + `compiler/stmt.rs` await-flag plumbing (the `is_await: _` discard — `:83` at `658cc302` and at HEAD, `git grep -n 'is_await: _'`; the `:103` this row used to carry predates 0a's `stmt.rs` split and was never re-derived) = own slice | now (Slices 6b/6c, and umbrella 6a's children as that derivation mints them) | 2026-10-31 |
