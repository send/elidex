...
found the class, not as the boundary. Which sub-slice each name falls to follows from the per-family
charters in §5.
...
`#11-vm-labelled-statement-break-target`, `#11-vm-per-iteration-loop-environment` and
`#11-vm-for-in-enumeration-semantics` (`grep -c -- '<slug>' <ledger>` → **0** for each of the last
three, run 2026-08-09). `#11-vm-computed-compound-assignment`
