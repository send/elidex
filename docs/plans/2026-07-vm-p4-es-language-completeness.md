...
and each acceptance condition the slot states is placed on the specific child that consumes that
...
- **Umbrella row** — ships **no implementation PR**. It carries **(1) the split** — its own boundary
  ...
  **never lands**; it *retires* once every child it minted has landed, and a partial landing is recorded
  ...
  out of each row's prose: `grep '^| ' <this file> | grep -Fc 'UMBRELLA, not a terminal unit'` → **32**,
  whose control is the same pipeline over the bare string `UMBRELLA`. The two counts must agree; they
  ...
  rows too** — the split is **26** §5 rows and **6** §8 slots. The narrower pipeline was what this rule
