...
- **Umbrella row** — ships **no implementation PR**. It carries **(1) the split** — its own boundary
  ...
  **never lands**; it *retires* once every child it minted has landed, and a partial landing is recorded
  ...
  out of each row's prose: `grep '^| ' <this file> | grep -Fc 'UMBRELLA, not a terminal unit'`, whose
  control is the same pipeline over the bare string `UMBRELLA`. The two counts must agree; they did not,
  because two rows spelled the kind in words alone and every reader deriving the kind from the marker
  read them as something else. ⚠ The pipeline is **row-scoped** because this passage states the literal
  itself; an unscoped count over the whole file is off by the number of times the rule is written down.
