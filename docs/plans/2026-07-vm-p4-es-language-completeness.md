...
mints carries its own coupled-invariant enumeration in its own memo, and each umbrella's children are
enumerated in their own. ⚠ **Which rows are terminal and which are umbrellas is not restated here** —
...
- **Shared prerequisite of Slices 2 and 3** (an ordering fact, so it is stated once here rather than on
  ...
  *and* runs this bullet's regression for its own producer — `class B extends A { x = super.x }` for 2aa,
  a **side-effect** form `class B extends A { #x = (seen = super.x, 1) }` for 5 (a private *read* is 5's
...
- **Shared prerequisite of Slices 2b and M** (an ordering fact, so it is stated once here rather than on
  ...
  **§15.7.1** Static Semantics: Early Errors, production
  ...
  cite one. The fix is the parser early error, and it must land with whichever comes first of **2b** and **the terminal child of umbrella M that enables `parse_module`** — ⚠ **naming "M" does not identify an owner**, for the reason the Slice-5 handoffs above already carry: M is an umbrella and ships no PR, so that child landing while `class C { static { await 0 } }` is still accepted in module mode would leave every named party able to retire. Assigning this obligation to that child is a **mandatory output of M's derivation** —
...
| **6c** | **`%AsyncIteratorPrototype%`** — the shared intrinsic, and nothing else. **It exists because 6a and 6b both inherit from it and neither can be told to consume the other's copy**: 6a's acceptance now requires `%AsyncGeneratorPrototype%` to inherit it (that row), and ECMA-262 **§27.1.5.2** puts `%AsyncFromSyncIteratorPrototype%` on the same intrinsic, which is 6b's adapter. Measured at `658cc302`: `git grep -lE 'AsyncIteratorPrototype\|async_iterator_prototype' -- crates/script/elidex-js/src` → **0** files, against the control `git grep -c 'asyncIterator' -- vm/well_known.rs` → **1** (the symbol id exists, the prototype does not) — so whichever consumer landed first would have created a private copy the other was under no obligation to reuse. ⚠ **Acceptance stays inside the prerequisite** — a row that must retire before either consumer exists cannot assert anything about their prototypes without taking their work: the intrinsic exists and carries `%Symbol.asyncIterator%` returning `this`, **and that is all** — ⚠ **with the observation path named, because §5 requires an *observable* result and nothing reaches this intrinsic from JavaScript before a consumer lands**: it is asserted from Rust against the intrinsic slot itself (the `vm/tests/` harness reads it directly, as the GC tests read `vm.inner`), not through a JS expression. A prerequisite whose artifact has no JS-reachable holder is testable only that way; requiring a JS path here would pull a consumer's exposure work into the prerequisite, which is the shape the row exists to avoid. The inheritance edge and its regression belong to each consumer — `%AsyncGeneratorPrototype%` to **6a**, `%AsyncFromSyncIteratorPrototype%` to **6b** | `vm/globals_async.rs` | `#11-vm-async-generators` (6c) | T2 | — |
...
(new machinery), the 2a family ordered by its Deps column. Slice 5's edges onto the 2a family are owed
for the artifacts its own row names — the field record and the instance-initialization timing a private
field is defined through — and which rows carry them is read there, not restated here. **S** follows **3**
