...
| **9b** | **`String` prototype surface.** Derives its inventory from ECMA-262 **§22.1.3** at its start. Named deliverables: `String.prototype.matchAll` (absent) and `String.prototype.replaceAll`'s RegExp behaviour — measurements in the §8 slot row, which is their permanent home — **plus the `natives_string.rs` §21.1.3 → §22.1.3 citation retag** (§21.1.3 is *Properties of the Number Prototype Object*; the String prototype is §22.1.3 — both verified with `webref heading ecma262`), which lands here because Slice P does not touch this file. ⚠ **The `Deps` cell read `8b` while that row was terminal, and an umbrella named there names nobody**: `matchAll` is ECMA-262 §22.1.3.14, whose step 3.c is `GetMethod(regexpOrPattern, %Symbol.matchAll%)`, and `replaceAll`'s RegExp behaviour is §22.1.3.20 — both consume the well-known-symbol dispatch 8b owns (the *dispatch*; the ids are 8d's), so implementing them first would either re-open-code the `ObjectKind::RegExp` special-casing 8b exists to remove or ship a `matchAll` that ignores a user `@@matchAll`. The consuming child is un-minted, so what the `Deps` cell names is umbrella 8b's derivation together with the child that derivation mints for that entry — the form §5's row-kind rule states for exactly this state — and **re-pointing the edge at the child once minted is a mandatory output of the same derivation**, which 8b's row records by name. ⚠ **And an `8d` edge the row did not carry, found by walking this row's own seed rather than its dependants: both named deliverables read a `%Symbol.*%` id *before* any dispatch happens.** **§22.1.3.14** `String.prototype.matchAll` reaches `Let isRegexp be ? IsRegExp(regexpOrPattern)` inside its `If regexpOrPattern is an Object` branch, and **§22.1.3.20** `String.prototype.replaceAll` reaches the same AO on its own `searchValue`; **§7.2.6** IsRegExp then performs `Let matcher be ? Get(arg, %Symbol.match%)` before it ever looks for a `[[RegExpMatcher]]` slot, and `replaceAll` goes on to `Let replacer be ? GetMethod(searchValue, %Symbol.replace%)` — two of 8d's five ids, neither of them 8b's dispatch (§-numbers from `webref heading ecma262 22.1.3.14 --exact` / `22.1.3.20 --exact` and `webref aoid ecma262 IsRegExp`; steps from `webref body ecma262 sec-string.prototype.matchall` / `sec-string.prototype.replaceall` / `sec-isregexp`). With the ids unallocated `IsRegExp` cannot be written at all, so this row landing first re-opens the `ObjectKind::RegExp` branding 8b exists to remove, under two more member names — the same relationship 8a carries onto 8d, and it sits in this row's own `Deps` cell rather than being routed through 8b because 8d is terminal and mints nothing. ⚠ **And an `8a` edge from the same walk, which was left unplaced last round on the doubt that RegExp creation already exists in tree — it does not, in the form this step needs.** `matchAll`'s fallback is not the `%Symbol.matchAll%` dispatch above but a RegExp *created from a runtime value*: **§22.1.3.14** step 5 is "Let regexp be ? RegExpCreate(regexpOrPattern, "g")", and **§22.2.3.1** RegExpCreate is two steps — "Let obj be ! RegExpAlloc(%RegExp%)" then "Return ? RegExpInitialize(obj, pattern, flags)" — which are step 7 and step 8 of **§22.2.4.1** `RegExp ( patternOrRegexp, flags )` verbatim, "Let obj be ? RegExpAlloc(newTarget)" and "Return ? RegExpInitialize(obj, patternSource, flags)", entered with `%RegExp%` in place of NewTarget and past that clause's steps 1-6. So what step 5 consumes is the creation half of the constructor surface 8a's charter carries, and the two AOs it is composed of live in **§22.2.3** *Abstract Operations for RegExp Creation*, listed there as §22.2.3.2 RegExpAlloc and §22.2.3.3 RegExpInitialize (§-number and title from `webref aoid ecma262 RegExpCreate` and `webref heading ecma262 22.2.3` / `22.2.4`; steps from `webref body ecma262 sec-string.prototype.matchall` / `sec-regexpcreate` / `sec-regexp-pattern-flags`). ⚠ **The in-tree literal path is a different artifact and cannot stand in for it.** RegExp objects are constructed at exactly two sites — `git grep -n 'kind: ObjectKind::RegExp {' -- crates/script/elidex-js/src` gives `vm/dispatch_helpers.rs:176`, the `Constant::RegExp` arm of `load_constant`, whose pattern and flags are **compile-time constants** and so cannot take step 5's runtime `regexpOrPattern`, and `vm/host/structured_clone.rs:590`, a deserialization path — while `git grep -nic regexpcreate` / `regexpalloc` / `regexpinitialize` over the same tree each return **0** files, so §22.2.3 has no implementation to reuse and this row landing ahead of 8a open-codes a third construction site beside the one 8a must ship | `vm/natives_string.rs`, `vm/natives_string_ext.rs` | `#11-vm-builtin-prototype-static-sweep` (9b) | T3 | **8a**, **8d**, **the child of umbrella `8b` that owns the String-side `GetMethod` entry**, via that umbrella's derivation, which mints it |
...
false). Each needs a cohesion verdict in its own
