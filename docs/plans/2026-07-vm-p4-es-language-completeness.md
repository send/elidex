...
tracing edge and a regression that forces a collection between the store and the read. Measured at
...
false — and **I-7 adds two more readers of that same file**, the child of umbrella 3 that carries the
`[[HomeObject]]` representation and Slice **N**, each owing an `ObjectKind::Function` edge for state it
moves onto the closure). Each needs a cohesion verdict in its own
...
| `#11-vm-arrow-lexical-new-target` | **(carved at PR-B)** Slice **N**. ⚠ **A `new.target` is an object, so by §4 I-7 this slot owes the `ObjectKind::Function` tracing edge for it and a forced-collection regression**: the frame spelling is marked (`vm/gc/roots.rs:294-295`, `vm/gc/trace.rs:353-355`) and the closure spelling this work adds is reached by nothing, so an arrow outliving its constructor frame is where it is observable. `Op::NewTarget` reads the current frame's `CallMode` and `FunctionObject` has no lexical `new.target` beside `captured_this`, so `new.target` is `undefined` inside an arrow where ECMA-262 §9.4.5 resolves the surrounding function environment. Separate from Slice 3 because the AO differs, though both add lexical state to the same closure/frame seam | now (Slice N) | 2026-09-30 |
