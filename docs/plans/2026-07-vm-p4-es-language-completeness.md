...
returns **17**, silently keeping `SetPrivate`:
`for op in …; do git grep -h "Op::$op" <rev> -- crates/script/elidex-js/src/compiler/ | grep -vE '^[[:space:]]*(//|\*)' | grep -cE "Op::$op([^A-Za-z0-9_]|$)"; done`
→ 0 for all of them — **18** zero-emit opcodes in total, see §2.3). So a fix that targets the dispatch stub touches dead code and changes nothing.
...
input contract), `Pa → Pb` (a bytecode transport), `Eb → Ea` (a representation reuse), and
...
- **Shared prerequisite of the 2a family** (an ordering fact, so it is stated once here rather than on
  ...
  **and accepts** the computed-name forms
  ...
  2aa/2ab first connects the initializer resolves it** — the terminal owner this bullet already names
  for the `arguments` half, not the 2a umbrella, which under §5's terminality rule gets no
  plan-review and no PR of its own — and until it does, nothing here asserts one. Writing a replacement citation from recall is how
...
| **4a** | **The template object** — [C23] **§13.2.8.4** GetTemplateObject. Its step 3 returns the already-registered Array when `[[Site]]` is the same Parse Node and step 16 appends the Record to `realm.[[TemplateMap]]`, so the object is keyed by call site *and* realm and is the **same object** on every evaluation of that site; steps 12.c/12.e define the cooked and raw elements non-writable and non-configurable, step 13 freezes `rawObj`, step 14 defines `"raw"` non-enumerable and non-writable, step 15 freezes `template`. Carries the per-realm registry the family needs, which is why 4b depends on it rather than building a second one (CLAUDE.md *One issue, one way*). The acceptance shape those steps require is an **identity** assertion across two evaluations of one site and a **frozen** assertion — never that a tag received an array. ⚠ **Two named deliverables that acceptance shape does not imply, both about the registry rather than the object.** (i) **The registry must be added to the GC root set by hand, because root enumeration is hand-written throughout.** Measured at `658cc302`: `GcRoots` (`vm/gc/roots.rs:21-262`) is a hand-written struct with one field per root set; its **only** construction site is a struct literal in `collect_garbage` (`vm/gc/collect.rs:52-1145`, 40 field lines) that names the `VmInner` fields one by one; and `mark_roots` (`vm/gc/roots.rs:266-619`) walks each field in its own lettered step. Nothing derives a trace from `VmInner`: `git grep -c 'trait Trace' -- crates/script/elidex-js/src` and `git grep -c 'derive(Trace' -- crates/script/elidex-js/src` each match nothing, against the positive control `git grep -c 'derive(' -- crates/script/elidex-js/src`, which matches in 65 files. So a per-realm template registry hung off `VmInner` is **invisible to the collector** until this slice adds the `GcRoots` field and the `mark_roots` arm for the map's cooked and raw arrays; both are 4a's, and 4b inherits a registry that is already rooted. (ii) **The identity regression must force a collection between the two evaluations** — `vm.inner.collect_garbage()`, as `vm/gc/tests.rs` does — because as written the assertion passes whenever no GC happens in between, so it cannot fail on the defect (i) describes. (iii) **I-5's non-`engine` consequence is not Slice 7b's alone.** 4a's field lands in that same hand-written root set, whose fields and `mark_roots` arms are individually `#[cfg(feature = "engine")]`-gated (and `vm/gc/collect.rs` carries a `#[cfg(not(feature = "engine"))]` counterpart for each), while a template object is pure ECMA-262 — so the field and its mark arm must be defined identically for non-`engine` builds, which is exactly what §4 I-5 requires of 7b for the same directory. (iv) **Grain — the realm axis is part of the key, not an assumption.** "Keyed by call site *and* realm" plus a plain `VmInner` field are consistent only under one realm per VM, and `docs/plans/2026-06-agent-scoped-ecsdom-world.md` §5 **design req 7** schedules that away: *"The current `Vm` (singleton `global_object` + singleton prototype slots) must generalize to **N realms** (one per Window)."* The in-tree sibling records this at the residence rather than as a TODO, and 4a's registry carries the annotation in the same shape — `HostData::ce_registry` (`vm/host_data/mod.rs:536-557`): *"the grain migration to a per-realm component rides agent-scoped EcsDom, docs/plans/2026-06-agent-scoped-ecsdom-world.md §5"*. (v) **Lifetime — realm-lifetime, so it survives `Vm::unbind`.** `unbind` (`vm/lifecycle.rs:15`) closes every script BATCH; `teardown_document` (`:536`) is document destruction and calls `unbind` as its last step. A realm outlives a batch, so a registry cleared on `unbind` rebuilds the template on the next batch and breaks §13.2.8.4 step 3's same-object guarantee *across* batches — release it at `teardown_document`, alongside `ce_registry` / `ce_constructors`, which `vm/lifecycle.rs` documents as precisely this class. The identity regression in (ii) cannot catch that, being single-batch: it must **also cross an `unbind`** — evaluate the site, `unbind`, evaluate again, assert identity. (vi) **Key — there is no Parse-Node identity in the VM, so `[[Site]]` needs a carrier.** `grep -rnw 'Arena' crates/script/elidex-js/src/vm/` → **0**, and the same for `NodeId` (control, identical shape: `ObjectId` → 2217); `NodeId<T>` is `{ index: u32, _marker }` whose `PartialEq`/`Hash` read `index` alone (`arena.rs:10-42`); and every `parse_script` / `parse_module` mints fresh arenas (`parser/mod.rs:109-111`), so index *n* from two separately parsed scripts compares equal. Keying on a raw `NodeId` would alias one site's frozen template onto another's — a worse violation than the unrooted one in (i). The carrier that disambiguates already exists and needs no minting: `FuncId` (`vm/value.rs:50-52`) indexes the VM-global `compiled_functions`, which is never truncated or recycled, and rides every `CallFrame` beside `ip` — so the registry keys by `(FuncId, ip)`, or by the per-call-site `(FuncId, call-IC slot index)` the compiler already allocates, together with (iv)'s realm axis. (vii) **Why this is a side store and not an ECS component**, since it is per-VM state holding `ObjectId`s: CLAUDE.md's exception (a) (per-VM identity handle) covers the values for the same reason `HostData::ce_constructors` claims it (`vm/host_data/mod.rs:576-582`) — and, as there, the key is not an `Entity` (`ce_constructors` is `HashMap<u64, ObjectId>`), so the per-entity precondition of the side-store→component rule is not met either. Two independent reasons; (iv)'s realm axis is what the eventual migration turns on | `vm/`, **`vm/gc/roots.rs`**, **`vm/gc/collect.rs`**, `compiler/expr.rs` | `#11-vm-tagged-template-literals` (4a) | T1 | — |
...
it at landing mints duplicates. That marker was stale on four slots in all — those two plus
`#11-vm-assignment-target-completeness` and `#11-vm-iteratorclose-precedence-convention`, which the
derivation above already reports nonzero — and is removed from all four. **This derivation, not a
marker in §5, is what a lander reads for registration state.**
...
- **I-3 tagged-template clause recorded, not discharged** — decide the helper's *input* shape
  (fixed-prefix count or item iterator) so Slice 4b has a live consumer path. ⚠ **Owner assigned:
  Slice 1b**, as an explicit deliverable of `compile_call_arguments` / `ArgsForm`, with Slice **4b**'s
  mandatory plan-review as the check (Slice 4 is an umbrella this document itself records as wrongly
  judged terminal; §6.3 already writes 4b). §3 already records the binding constraint (`ArgsForm` as
