...
remains. **O**, **N** and **B** sit outside that chain — no row's deliverable is an input to any of
them. N's own row records the seam it shares with Slice 3, which is a coordination note, not an
ordering. **R** is outside the sequence entirely, like **M**.
...
`#11-reflect-apply-ce-test` — those are what is still to register. The two slots this revision adds,
`#11-vm-block-declaration-instantiation` and `#11-vm-explicit-resource-management`, also return
**zero** (same ledger, `stat -f %Sm` = 2026-08-08 11:51), so they join that set. `#11-vm-computed-compound-assignment`
