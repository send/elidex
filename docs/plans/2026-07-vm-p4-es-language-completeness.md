...
- **Umbrella row** — ships **no implementation PR**. It carries **(1) the split** — its own boundary
  ...
  **never lands**; it *retires* once every child it minted has landed, and a partial landing is recorded
  ...
  rows too** — the split is **28** §5 rows and **8** §8 slots. The narrower pipeline was what this rule
...
- **Evidence for Slice K** — the class-name binding is initialized **before** static elements run.
  ...
  as a terminal prerequisite **row**, so the ordering is read from 2ac's and 2b's `Deps` cells and this
  bullet is evidence only.** What the prose form could not do is what a slot could not do either: moving
...
- **Shared prerequisite of the 2a family** (an ordering fact, so it is stated once here rather than on
  ...
  **and accepts** the computed-name forms
  ...
  by landing and it became the terminal prerequisite row **K** that both rows now depend on. This bullet
...
to any of them, and inside E the Deps column orders `Ea → Eb`. **O** and **N** are
