...
| **9b** | **`String` prototype surface.** Derives its inventory from ECMA-262 **§22.1.3** at its start. Named deliverables: `String.prototype.matchAll` (absent) and `String.prototype.replaceAll`'s RegExp behaviour — measurements in the §8 slot row, which is their permanent home — **plus the `natives_string.rs` §21.1.3 → §22.1.3 citation retag** (§21.1.3 is *Properties of the Number Prototype Object*; the String prototype is §22.1.3 — both verified with `webref heading ecma262`), which lands here because Slice P does not touch this file. **Ordered after the String-side `GetMethod` child of umbrella 8b** — ⚠ **this cell read `8b` while that row was terminal, and an umbrella named in a `Deps` cell names nobody**: `matchAll` is ECMA-262 §22.1.3.14, whose step 3.c is `GetMethod(regexpOrPattern, %Symbol.matchAll%)`, and `replaceAll`'s RegExp behaviour is §22.1.3.20 — both consume the well-known-symbol dispatch 8b owns (the *dispatch*; the ids are 8d's), so implementing them first would either re-open-code the `ObjectKind::RegExp` special-casing 8b exists to remove or ship a `matchAll` that ignores a user `@@matchAll`. The consuming child is un-minted, so the edge cannot be written in the cell yet: **placing it is a mandatory output of umbrella 8b's derivation**, which that row records by name, and this `Deps` cell reads `—` *pending that child* rather than asserting this row is unordered — the one state §5's row-kind rule leaves without a structured home, and the reason it is spelled out here | `vm/natives_string.rs`, `vm/natives_string_ext.rs` | `#11-vm-builtin-prototype-static-sweep` (9b) | T3 | — |
