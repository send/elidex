...
13. **§6.2a — the reused drain's `IteratorClose` divergence — RESOLVED as two units.** ⚠ **Round-8
    ...
    while `op_array_spread`/`spread_iter_loop` is the only in-tree drain-into-array and the child of umbrella 1a that removes `op_array_spread`'s `return()` **deletes**
    its `return()`. So either 0bc routes through it (and that child silently falsifies 0bc's [C39] claim) or 0bc
