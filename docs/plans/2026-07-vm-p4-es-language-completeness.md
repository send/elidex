...
|---|---|---|---|
| [C19] | ECMA-262 §13.3.8.1 Runtime Semantics: ArgumentListEvaluation | `#sec-runtime-semantics-argumentlistevaluation` | Slice 1b, 4a |
| [C20] | ECMA-262 §13.3.6.2 EvaluateCall | `#sec-evaluatecall` | Slice 1b, 4a |
...
| [C23] | ECMA-262 §13.2.8.4 GetTemplateObject | `#sec-gettemplateobject` | Slice 4a (steps 4-15, construction) + 4d (steps 3/16, the registry) |
| [C40] | ECMA-262 §13.3.11.1 Runtime Semantics: Evaluation (tagged template) | `#sec-tagged-templates-runtime-semantics-evaluation` | Slice 4a |
...
| `compiler/expr.rs:237-239` | `` t`a${1}` `` | `PushUndefined` | tag never called | T1 | **4a** |
...
- ⚠ **The inbound rule has no membership test, and core dispatch already carries host knowledge.**
  ...
  the same is available in this direction. **The test is settled per surface by that surface's own
  owner, and the reason is that the two rows carry no edge between them** (§5): Slice **10a**'s
  mandatory plan-review settles the `Reflect` half — §5 gives the `Reflect` surface to 10a — and Slice
  **10b**'s settles the `Proxy` half, with whichever runs first auditing these four sites under the
  test it adopts and the second inheriting that test rather than re-opening it. Assigning the whole
...
**4a + 4d + 4c** below, and the umbrella row carries the *derivation* — the criterion plus every
candidate seam tested against it — rather than a fourth verdict. Three wrong verdicts in a row on one
split is the signal §13's enumerate-by-command rule already names: **a seam set is an enumeration,
and every enumeration this document curated by inspection has failed.**
...
- **Shared prerequisite of the 2a family** (an ordering fact, so it is stated once here rather than on
  ...
  **and accepts** the computed-name forms
  ...
  So the divergence is measured and the governing rule is not — **the mandatory plan-review of whichever of
  2aa / 2ab / **2ac** first reaches a field initializer resolves it** — the terminal owner this bullet already names
  for the `arguments` half, not the 2a umbrella, which under §5's terminality rule gets no
  plan-review and no PR of its own — and until it does, nothing here asserts one.
...
- **Shared prerequisite of Slices 2 and 3** (an ordering fact, so it is stated once here rather than on
  ...
  **§15.7.10** ClassFieldDefinitionEvaluation takes `homeObj` and, for
  ...
  the owner is **whichever lands second** of (a) the slice that creates the producer's closure — the
  field-initializer closure for the field half, **2b** for the static-block half — and (b) **3**; that
  slice integrates the home object *and* runs the regression above for its producer. Stated as a
  derivation rather than a pair of slice ids because the field half's creating slice is settled by the
  2a family's own split, not here.
...
| **0b** | **Assignment/update target completeness — UMBRELLA, not a terminal unit.** The charter as written spanned binding-pattern storage, `IteratorClose` conformance ([C39]→[C36]), optional-call receiver preservation, parser-level early errors and catch-parameter binding initialization — and the row itself recorded that `peel_paren` does **not** reach the optional-call case, so there was no shared lowering chokepoint holding them together. Split below along the mechanism boundaries the charter already named. ⚠ **"the sub-slices are ordered and dependent" stood here and is withdrawn — it was prose restating an *authoritative* field, and it had gone stale against the cells**: `0ba` and `0bb` carry empty Deps, `0bc` carries only `Pa`, and the ordering rationale says the three are independent since the `0bb → 0ba` and `0bc → 0bb` edges were deleted at PR-B. **The three sub-slices are independent of one another**, and the Deps column — not this sentence — is where that is read | — | `#11-vm-assignment-target-completeness` (umbrella) | T1 | — |
...
| **9a** | **`Array` prototype/static surface.** Seeded by the §1.2 absence probe (`at` / `findLast` / `findLastIndex` / `toSorted` / `toReversed` / `toSpliced` / `with`); derives its own inventory at its start from ECMA-262 **§23.1.2** (Properties of the Array Constructor = the statics) and **§23.1.3** (Properties of the Array Prototype Object), rather than from that probe. **Depends on Pa, for the reason 9c's cell carries**: **§23.1.2.1** `Array.from` closes the iterator on the error path — step 5.e.i.2 is `Return ? IteratorClose(iteratorRecord, error)` with `error` a ThrowCompletion — and its steps 5.e.v.2 and 5.e.viii are `IfAbruptCloseIterator` (`webref body ecma262 sec-array.from`, over which `grep -c IteratorClose` → **1** and `grep -c IfAbruptCloseIterator` → **2**). Same §7.4.11 abrupt-completion contract, so a drain written against today's `iter_close` inherits the inverted precedence here too | `vm/natives_array.rs` | `#11-vm-builtin-prototype-static-sweep` (9a) | T3 | **Pa** |
...
for, and Pd's is §14.7.5.7 sequencing — neither consumes what Pa or Pb produces. ⚠ **Pa's dependents
...
for the reason its row states. Inside Slice 4 the chain is **4a → 4d**: 4a depends on **1b** (I-3
requires reusing the arg helper) and makes the tag path live with §13.2.8.4 step 3's same-object
guarantee pinned as diverging, then **4d** retires that pin with the registry; **4c** is outside the
chain. Slice 4's own Deps cell is `—`. D runs after the connecting slices so it deletes only what
...
**6b** are T2 while **9d** is T3. They hold anyway, because a Deps edge records that the earlier row's
deliverable is an *input*, not that its defect is worse: 6a's `.return` and 6b's `for await` drain
both reach §27.5.4.7.1 PromiseResolve and therefore the resolving-functions closure 9d delivers (each
cell traces its own chain), so landing either first writes the async settlement path against a
`settle_promise` that adopts only `ObjectKind::Promise` and rewrites it when 9d lands. Severity says
...
**Acceptance condition Slice 4a consumes**: a 1b test drives `compile_call_arguments` with a
non-empty fixed prefix plus *n* substitutions and asserts the emitted layout and the returned
`ArgsForm` for *n* = 0 and *n* > 0, so Slice 4a reaches its tagged-template lowering by *calling* this
helper — no signature change, no second argument-emission path. Slice 4a's mandatory plan-review
checks that it did.
...
16. **§7.2's conformance-row outcome type — RESOLVED (crash-aware outcome), owner Slice 0cb.** The
    ...
    **1b**'s, §5 gives 1b `Deps: 1a`, and §15's ship order runs `… → 0b → 0c → 1a → 1b`, so hoisting
    it inverts both. ⚠ **This sentence also cited "1a `Deps: 0cb`" until PR-B; that edge was deleted
...
- **I-3 tagged-template clause recorded, not discharged** — decide the helper's *input* shape
  ...
  specified cannot express `« siteObj »` ++ substitutions) and §5 gives Slice **4a** `Deps: 1b (I-3
