...
- **I3 — guard bracket (axis b × e).** `running_nested_apply_history_step` is set **before the peek** (`peek_back`/`peek_forward`/`peek_go`) and cleared **after the commit** (`commit_index`), covering the *entire* peek→commit window. A reentrant nav-mutating message (SW-pump) arriving inside the bracket is serialized onto the queue, not applied under the held peek — making peek→commit atomic **by construction** (retiring the `commit_index` `debug_assert`). A guard set after the peek or cleared before the commit leaves the race reachable and is a bug. **Liveness — bounded next-turn drain (Codex PR#469 R3 T1, as-shipped):** Phase-2 (`drain_traversal_queue`) drains a **bounded snapshot** — the steps present at drain-start (`pending_len()` captured once) — and **terminates by construction** even if an apply re-enqueues. A step serialized *mid-apply* (a reentrant nav-mutating message pushed onto the back of the queue) is **NOT** drained in this pass; it is deferred to the **next** `run_deferred_traversals` turn. **Liveness is preserved by content-mode's async pump running Phase-2 every turn** (`event_loop.rs` per-turn `run_deferred_traversals`, at **step 3** of the message-held pump — not a top-of-loop replay pass, retired by the pump-turn drain unification), *not* by draining to exhaustion. The earlier "re-check the queue until empty before returning" invariant is **RETIRED** — a re-check-until-empty loop is unbounded (a host that re-enqueues on every apply would hang the single-writer renderer thread) and is exactly what the bounded snapshot replaces. The loop-bound is **no longer a Slice-4 item** (it is implemented here); what remains Slice 4 is only the reentrant DIRECT-nav *serialization* semantics (§7.3.1.1 running-nested-apply guard WIRING for a reentrant direct nav that does not consult `is_applying()`), not the loop bound. ⚠ **App-mode (Slice B) caveat:** this bounded-drain liveness argument is **content-mode-specific** — it rides the every-turn async pump. App-mode's `drain_same_turn` has **no async pump**, so a mid-apply-serialized step would strand until the next input event; Slice B must NOT adopt the bounded drain without either re-adding an end-of-handler re-check **or** keeping the reentrancy vector dead (see the Slice-A plan-memo scope fence).
...
- **Slice 4 — reentrancy guard + retirements:** wire `running_nested_apply_history_step`, serialize the
  ...
  ⚠ **CARRY (Slice-1 `/code-review` high, 2026-07-13) — NARROWED: the loop bound is now IMPLEMENTED (Codex PR#469 R3 T1).**
  `drain_traversal_queue` no longer runs an unbounded `while let Some(step) = pop_next()`; it drains a **bounded
  snapshot** (`while remaining > 0`, `remaining = pending_len()` captured at drain-start) and **terminates by
  construction** — a wired host whose `apply_traversal` re-enqueues on every call can no longer hang the
  single-writer renderer thread (a mid-apply re-enqueue defers to the **next** `run_deferred_traversals` turn;
  content-mode's async pump drains Phase-2 every turn = liveness). The **loop bound is NOT a Slice-4 item**. What
  **remains Slice 4** is only the reentrant-message *serialization* semantics: wiring the
