...
- **D1 — C2 adds no `ScaleFactorChanged` handler.** winit 0.30.13 emits a `Resized` *after* every
  `ScaleFactorChanged` on all four platforms elidex targets (§1, source-verified), and at that `Resized`
  `window.scale_factor()`/`inner_size()` already return the new values. The existing `Resized` arm
  (`app/mod.rs:947-972`) therefore already performs the full DPI refresh (gpu resize + placement recompute
  with new scale + content notify + reclip + redraw). A parallel handler is dead/redundant mechanism —
  rejected by *Ideal over pragmatic* + *dead code は接続するか削除* + *設計優先 (場当たり reactive fix 禁止)*.
...
| **X11** | changelog v0.18 — "a `Resized` event will **always** be generated after a DPI change to ensure the window's logical size is consistent with the new DPI." |

In every case `Resized` follows `ScaleFactorChanged`, and at that `Resized` `window.scale_factor()` returns
the **new** scale and `inner_size()` the **new** physical size. So elidex's existing `Resized` arm already
performs the full DPI refresh the slot asked for:
...
verified 2026-06-26. Exact-`f32` equality is correct: `size_logical` is recomputed deterministically from
the same `inner_size`/`scale_factor`, so an unchanged window yields a bit-identical value — a "did the
producer compute a different value" test, not a tolerance question; see §9 Q4 for the reordering audit.)
...
- No `WindowEvent::ScaleFactorChanged` arm (D1 — redundant on winit 0.30; the `Resized` follow-up covers it).
...
- **`#11-shell-viewport-delivery` axis-3** — **discharged** by this PR (seq-invariant fix) + the winit-0.30
  finding (the "handler" half was a non-problem). Record the winit-contract finding in the slot so it is not
  re-carved. axis-2 (multi-tab) already discharged by C1; **C3 (prefers/dppx device facts) remains** the open
  PR-C axis.
- **Candidate `#11-winit-scale-contract-regression-guard`** — *only if* plan-review wants insurance against
  a future winit dropping the `Resized`-after-`ScaleFactorChanged` guarantee: an integration guard that
  fails if a `ScaleFactorChanged` is seen without a following `Resized`. **Recommend NOT carving** (no
  consumer; speculative — *ideal over pragmatic*; fails `feedback_defer-slot-eligibility-audit-at-create`).
  Listed for the panel to rule on.
...
- **Q1 — Is reframing axis-3 from "add a `ScaleFactorChanged` handler" to "fix the publish seq-invariant"
  ...
  genuinely misses on a supported platform?)
...
- **Q4 — Exact-`f32` equality in `publish_if_changed`.** `size_logical` is recomputed from identical inputs
  ...
  reordering)? If so, a no-op resize could still bump. (My read: deterministic same-input → same-output,
  safe. Verify the arithmetic is reordering-free.)
