...
2. **`test_preflight.py` will already exist** (A-ii creates it with its §6 pin set, P1…P13 plus T-raise/T-net). B's `parse_spec_cell` and
...
- **Q2 — what caps the probe window (`MAX_LABEL_WORDS`)?** Measured: the catalog's longest label is **24 words** (an RFC title ending `| RFC Editor`); **169 of 1896** label keys exceed 6 words. An earlier revision recommended a cap of 6 asserted against the *pinned* set only — which left 169 catalog labels unreachable by the detector while `shortname_for` resolves them, contradicting I2 (attribution reach = `shortname_for` reach) and under-reporting the supersession audit by design (Codex R26). **Recommendation: no fixed cap — derive the window from the index**: `MAX_LABEL_WORDS = max(len(k.split()) for k in index)` at index-build time, so reach is equal by construction. The cost is bounded by the index, not by the regex §4.6.1 deletes: at most 24 dict lookups per comment line, each O(1); a label that is never cited costs nothing beyond its index entry. Review should confirm that bound rather than a number. **Offline** (catalog unavailable, §4.1.7): the index holds only the pinned keys, so the window is their maximum — and that is correct, because no catalog-only label can be *resolved* offline whatever the window; what matters is that it is *classified*. A label-shaped run (the probe's left edge, capitalised words before `§`) that the pinned index does not resolve is emitted as **`UNKNOWN-SPEC`** whenever `_catalog().available is False`, never as UNATTRIBUTED, so an offline run under-resolves loudly rather than silently (Codex R27). `max()` over an empty index cannot occur: the pinned set is never empty (A-i's `test_empty_specs_would_still_import` pins the import, not an empty map in production; `SPECS` has twelve rows). Pinned by **T3b**: a 9-word catalog label under a poisoned `urlopen` → `UNKNOWN-SPEC`, and the same label online → attributed.
- **Q3 — should `resolver.lookup_heading` itself use the shared index, or only `cite_audit`?** Using it everywhere makes `coverage-map` and `preflight` faster and is the one-issue-one-way answer; it also changes a generic-core function's memory profile (one index per spec per process). **Recommendation: yes, in `resolver`** — *once B adds* the `@lru_cache` on `try_fetch_data` (A-i §13 routes that edit to B; at A-i's landed head the decorator is absent, so an earlier revision's "already cached" premise was false — Codex R27). With the cache in place the extract is retained per process and the index is strictly cheaper than re-scanning it; without it the index is a second copy of a re-fetched extract, which is why the decorator is in B's edit set (§8) and not an assumption.
