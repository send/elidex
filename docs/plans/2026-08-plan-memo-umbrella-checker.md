...
| CommonMark §4.7 | orphan detection | a definition-shaped line the grammar could not read (it interrupts a paragraph), parsed with its permitted continuation line; linear — a hit records every returned definition and resumes at the grammar's rest offset | `plan_memo_lexer.py::Lexed.orphan_definitions` | ✓ controls "(def) a would-be MULTILINE definition that interrupts a paragraph is an orphan…", "orphan_definitions() is linear: 3000 definition lines in < 50 ms" | no |
...
| **local policy** (no spec clause) | sibling destinations (percent-decoded) | the path component of a destination is percent-decoded before resolving (`slice%20sib.md` = `slice sib.md`, as `<slice sib.md>`) | `plan_memo_tables.py::Memo.linked_files` (`urllib.parse.unquote`) | ✓ control "(link) a percent-encoded destination `slice%20sib.md`…" | no |
