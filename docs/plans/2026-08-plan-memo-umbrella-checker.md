...
| CommonMark §4.7 | orphan detection | a line that OPENS like a definition (label + colon, `definition_shape`) but is not one — inside a paragraph, or invalid (a title crossing a blank line, junk after the destination) — is an orphan; linear — the runs are joined once, one shape parse per line | `plan_memo_tables.py::Memo._blocks` (`Memo.orphans`), `plan_memo_lexer.py::definition_shape` | ✓ controls "(def) a would-be MULTILINE definition that interrupts a paragraph is an orphan…", "Phase-1 orphan detection is linear: <= 4 link_label calls per line, t(4N)/t(N) < 8" | no |
...
| CommonMark "Appendix: A parsing strategy", Phase 1 → Phase 2 | block structure before inline structure | Phase 1 over RAW lines (`Memo`): fences, reference definitions as blocks of their own recognised only at a block start and parsed over the REST OF THEIR RUN (the lines up to the next blank line or fence, joined once — §6.3 label up to 999 chars and may span lines, `link_title`; §4.7 "may not contain a blank line" by construction; a definition-shaped line inside a paragraph is an orphan, `Memo.orphans`), GFM tables ending at a blank line or any block start (a definition included, GFM §4.10), paragraphs; Phase 2 (`inline_pass`) over each paragraph's / cell's content only, with `defs` from Phase 1 | `plan_memo_tables.py::Memo._blocks` / `Memo.block_start` / `Memo.definition_at`; `plan_memo_lexer.py::definition_block` | ✓ controls "(def) a definition is read from RAW lines at a block start…", "(table) a reference definition right after a schema table ENDS the table…", "(def) a definition cannot interrupt a paragraph…", "orphan_definitions() is linear…" | no |
| WHATWG URL (scheme before decoding) + **local policy** | ONE destination → sibling resolver | `Memo.sibling_path`, stages in spec order: (a) scheme on the RAW path (`notes%3Achild.md` is the file `notes:child.md`), (b) percent-decode, (c) reject absolute / C0 control, (d) `.md`, (e) `resolve()` with `OSError` read as an UNAVAILABLE sibling (the exit-2 missing-memo path); `linked_files` dedups with a set | `plan_memo_tables.py::Memo.sibling_path` / `linked_files`, `_is_file` | ✓ controls "(link) `notes%3Achild.md` has no scheme…", "an OSError from resolve() is the unavailable-sibling schema miss…", "linked_files scales linearly…" | no |
| **local policy** (no spec clause) | sibling destinations (control characters) | a decoded destination holding a C0 control (`child%00.md`) is rejected in the same validation line as a scheme / absolute path, so `Path.resolve()` never raises | `plan_memo_tables.py::Memo.linked_files` (`_CONTROL`) | ✓ control "a decoded destination with a C0 control character is rejected, never resolved" | no |
