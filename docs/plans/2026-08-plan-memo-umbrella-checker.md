...
| §4.6 | HTML block | PROSE-AS-WRITTEN (+ SEED on EVERY line from the opener to the end condition: types 1–5 by content, 6–7 at a blank line; a type-7 opener only at a block start); type 1–6 openers interrupt a paragraph | `unsupported_block` / `html_block_type` / `html_block_ends`, `Memo._blocks` | "(lex-seed) an HTML-block opener holding a declared id is a seed" |
