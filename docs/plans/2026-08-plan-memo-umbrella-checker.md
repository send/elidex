...
- **I-A Lexical masking** — text inside fenced blocks and inline code spans is never read as a
  ...
  **block's inline content** (paragraph / cell), never per line — CommonMark §6.1 spans may contain
  ...
  0.31.2): a blank line (§2.1), a fenced-block line (§4.5), a paragraph-interrupting block start —
  ...
  blockquote), HTML-block opener of type 1–6 (§4.6; type 7 cannot interrupt) — or a GFM table header
  ...
  it is parsed over) and a GFM table's body, so `[foo]:\n---` is a setext heading, `[foo]:\n#` /
...
| §4.5 | Fenced code block | LEXED — a RAW extent: never inline-parsed, always a run / paragraph / table end; one extent map (`raw_lines`) shared with HTML blocks | `plan_memo_blocks.py::fenced_lines` / `raw_lines` | "(fence) …" family (10 controls) |
| §4.6 | HTML block | RAW (+ SEED) — a leaf block "treated as raw HTML", exactly like a fence: its lines from the opener to the §4.6 end condition (types 1–5 by content, possibly the opener itself; 6–7 at the next blank line) are a RAW extent in the same map as fences (`raw_lines`), never inline-parsed, always a run / paragraph / table end; a type-7 opener only at a block start (cannot interrupt a paragraph), type 1–6 openers interrupt; every raw HTML line holding a `\|` or a declared id is a `[LEX-UNSUPPORTED?]` seed (commonmark.js: `<pre>\n`\n</pre>` is raw, `text\n<span>` is a paragraph, `<div>\nx\n</div>\ny` runs to the blank) | `plan_memo_blocks.py::raw_lines` / `html_block_type` / `html_block_ends`, `Memo._blocks` (seed) | "(lex-seed) an HTML-block opener holding a declared id is a seed" |
| §4.7 | Link reference definition | LEXED — a block of its own at a block start, parsed over the rest of its run; elsewhere an orphan | `Memo._blocks` / `definition_block` | "(def) …" family |
...
| GFM §4.10 | Table | LEXED — see the §3 rows above; the id cell of a schema row is scanned too, with the row's own id suppressed (`**7z** — Slice 9z lands first` names `9z`) | `find_tables` / `split_row`, checker `blocks()` | "(row)" / "(table)" families; "(id) the id cell's trailing prose is scanned…" |
