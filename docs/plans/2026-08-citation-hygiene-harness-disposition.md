...
not be applied. **The section scope this paragraph used to assign alongside it has landed** — `b088dacc`
scopes the extraction to `^## §3 ` (`-audit.sh:273`, extracting at `:277`) — so the content test is the
only half still owed. (**This program already owns a machine check over this memo's own claims — the gate
...
written on the match branch — `_pop` at `-audit.sh:366-367`, joined at `:469` — so a needle matching
...
PR-1a-i's, stated in §3; the check over this memo's own quantities landed as `b088dacc`, extending the memo gate this program
already owns at `-audit.sh:265-281`; §9 judges that commit against its own clause. Both have a site and an owner, so neither
is a slot and neither is an unowned check.
...
at the end of §1: **no mechanism that decides what a PR decides**. Mechanism whose only subject is *this
...
census output §3 calls the work list*, and the split moved it (`HOMES: 72 … in 8 files` at `b088dacc`
against `HOMES: 70 … in 9 files` at `a5fab499`; the two rows it dropped are `prose` rows, which is
PR-1a-i's own `prose` rule executed by a prereq). It was compelled by CLAUDE.md's touch-time split
discipline, which this clause had not written as an exception. **The clause was incomplete, and is
completed here**: *no mechanism that decides what a PR decides, except a split CLAUDE.md's touch-time
discipline compels* — and the exception carries no discharge, so the split's scope effects are PR-1a-i's
to absorb rather than facts PR-1a-i may assume.
...
the `partset` rule exists to collapse. PR-1a-i collapses three, not two.
