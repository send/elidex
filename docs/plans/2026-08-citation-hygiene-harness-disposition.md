...
|---|---|---|
| **1a-i-β** the classifier | the `prose` subject test; the coverage gate's content test; `classify`'s command-position predicate | all three change **how the census classifies a line**, and none of them changes an answer about where a block ships |
...
2. **The file tier's position relative to T1 is a decision to state, not to inherit.** Placed ahead of T1 it moves seven blocks off `T1 declared` with identical values, so nothing visible changes and the ordering would be adopted by accident. This memo's own rationale settles it — T1 is a heuristic parse of prose on a branch under revision and *reports*; a declaration *binds* — so **the file tier outranks T1**, and it is written down here rather than left to where the edit happens to sit.
...
guard column, a roster column) and therefore cannot shrink into a false green. It carries **eight
obligations over the nine classes**: `mention` and `callsite` are *nothing to do*, their observable is
already true at HEAD, and an observable that is already true cannot discriminate — they are regression
guards, not obligations. `covgate` joins as the ninth entry, from §3's own keep.
