...
**PR-1a-i's and PR-1a-ii's scopes as stated in §3, §2 and §3a**. ⚠ **The last two are named because §3's
rule rows are not the whole statement of a slice's scope** — §2's slice table and §3a's apportionment carry
obligations §3's rows do not, and a slice whose mechanism falsifies a §3 row must edit that row, so an
authorisation resolving through §3 alone would resolve through text the slice itself wrote. It does **not**
certify PR-1a-i as a terminal slice under CLAUDE.md's base case (§2).
