...
Spec column carries each spec's label **as the plan-review gate's pinned map spells it** (`SPEC_LABEL_REVERSE`, `preflight.py:51`) — ⚠ **an earlier draft said *"authoritative label"***, and none of the four rows carries one: ⊕ `webref specs fetch` → `Fetch Standard`, not `WHATWG Fetch`; `cssom-view` → `CSSOM View Module Level 1`, and the repo's own generator emits `CSSOM VIEW 1`, neither of which is row 4's `CSSOM View 1`; a fixture deliberately carries a different one, because
...
⚠ **PR-1a-i did not meet `axes.md`'s single-intersection clause** — ⚠ **an earlier draft attributed it to CLAUDE.md**, which says only *narrowly-scoped* (⊕ `grep -ci 'single-intersection' CLAUDE.md` → 0; `axes.md` → 1), and this section's own parenthetical says `axes.md` is the narrower of the two. **PR-1a-i also did not meet the base case, and it is re-sliced here rather than
...
| **1a-i-δ** the guards | `reads` — a named failure on every read of the harness's or a memo's text | ⚠ **It was assigned to no slice at all when the partition was first drawn, and §3a counted it as an obligation, so the two sections contradicted each other.** It is its own slice because it fits none of the other three predicates: it changes no classification, no set and no routing answer. ⚠ **It was also called *"the only slice with no ordering constraint — nothing reads its output — so it may land at any point, and that independence is the evidence the partition is right"*, and round 9 falsified that by running it.** ⊕ Plant one δ-conformant edit — route an unguarded read through the validity primitive, the idiom `-common.sh:212`/`:233` already use and the `reads` row blesses — and `homes` moves `HOMES: 70 (31 code, 39 prose)` → `71 (32 code, 39 prose)`, `callsite` 2 → 3, **with the new row landing in β's class**. δ's output IS read: by the census, whose membership is §9's operative test and whose rows are the work list §3 is written against. **So the sentence this row offered as the evidence that the partition is right is withdrawn, and no replacement evidence is offered here** — the partition's correctness is an open question, raised below |
...
scopes the extraction to `^## §3 ` (`-audit.sh:316`, extracting at `-audit.sh:320`).
...
above is one**, and since `b088dacc` it also checks this memo's **quantities**: `-audit.sh:308-323` reads this
...
population print was written on the match branch — `_pop` at `-audit.sh:409-409`, joined at `-audit.sh:529-530` — so a
...
`-audit.sh:342-342` — did not hold. Draft 13 declined the fix on the ground that it would shift `homes`'s
...
kernel blocks**, so one-file-per-group merges them back into a single file — ⊕ `wc -l` over the three today (the figure is not written here: this memo's own commits have falsified it twice); ⚠ an earlier draft said 1192, which was the sum before this session's parts grew, undoing
...
`b088dacc`, extending the memo gate this program already owns at `-audit.sh:308-323`; §9 judges that commit
against its own clause. Its population defect, which draft 13 recorded as a third owed item, was fixed in
`96d8fae3` rather than carried. ⚠ **The provenance block is landed** (`-inventory.sh`, block `attest`) — ⚠ **and D18, which this sentence points at for the sequence, still headlines it *"REVERTED … It cannot land on this branch"* and carries two ⊕ figures that no longer reproduce** (⊕ `git diff 84a7bd67^ -- …-inventory.sh …A-rederive.sh` prints **147** lines, not nothing; ⊕ `selfcheck` reports **35 blocks, 25 on the roster**, not 34/24). An implementer following the pointer would revert `bd52f296`. The re-statement is D18's, not this section's; recorded here so the pointer is not read as agreement. It was landed at `84a7bd67`, reverted at `dae569d4` on a whole-output reading of §9's stopping rule, and re-landed once three review axes measured that the reading convicts the whole branch and acquits nothing; §9 grades it on membership, where it passes. Nothing is owed for it, and D18 carries the sequence. Every item the
...
| the partition is correctly drawn | **OPEN.** Raised by β (round 8) with this memo as owner; round 9 fired the trigger and measured that a **class-keyed** partition cannot express this harness's layering — §3 apportions census *classes*, the harness's seams are *what a part ranges over* (`-audit.sh:8-14`, `-inventory.sh:7-14`, `-integrity.sh:7-12`) and *where a helper's callers are* (`A-rederive.sh:41-42`). Three axes declined to say whether the remedy is a sixth slice or an owner inside the existing rows. **This memo does not get to claim the partition is settled** |
| who owns the harness's own shell-lexical structure | **NOBODY, measured.** One family: payload spans (`-audit.sh:617` and `-inventory.sh:137`, **byte-identical** regexes, with the incident at `-inventory.sh:179-183`), quoted spans (three answers), command position (six homes), and the **admission gate** `len(hits) >= 2` (`-audit.sh:231`, raw line) which decides the census's population. It sits *upstream* of every class §3 apportions. *Trigger*: before any slice edits a second member of the family |
| the `-audit.sh` seam ε's split needs | **UNASSIGNED.** ⊕ 698 lines; ε's own delta lands above the band's upper edge, so the cut cannot happen inside ε's commit |
| this memo set can absorb another correction round without a seam cut | **NO.** ⊕ `wc -l docs/plans/2026-08-citation-hygiene-harness-*.md docs/plans/2026-07-citation-hygiene-A-rederive-audit.sh` — several artifacts sit within a few lines of the authoring band, and rounds 8 and 9 each forced seam cuts to land their own corrections. ⚠ **No figure is written here: the first draft of this row carried four, and inserting the row falsified two of them.** The cause is the shape rather than the volume — ⊕ `grep -c '⚠'` on this memo against `wc -l` — because superseded text stays inline by this program's own recording discipline, so the surface each next round must read grows monotonically. *Trigger*: the next round that cannot land its corrections without a cut |
...
here and the external-state memo is its evidence**, not its home. ⚠ **And "this memo pair" is now SIX
files** (β's §1 and §2a seam cuts added the sixth) — the two splits created two of them, so an unswept transfer clause would leave both outside what it
carries.
...
⚠ **WHAT PASSING MEANS, because neither this clause nor §2's base case defined it and β ran EIGHT rounds
against an undefined terminal condition** (CRIT 7/9/6/8/9/14/12/16 — rising, not converging). A slice's
plan-review has **passed** when a round moves **none of three faces**: no finding changes a §3 rule cell, a
§3a obligation, or a §9 authorisation. Round-count, fatigue and CRIT totals are **not** inputs. ⚠ **And one
signal is about the author rather than the plan**: when a round's findings land predominantly on text the
previous round wrote, the next move is to stop authoring, not to author again — rounds 7 and 8 were that,
measured, and round 8's response was subtractive for that reason.
...
`-audit.sh:308-323`'s memo gate from §3's class-rule row keys to this memo's **quantities** was exactly that,
...
**The grades are D21's table, not a second copy here.** ⚠ **They were spelled out in this clause as well, and a second copy of a claim is not a check on the first (§8) — it is a second thing to keep true, which is what let *"the two newest"* and *"four of the eight"* go stale in it.** One verdict is stated here because it is the clause's own finding rather than a measurement: **`a5fab499` violates**, and `84a7bd67` was convicted and reverted at `dae569d4` **in error** — the whole-output test that convicted it acquits nothing on this branch and, applied consistently, convicts `b088dacc`, which this clause permits by name. The revert is undone; ⊕ `git diff 84a7bd67 -- …-A-rederive-inventory.sh` prints nothing.
...
**`473b9d56` and `9f0fe33d` fall under this clause and were ungraded until it was run; both pass.**
⚠ **An earlier draft called them *"the two newest"*, which the next commit falsifies — the same
position-dependence the ⚠ below diagnoses for a count, re-committed as a superlative. D21 carries which
commits exist; this clause carries only the criterion.** ⚠ **An earlier draft wrote *"two of the seventeen"*, pinning a count D21's own closing
...
this memo re-entered the authoring band. **The clause stands unamended**, and its precondition is unchanged:
a mechanism commit this clause permits must not be the commit that crosses the size trigger — cut the seam
while writing, at the band, so that no later split has to decide a PR's scope.
