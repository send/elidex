...
scopes the extraction to `^## §3 ` (`-audit.sh:315`, extracting at `-audit.sh:319`).
...
above is one**, and since `b088dacc` it also checks this memo's **quantities**: `-audit.sh:307-323` reads this
...
population print was written on the match branch — `_pop` at `-audit.sh:408-409`, joined at `-audit.sh:528-530` — so a
...
`-audit.sh:341-342` — did not hold. Draft 13 declined the fix on the ground that it would shift `homes`'s
...
`b088dacc`, extending the memo gate this program already owns at `-audit.sh:307-323`; §9 judges that commit
...
`-audit.sh:307-323`'s memo gate from §3's class-rule row keys to this memo's **quantities** was exactly that,
