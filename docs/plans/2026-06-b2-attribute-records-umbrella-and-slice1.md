...
- **Attribute-name HTML-namespace casing — WHOLE surface** → NEW slot `#11-attribute-name-html-namespace-casing` (Codex R3+R4). DOM §4.9 lowercases an attribute's qualified name ONLY for HTML-namespace elements in an HTML document; SVG/MathML keep their case-preserved local name (parser stores `attr.name.local` verbatim, `convert.rs`). elidex currently lowercases UNCONDITIONALLY at EVERY attribute-name-resolution site — `getAttribute`/`hasAttribute`/`setAttribute`/`removeAttribute`/`toggleAttribute` handlers (`props.rs:31/61/115`, `attrs.rs:33/65`) + the VM natives + the `Attr`/`NamedNodeMap` surface (`getAttributeNode` cache keys). **The fix is a single canonical `is_html_namespace`-aware name resolver applied at ALL those sites** — one issue, one way. **Why deferred whole (NOT partially gated in Slice-1)**: it is pre-existing (`setAttribute`/`getAttribute` lowercased pre-B2) and cross-cutting across the entire attribute IDL surface incl. reads + the Slice-3 Attr surface. Codex R3→R4 demonstrated the trap directly: gating only `removeAttribute`/`toggleAttribute` (R3) produced a `setAttribute`-vs-`removeAttribute` desync for SVG IDL (R4) — i.e. a "new seam + N legacy" **strangler** that the "One issue, one way" mandate forbids. So Slice-1 keeps ALL attribute methods at the **uniform unconditional-lowercase baseline** (consistent, no desync — R1's HTML mixed-case wrapper-snapshot fix is preserved by construction since the snapshot keys on the same uniformly-lowercased name the handler removes), and this slot owns the whole-surface unification in one plan-reviewed slice. **Known consequence at the uniform baseline (the slice resolves it)**: an SVG/MathML element's parser-set camelCase attribute (`viewBox`) is uniformly inaccessible via IDL by its original case (`getAttribute`/`removeAttribute('viewBox')` look up `viewbox`); pre-B2 `removeAttribute` was the lone raw-name exception — Slice-1 normalizes it to the uniform baseline. **Re-eval trigger**: Slice-3 (Attr/NamedNodeMap) OR an SVG/MathML mixed-case attribute WPT/site. **Date**: 2026-08-28. **Registers in `project_open-defer-slots.md` at landing.**
