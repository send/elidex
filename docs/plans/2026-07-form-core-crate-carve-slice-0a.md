...
**STAY-UP** (systems / bindings / dom-api- or script-session-coupled): `reconciler.rs`, `init.rs`, `radio.rs`, `submit.rs`(+tests), `select/mod.rs`(+tests, `elidex_dom_api` dep), `clipboard.rs`, `selection.rs`, `sizing.rs`, `label.rs`, `clone.rs` (`elidex_dom_api::ClonedFrom`), `focus_snapshot.rs`, `ancestor_cache.rs`, `inert_document.rs` (`elidex_script_session` — hard UP).

**Judgment items kept UP** (dependency-clean for DOWN, but **outside** the enumerated derivation closure → keep UP to hold 0a to "move the enumerated set only"): `selection.rs` (editing ops), `sizing.rs` (`form_intrinsic_size`). Rationale recorded in §11 so a later slice can revisit if a leaf consumer emerges.
...
- **J1 — `selection.rs` / `sizing.rs` kept UP** despite being dependency-clean for DOWN. Rationale: 0a moves the *enumerated derivation closure* only (validity/disabled/sanitize/step/datetime), holding the split behavior-invariant and minimal. Neither has a leaf consumer today (nothing below `elidex-form` needs intrinsic-size or editing-ops). Revisit if/when a lower crate needs them. Is "minimal enumerated set" the right boundary, or should dep-clean modules move eagerly? (Recommend: minimal — a crate boundary should be pulled by a consumer, not pushed speculatively.)
