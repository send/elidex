...
- **J5 — hand-off to Slice 0b** (`/simplify` A4, an *edit* not a move → out of 0a's contract): `reconciler.rs:240` hand-rolls what `FormControlState::update_pattern` (form-core) already does — provably identical on both arms. Calling `fcs.update_pattern(new_value)` inside the existing `!=` guard would drop **both** the `compile_pattern_regex` `pub` widening and its `pub(crate) use` re-export line. This is exactly Slice 0b's "IDL hand-mirror 削除" class. Until then, `compile_pattern_regex` stays `pub` in form-core with a `pub(crate) use` alias in `elidex-form` (note: the alias hides only the `elidex_form::` path — the item is workspace-reachable via `elidex_form_core::` regardless, so the widening is real until 0b removes it).
