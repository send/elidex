...
|---|---|---|---|---|
| A-i | The shared spec-label map | `webref-cite-audit-tool` (current) | Create `.claude/tools/_webref/spec_labels.py` **pinned-map-only**; collapse the three hand-maintained copies (`preflight.SPEC_LABEL_REVERSE`, `coverage_map._SPEC_LABEL_MAP`, `cli.COMMON_SHORTNAMES`) onto it; the `DESIGN.md` bullet; move the consumer-derivation assertion off the tools package. **Pure refactor — no gate semantics, no CI topology.** | A-ii's whole subject is the failure mode this import *creates*. If A-ii landed first there would be nothing to fail closed; if B landed the import, `main` would carry a fail-open plan-review gate for the duration of B. |
