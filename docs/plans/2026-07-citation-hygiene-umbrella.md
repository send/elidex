...
|---|---|---|---|---|
| A-i | The shared spec-label map | `webref-cite-audit-tool` (current) | **Generic tree, plus one comment in the adapter.** Create `.claude/tools/_webref/spec_labels.py` **pinned-map-only**; point `coverage_map` and `cli` at it; the `DESIGN.md` bullet; author the generic-tree `test_spec_labels.py`; rewrite every consumer list and rationale naming an elidex file path (by role) or a Slice-B artifact, and author the copy-count claim at the five sites that carry it. ⚠ **The K2 enforcement is NOT in this row — it is slice A-i-wire below** (#501 design re-gate 5). It was added here at review round 68, *after* A-i's plan-review, and carried the wire (**a figure that was quoted here as "1089 lines" and is retired** — it conflated the scanner with its controls, and A-i-wire's memo §0 derives both halves with a command instead), its `REQUIRED_WIRES` registration and edits to `ci.yml` and `CLAUDE.md`; CLAUDE.md makes plan-review a rule for work of that shape, and the base-case clause that lets an approved slice be one PR does not reach an instrument added after the approval. A-i-wire carries it with its own plan-review. ⚠ **Amended at landing per A-i §13 item 2 (the four scope-grant clauses), measured against the `origin/main` base** — this row used to say *touches no adapter file* (A-i §4.1 rewrites one three-line comment in `preflight.py`, nothing else), *delete the 8 inert parse aliases* (`origin/main` has none; A-i **omits** them), *move the 8 label-map tests* (`git grep -lI '_SPEC_LABEL_MAP\|COMMON_SHORTNAMES' origin/main -- '*test*'` → empty; A-i **authors** them) and *correct the copy-count claim at five sites* (`origin/main` carries none under `.claude/`; all five are **authored**). §9 barred A-i from amending its own approval boundary mid-review; the amendment narrows and relabels, it grants nothing (Codex R10). ⚠ **`preflight.py`'s copy migrates in A-ii, not here** (revised 2026-08-01 after A-i round 2). Two drafts tried to land it in A-i and both regressed the gate, in opposite directions — measured against `origin/main` with the tools tree absent: a **guarded** import takes default mode `exit 1 → exit 0` (fail-open), and a **hard** import takes `--no-verify` `exit 0 → traceback`. Preserving both requires a capability check at the verification stage suppressed by `--no-verify`, which *is* A-ii's act-site 1. **The gate's copy is not separable from the gate's failure semantics**, so it goes with them. A-i therefore collapses two of the three copies and A-ii the third; K1 completes across the pair. | A-ii's whole subject is the failure mode this import *creates*. If A-ii landed first there would be nothing to fail closed; if B landed the import, `main` would carry a fail-open plan-review gate for the duration of B. |
| A-i-wire | The K2 trip-wire | `webref-generic-core-trip-wire`, stacked on A-i | **The slice memo is the one list, and this row does not restate it**: `docs/plans/2026-09-citation-hygiene-Ai-wire-k2-trip-wire.md` §4 = the artifacts, §8 = the defer slots and the declared blind spots. ⚠ This row used to copy both — the file list, a slot verdict and a blind-spot count — and was stale on all three once the memo moved on (Codex PR519 R8). **Carved out of A-i at #501 design re-gate 5** — see that row. ⚠ Its plan-review owned two decisions A-i could not make, and **both are answered in that memo**: (a) how a required, ungated wire records what it cannot see — §8, and (b) **the contention with open PR #510** — answered **no, the required ungated wire set may not require an interpreter** (§6), on the ground that A-iii's row below already plans an ungated `tools` job *with* an interpreter floor, so each job gets one rule. ⚠ That answer is **raised to #510, not imposed**: its write path is in that PR's files. The *budget* half (what the always-run job may **cost**) is separate and is NOT settled by it — A-i-wire re-derives only the `timeout-minutes` line it is itself spending, with the derivation at that line. | K2's closed part is unenforced until it lands: A-i discharges K2 by diff review, which is finite but not a gate. Stacked rather than deferred for that reason. |
| A-i-launch | The trip-wire launch contract | new, cut from `origin/main` (no dependency on A-i / A-i-wire) | **Every required wire's environment, closed where bash is started** — the `trip-wires` CI job's command line and the `mise` task, not inside any wire: a bash script cannot close what bash consumes at startup (`BASH_ENV`, `SHELLOPTS`, `BASHOPTS`, `BASH_FUNC_*`; measured in A-i-wire's §11 plan-review). The driver confirms positively that it was launched that way and refuses loudly otherwise. Owned open defects, carved from A-i-wire (its memo §11.1): **D1** `GREP_OPTIONS` (BSD grep honours it; a **local** false green on macOS, CI's GNU grep ignores it), `GIT_TEMPLATE_DIR`, `GIT_TRACE*` and trace2 config, `CDPATH`, `umask`, the `GIT_CONFIG*` channel (any config key, `core.fsmonitor` included), `XDG_CONFIG_HOME`/`SUDO_UID` for `safe.directory`, and the driver's own `TRIP_WIRES_SELFTEST` (entered from the environment). A gate subverted by the environment of the person running it is declared out of scope; an environment that makes a gate lie by accident is not. ⚠ `scripts/trip-wires.sh` belongs to no lane, and #510 and `stale-claim-detector` both edit it — this slice's plan-review settles the order with them. | The class covers every required wire, not one: `GREP_OPTIONS` changed a sibling wire's verdict too. Independent of the slices around it, so it may land in any order; until it does, D1 stays open and is named in A-i-wire's PR. |
...
| A-i-wire | `2026-09-citation-hygiene-Ai-wire-k2-trip-wire.md` | on **#519**, in this checkout. Its `preflight` EXIT **1 is by design** — the same no-spec-surface declaration A-iii and C make, and for the same reason (A-ii's §4.2.5 is not landed). ⚠ Listed here because the repo-half command below runs preflight over **every** tracked plan memo; a by-design exit 1 with no row to explain it reads as a regression |
...
(`git grep -hF urlopen origin/main -- '.claude/tools/_webref/test_*.py' | wc -l`): **0 `urlopen` calls**. The figure came from a
...
- **`docs/plans/2026-07-citation-hygiene-A-rederive.sh` was shared and owed a split.** ✅ **Moot — the
  ...
  enforcement, is the trip-wire — registered in `scripts/trip-wires.sh`'s `REQUIRED_WIRES` and run on
  every PR to `main`. ⚠ Since #501 design re-gate 5 it ships in **slice A-i-wire**, not A-i.
...
- **Slice A-ii changes `preflight.py`'s failure semantics** from silent-exit-0 to fail-closed. Every lane runs that gate. A-ii's landing checklist must re-run preflight on every worktree that authors a plan-memo and record the result. ⚠ **The worktree set is machine-local and is not derivable from a landed tree** — it exists in no commit, so a checklist command here cannot produce it and a hand-written list goes stale (the one this bullet used to carry named `elidex-wt-c4fix`, which does not exist, and omitted `elidex-wt-submittable` and `elidex-wt-tripwire-ci`, which do). Split the obligation: the local half is `git worktree list`, run by A-ii's author at landing time; the **repo** half — every *tracked* plan memo still passing the fail-closed gate — is not a checklist item at all but a command over the landed tree, `git ls-files 'docs/plans/*.md' | xargs -n1 python3 .claude/skills/elidex-plan-review/preflight.py`, which is CI-shaped and is where A-ii should put it. ⚠ **This bullet named `docs/plans/2026-07-citation-hygiene-A-rederive.sh lanes` until #501 R66** — a file the harness drop removed, twelve lines below a bullet that says so.
...
- **CI topology is decided across two lanes, not one.** The Layout lane's [PR #496](https://github.com/send/elidex/pull/496) lands an **ungated** trip-wire job and argues in-file that gating `.claude/tools/**` behind a path filter makes the tamper path of an allowlist gate itself an allowlist entry. Slice A-iii **adopts that shape** rather than adding a competing filter — one question, one answer. ⚠ **"Whichever lands second is a textual merge, not a decision" was false and is withdrawn** (#501 design re-gate 5): open PR [#510](https://github.com/send/elidex/pull/510) registers a wire that requires `python3` and raises the `trip-wires` job's `timeout-minutes` from 2 to 10, while the A-i-wire slice's rationale rests on the wire set needing **no toolchain**. That is a design decision about what the ungated job may cost, and A-i-wire's plan-review owns it. The sentence was written from a hand-listed set of contending branches; the derivation over every local branch returns a third, and it is the one with the open PR. ⚠ **ANSWERED (2026-09-21, A-i-wire §6): NO — `trip-wires` keeps a no-interpreter floor and Python checks belong in A-iii's ungated `tools` job**, so each job carries one rule instead of two jobs carrying the same one. Raised to #510 ([comment](https://github.com/send/elidex/pull/510#issuecomment-5757095202)); **not** imposed, since the write path is in that PR's files. ⚠ **The budget half is still open and is nobody's yet**: what the always-run job may *cost* has three claimants (#510, `stale-claim-detector`, A-i-wire), and A-i-wire re-derives only the `timeout-minutes` its own controls spend. Whoever lands a fourth heavy wire inherits the question.
