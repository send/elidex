...
| A-i-wire | The K2 trip-wire | `webref-generic-core-trip-wire`, stacked on A-i | `.claude/tools/webref-generic-core-trip-wire.sh` + `…controls.sh`; the one line registering it in `scripts/trip-wires.sh`'s `REQUIRED_WIRES`; the `trip-wires` rationale in `.github/workflows/ci.yml` and the paragraph restating it in `CLAUDE.md`. **Carved out of A-i at #501 design re-gate 5** — see that row. ⚠ Its plan-review owned two decisions A-i could not make, and **both are now answered in that memo**: (a) whether a required, ungated wire may declare blind spots without registering them as slots — **no slots** (§8: a slot records work owed, a blind spot records the reach of a predicate; the list is **five**, not the four this row used to say), and (b) **the contention with open PR #510** — answered **no, the required ungated wire set may not require an interpreter** (§6), on the ground that A-iii's row below already plans an ungated `tools` job *with* an interpreter floor, so each job gets one rule. ⚠ That answer is **raised to #510, not imposed**: its write path is in that PR's files. The *budget* half (what the always-run job may **cost**) is separate and is NOT settled by it — A-i-wire re-derives only the `timeout-minutes` line it is itself spending (2 → 5, with the derivation at that line). | K2's closed part is unenforced until it lands: A-i discharges K2 by diff review, which is finite but not a gate. Stacked rather than deferred for that reason. |
