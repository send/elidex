...
| A-ii | The gate's copy **and** its failure semantics | new, stacked on A-i | Migrate `preflight.SPEC_LABEL_REVERSE` onto `spec_labels.py` **together with** the fail-closed capability verdict, both act-sites, the four remedy strings, the no-spec-surface declaration, `SKILL.md`'s contract of record, the two gate-output strings that name the deleted symbol, and the shared fence tracker — new `fences.py`, `grep_pass.py` moved onto it, `test_grep_pass.py` P11g (A-ii §4.2.5/§4.3) | Every lane runs this gate. It must be correct before anything downstream relies on its verdict. |
