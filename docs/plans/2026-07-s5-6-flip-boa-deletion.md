...
- **IDB versionchange, emit + receive TOGETHER** (B6 + B21, IndexedDB-3 §4.2 *fire a version change
  ...
  correlation `request_id` **is minted in the ENGINE at the enqueue site** (the VM request's
  `ObjectId` value, carried on `IdbVersionChangeRequest.request_id`) — the request identity is gone
  by drain time, so the shell cannot invent it after the fact; the shell echoes it back
  (`IdbUpgradeReady` / `IdbConnectionsClosed`) to unblock the opener. (This corrects the earlier
  "shell mints it, never crosses into the engine" plan text — the correlation would break under that
  ownership. The `request_id`-CONSUMING blocking round-trip stays deferred to `#11-idb-connection-queue`;
  the id itself is engine-minted now.) Likewise the versionchange payload carries the
  identity-preserving storage-key `origin` (`storage_origin_key`, §above) so the origin-keyed shell
  broadcast targets same-origin tabs without aliasing opaque documents. The boa-inherent
...
| B6 IDB emit drain | open-with-higher-version enqueues `IdbVersionChangeRequest {origin, request_id, db, old, Some(new)}` (`origin` = identity-preserving `storage_origin_key`; `request_id` = the VM request `ObjectId`; a fresh 0→1 creating open included); `deleteDatabase` enqueues `{origin, request_id, db, old, None}` and a nonexistent-db delete enqueues NOTHING (§5.3 step 4 gate); drain shape/order pinned |
