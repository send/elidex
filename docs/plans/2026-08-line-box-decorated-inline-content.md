...
the **predicate** PR (§9) is the third required before PR-1a, and the **end-of-line
white-space**, **min-content** and **reconciler** PRs are all required before PR-1b (ledger
**A80**, **A88**). The memo does **not** re-anchor
...
behaviour** of §6's cells — 49 at rev 34, **58** after the amendments recorded next, and from there **no total is restated**: `python3 .claude/tools/plan-xcheck.py <memo>` prints it (ledger **A87**). What is history rather than a total is which revision added which cell: R22's 10c, R26's 25b, rev 60's 10d and 17g, rev 65's 25c, rev 66's 3c and 25d, rev 67's 12g, rev 68's 25e, rev 70's 24g, rev 71's 25f (ledger A54, A59, A60, A70, A71, A72, A76, A79, A86, A90); rev 63 adds none (R2 adds
...
cumulative design re-gate it does not silence either — and *reopened* three times besides, by
decision rather than by amendment: M4 at rev 60 (ledger **A60**), M3's hang gate at rev 63
(**A63**), and M4's flush-time **emit rule at rev 71** (2026-09-26, ledger **A92**: at a forced
break the hook emits for an open box whose cursor span is empty, because css-inline-3 §2.1 splits
a box containing a forced line break into fragments, css-break-3 §2 makes each a *box fragment*
and cssom-view-1 asks a rect of each "including those with a height or width of zero"; OQ-1's
soft-wrap answer is untouched — there the box is not on the earlier line at all). Each is recorded
in its own paragraph above rather than in this count** (Codex R2, R3 then R4 on #515, 2026-09-20 —
...
* **PR-1a — item stream. Behaviour-neutral.** Owns coupling 1×2 (both its rows): M1 + M2. Marker
  ...
  until PR-1d. **Characterization tests for §6 cells 1, 2, 5, 6b, 6c, 6f, 6d, 6e, 6i, 6g, 6h, 7–12, 12d and 12g land here
...
3c. **`border` alone, in the advance and in both intrinsic sizes** (M3, M8; R32) — the one
    ...
    every other M8 cell — 25, 25b, 25c, 25d, 25e — carries padding or margin and no border. Two markups of its own, because the two
...
25f. **An atomic's boundary keeps two texts in separate min-content candidates** (M8; R38) —
    ...
    `measure_width` (`tests/mod.rs:31`) and comparing, **not** `measure_width("ab")`. css-text-3
...
25e. **Marker-only content still contributes its edges to both intrinsic sizes** (M8; R35) —
    ...
    intrinsic cell, since 25, 25b, 25c and 3c all carry text and 25d's marker-only case is
    negative and floors to 0 either way (ledger **A79**). ⚠ This is §4.2's **Shape B**, the shape
...
25d. **Signed margins in intrinsic sizing: the used width floors at zero, and the margin is not
    ...
    * **the floor** — `<div style="display:inline-block"><span style="margin:-10px"></span></div>`,
      ...
      this markup. They are not undecidable — css-sizing-3 §2.2's "Intrinsic size contributions are
      based on the outer size of the box" licenses the negative sum, and M8's arithmetic gives
      `−20` — but **where the floor is applied is PR-1b's**, and a pass-level value asserted here
    ...
    * **the discriminator** — the arm below, which is what separates the intended implementation
    ...
    (`tests/mod.rs:31`) rather than stated as a literal. **Dropping margins from M8** reports
...
17c. **A box opened at a soft wrap has no rect on the line before it** (M4; rev 60, ledger
    ...
    **registered deviation** §3's css-text-3 §5.5 *adjacent soft wrap opportunity* row marks ✗
    ...
    which is why the rev-71 forced-break amendment to M4's emit rule leaves this cell untouched:
    there the break is *inside* the box and a fragment exists, here it is not, ledger **A92**);
...
17d. **`getClientRects`, both channels, one asserting a divergence** (M4's ⚠). Two markups:
    ...
    **two** `line_rects` — ⚠ an earlier drafting stated the fit test with the *untrimmed* width,
    ...
    §5.5 putting the break at the box's margin edge, and the rev-71 forced-break amendment (A92)
    not reaching it — `line_rects.len() == 1`, and `getClientRects` takes
...
17g. **A forced break inside a box whose own content has no run across it** (M4; A55's
    ...
    **Asserted**: the outer span's `line_rects.len() == 2`; line 1's rect starts at the outer's
    ...
    How the producer records line 1's zero-span entry is PR-1c's; **that it exists** is M4's, by
    the rev-71 freeze amendment to the emit rule (ledger **A92**) — before it, the hook's
    non-empty filter made this cell's `len == 2` unreachable under the stated mechanism, which is
    the defect the amendment answers.
...
* **Every reader of an inline `LayoutBox`.** **Three** distinct changes, in three different PRs:
  1. ⚠ **Position — PR-1b, and the re-slice's shippability argument does not cover it.** M3's
     ...
     the run its own entity carries (`pack/mod.rs:706-714`, `pack/boxes.rs:56-81`) and PR-1b moves
...
**PR-1a** (item stream, behaviour-neutral; cells 1, 2, 5, 6b, 6c, 6d, 6e, 6f, 6g, 6h, 6i, 7–12, 12d and **12g**): both marker variants reach `pack/items.rs`; every
...
every non-`Text` variant, so it cannot observe a marker; cells 1, 2, 5, 6b, 6c, 6f, 6d, 6e, 6i, 6g, 6h, 7–12, 12d and 12g land
...
16, 16b, 25, 25b, 25c, 25d, 25e and 25f. **It owes three dependencies**: it is not cut from `origin/main` until the
...
ledger **A58**), and cell 16 asserts step 3's removal (`white-space: normal`), which the **end-of-line white-space** prereq — the first of the three to land — makes conformant.
...
**PR-1d** (existence): cells 18, 19, 20, 21, 22, 23, 23b, 24, 24b, 24c, 24d, 24e and 24g, plus the flip set — **cells 1, 2, 7, 8, 10, 11, 12, 12d and 12g flip; 5, 6b, 6c, 6f, 6d, 6e, 6i, 6g, 6h and 9 do not**; this is the one normative statement of it — every §7
...
instances are enumerated below and **no total is restated** (ledger **A87**'s rule): all but the
first are the one defect, the segmentation — seen across items (2), inside one run (3, 4), in the
other pass (5), at an atomic's boundaries (6) and at a forced break (7) — while 1 is the edge term
PR-1b adds on top of it (ledger **A58**, **A64**, **A68**, **A90**, **A91**).
...
are **pending** and are ordered against a later PR rather than PR-1a — all three against
PR-1b: the **end-of-line white-space** and **min-content PRs** in that order, the first before the
second (ledger **A69**), and the **reconciler PR** beside them (ledger **A80**), so each may land
any time before PR-1b and after whatever it follows. ⚠ Whether the
