...
instruction.** The program has **five** prereq PRs (§8), and **two** of them move code: the
...
*Collapsing margins* pair and the count was not re-derived. §9's CSS 2 cite list runs to **nine**
sections because it counts every CSS 2 section this memo *cites*, produced by its own grep; the
other two, CSS 2 §9.5 (§3's css-inline-3 §2.1 row, in a cross-reference) and CSS 2 §16.6.1 (§1.6),
are cited without a title, so they carry no section↔title pair to
...
eight; the same command on this revision → nine). The
**seven** above is unaffected: seven titled pairs, two untitled cites, nine sections.
...
behaviour** of §6's cells — 49 at rev 34, **58** after the amendments recorded next (R2 adds
...
(`.claude/tools/plan-xcheck.py:135-145`) fails a row whose Touch names a `#11-` slot while the
mark starts `✓` (`:140`), and a `✗` whose Touch names no slot, no `PR-`, no `M<n>` and no §9
(`:141-142`). That second rule is also why no row below can be read as the *enumeration*
...
| CSS Sizing 3 §5.2 Intrinsic Contributions | min-content | inline-axis edges occupy space in the widest unbreakable segment | M8 — the **min-content prereq PR**, in `main` before **PR-1b** (§8) | ✓ | yes |
| CSS Sizing 3 §5.2.1 Intrinsic Contributions of Percentage-Sized Boxes | rule 4 | "For the min size properties, as well as for margins and paddings (and gutters), a cyclic percentage is resolved against **zero** for determining intrinsic size contributions" (`body css-sizing-3 cyclic-percentage-contribution`) | ⚠ **the gap is in this memo rather than (yet) in the engine: §8 threads `containing_inline_size` through every `collect_inline_items` caller and never says what the intrinsic passes thread, which is exactly what this rule decides.** The callers are four (`git grep -n "collect_inline_items(" 154bac3f -- crates/`), one of them a test helper, and **two are the intrinsic passes** — `min_content_inline_size` (`inline/measure.rs:22`) and `max_content_inline_size` (`:51`) — where the containing inline size is the quantity being computed. §6 cell 4 resolves `padding: 5%` against a real containing size in the layout pass and cell 25 uses a fixed `10px` under shrink-to-fit; neither is their intersection. Owned by the **min-content prereq PR** and **PR-1b**, the two that put an edge term into those passes, and answered in their own plan-reviews (§8 mandates one each) rather than here | ✗ (owner named: the min-content prereq PR and PR-1b) | yes |
...
CSS Sizing 3, CSS Writing Modes 4, CSS Display 3, CSS Pseudo-Elements 4, CSS Content 3, CSS Backgrounds 3, CSSOM View 1, Resize Observer 1), M=48 entries (`Split decision` below restates K; both are recomputed) — ⚠ **M read 42 until R20**, whose enumeration-completeness audit (§9) added six rows; K is unchanged, every section those rows cite belonging to a module the table already carried. Both figures are recomputed
...
| **M8** | Intrinsic sizing | **Both intrinsic passes account for the edges, and the min-content half is a prerequisite PR ahead of PR-1b rather than a deferral** (ledger **A47**; §8's *Min-content prereq PR*). `max_content_inline_size` is **PR-1b's** (`inline/measure.rs:44`): each marker adds its inline-axis edge sum to the running total, matching that pass's existing `total +=` shape. `min_content_inline_size` is the prerequisite's, because it has **no running candidate and no cross-item joining** — it is `max_word = max_word.max(m.width)` per word per item (`:15-37`), so a box's edges have nothing to attach to, and the same absence already loses run boundaries (`a<b>b</b>c` yields `max(\|a\|,\|b\|,\|c\|)` today, never `\|abc\|`). ⚠ **That was read here as a ground for deferring it and is not one**: it measures what the fix *costs*, not whether PR-1b is correct without it. | Deferring either intrinsic half past PR-1b ships an inconsistency between **layout and intrinsic sizing**, not between the two intrinsic sizes. `shrink_to_fit_width` is `min(max_content, max(min_content, available))` (`elidex-layout/src/intrinsic/mod.rs:134`), fed by `intrinsic/block.rs:39`/`:49` and consumed at `elidex-layout/src/layout/mod.rs:57` for an `auto`-width inline-block: at a small available width it settles on the **word** width, while PR-1b's line needs word + edges, and the box overflows. css-sizing-3 §5.2's note that the spec "does not define precisely how to determine these sizes" licenses a choice of *procedure*; it does not license an intrinsic size the engine's own layout cannot fit into. |
...
| `crates/layout/elidex-layout-block/src/inline/measure.rs` | `max_content_inline_size` (`:44`) — M8's contribution. `min_content_inline_size`'s **accumulator** — `max_word` at `:23`/`:31` — is **not this program's**: the min-content prereq PR builds it, in `main` before PR-1b (§8), and PR-1b re-anchors against that base. The function itself (`:15-37`) PR-1b does touch, because its `collect_inline_items` call (`:22`) takes the new argument like every other caller. |
...
* **PR-1b — inline-axis advance.** Owns couplings 7×2 and 7×(intrinsic): M3 + M8, **plus M5's
  ...
  both read it — **per marker side**, the derivation site producing the two halves of which the
  ...
  inline-axis advance, shaping break at a decorated boundary, the hang gate, and the
  max-content contribution. §6 cells 3, 3b, 4, 12b, 12c, 12f, 12e, 14, 14b, 15, 15b, 15d, 16,
  16b and 25 land here — cells 3, 3b, 4, 12b, 12c, 12f and 12e among them because each asserts a
  *payload* fact
  ...
  * **Shape B never reaches the packer at all**: PR-1a holds `items.is_empty()`
  ...
  min-content half of M8 — and that slot is **withdrawn, not re-tagged**: the work is ordered
  *before* PR-1b as a prerequisite (§8's *Min-content prereq PR*; ledger **A47**), so the gap is
  never open and there is nothing to register. **No close is owed either**: the slot was never in
...
* **PR-1c — box geometry.** Owns coupling 3×7: M4. The open-box stack and its flush-time
  ...
  **a11y node bounds** (`elidex-a11y/src/tree.rs:121-125`, an unconditional `border_box()` read).
  ...
  point hits the `<p>`. §6 cells 6, 10b, 10c, 13, 13b, 13c, 13d, 14c, 15c, 17, 17b, 17c, 17d and
  17f land here.
...
Own deferrals **per PR** (the policy's unit), for all **nine crate PRs** of the program (§8's two
...
⚠ The **reconciler prereq** (R15's carve, registered here at R16; §8) and the **min-content
prereq** (R17's carve, registered here at rev 53; §8) take the same treatment for the same
...
until then** — reads **six** of this program's nine `opens` statements, not
nine. ⚠ **It read five until rev 34, and the two it missed failed for *different* reasons — one
...
`predicate prereq` is unmatched by name — and `reconciler prereq` (R16's carve) and
`min-content prereq` (R17's) are the **second and third** names it does not reach, which is the
...
for PR-1a, PR-1b, PR-1c, PR-1d, the seam-3 prereq and the dead-arm prereq — PR-1b's "none" against
no row is as live a pair as a "1" against one — while `predicate prereq`, `reconciler prereq` and
`min-content prereq` still hold in the reverse direction alone: an `(own)`-tagged §10 row naming
...
closed world is the reason none of the three carves takes a §10 row of its own**: its prereq
alternation is the same closed three (`(seam-3|dead-arm|predicate) prereq PR`), so a row tagged
to a fourth or fifth prereq routes to nowhere by the checker's own reckoning (`:339` admits only
those three; `:355` is where the fourth tag fails). The precedent is already the predicate
prereq's — it carries no §10 row, its registrations sitting in §5.3, §8 and §9 — and the
reconciler and min-content prereqs follow it; what §10 does carry for the reconciler is the
...
R20's enumeration-completeness audit, §9) and the
dead arm's own disposition are pre-existing class, so none of them enters a per-PR count. ⚠ **Three
of them were added at R10, two of those having been missing before it — and the last twelve, R11's,
R12's, R15's three, R16's and R20's six, were added to this list in the same revision that opened them, which is the
...
3b. **Cancelling pair, the sum/disjunction contrast on the advance side** — the edge pair is
    ...
    **disjunction**. The cell carries **two markups of its own**, because its two observables are
    not visible on one — a shaping break needs two same-entity text runs, and a hang needs a
    collapsible space that the box's inline-**end** side follows:
    ...
    * **hang** — `<p style="text-align:center"><span style="margin:-10px;padding:10px">abc </span></p>`:
      the trailing space is inside the box and the `InlineBoxEnd`'s **own** inline-end components
      are non-zero, so M3 zeroes `current_line_last_hang` — again at zero advance.
    ⚠ **The side is load-bearing, and putting the pair on the inline-start side alone inverts the
    assertion**: with `margin-left:-10px;padding-left:10px` the `InlineBoxEnd` contributes nothing
    after the space, M3's side-specific gate passes `hang = None`, and the hang is left standing at
    the space's own width rather than zeroed. That is cell 16's arm (a), not this cell, and it is
    why the pair here is all-sides.
...
14b. **The gate's negative arm** — `<p>a<span style="padding-top:1px"></span>b</p>`: a marker
    ...
    coalesce, the hang is untouched
    (M3's `hang: None` path — the same arm cell 16's **(a)** reaches from the other side, at an
    `InlineBoxEnd` whose own set is zero; ⚠ this pointed at "cell 16's contrast" until R5, and
    after R2 that contrast is between two arms **both** carrying an inline-axis edge, so it no
    longer names a no-edge case at all) and **`b` is not displaced — the cursor advance
...
16. **`text-align`, and *which side* of the box follows the space** (css-text-3 §4.1.2) — a
    **contrast pair**, because M3's hang gate is side-specific and a symmetric fixture cannot tell
    that gate from the whole-box one (the fixture this cell carried until rev 36 was symmetric; the
    freeze amendment records it). Both arms are centred at the same width `W`, hold the same text
    and advance the cursor by the same 10px; they differ only in which marker carries the edge.
    * **(a) no edge after the space** — `<p style="text-align:center"><span
      style="padding-left:10px">abc </span></p>`: the trailing space is *inside* the box and the
      `InlineBoxEnd` contributes no inline-end advance, so the space is still at the end of the
      line and css-text-3 §4.1.2 **step 3** ("A sequence of collapsible spaces at the end of a line
      is removed, as well as any trailing U+1680 OGHAM SPACE MARK whose `white-space` property is
      `normal`, `nowrap`, or `pre-line`") applies to it. The hang stands, and the run's
      `inline_start` is `10 + (W − 10 − measure_width("abc")) / 2`.
    * **(b) an edge after the space** — the same markup with `padding-right:10px`: the box's 10px
      now *follows* the space, so the space is no longer line-final, step 3 no longer applies to
      it and step 4's hanging — which acts on what step 3 leaves — is not reached either. M3
      zeroes `current_line_last_hang`, matching the engine's `top_group_is_line_last` convention
      (`inline/pack/mod.rs:264-279`), and the run's `inline_start` is
      `(W − 10 − measure_width("abc ")) / 2`. The premise is css-inline-3 §2's "inline-axis
      margins, borders, and padding are respected between inline-level boxes": the marker's
      advance *is* line content after the space.
    ⚠ **What makes the pair discriminate**, stated rather than assumed: `place_item` rewrites
    `current_line_last_hang` unconditionally (`inline/pack/mod.rs:701`), so among the markers that
    follow the text only the **last** one decides — here the `InlineBoxEnd`. In (b) its own
    inline-end set is non-zero and both gates zero the hang, which makes (b) the control. In (a)
    its own set is zero, so the side-specific gate leaves the hang standing while the whole-box
    disjunction — true for this box at *both* markers — clears it: (a)'s **two readings** differ
    by `(measure_width("abc ") − measure_width("abc")) / 2`, and that is the discriminating
    quantity. ⚠ The clause "and so do (a) and (b)" stood here until R5 and mis-distributes the two
    arms' own formulas, which are right: subtracting them gives
    `10 + (measure_width("abc ") − measure_width("abc")) / 2`, because (a)'s run starts after the
    box's 10px inline-**start** edge and (b)'s does not. The arm-to-arm difference is therefore
    not the discriminator; arm (a)'s two readings are. Expectations are
    computed with the harness's `measure_width` (`tests/mod.rs:31`), as the crate's justify tests
    do. ⚠ Both arms are LTR `horizontal-tb`, where `padding-left`/`padding-right` are the
    inline-start / inline-end pair; the mapping itself is cell 12e's.
...
16b. **Paired contrast** — `<p style="text-align:center">abc </p>`, either arm of cell 16 with the
    span removed: the trailing space must still hang, exactly as today. Pins that M3's zeroing is
    scoped to the marker and does not leak.
...
25. **Shrink-to-fit, both intrinsic sizes** (M8): a `float: left` / `display: inline-block`
    ...
    wider than the same box without the padding — PR-1b's own contribution — **and** a
    **min-content** size 20px wider, which the min-content prereq PR put in `main` ahead of PR-1b
    (§8). ⚠ **Both halves, because shrink-to-fit reads the pair**
    ...
    (ledger **A47**).
    ⚠ **The ordering this half rests on is under a recorded defect** (R22-e; §8's *Min-content
    prereq PR*): at that PR's own base the markers carry no `EdgeSizes`, so which PR delivers
    the min-content 20px is open, while the max-content half is PR-1b's either way.
...
10b. **Nested boxes, stack depth > 1** (M4) — `<p>a<span style="padding:5px">b<span
    ...
    **stack** rather than one open slot. ⚠ **Cell 10c is the other PR-1c cell with depth > 1**
    (R22), and the containment clause above is a fact about *this* cell's all-positive edges
    rather than about the class: 10c is the depth-2 case where the inner span's advance is
    negative and the outer's rect does not enclose it.
10c. **Nested negative edges invert the outer box's rect, and nothing downstream normalises it**
    (M4; R22-c) — `<p>a<span style="padding-top:1px"><span
    style="margin-left:-20px;padding-left:1px"></span></span>b</p>`. The outer span's own
    inline-axis edges are zero, so M4 saves its content-start at the cursor unmoved; the inner
    marker's inline-start sum is `-20 + 1`, which M3 applies **signed** (its shaping break reads
    the same pair "on non-zero **components**", so a negative value is no special case there), so
    the outer box's end-cursor sits **behind** its own content-start and the entry `InlineBoxEnd`
    pops has `inline_end < inline_start`. The ordering then survives every step to the component:
    `commit_aligned_entity_rects`' fold takes `min`/`max` only where an entity **already** has an
    entry on that line (`pack/mod.rs:481-486`), and the outer span owns no run of its own — the
    `a` and `b` runs carry the `<p>`'s entity — so its sole entry is pushed verbatim (`:485`);
    `or_insert` copies it (`:505-511`); and `assign_inline_layout_boxes` writes
    `inline_end - inline_start` into `LayoutBox.content` with no clamp (`pack/boxes.rs:76`,
    `:81`), which `Rect::new` stores as given (`layout_types/rect.rs:141-146`). The construction
    is an **ordering**, so it carries no `measure_width` relation and does not turn on the
    harness's font, unlike 17c's.
    ⚠ **What this cell asserts about the width is the open half, and leaving it open is the
    record.** `LayoutBox`'s own doc contemplates a negative extent — "negative margins can produce
    a `Rect` with negative width or height" (`layout_types/boxes.rs:146`) — but it says that of
    `margin_box()`, a box *derived* at the read, not of the stored content rect the whole
    `border_box()` reader family §5.3 enumerates derives from — `getBoundingClientRect` /
    `offsetLeft`, hit-testing (`hit_test.rs:165`'s `bb.contains`) and a11y node bounds — plus
    the `border_box()` a relatively positioned inline's background is painted at (cell 13d). Which side
    encloses what, and whether anything normalises at all, is PR-1c's plan-review's; this cell is
    the construction that makes the question observable, and it pins neither answer.
    ...
    zero-width rather than inverted. Depth > 1 is what makes the inversion reachable at all, which
    is why this cell and 10b are the two PR-1c cells that have it.
...
17c. **A box opened at a line end** — `<p style="width:60px">aaaa <span style="padding:5px">cccc</span></p>`:
    ...
    **must** still be rebased to 0 or line N+1's rect comes out inverted (M4). The shape, as a
    relation the test computes with the harness's `measure_width` (`tests/mod.rs:31`) rather than
    from literals: `measure_width("aaaa ") + 5 ≤ 60`, so the start marker is placed after
    `"aaaa "`, advances by its 5px start edge and **stays on line N** — which is what makes this
    cell's shape reachable — while `measure_width("aaaa ") + 5 + measure_width("cccc") > 60`, so
    `"cccc"` opens line N+1. Line N's open-box stack entry therefore spans content-start → flush
    cursor = **zero**, and M4's flush-time emit — non-empty spans only — produces nothing, which
    is the arm M4 names this cell for (the *other* rule, `InlineBoxEnd`'s unconditional push, is
    cell 14c's); line N+1's entry must be rebased to 0, or its rect runs from the line-N cursor
    down to the fragment's own width — inverted, which is the whole assertion.
    `line_count` is **2 before and after PR-1b**: the 5px start edge does
...
19. **Decorated inline adjacent to a forced break — a non-regression cell, and only one of its two
    ...
    `offset >= text.len()` return at `:31`; the packer's caller is `pack/items.rs:74`, stated at
    ...
    statements agree. ⚠ **The filter was credited to `pack/items.rs:74` until R22**: that
    line is where the packer *calls* the finder and splits the run on what comes back, while the
...
* `current_line_last_hang` — **zeroed at a marker whose own side contributes an inline-axis edge**
  — the `inline_start` components at an `InlineBoxStart`, the `inline_end` components at an
  `InlineBoxEnd` — and left alone otherwise (M3); cells 16, 3b and 14b. ⚠ **This bullet stated the
...
16b and 25. **It owes one dependency**: it is not cut from `origin/main` until the min-content
prereq PR has landed, and cell 25 asserts both intrinsic sizes on that base. ⚠ **The
paragraph below records that this dependency does not deliver the *edge* half of that assertion**
(R22-e): at the prerequisite's own base the marker payload carries no edges, and the disposition
is that PR's plan-review's.
...
`LinePacker` exists; the shaping break and the hang gate both read the predicate **per marker
side** — the derivation produces the two halves and `has_inline_axis_edge` is their disjunction
...
therefore that at each of the two gates, and at every site **describing** either, the quantity
named is the marker's own side. ⚠ The identifier grep missed the one site that stated the
whole-box rule outright — §7's `current_line_last_hang` bullet, which names no identifier at all,
so `grep -c has_inline_axis_edge` on it returns 0. Harvested by concept instead
(`grep -noE 'hang gate|current_line_last_hang|shaping break|the hang' <memo>`, each hit read) the
population is **ten** sites: the freeze's R2-F3 record, §5.1 M3 and M5, §5.3's PR-1b bullet, §6
cells 3b, 14, 14b and 16, §7's bullet and this clause. Two more named the whole-box identifier at
a gate where the readings happen to agree; all ten now state the marker's own side;
M8 adds the max-content contribution **in this PR** and not the min-content one, which is in
`main` before it (§8's *Min-content prereq PR*; ledger **A47** — an earlier drafting deferred
it past PR-1b instead).
...
**PR-1c** (box geometry): cells 6, 10b, 10c, 13, 13b, 13c, 13d, 14c, 15c, 17, 17b, 17c, 17d and 17f —
...
after the min-content prereq has, and PR-1c
...
branch and neither is on `origin/main`**
(`git ls-tree -r origin/main --name-only | grep -cE '\.claude/tools/plan-(sweep|xcheck)\.py'` → 0,
against `git ls-files | grep -E 'plan-(sweep|xcheck)\.py'` → both files, on this branch).
⚠ **The command read `grep -c plan-` until R22, and that form returns 5** — it matches the four
`.claude/skills/elidex-plan-review/` files and `.plan-z1b-consume-delta.md`, none of them a
checker, so it could not have returned 0 and did not discriminate what the sentence claims. The
conclusion is unchanged; the transcript was not reproducible, which is the defect.
An approval PR carrying **only** the memo would therefore land a document on `main` whose
...
hold this one, and now cannot hold the approval PR through it either. The predicate, reconciler
and min-content prereqs are
...
the predicate prereq is approval-independent like its four siblings — an earlier drafting called
...
⚠⚠ **That ordering does not admit instance 1, and the defect is in this carve rather than in
the prerequisite** (R22-e). §5.1 M1 splits the marker payload by §8's own dead-field rule: PR-1a
emits `InlineBoxStart { entity }` / `InlineBoxEnd { entity }`, and the three `EdgeSizes` + the
`WritingModeContext` *join the payload in PR-1b*. This PR branches off `main` like its four
siblings and is ordered against PR-1b alone, so its base is one of two states and neither carries
an edge: **before PR-1a** there are no markers at all and an empty decorated inline is absent from
the item stream entirely (§4.2's Shape B), and **between PR-1a and PR-1b** the markers carry
`entity` and nothing else. Instance 1's edge sum therefore has no input at the base its own
ordering gives it. Closing it there is either payload work moved out of PR-1a/PR-1b, or a
derivation of the edges from style inside `measure.rs`, beside the marker payload M4's invariant
(i) calls the single derivation site — and (i) is scoped **per pass** while the intrinsic pass runs
its own `collect_inline_items` (`measure.rs:22`), so whether it even reaches there is unsettled
too. §8's **PR-1b** item states the obligation this falsifies: PR-1b "is not cut from
`origin/main` until the min-content prereq PR has landed, and cell 25 asserts both intrinsic sizes
on that base."
⚠ **Instance 2 is not in that position, and the grounds on record are instance 2's.** Cross-item
joining is about `InlineItem::Text` — instance 2 above is three of them walked alone, and its
markup stays that instance's — and it reads no marker, so it is realisable at this PR's own base. A47's ground for
withdrawing the deferral (the inconsistency is between intrinsic sizing and the layout PR-1b ships)
and this paragraph's ground for making it a PR of its own (`max_word` is the pass's whole state, so
the change reaches the pass's **structure** and not only its arithmetic) are both read off that
half. What is open is whether the two instances travel together.
⚠ **The tension is stated, not settled, and one candidate objection is recorded as absent.**
Nothing in A47 or in this paragraph grounds the carve on "PR-1b would otherwise do two things":
the grounds on record are the two above, and §6 already routes **cell 25** — the assertion of both
intrinsic halves — to **PR-1b**, whose DoD lists it. An edge term added to an accumulator this
prerequisite has already built is arithmetic on that accumulator and carries neither ground.
Against that, §5.1 M8 reads "the min-content half is a prerequisite PR ahead of PR-1b", and any
split of the two instances parts from that sentence. This memo owns the **ordering** and not this
PR's content (above), so which of the two moves is the min-content prereq PR's plan-review's, with
the umbrella re-deriving the ordering from what it settles. ⚠ A third part — `InlineItem::Atomic`
contributing zero — is **not** one: `measure.rs`'s own doc states it of `min_content_inline_size`
(`:13-14`) and `max_content_inline_size` (`:42-43`) alike, so it is neither min-content-specific
nor decoration-specific, and §8 names two measured instances rather than three.
...
§6 cell 6c). ⚠ This memo does **not** claim it is disjoint from the other four: its touch set
follows
...
claims no disjointness for either.
All five branch from `main`;
...
**five** prereqs branch off `main`, as does the **tooling PR** (approval-independent, ordered on
...
the predicate PR (**pending**) because PR-1a's M1 consumes what it establishes. The remaining two
are **pending** and are ordered against a later PR rather than PR-1a: the **min-content PR**
against **PR-1b** and the **reconciler PR** against **PR-1c**, so each may land any
time before its own. ⚠ Whether the
predicate prereq is ordered against the other four depended on its touch set, which §9 hands over;
...
the reconciler and min-content PRs. PR-1b,
...
* ⚠ **This memo's own length, and why the split is booked rather than done.** It is far past 1000
  ...
  (the four shipping PRs, the three pending prereqs and the tooling PR — **eight** reviews), which
...
* **Intrinsic sizing** — split by pass, per M8: the max-content contribution is **in PR-1b**; the
  min-content one is a **prerequisite PR in `main` before PR-1b** (§8), because that pass has no
  accumulator to attach edges to and PR-1b is not correct without it. ⚠ **It was slotted until
  rev 53** and the slot is withdrawn, not re-tagged (ledger **A47**). The intrinsic passes pass
  `0.0` as the containing inline size (§5.2) — the standard treatment for percentages.
* **`#11-css2-spec-label-normalisation`** — this memo cites css-inline-3 for the model, so its
  remaining CSS 2 cites are **§8.3, §8.3.1, §9.2.1.1, §9.2.2.1, §9.4.2, §9.4.3, §9.5, §10.8.1 and
  §16.6.1** — nine. ⚠ **Seven until rev 34, and the eighth was invisible to the very command
  ...
  **CSS 2 §9.5** entered in that revision's own commit, in §3's css-inline-3 §2.1 row, where the
  ...
  returns eight and the same command on this revision returns nine, which is what "the list is
  ...
  both untitled cites, CSS 2 §9.5 (quoted as a cross-reference, no title) and
  CSS 2 §16.6.1, which §1.6 cites with no title. ⚠ **Two untitled, not one**: the front matter
...
* **The canonical *inline box* predicate, with the `client*` guard as its first consumer — carved
  ...
  **Ordering**: in `main` before **PR-1a**, because M1's emit test is one of its two consumers
  ...
  the other four prereqs, not a sibling topology, not a cold-gate width. All three follow from the
...
* **The enumeration-completeness audit (R20) — the standard it used, what it found, and the six
  ...
  **The remaining three take no token, each for a stated reason.** css-break-3 §5.4's
  ...
  what elidex satisfies today. css-sizing-3 §5.2.1 rule 4: the gap is in **this memo** rather
  than in the engine — §8 threads `containing_inline_size` and never says what the two intrinsic
  passes thread — so it is an input to the min-content prereq PR's and PR-1b's own plan-reviews,
  not a platform slot. css-content-3 §1's per-value box count: conformant today by an empty
