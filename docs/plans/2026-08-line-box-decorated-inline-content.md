...
verifiable with webref under the shortname **`CSS2`** (`webref heading CSS2 8.3`); the **six** CSS 2
section↔title pairs this memo carries were checked that way. ⚠ **Six, not the five an earlier
drafting counted** (round 25, Axis 4). §9's CSS 2 cite list runs to **seven** sections because it
counts every CSS 2 section this memo *cites*, produced by its own grep; the seventh, CSS 2 §16.6.1,
is cited in §1.6 without a title, so it carries no section↔title pair to check — the two figures
count different things and each is stated where it is produced.
...
gate). So **rounds 25–26** decide. #515 stays a **draft** until then and is
...
**CSS 2 confirms both halves independently, in two different sections.** For **margins**, §8.3
...
any effect on non-replaced inline elements". For **padding and border**, §10.8.1 *Leading and
...
⚠ **Narrowed in rev 33** (round 25, Axis 4): an earlier drafting wrote that §8.3 "says nothing
about padding or border", which the section refutes — §8.3.1 *Collapsing margins*, which
`body CSS2 margin-properties` returns as part of §8.3, contains "no padding and no border separate
them". That is a **margin-collapsing** condition on block boxes, not a line-box-sizing rule, so
§8.3 is not the authority for the padding/border half either way.
...
counts among the six checked CSS 2 section↔title pairs above. CSS 2 §10.8.1 is that complement,
...
partitioned across multiple line boxes." (Both `…` mark an elision: the first the §5 title, the
...
| `crates/layout/elidex-layout-block/src/inline/pack/mod.rs` | `LinePacker` line state. **M3's `note_line_occupancy` lives here**, beside `place_item` (`:679`), its first caller — and it is the **single writer** of the line-occupancy ordering, M7's glyph-bearing state included: the two raise sites are `place_item`'s call and the marker path's. `place_item` **derives** the occupant it passes from the two arguments it already has — `contributes_content` (`:686`) and `member: FlowMember<'_>` (`:687`) — raising the glyph-bearing rung only for `(FlowMember::Text(_), true)` and `Content` for every other pair, its own literal-`true` atomic call (`:652` → `:658`) included; the marker path passes `BoxEdgeOnly` and so raises no higher than the non-glyph state (M5, M7). ⚠ **Not "the Text arm's call is the only site whose value means glyph text"** — that is what this row said until the rev-33 gate, and `place_item`'s two callers (`:591`, `:652`) feed one parameter and one `note_line_occupancy` call, so a *call site* is not a condition the raise can test (M7's Decision carries the withdrawal). No other function writes the ordering. `flush_line` (`:209`) **calls** the open-box hook that `pack/inline_box.rs` owns (M4, **PR-1c**), promotes M7's tentative baseline inside its `:210` arm and **reads the occupancy state there** (**PR-1d**), and grows its per-line reset block by exactly **one** field per PR — M3's `line_occupancy` in **PR-1b** and M7's tentative in **PR-1d**; M7's glyph state is a widening of M3's field, so the `:432-439` reset it already has covers it and no second reset is added. The fabricated shaping citation at `:726` is rewritten by **PR-1b** — the one cite this program corrects, because PR-1b changes what that comment documents (§9). |
...
* **PR-1b — inline-axis advance.** Owns couplings 7×2 and 7×(intrinsic): M3 + M8, **plus M5's
  ...
  *positioned*. That is the property that makes this PR shippable ahead of PR-1c rather than after
  ...
  observable), and **no cell pins a `line_count` change** — the only
  markup that gains a line does so through the item-boundary flush
  `#11-inline-item-boundary-soft-wrap` records, which is not pinned as expected (an earlier
  drafting said cells 3, 12b and 15 pin it; round 22, Axes 3/5). The two shapes stay lineless in
...
tuple. Against them §10 carries **four** `(own)`-tagged rows (PR-1a, PR-1b, PR-1c, seam-3 prereq —
`awk '/^## §10\./,0' <memo> | grep -F '(own)'`), so check 9 is **live in both directions** for
PR-1a, PR-1b, PR-1c and the seam-3 prereq; for `dead-arm prereq` only the reverse direction holds — an
...
three write sites; **M7** (rev 33) — the per-line *glyph* fact is the **same** answer applied a
...
leaves it, three-valued; the widening is PR-1d's and adds no field, no reset, no second writer and
no new parameter;
...
5. **Block-axis edges only** (`padding-top`, `margin-top`) — **line stays suppressed in
   ...
   independently — §8.3 for margins, §10.8.1 for padding and border. The authorities are §1.2's;
   this cell refers to them rather than restating them. ⚠ It used to restate them, in the two forms
   §1.2 has since withdrawn: "§8.3 says nothing about padding or border" (refuted by §8.3.1) and
   then the asymmetry that only `css-inline-3` covers the padding/border half (refuted by §10.8.1)
...
12e. **Own-vs-inherited direction** (M1): `<p dir="rtl"><span dir="ltr" style="padding-left:10px">x</span></p>`
    ...
    §6.4 sentence, "based on the used `direction`… of the box"; "of the box" is not in §6.4, which
    names no owner. §3's own css-writing-modes-4 §6.4 row already renders it correctly, with the
...
22. **Both clauses are per *box*, and that is now said** (§1.5): a decorated inline **with** text
    ...
    does contain a strut, so M7 records a tentative for it. ⚠ **Neither clause says which line's
    baseline that strut becomes** — the line-level question is cell 24's, and under M7's rule a
    glyphless box co-resident on a line that carries glyph text does **not** give the line its
    baseline. An earlier drafting left clause 2 tagged bare "(M7)" and unqualified as to line
...
24. **No usable font: the tentative baseline must not promote** (M7): on a line whose text has no
    ...
    **occupancy state**, which reached the glyph-bearing rung because `place_item` was handed a
    `FlowMember::Text(_)` with `contributes_content` (M3's derivation, `pack/mod.rs:686-687`) even
    though `measure_text` returned `None`; that is what
...
`grep -rEn '\.(border|padding|margin)\.(top|right|bottom|left)\b'`
over the crates §7's reader family names (`elidex-render`, `elidex-dom-api`, `elidex-layout`,
`elidex-shell`, `elidex-a11y`, `elidex-js`, `elidex-layout-block` and its four siblings,
`elidex-ecs`), **run in the PR with its output recorded**, each hit classified as a `LayoutBox`
read or not. It deliberately
...
correspondence to those two fields is that crate's contract, so a cell of *this* program could only
assert it through a hand-inserted `LayoutBox` — the marshalling-only shape §5.2 rejects for
`elidex-dom-api`. So the moved origin is **accepted, on the record**, not absent (round 25, Axis 3;
...
citations to their §3 rows **and its glyph-state gate, which is four checkable items and not a
field pair**: (i) the line-occupancy ordering gains its glyph-bearing state — a fourth variant or
the sibling monotone enum PR-1d's own memo may pick, since M7 leaves the mechanism there;
...
glyph-bearing arm is `(FlowMember::Text(_), true)`; **neither `place_item` call site changes and no
...
written**: `grep -rn '<the glyph rung's identifier>' crates/layout/elidex-layout-block/src/` must
return exactly three things — the rung's declaration, that one arm inside `place_item` in
...
higher than the non-glyph state. A fourth hit is a second raise site and fails the item. Beside it,
...
harness switch. **No field joins the `:432-439` reset block for this gate** — M3's reset covers the
...
6. ⚠ **Its own lane-eligibility check.** The candidate home for the replacedness half is
   ...
   `elidex-plugin` (`computed_style/display.rs`) that **both** consumers already depend on with zero
...
* **`#11-css2-spec-label-normalisation`** — this memo cites css-inline-3 for the model, so its
  ...
  ⚠ Two corrections to how this list is produced. **CSS 2 §9.2.1.1** (*Anonymous block boxes*)
...
* **`<br>` and `<wbr>` carry no break behaviour engine-wide.** Both are inline boxes under the
  ...
  `grep -rnE '"br"|BrMarker' crates/layout crates/core/elidex-render crates/core/elidex-ecs` and
  `grep -rniE '\bwbr\b' crates/layout crates/core/elidex-render crates/core/elidex-ecs` both return
  nothing at `154bac3f`, and `force_break()`'s single caller (`pack/mod.rs:603`) is the
  preserved-`\n` path, not a `<br>`. **What is unimplemented is the *break*, not the element**, and
  ...
  **Why out of scope**: each is a distinct inline-level feature —
  ...
  a slot would track; recorded so that §5.1 M1's attribution names something. ⚠ **Two claims rev 32
