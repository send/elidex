...
* **R2-F1 / R2-F2 / R2-F4** — one root: elidex has **no per-box layout-bounds model** (css-inline-3
  ...
  R4** — M6's and M7's disclosures are corrected
...
two write sites — `flush_line`'s reset (`pack/mod.rs:439`) and `place_item`'s set (`:772`) — so
entity identity is the sole discriminator. ⚠ Widening M1 to every inline box
...
(`builder/inline_flow.rs:107-124` → `elidex-shaping/src/shaping.rs:133`). So
...
`normal` branch turns on. **`#11-inline-height-normal-layout-bounds`** (§3, §9, §10),
...
| 5 | **Line-box height composition** — `css-inline-3` §5.3 layout bounds, incl. the root inline box; the whole model is `#11-inline-root-inline-box`'s, in five facets | `inline/pack/mod.rs:696` (the scalar `max`), `:493` (the committed rect's block extent), `:539-543` (the vertical block-advance source), `:586` (first-wins baseline) |
...
* **PR-1c — box geometry.** Owns coupling 3×7: M4. The open-box stack and its flush-time
  ...
  **a11y node bounds** (`elidex-a11y/src/tree.rs:121-125`, an unconditional `border_box()` read).
  ...
  point hits the `<p>`. §6 cells 6, 10b, 13, 13b, 13c, 13d, 14c, 15c, 17, 17b, 17c, 17d and 17f
  land here.
...
* **`#11-inline-root-inline-box`** (new slot, **pre-existing** class): ⚠ **the whole css-inline-3
  §5.3 layout-bounds model** — widened here from "the block container's root inline box" by Codex
  ...
  * **(e) composition on a *decoration-only* line carrying more than one box** (R4; **M6 and
  ...
  gives inline boxes layout bounds of their own** (the five facets are instances of their absence),
  or a compat-survey hit. Re-eval: 2026-11-01. ⚠ **A further disjunct — "any work needing
...
* **`#11-inline-item-boundary-soft-wrap`** (new slot, **pre-existing** class): `place_item`
  ...
  **cell 15c**, recorded here as outside the class because it "declines to assert where `b`
  ...
  `position:absolute` arm** puts an `InlineItem::Placeholder` (`inline/collect.rs:230`) between
...
* **`#11-shaping-break-vertical-align-and-isolation`** (new slot, **pre-existing** class, opened
  ...
  two write sites, `pack/mod.rs:439` (the `flush_line` reset) and `:772`, so no property other
  than identity can end a run. **Why deferred**: the fix is a shaping-break predicate over §7.3's
...
13b. **The committed rect takes the *line's* block extent, not the box's — pinned as accepted**
    ...
    **line's** block extent and its `border_box()` that plus the padding on each side, where
    ...
    border. **Required test**: the cell computes both from the produced `LayoutBox` — the content
    height equals the `<p>`'s used `line-height` and the border box exceeds it by twice the
    padding — rather than from literals, so the numbers live in the assertion.
    ...
    `line-height` has no impact on the size of an inline box**" — and then declines to fix it:
...
20. `display: none` ⇒ no marker (`inline/collect.rs:218`); `position: absolute` ⇒ out of flow,
    ...
    `InlineItem::Placeholder` (`inline/collect.rs:230`). Measured at `22de3078`, `pack()`'s
...
prereq PR has landed, and cell 25 asserts both intrinsic sizes on that base.
...
**PR-1c** (box geometry): cells 6, 10b, 13, 13b, 13c, 13d, 14c, 15c, 17, 17b, 17c, 17d and 17f —
...
* **`#11-resize-observer-inline-empty-content-rect`** (new slot, **pre-existing** class; Codex
  ...
  **What the fix needs**: the same canonical *inline box* predicate the `client*` prereq PR
  ...
  §A's three-conjunct definition including the **non-replaced** half, which is precisely the half
  §3.3.1 keys on and the half elidex does not answer today (`is_atomic_inline`,
  `inline/collect.rs:14`). The prereq PR is this slot's natural **input**, not its owner: it lands
...
* **The enumeration-completeness audit (R20) — the standard it used, what it found, and the six
  ...
  * **css-inline-3 §5.3's `line-height: normal` branch** →
    **`#11-inline-height-normal-layout-bounds`** (new, **pre-existing** class). *Gap*:
    ...
    branch takes. *Trigger*: any line-height correctness work reaching `LineHeight::resolve_px`,
    or any work that gives inline boxes layout bounds of their own. *Re-eval*: 2026-11-01.
  ...
  * **css-pseudo-4 §4.1's suppression clause** →
    ...
    *Gap*: `generate_pseudo_entity` (`crates/css/elidex-style/src/pseudo.rs:27-50`) carries no
    ...
    canonical predicate the prereq PR establishes, whose **non-replaced** conjunct is the half
    this keys on — a consumer beside the two that bullet enumerates and beside
    `#11-resize-observer-inline-empty-content-rect`, which is the precedent for routing a
    consumer this way. *Trigger*: the predicate prereq PR's landing, or the next change to
    `elidex-style`'s pseudo generation. *Re-eval*: at the predicate prereq PR's landing.
...
* **The two plan checkers are not on any loop.** `plan-sweep.py` / `plan-xcheck.py` live in
  ...
  *quotation* was truncated, so a predicate reading only the row's own text would have answered
  ...
  the row, which is the whole of why it is keyed on the spec's case count. ⚠ The
  `SPEC_LABEL_REVERSE`
...
| Open `#11-inline-root-inline-box` (pre-existing) with the Why / trigger / date in §5.3 — whose **subject is the whole css-inline-3 §5.3 layout-bounds model in five facets**, widened from "the root inline box" by Codex R2 (F1/F2/F4) and again by R4 (facet (e)); the registration must carry the widened subject, since a slot opened on the narrow one would leave facets (b)–(e) owned by nothing. ⚠ A pre-existing slot whose Why / trigger live only in this memo is registered when the memo is on `main` — the approval PR — not at a code PR (§10's first row: a registration is made true by existence; an earlier drafting routed this row to PR-1d and the next to PR-1b, the same deferral-dressed-as-routing that row withdraws). Own deferrals stay on the PR that opens them — the per-PR ≤3 count is measured at that landing | approval PR |
...
| Open `#11-inline-height-normal-layout-bounds` (**pre-existing** class, opened by R20) with the Why / trigger / re-eval in §9 — css-inline-3 §5.3's **`line-height: normal`** branch, the one an author-`line-height`-free document is in, which elidex never enters because `LineHeight::Normal` collapses to `font_size * 1.2` at computed-value time. The registration carries **why it is not a sixth facet of `#11-inline-root-inline-box`** (that slot's five facets are each pinned by a §6 cell and none turns on per-glyph provenance), so the two are not re-merged later by someone reading only the section number. Registered at the approval PR; **not** an own deferral | approval PR |
...
| Open `#11-pseudo-generation-on-replaced-originating-element` (**pre-existing** class, opened by R20) with the Why / trigger / re-eval in §9 — css-pseudo-4 §4.1's suppression clause, unimplemented: `generate_pseudo_entity` and its call sites test the cascade, `content` and `display` and never replacedness. The registration carries that the canonical *inline box* predicate the prereq PR establishes is this slot's **input** and not its owner — the `#11-resize-observer-inline-empty-content-rect` shape — and that it is a **further** consumer of that predicate beside the two §9's canonical-predicate bullet enumerates. Registered at the approval PR; **not** an own deferral | approval PR |
