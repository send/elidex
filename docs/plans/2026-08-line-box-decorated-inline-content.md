...
decorated pseudo-elements and states the non-zero-edge ground; cell 15 no longer pins a §5.5
...
* **`#11-inline-zero-edge-box-in-item-stream`** (new slot, **own** deferral — PR-1a's): M1 emits
  ...
  contrast, **14c**, **21** and §7's "a box with all edges zero gains nothing" are written against
  the conjunct and **flip** when this slot lands. How: drop the conjunct from M1's emit test — the
  edges are already derived downstream (M3's sums, M5's disjunction), which also retires the
  resolve-then-discard oddity M1 records for PR-1a. Trigger: `#11-inline-root-inline-box` work
...
6g. **A decorated pseudo-element inline box gets markers** (M1's routing special-case; Codex on
    ...
    **behaviour-neutral** here (the markers are inert until PR-1b). Paired with 6c–6e it pins that
    ...
    idiom (`inline/tests/inline_flow/persist.rs:133-140` at `22de3078`) — a hand-inserted entity
...
| Open `#11-inline-root-inline-box` (pre-existing) with the Why / trigger / date in §5.3, including the zero-edge empty-inline §2.2 case it also owns (Codex on #515) | PR-1d |
