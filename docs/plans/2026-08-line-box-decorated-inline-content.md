...
* **R2-F1 / R2-F2 / R2-F4** — one root: elidex has **no per-box layout-bounds model** (css-inline-3
  ...
  §5.3 model with four enumerated facets (§5.3, §9, §10), M6's and M7's disclosures are corrected
...
| 5 | **Line-box height composition** — `css-inline-3` §5.3 layout bounds, incl. the root inline box; the whole model is `#11-inline-root-inline-box`'s, in four facets | `inline/pack/mod.rs:696` (the scalar `max`), `:493` (the committed rect's block extent), `:539-543` (the vertical block-advance source), `:586` (first-wins baseline) |
...
* **PR-1c — box geometry.** Owns coupling 3×7: M4. The open-box stack and its flush-time
  ...
  cells 6, 10b, 13, 13b, 14c, 15c, 17, 17b, 17c, 17d and 17f land here.
...
* **PR-1d — existence.** Owns couplings 1×3, 1×5 and 1×4: M5's *flip* + M6 + M7. The clause-3
  ...
  cells 18, 19, 20, 21, 22, 23, 23b, 24, 24b and 24c land here, plus the flip set §8 states.
...
* **`#11-inline-root-inline-box`** (new slot, **pre-existing** class): ⚠ **the whole css-inline-3
  ...
  **row 5** is this slot's invariant. **Four facets, each pinned by a §6 cell rather than fixed
  here**:
  ...
  * **(d) baseline composition on a mixed line**: a glyphless box's strut is suppressed rather
  ...
  `vertical-rl`, the convention being the *text* path's; (d) elidex composes no struts at all), so
  `origin/main` already fails all four. Trigger: any line-height correctness work, **any work that
  gives inline boxes layout bounds of their own** (the four facets are instances of their absence),
  ...
  the four facets needs **per-glyph** provenance — composing the bounds of a box elidex can already
...
13b. **The committed rect takes the *line's* block extent, not the box's — pinned as accepted**
    ...
    *from* each box's bounds rather than assigning the line's extent back to them; every
    `clientHeight` / `getClientRects` / background-and-border / hit-test reader of that rect
    inherits the 40.
...
**PR-1d** (existence): cells 18, 19, 20, 21, 22, 23, 23b, 24, 24b and 24c, plus the flip set — **cells 1, 2, 7, 8, 10, 11, 12 and 12d flip; 5, 6b, 6c, 6f, 6d, 6e, 6g, 6h and 9 do not**; this is the one normative statement of it — every §7
...
**per-glyph** provenance anywhere in its four facets (§5.3), whereas the fallback-only condition
...
| Open `#11-inline-root-inline-box` (pre-existing) with the Why / trigger / date in §5.3 — whose **subject is the whole css-inline-3 §5.3 layout-bounds model in four facets**, widened from "the root inline box" by Codex R2 (F1/F2/F4); the registration must carry the widened subject, since a slot opened on the narrow one would leave facets (b)–(d) owned by nothing. ⚠ A pre-existing slot whose Why / trigger live only in this memo is registered when the memo is on `main` — the approval PR — not at a code PR (§10's first row: a registration is made true by existence; an earlier drafting routed this row to PR-1d and the next to PR-1b, the same deferral-dressed-as-routing that row withdraws). Own deferrals stay on the PR that opens them — the per-PR ≤3 count is measured at that landing | approval PR |
