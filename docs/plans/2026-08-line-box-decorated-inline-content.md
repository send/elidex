...
each decision, stated affirmatively, plus an explicit refutation of the competing readings where
one keeps being reached for.
...
supports it only by implication and is the superseded text.
...
laxity adjacent to it.

| Spec section | Step | Branch | Touch (compile/dispatch site) | Full enum? |
|---|---|---|---|---|
| CSS Inline 3 §2 Inline Layout Model | inline-axis edges respected between boxes | start/end edge advance | M3 shared core (`note_line_occupancy`, NEW) — **PR-1b** | ✓ |
| CSS Inline 3 §2 Inline Layout Model | root inline box | block container's anonymous inline box | **NOT implemented — M6, `#11-inline-root-inline-box`** | ✗ (pre-existing, disclosed) |
| CSS Inline 3 §2.1 Layout of Line Boxes | box exceeding the line, or containing a forced break | split into fragments across line boxes | **`#11-inline-box-decoration-splits`** | ✗ (deliberate, §5.3) |
| CSS 2 §9.4.2 Inline formatting contexts | box that **cannot** be split | overflows the line box | M3 — no wrap check on the marker path; §6 cell 15 | ✓ |
| CSS Break 3 §5.4 Fragmented Borders and Backgrounds: the box-decoration-break property | `box-decoration-break: slice` | "no border and no padding are inserted at a break" (the "no visual effect where the split occurs" phrasing is CSS 2 §9.4.2's, not this section's) | **`#11-inline-box-decoration-splits`** | ✗ (deliberate) |
| CSS Inline 3 §2.2 Layout Within Line Boxes | Note on empty inline boxes | they still have a line-height and influence the calculation | M6 — **PR-1c** | ✓ |
| CSS Inline 3 §2.3 Phantom Line Boxes | consequence (a) | zero-height for positioning descendant content (abspos) | `static_positions` (`inline/pack/mod.rs:97`) — **PR-1c** | ✓ |
| CSS Inline 3 §5.3 Calculating the Logical Height Contributions ("Layout Bounds") of Inline Boxes | layout-bounds inflation by edges | applies only when `line-fit-edge` ≠ `leading`; initial is `leading` | no code touch; grounds §1.2's block-axis exclusion | ✓ |
| CSS Inline 3 §2.3 Phantom Line Boxes | clause 3 | non-zero **inline-axis** margin/padding/border | clause-3 predicate → `has_inline_axis_edge` (M5); the predicate lands in **PR-1b** with its M3 consumers, **PR-1c** substitutes it for the constant | ✓ |
| CSS Inline 3 §2.3 Phantom Line Boxes | clause 5 | forced line break | `force_break` (`inline/pack/mod.rs:781`) — untouched | ✓ (pre-existing) |
| CSS Inline 3 §2.3 Phantom Line Boxes | consequence | line box *and its in-flow content* do not exist | commit/discard seam (`inline/pack/mod.rs:210` vs `:423`) — **PR-1c** | ✓ |
| CSS Inline 3 §5.3 Calculating the Logical Height Contributions ("Layout Bounds") of Inline Boxes | glyphless / fallback-only box | strut with first-available-font metrics | tentative baseline — **PR-1c** | ✓ |
| CSS Inline 3 §5.3 Calculating the Logical Height Contributions ("Layout Bounds") of Inline Boxes | half-leading | `A′ = A + L/2` | existing formula (`inline/pack/mod.rs:581`) — unchanged | ✓ (pre-existing) |
| CSS Text 3 §5.5 Line Breaking Details | break opportunities | inline box boundary is **not** one | M3 — the marker path calls the shared core without a wrap check — **PR-1b** | ✓ |
| CSS Text 3 §5.5 Line Breaking Details | intra-word shaping | "the characters must still be shaped … as if the word were still whole" | the coalescing `pack/mod.rs:744` documents; its comment's citation is corrected in **PR-1b** (§3.1) | ✓ (pre-existing) |
| CSS Text 3 §5.5 Line Breaking Details | adjacent soft wrap opportunity | break lands at the box's **margin edge** | **`#11-inline-box-decoration-splits`** | ✗ (deliberate, §5.3) |
| CSS Sizing 3 §5.2 Intrinsic Contributions | max-content | inline-axis edges occupy space | M8 (`inline/measure.rs:44`) — **PR-1b** | ✓ |
| CSS Sizing 3 §5.2 Intrinsic Contributions | min-content | no accumulator to attach edges to | **`#11-inline-min-content-box-edges`** | ✗ (deliberate, M8) |
| CSS Text 3 §7.3 Shaping Across Element Boundaries | shaping break | trigger 1 of 3: non-zero inline-axis edge | `last_placed_entity` coalescing (`inline/pack/mod.rs:744`) — **PR-1b** | ✗ (no shaping-break handling for trigger 2, `vertical-align` ≠ `baseline`, or trigger 3, "the boundary is a bidi isolation boundary" — set by `unicode-bidi`. Both properties resolve; the packer does not read either. Neither is this umbrella's subject) |
| CSS Text 3 §4.1.2 Phase II: Trimming and Positioning | steps 3–4 | a collapsible space is line-final only if nothing follows it on the line | `current_line_last_hang` (`inline/pack/mod.rs:701`) — **PR-1b** | ✓ |
| CSS Text 3 §4.1.1 Phase I: Collapsing and Transformation | step 4 | collapsing crosses inline box boundaries | `collapse_inline_whitespace` (`inline/whitespace.rs:41`) — **PR-1a** | ✓ |
| CSS Inline 3 §5.3 Calculating the Logical Height Contributions ("Layout Bounds") of Inline Boxes | Quirks Mode | an inline box fragment with **zero borders and padding** and no direct text is ignored when sizing the line box | not implemented — no quirks-mode layout switch exists; **§9** | ✗ (deliberate, §9) |
| CSS 2 §10.8.1 Leading and half-leading | glyphless inline box | strut with first-available-font metrics (superseded by CSS Inline 3 §5.3, kept as the historical anchor) | M7 — **PR-1c** | ✓ |
| CSS 2 §8.3 Margin properties: margin-top, margin-right, margin-bottom, margin-left, and margin | non-replaced inline elements | vertical margins have no effect | consistent with CSS Inline 3 §2.3 clause 3; no code touch | ✓ |
| CSS 2 §9.4.3 Relative positioning | relpos inline in flow | decorated relpos inline; sub-flow keying | `collect.rs:286` — **`#11-inline-box-decoration-splits`** (the `group_key` field arrives with its only reader, §2 pair 2×6) | ✗ (deliberate, §5.3) |
| CSS Box Model 3 §3.1 / §4.1 | percentage margin/padding | logical width (= inline size) basis | `resolve_box_model` (`helpers.rs:116`) — **PR-1a** | ✓ |
| CSS Writing Modes 4 §6.1 Abstract Dimensions | inline size ≡ logical width | basis identity in vertical modes | same | ✓ |
| CSS Backgrounds 3 §3.2 Line Patterns: the border-style properties | `none` / `hidden` | width ignored ⇒ 0 | already zeroed at computed-value time (`elidex-style resolve/box_model/mod.rs:260`) | ✓ |

**Breadth**: K=8 specs (CSS Inline 3, CSS Text 3, CSS 2, CSS Break 3, CSS Box Model 3,
CSS Sizing 3, CSS Writing Modes 4, CSS Backgrounds 3), M=28 entries. Both figures are recomputed
from the table above by `.claude/tools/plan-xcheck.py`, which fails on drift.

**Split decision**: K=8 ⇒ SPLIT-DEFAULT. The plan **is** split into three shipping PRs, each
...
* **`crates/layout/elidex-layout-block/src/inline/pack/mod.rs:726` cites "CSS Text 3 §5.6
  ...
  decorated boundary). PR-1b rewrites the comment, so both land: §5.5 for the surviving intra-word
  coalescing, §7.3 for the new boundary break.
* **A nonexistent "CSS Box Model L3/Level 3 §5.3" is cited across `elidex-layout-block`**
  ...
  whose hits on `154bac3f` are `lib.rs:178`, `helpers.rs:59`, `helpers.rs:114`,
...
| the bogus-cite sweep (`helpers.rs`, `lib.rs`, `positioned/layout.rs`, `block/mod.rs`, `block/children/helpers.rs`) | PR-1a's one-commit class fix across every hit of §3.1's concept grep. Listed as its own row because five of the files it edits are not otherwise this program's. |
...
| the dead-arm prereq PR's surface | `flush_line`'s `else` arm and its unmerged rect loop (`pack/mod.rs:393-421`) **and** the `inline/mod.rs` half the reachability argument kills: `persist_candidate` (`:239`), `flow_align`'s `Option` construction (`:240-251`), `persist_flow`'s now-redundant conjunct (`:322`) and the comments that explain the two-path model (`:227-230`, `:309-320`, `:329-330`). Listed as a layer of its own because all three `inline/mod.rs` sites lie **above** seam 3 and therefore shift its coordinates (§8's ordering). |
| `crates/layout/elidex-layout-block/src/inline/whitespace.rs` | `collapse_inline_whitespace` (`:41`) — M2's transparent arm. |
| `crates/layout/elidex-layout-block/src/inline/measure.rs` | `max_content_inline_size` (`:44`) — M8's contribution. `min_content_inline_size`'s **accumulator** (`:15-37`) is not touched (see `#11-inline-min-content-box-edges`), but its `collect_inline_items` call (`:22`) takes the new argument like every other caller. |
...
| `elidex-render` tests | PR-1b's paint assertions for cell 13(c) — the background-colour and border rects that `paint/mod.rs:68`/`:382` derive from `border_box()`. §7 lists the family; §8 makes dispositioning it a PR-1b DoD item. |
| `elidex-dom-api` tests | PR-1b's `getBoundingClientRect` cell and PR-1c's `getClientRects` agreement cell (17d), against `element/layout_query.rs`. The two channels M4's ⚠ names are only observable from this crate. |
...
* **PR-1a — item stream. Behaviour-neutral.** Owns couplings 2×(all) and 1×2: M1 + M2. Marker
  ...
  rather than in two lists — `containing_inline_size` threaded, every bogus §5.3 cite fixed
  ...
  until PR-1c. **Characterization tests for §6 cells 1–12, 3b, 12b, 12c, 12d and 12e land here
  asserting today's behaviour**, since `grep -rn padding
...
* **PR-1b — geometry.** Owns couplings 7×2, 3×7 and 7×(intrinsic): M3 + M4 + M8, **plus M5's
  ...
  **This is the engine-wide correction**: it changes painted output for every decorated inline
  with content. §6 cells 13, 14, 14b, 15, 15b, 16, 16b, 17, 17b, 17c, 17d and 25 land here.
...
* **`#11-inline-box-decoration-splits`** (new slot, **own** deferral): css-inline-3 §2.1 box
  ...
  appears nowhere in css-inline-3 §2.x), plus fragmentation across the marker pair;
  **css-text-3 §5.5's adjacent-soft-wrap rule** (a break next to a decorated boundary lands at the
  box's *margin edge*, which M3's unconditional start-edge advance does not honour — §3's §5.5 row
...
the seam-3 prereq opens 1 (`#11-plan-memo-consistency-checkers-on-ci`), the dead-arm prereq opens
none. `#11-inline-root-inline-box` and the dead arm's own disposition are pre-existing class. All
...
6. All zero ⇒ no marker emitted at all (M1). **Block-axis-only** (`padding-top`) ⇒ marker **is**
   ...
   ⚠ On a phantom line the box gets **no rect and no paint**: css-inline-3 §2.3 makes the line box
   "and its in-flow content" non-existent, and the discard arm (`:428`) clears the tentative
   rects. The marker earns its keep only on a line that exists for another reason — `<p>text
...
17d. **`getClientRects` agrees with `getBoundingClientRect`** — a decorated inline that wraps
    must return per-line **border-box** fragments (edges included), matching what the single-line
    case returns through the `LayoutBox`. Constructible as
    `<p style="width:60px">aaa <span style="padding:10px">bbb ccc</span></p>`.
...
* **Every reader of an inline `LayoutBox`'s edges.** M4 flips `padding`/`border`/`margin` on inline
  ...
  * `elidex-layout-multicol`, `-table`, `-flex`, `-grid`, `block/mod.rs`, `block/children/stack.rs`,
  ...
  None is wrong-after-M4; the point is that PR-1b's DoD must reach past paint, which §8 says.
...
behaviour; **every hit of §3.1's concept grep** corrected; new variants carry docstring citations
to their §3 rows; **`setup_inline_test` (`inline/tests/mod.rs:54`) gains a deterministic way to
force `any_font == false`**, without which cell 12d is unconstructible; cells 1–12, 3b, 12b, 12c,
12d and 12e land as characterization tests; zero behaviour change. **Dead-field rule**: fields
...
`#[allow(dead_code)]` is needed. ⚠ PR-1a's marker fields are nonetheless read only by the *edge
resolution* it performs, not by the packer (whose `match pi` arm is a no-op); if that leaves
`field is never read` under `-D warnings`, the characterization tests in §6 are the intended
reader, and PR-1a's DoD includes making that so rather than silencing it.

**PR-1b** (geometry): cells 13, 14, 14b, 15, 15b, 16, 16b, 17, 17b, 17c, 17d and 25; **§7's full
`LayoutBox`-edge reader list dispositioned, not just paint** (a `getBoundingClientRect` cell
included); no double-counted edges on either side; the `inline/pack/mod.rs:726` shaping citation
corrected to css-text-3 §5.5 + §7.3 (§3.1); `note_line_occupancy`, `pack/inline_box.rs` and M8's
contribution carry docstring citations to their §3 rows.

**PR-1c** (existence): cells 18–24, plus the flip set — **cells 1–4, 3b, 7, 8, 10, 11, 12 and 12d
flip; 5, 6, 9, 12b, 12c and 12e do not**; this is the one normative statement of it — every §7
...
**Ordering: the seam-3 PR lands first.** Not because the dead arm sits in seam 3 — it does not,
it is in `pack/mod.rs` — but because the dead-arm PR's *other* half edits `inline/mod.rs` at
`:239`, `:240-251` and `:322`, all **above** seam 3's `:413-639`, so landing it first shifts the
range the seam-3 PR is measured against.
...
* **`#11-inline-fragmented-fn-decomposition` — trigger fires, and this program honours it.** The
  ...
  **Grounds**: the slot's **own** trigger, which carries no size qualifier and has fired. That
  ground alone carries it, and it is the only one stated. ⚠ CLAUDE.md's prereq-split clause is
  ...
  and its own text routes pre-existing files to the >1000 clause.
...
* **File growth**: measure with `wc -l` at each PR rather than against a number written here, which
  ...
  predicates and the persist-block touch — no new seam decision needed. Both `inline/mod.rs` and
  `pack/mod.rs` are re-checked against the ~1000-line convention at PR-1c, on their then-current
  size.
* **`#11-inline-relayout-box-staleness`** (`inline/pack/boxes.rs:96`) — pre-existing, but
  **PR-1b widens its observable surface**: today an inline's stale `LayoutBox` is
  ...
  box carries stale edges. PR-1b's widening is a note on this slot.
...
* **The two plan checkers are not on any loop.** `plan-sweep.py` / `plan-xcheck.py` live in
  ...
  PR**, not folded into this umbrella. New slot `#11-plan-memo-consistency-checkers-on-ci` (own
  deferral). ⚠ Until it lands, **this memo's own remaining rounds depend on running them by
  hand** — the habit the slot exists to end. Trigger: **this memo's next plan-review round**, not
  a PR landing. Re-eval: 2026-11-01.

## §10. Slot ledger actions at landing

| Action | PR |
|---|---|
| Register this umbrella's slot in `project_open-defer-slots.md` (the SoT per MEMORY.md), together with the two other unregistered Layout-lane slots — `#11-css2-spec-label-normalisation` (a #497 carve, **pre-existing** class) and `#11-inline-fragmented-fn-decomposition` (carved from #495, **pre-existing** class). This umbrella's own slot is **pre-existing** class too: Codex opened it on #497, not this program. | seam-3 prereq PR |
| Register **and** close `#11-inline-fragmented-fn-decomposition` in one row — it is registered as closed-on-landing, not registered then closed — **as a partial close**, naming the seams §9 measures as still open, in the successor slot `#11-inline-fragmented-fn-seams-1-2` (**pre-existing** class — the seams predate this umbrella; Why: the prereq PR discharges seam 3 only; trigger: the first change **after this umbrella's three shipping PRs** that touches the residue — every touch site §5.2's `inline/mod.rs` row lists is this program's own and does not fire it; re-eval 2026-11-01). | seam-3 prereq PR |
