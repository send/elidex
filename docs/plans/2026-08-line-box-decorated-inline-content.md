...
6h. **An empty decorated pseudo-element gets an adjacent marker pair** (M1; round 22) —
    ...
    (`elidex-style/src/pseudo.rs:47` creates one for any `ContentValue::Items`, `content: ""`
    included; its `TextContent` is `""`, `generated_content.rs:158`) and is an inline box, but the
