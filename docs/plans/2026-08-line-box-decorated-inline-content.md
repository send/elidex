...
instruction.** The program's prereq PRs are the ones §8 names, one block each and no count anywhere (ledger
**A87**, **A117**) — and **two** of them move code: the
...
**item-boundary opportunity** PRs before PR-1b, ordered among themselves in §8 rather than here
(ledger **A80**, **A88**, **A94**, **A95**, **A105**, **A110**). The memo does **not** re-anchor
...
*Collapsing margins* pair and the count was not re-derived. §9's CSS 2 cite list counts every CSS 2
section this memo *cites* and **no total is restated here**: it is whatever
`grep -o 'CSS 2 §[0-9.]\+' <memo> | sort -u` returns (ledger **A87**). The cites carrying no
section↔title pair to check — CSS 2 §9.5 (§3's css-inline-3 §2.1 row, in a cross-reference),
CSS 2 §16.6.1 (§1.6), CSS 2 §10.2 (rev 59's A59 and rev 60's M4 outcome) and CSS 2 §10.4
(rev 69's cell 25d) — are named rather than counted, for the same reason: the two populations are
different and only one of them has a command.
...
**seven** above is the one figure kept, because it has no command: the section↔title pairs are
checked one at a time with `webref` and nothing enumerates them (ledger **A87**).
...
one — one per row of the unit's edge matrix that follows it that names an arm, and states flip or invariance per arm (A123); rev 63 adds none (R2 adds
...
non-`normal` `white-space` values** (ledger **A67**). M3 keeps three things: a marker does not make
a trailing space non-line-final, whatever its edges; under `normal` step 3 removes a
**collapsible** trailing space; and what survives step 3 hangs under step 4's first bullet
(ledger **A82**). A freeze amendment to **M3**'s
...
cumulative design re-gate it does not silence either — and *reopened* twice besides, by user
decision: M4 at rev 60 (ledger **A60**) and M3's hang gate at rev 63 (**A63**), each recorded in
its own paragraph above rather than in this count. ⚠ **A third reopening was taken at rev 71 and
withdrawn at rev 72** — M4's flush-time emit rule, on a premise nobody had measured; the rule is
back in its frozen state and the count with it (ledger **A92**, withdrawn by **A97**)** (Codex R2, R3 then R4 on #515, 2026-09-20 —
...
* **R7-b — the bidi reorder discards the gap at paint, and the disposition is decided on a
  ...
  **identical** glyph positions, against the crate's own `converged_ltr_identity_no_reorder`
  ...
  program adding reach and not the defect. ⚠ **The predicate this was stated on was wrong and is
  corrected at §9** (rev 73): the branch fires on `direction == Rtl` **or** any `Text` run holding RTL
  characters (`builder/inline_flow.rs:140-143`), so an **LTR** paragraph with two Hebrew runs reorders
  while an RTL paragraph whose runs are all level-2 does not — and on that identity path an `rtl` box's
  own edges come out **mirrored** rather than unaffected. §6 cells **12b**, **12h** and **12i** carry
  the pins, §9 states the divergence once, and the sites that speak of glyphs moving carry a pointer,
  not a copy, per rule 2. Ledger **A24**, **A101**, **A103**.
...
5. **The umbrella pins outcomes; each per-PR plan-review owns mechanism** (R26; ledger **A59**).
   ...
   **(a)** the mechanism and **(b)** the outcomes for the `white-space` values other than
   `normal` — `nowrap` and `pre-line` wherever they differ from `normal`, `pre-wrap`'s hang and
   what css-text-3 §8.2 lets block it, `pre`'s preserved spaces, and `break-spaces` — to the end-of-line white-space prereq's and PR-1b's own plan-memos and
...
| A74 | R33-F1; **on rev 66's own text** | §5.2's `elidex-render` row and §6's test-placement paragraph still assigning cell 13d to `elidex-render` after **A73** moved it | rev 66 rewrote the row's opening ("no cell of this program lands here") and left its tail saying "cell **13d** lands here to assert the positioned one", while the test-placement paragraph still sent the implementer to `elidex-render`'s `consumes_relpos_inline_subflow_with_gap` harness — the hand-populated box A73 exists to stop. Both now name the `elidex-shell` pipeline. **Sites swept, by concept rather than phrase** (every statement that *places* 13d, `grep -n '13d' <memo>` read in full): §5.2's `elidex-render` row (re-stated), §6's test-placement paragraph (re-stated), §5.2's `elidex-shell` row (already updated at rev 66) and the cell itself (rev 66). **Considered and left**, because they cite 13d for what it asserts rather than where it lives: the freeze's amendment record, A8/A33/A38/A62/A73, §5.3's PR-1c bullet, §6's "not a counter-example" note, cells 10c and 17-family references, §7's two paint bullets, §8's PR-1c list and reconciler paragraph, §9's staleness bullet. Found by Codex on #515 |
| A75 | R33-F2 | §5.2's `collect.rs` row permitting **one** new parameter and declaring `root_horizontal` unchanged | M1's payload is `WritingModeContext::new(<the IFC root's writing mode>, style.direction)`, and `root_horizontal` is a `bool` — `parent_style.writing_mode.is_horizontal()`, computed at `collect.rs:150` and taken at `:211` (`154bac3f`) — which cannot separate `vertical-rl` from `vertical-lr` or a sideways mode, while `collect_inline_items_inner` sees only the current ancestor's `parent_style`. The row therefore forbade what M1 requires. It now permits the root's writing mode to reach the recursion and leaves **which** shape — widening `root_horizontal` to the `WritingMode` or a sibling parameter — to PR-1a/1b's plan. The case is author-reachable because css-writing-modes-4 §3.2 blockification is unimplemented (`#11-writing-mode-inline-blockification`; cell 12f). Found by Codex on #515 |
| A76 | R33-F3 | §5.2's "`items.is_empty()` … excludes markers" as a statement of **today's** behaviour | from PR-1a §4.2's Shape B is an adjacent marker pair and nothing else, so the test is **false** unless PR-1a makes it marker-aware, and the two paths differ where no line count can see: the early return clears stale `InlineFlow` unconditionally (`inline/mod.rs:161-162` at `154bac3f`) while the tail reconciler's clear is `!env.is_probe`-gated (`:637-639`), and `line_count` is 0 down either path. Re-stated as a **requirement on PR-1a's predicate**: a marker-only stream still takes the early return, and PR-1d flips it; the predicate's shape is PR-1a's. Pinned by new PR-1a cell **12g**, which lays out Shape B with `is_probe: true` after seeding an `InlineFlow` on a candidate key and asserts the component is gone — the state leak, not a count — and which joins §8's flip set (it flips at PR-1d with cell 12). The harness already writes the env by literal (`inline/tests/baseline.rs:19`). Found by Codex on #515 |
| A77 | R33-F4; **on rev 66's own text** | cell **25d**'s edge-only fixture as a discriminating intrinsic assertion | it reports 0 under three implementations — the intended one (accumulate `−20`, floor at the inline-block), one that omits margins from M8 entirely, and the per-marker clamp **A72** rejects — so PR-1b could pass every intrinsic cell with margins never reaching intrinsic sizing. The zero case stays (it is A72's outcome) and the cell gains a second markup that puts the signed sum out of the outer floor's reach: `<span style="margin-left:-10px">verylongword</span>` in an `auto`-width inline-block, with `measure_width("verylongword") > 10` asserted as the precondition, gives both intrinsic sizes `measure_width("verylongword") − 10`, where dropping margins and the per-marker clamp each report `measure_width("verylongword")`. Found by Codex on #515 |
| A78 | R35-F1 | cell 24d's "the IFC's `first_baseline` is **one** box's strut ascent", and M7 leaving which `InlineBoxStart` records the per-line tentative unsaid | with one `Option<f32>` per line, keeping the first tentative and overwriting with the second both satisfy the plan, and on 24d's fixture they differ by the whole gap between a 100px and a 10px font — so the cell asserted no value. The engine's rule, measured at `154bac3f`: `first_baseline` is written only under `self.first_baseline.is_none()` (`inline/pack/mod.rs:575`, the atomic path's second writer at `:621` under the same guard), the field is initialised once per IFC (`:187`) and no per-line reset clears it — **first-wins, IFC-wide**. M7 now states the tentative under the same idiom (recorded only when unset) and 24d asserts the **first** span's strut ascent. ⚠ **Interim, and the cell says so**: css-inline-3 §5.3's composition is facet (e) of `#11-inline-root-inline-box`, which supersedes the value; what 24d is *for* — maximal `A′` and `D′` on different boxes, their sum beyond the scalar `max` — is unchanged. Found by Codex on #515 |
| A79 | R35-F2 | the intrinsic cells as coverage of the shape this program **introduces** — marker-only content | 25, 25b, 25c and 3c all carry text, and 25d's only marker-only fixture is negative and floors to 0, so an implementation that attaches the edge term only where it commits a text segment returns **0** for `<div style="display:inline-block"><span style="padding:10px"></span></div>` and passes every intrinsic cell while PR-1b's line advances 20px — the box then narrower than its own content. New PR-1b cell **25e** asserts **20px on both intrinsic sizes** for that fixture, the same quantity M3 advances by, and names that implementation. Routed in §6, §5.3 and §8; the freeze's cell figure goes **66 → 67**. Found by Codex on #515 |
| A80 | R35-F3 | the reconciler prereq ordered **before PR-1c** alone | the desynchronisation it repairs is created by **PR-1b**, not PR-1c: PR-1b moves the position of `LayoutBox.content` for every decorated inline with content (§8's PR-1b item, §7's first numbered change), `assign_inline_layout_boxes` skips an entity that already has a `LayoutBox` (`pack/boxes.rs:62-64` at `154bac3f`), and a `position:relative` inline's background is painted from `border_box()` **today** (cell 13d's measurement) — so after a restyle the glyphs move and the painted background stays at the first-layout box, on the PR-1b commit. PR-1c widens the same defect to the box's edges. **Ordered in `main` before PR-1b**, and so before PR-1c. Swept: the reconciler block's ordering line, the topology paragraph's two sentences (its ordering sentence and the pending-three sentence), §8's PR-1b dependency clause (**two → three**), §8's PR-1c dependency clause, §8's PR-1a/PR-1b/PR-1c landing sentence and §9's staleness bullet. **No cell here**: observing it needs a restyle, a second layout and a display list — the reconciler prereq's own surface, and its plan's to cover; this umbrella states the ordering and PR-1b's DoD names it. ⚠ The rewrite briefly opened a line with `**PR-1b**:`, which `plan-xcheck.py` reads as a second §8 DoD heading and which reported 20 contradictions until the line was re-wrapped — the checker caught it, and the wrap is why the sentence reads as it does. Found by Codex on #515 |
| A81 | enumeration attestation of revs 62–67 | two claims the attestation measured false, both **self-introduced** in that window: §8's PR-1c set-level argument excusing cell **13d** as "painted from a `LayoutBox` its render harness sets by hand, so it tests the reader and not the producer" (**A73** had already made 13d a real-producer cell, and **A74** declared the sweep complete), and §5.1 M1 assigning the writing-mode widening to **PR-1a** | **The set-level argument, re-derived** rather than patched: with 13d running the producer through `elidex-shell`, the **border** term has two independent cells (13c's field, 13d's paint) plus the end-to-end clause; the paragraph's conclusion survives on the **margin** term, which is 13c's alone, cell 3 being PR-1b and reading the cursor. **A74's exhaustiveness claim was false**: a second placement site survived at §9's staleness bullet ("at cell 13d's measured scale on `elidex-render`'s relpos harness"), now re-stated as the *probe's* host with the cell's own host named; and A74's "considered and left" list named **A8**, which contains no `13d` at all — the list is a record, so it is corrected here rather than rewritten there. **M1's owner**: the widening lands in **PR-1b** with the `WritingModeContext`'s first readers, since a value threaded in PR-1a would have no PR-1a reader (§8's dead-field rule), and the two shapes are distinguished — a sibling parameter leaves `positioned_subflow_key`'s reader (`collect.rs:107`) untouched, widening `root_horizontal` does not. Found by an enumeration attestation of revs 62–67 |
| A82 | enumeration attestation of revs 62–67 | M3's "under `white-space: normal` step 3 removes the space" as the whole of `normal`, and §3's step-4 row handing the **entire** step to the end-of-line white-space prereq | step 3 removes a sequence of *collapsible* spaces and a trailing U+1680; **other space separators are neither collapsible under §4.1.1's `normal | nowrap | pre-line` bullet nor named by step 3**, so they reach step 4, whose first bullet — "If `white-space` is set to `normal`, `nowrap`, or `pre-line`, the UA must hang this sequence (unconditionally)" (`body css-text-3 white-space-phase-2`) — is a `normal` outcome. Freeze discipline 5's exception covers the mechanism and the values **other than** `normal`, so that bullet is this umbrella's: M3 now scopes its removal sentence to collapsible spaces and states the hang **by reference**, and §3's step-4 row splits accordingly (the `pre-wrap` and `pre` outcomes stay the prereq's, `break-spaces` stays vacuous). **No cell**: the engine's trim reaches `' ' | '\t'` alone (`inline/whitespace.rs:162`), which is the prereq's instance 3, and a marker changes none of it. Found by an enumeration attestation of revs 62–67 |
| A83 | enumeration attestation of revs 62–67 | seven smaller defects, grouped because each is one sentence and none changes a decision | **(a)** cell **17c** handed a `normal` outcome to the prereq's plan and weakened its precondition on that basis; its fixture carries no `white-space`, so the outcome is removal and the precondition is `measure_width("aaaa") + 5 > measure_width("cccc")`. **(b)** M3's "§4 enumerates eight" — `place_item` writes **nine** line-state fields and §4 names nine; the count is not restated at all now, per the rule that a memo carries no running totals. **(c)** §8's min-content block said "four measured instances" beside "Five" after A68, and called the atomic gap a "third instance"; both re-derived to five and "instances 2–5". **(d)** the topology's "ordered against none of the other prereqs" stood four lines above the sentence ordering end-of-line before min-content. **(e)** §8's PR-1b dependency credited step 3's removal to "the second" prereq, which under "in that order" is the min-content one; it is the **end-of-line** prereq's. **(f)** `positioned/constraints.rs:318` sat unqualified beside two `elidex-layout/src/` paths in the clause that exists to keep the homonyms apart — now crate-qualified at the cell (A72's copy is committed and stands). **(g)** cell 12g cited `:637-639` without the post-#508 note that the call lives in `inline/reconcile.rs`, and said "two paths" where there are **three** `clear_inline_flows` sites — two early returns clearing unconditionally (`:162`, `:201`) and the gated tail. Also: 25e's max-content gloss now quotes css-sizing-3 §2.1's "usually" rather than stating it flatly. Found by an enumeration attestation of revs 62–67 |
| A84 | enumeration attestation of revs 62–67; user decision, 2026-09-23 | cell **25d**'s "both intrinsic sizes are 0" and the ground A72 published for it — "css-sizing-3 §5.2's Note" | the Note is not §5.2's: `body css-sizing-3 intrinsic-contribution` prints §5.2, then the §5.2.1 heading, and only **then** the Note, between rules 3 and 4 — and §5.2.1's procedure is entered by a **percentage-sized box with a cyclic dependency**, which 25d's all-lengths fixture is not. Re-derived from the spec, and the answer is narrower than the claim: **the spec fixes the used width, not the contributions**. `min-width: auto` "resolves to a used value of 0" (css-sizing-3 §3.2), and the width algorithm is re-run against it — CSS 2 §10.4 step 3, quoted at the cell, css-sizing-3 §3.1.2 defining the property and restating no algorithm. Meanwhile css-sizing-3 §2.2 **presupposes a negative contribution**, which is why it floors the max-content contribution by the min-content one "(e.g. due to the use of negative margins)". So 25d's first markup now asserts the **used width is 0** and says in its own text why it asserts neither intrinsic size; the second markup, which is what discriminates the intended implementation from "margins dropped from M8" and from the per-marker clamp, is unchanged; the cell's title names both arms. **A72 is committed and stands**; this row is its correction. CSS 2 §10.4 joins §9's cite list (ten → **eleven** sections, three → **four** untitled), re-derived by that list's own grep. Found by an enumeration attestation of revs 62–67, decided by the user |
| A85 | enumeration attestation of revs 62–67 | six presentation defects held behind **A84** and now applied, grouped because none changes a decision | **(a)** the freeze's "Amended eleven times" counted the external channel's amendments only; the two **reopenings** by user decision — M4 at rev 60 (**A60**) and M3's hang gate at rev 63 (**A63**) — now stand beside it rather than inside the count. **(b)** §3's css-text-3 §8.2 *Covers* column named two of the section's normative sentences; it now names the **three** the memo relies on, including "A hanging glyph is **also not taken into account when computing intrinsic sizes** (min-content size and max-content size), and any sizes derived thereof", which is the sentence §8's end-of-line white-space prereq block reaches, and says which paragraphs the memo does **not** reach. **(c)** step 4's `pre-wrap` quote dropped its second sentence with no ellipsis; it is quoted whole. **(d)** cell **13d**'s "five where today's pipeline emits one" counted the span's own rects while the display list also carries the line's text runs — scoped; and its `tests.rs:41`/`:127` cited two tests with different entry points as one site — now named, `build_pipeline_interactive_returns_all_fields` and `background_color_in_pipeline`. **(e)** **A68**'s swept list omitted §5.3's PR-1b bullet, which rev 64 did sweep; **A68 is committed**, so the omission is recorded here. **(f)** **A67**'s three review tallies ("1 CRIT / 15 IMP; 1 CRIT / 4 IMP; 1 CRIT / ~14 IMP") name no command or artifact and cannot be re-derived from the repository — they are **process figures, not measurements**, and are marked as such here rather than given a provenance they never had. Found by an enumeration attestation of revs 62–67 |
| A86 | R37 | §5.3's `#11-inline-box-decoration-splits` routing recording only the lost start edge and the rect geometry of a box that opens at a soft wrap | the same advance places the box's **metric** too, and nothing recorded it: M6 applies the marker's `block_advance` at `InlineBoxStart` (§5.1 M6, whose own ⚠ then replays it for every continuation line through M4's hook), and M3's unconditional start-edge advance keeps that marker on **line N**, so after PR-1d a box opening at a wrap raises line N's height by its own `line-height` as well as its continuation line's — on a line css-text-3 §5.5 does not put it on and where this memo's M4 says it has no fragment. Nothing discriminated it: 17c carries the inherited line height and is PR-1c, 24e's outer box opens on a line it **has content on**, and the remaining PR-1d height cells are single-line (checked: 23, 23b, 24, 24b, 24c, 24d). The consequence is routed into that slot beside the edge and rect geometry — one deviation, three observables — and new PR-1d cell **24g** pins **elidex's value** (both lines at 100px) in the shape cell 23 uses, naming the conformant one (line N inherited, line N+1 at 100px) and rejecting a producer that drops the replay. ⚠ **The cell asserts the divergence rather than the conformant outcome**, which is the opposite of the finding's suggested assertion: under the planned mechanism the metric lands where the marker is consumed, so a cell demanding the conformant value would be red at PR-1d and would be a requirement on work this memo routes elsewhere — the memo's rule is to state the defect and pin what ships (A48). Found by Codex on #515 |
| A87 | attestation of revs 68–69 | the memo restating **derivable totals** in prose — the front matter's "ten sections … the other three untitled" against rev 69's "eleven / four" six lines below it, and the freeze's running cell figure | M7's own rule already says it: a count restated away from the enumeration it summarises drifts from it. So the totals are **removed, not re-synced**, wherever a published command derives them. **Retired**: §9's CSS 2 cite list total and the front matter's copy of it — the list is `grep -o 'CSS 2 §[0-9.]\+' <memo> \| sort -u` (11 lines at this revision, a number this row states as an observation and the memo does not); the front matter's untitled-cite count, whose members are now named instead; and the freeze's per-revision cell totals, `python3 .claude/tools/plan-xcheck.py <memo>` printing the figure (its `cells N defined / N routed` field) while the memo keeps only which revision added which cell. **Kept, with the reason**: the front matter's **seven** section↔title pairs — no command enumerates them, each is checked with `webref` one at a time — and §3's `K=`/`M=`, which the checker itself recomputes and fails on drift, so the figure and its checker are the same artifact. **Undecided, reported rather than edited**: the ledger's own per-round finding tallies (**A67**'s three, and the review counts in A63/A81), which name no artifact; they are process figures and A85(f) already marks them as such. Found by an enumeration attestation of revs 68–69 |
| A88 | attestation of revs 68–69 | ten sites left stale by revs 68–69's own corrections, and the **enumerated-sweep idiom** that hid them: A80's "Swept: …" list and A82's scoping each claimed completion while live sites survived | an enumerated sweep is a snapshot with a completeness claim attached; the population belongs to a command. **This row's sweeps, as commands and their output at the fixed head**: ordering — `grep -n 'ahead of PR-1c\|before PR-1c\|PR-1c depends on it' <memo>` returned seven lines, of which four were stale (the front matter, §5.3's slot bullet, §9's staleness bullet and §8's PR-1c block) and three are history or PR-1c's own inheritance; the paint-creation claim — `grep -n 'created here' <memo>` returned three, one live (§8's PR-1c block) and two ledger records. **Corrected**: the front matter and §5.3/§8/§9 now order the reconciler before **PR-1b** (**A80**); §8's "Before PR-1c nothing was painted … created here" is false of the positioned arm and says so; §7's item 1, which had "PR-1c — **not** PR-1b — moves it", now states that **both** move a `position:relative` box's painted rect — PR-1b through the `LayoutBox.content` written from `place_item`'s rect (`pack/mod.rs:706-714`, `pack/boxes.rs:56-81`), PR-1c through M4's edges — A33's probe having varied the edges only; §3's step-4 row and discipline 5's enumeration now scope the umbrella's share to **`normal`**, `nowrap` and `pre-line` going to the prereq with every other value (**A82** scoped); §3's §8.2 *Covers* column is marked a **seed** with the section's own enumeration command, claiming no completeness, and names the conditional-hang paragraph the memo reaches elsewhere; §5.2's `collect.rs` row says **PR-1b's** plan (**A81**); the freeze summary says M3 keeps **three** things; cell 17c scopes `normal`'s removal to a **collapsible** space; cell 25d arm 1 re-states its reason — the negative sum is licensed by §2.2's "Intrinsic size contributions are based on the outer size of the box", and what the arm withholds is a **pass-level** value, because where the floor is applied is PR-1b's; cell 3c's "every M8 cell is padding" is re-derived; `tests.rs:119-132`; §8's PR-1b "first of the **three**"; §2.1's "Usually" capitalised as the spec has it. ⚠ **Committed rows corrected here, not edited**: **A68**'s flat "single unbroken line" (A83 credited its fix to cell 25e; the site is §8's min-content instance 5), **A80**'s and **A82**'s completion claims, **A67**'s and **A76**'s superseded clauses — each stands as written. Found by an enumeration attestation of revs 68–69 |
| A89 | R38-F2 | rev 70's own ordering sweep writing "shippable **ahead of PR-1b**" inside §5.3's **PR-1b** bullet | a phrase-level replacement of `PR-1c` by `PR-1b` across a grep's hits, which is the method A88 adopted and the defect it did not anticipate: the sentence's argument — edges stay `EdgeSizes::default()` until PR-1c, so no `border_box()` reader changes extent — establishes that PR-1b may precede **PR-1c**, and nothing may precede itself. Restored, with the mis-replacement recorded in place. **Re-read of every site that sweep touched**, by its own commands at this head: `grep -n 'ahead of PR-1[bc]\|before PR-1[bc]\|PR-1[bc] depends on it\|inheriting that' <memo>` returned 28 lines at the head this row was written against (it returns more once these rows are in the file, which is why the number is dated rather than carried); each was read **in full**, and two were wrong — this one and §10's staleness row, which still said the slot is discharged "ahead of PR-1c". The remaining 26 are correct: 14 in ledger rows (history), 8 stating the new order, 4 about other PRs' own ordering. **The lesson recorded with the correction**: a sweep by phrase needs the hit read for what the sentence argues, not only for the token — A88's commands give the population, they do not give the verdict. Found by Codex on #515 |
| A90 | R38-F3 | the min-content prerequisite's defect statement leaving **atomics** out of the joining — "cross-item joining reads `InlineItem::Text` and no marker" | an atomic's two boundaries are soft wrap opportunities whatever its width (css-text-3 §5.5's Web-compatibility bullet), which is why §8's suite-level break invariant encodes every atomic item as **U+FFFC** before calling `find_break_opportunities` (ledger **A37**'s measurement). So a joiner that joins across every item boundary reports `measure_width("ab")` for `a<span style="display:inline-block;width:0"></span>b` where min-content is `max(|a|, |b|)` — and today's function passes by accident, walking each `InlineItem::Text` alone, so the defect is one the **joining introduces**. Folded in as the prereq's instance **6**, with §6 cell **25f** pinning it as a guard on that PR's base, like cell 25. **Not** `#11-inline-atomic-intrinsic-contribution`'s: that slot's subject is the atomic's own contribution (zero), and this instance holds with a zero-width atomic. Found by Codex on #515 |
| A91 | R38-F4 | `max_content_inline_size` summing every run and every marker edge into **one** running total across a **forced** break | css-sizing-3 §2.1 suppresses the **soft** wrap opportunities only — the max-content inline size is what the box would take "if none of the soft wrap opportunities within the box were taken" — so a preserved newline still breaks the line and the contribution is the widest forced line, not the sum. Measured: the pass sums `measure_text(&run.text)` over each `InlineItem::Text` (`inline/measure.rs:52-58` at `154bac3f`) and a `white-space: pre` run reaches it with its `"\n"` inside, the forced split being `pack/items.rs`'s at layout time, so the pass has no notion of a forced break at all. **Folded into the min-content prereq's segmentation defect as instance 7 rather than into M8**: the defect is that the pass's segmentation is not layout's, which is that prereq's subject and now covers both passes (**A68**), and M8's edge term rides on whatever accumulator it builds — as instance 1 already does for min-content. ⚠ **M8's Decision goes stale when that PR lands**: it states the edge term "matching that pass's existing `total +=` shape", and the shape is what instance 7 changes. M8 is a **frozen** column, so this row **flags** it and amends nothing. No M8 cell contains a mandatory break, so nothing in §6 is falsified today. Found by Codex on #515 |
| A92 | R38-F1; decision, 2026-09-26 | M4's flush-time emit rule — "emits a partial rect only for an entry whose span is non-empty" — as a universal, and with it cell 17g's `line_rects.len() == 2`, which that rule cannot produce | measured at `154bac3f`: in 17g's `<pre><span style="padding-left:10px"><span>\nx</span></span></pre>` every run is keyed to the **inner** span (`place_item` pushes the run's own entity under `entity != self.parent_entity`, `pack/mod.rs:703-714`), the outer box's content-start is the cursor after its 10px and the preserved `"\n"` advances it by that run's width alone, so at the forced flush the outer entry's span is empty and the hook emits nothing — `len` is 1, not 2. **Both repairs touched a frozen column**: narrowing 17g contradicts M4's own outcome paragraph ("a forced break inside it fragments it … so it has a rect on both lines (cell 17g)"), and changing the filter is the Decision's mechanism half. The **spec picks**: css-inline-3 §2.1 splits a box that "contains a forced line break" into fragments partitioned across line boxes; css-break-3 §2 makes each a *box fragment* with "its own share of the box's border, padding, and margin" and says inline breaking "similarly creates box fragments"; cssom-view-1's `getClientRects()` step 3 asks a rect of each "(including those with a height or width of zero)". **A55** had already withdrawn the filter's only ground for this case. So the emit rule is **amended, scoped to forced breaks**, as a freeze amendment in rev 60's and rev 63's convention, recorded in the freeze block with its date; the outcome (the fragment exists) is the umbrella's and recording a zero-span entry stays PR-1c's. ⚠ **OQ-1 is untouched**: at a soft wrap §5.5 puts the whole box on the next line, so no fragment exists on the earlier one — the amendment turns on *where the box is*, not on whether a zero-span rect may be emitted. **Sweep, two commands, every hit read in full.** The filter's own wording: `grep -nE 'non-empty|no partial rect|partial rect|zero[- ]span|emits nothing|emit filter' <memo>` returned 23 lines at the head this row was written against — exactly **one** of them states the rule (M4's Decision, amended here), one is **A55**'s record (committed, standing), and the other 21 are unrelated uses of those words, so the first command does **not** reach the cells. The cells were found by asking what depends on the rule instead: `grep -nE 'no rect on line|contributes no rect|line_rects\.len|yields none' <memo>` returns the three that do — **17c** and **17d(b)**, both soft-wrap cases, each now saying the amendment does not reach it, and **17g**, which the amendment makes reachable — plus OQ-1's own statement, §3's CSSOM row and `boxes.rs:102`'s `len() > 1` gate, none of which the amendment moves. Found by Codex on #515, decided on the lens |
| A93 | R40 (four P2s, one class); decision, 2026-09-26 | the memo's own split between **ordering a pre-existing defect before a PR** and **registering it as a slot** as a per-case judgment, argued locally at each of the three carved prerequisites and nowhere stated | the two are one rule, and §5.3 now states it: a pre-existing defect this program makes **newly reachable, or newly visible to an author** is ordered before the PR that makes it so; one the program leaves as it found it stays a slot. The three carves are its precedents (the reconciler PR, A80; the min-content PR's own words; the end-of-line white-space PR, A66), and the **pre-existing** test the memo already had decides *class* where this decides *order*. The audit is the population command §5.3 carries, `grep -o '\`#11-[a-z0-9-]*\`' <memo> \| sort -u` filtered by check 8's definition test, and its outcomes are **A94**, **A95** and **A96**. ⚠ Two entries it does **not** dispose of: `#11-inline-box-decoration-splits` is newly visible at PR-1c by this rule, but the memo's standing decision pins elidex's values there as accepted divergences (cells 17d(b), 17f, 24g), so applying the rule would reverse a decision rather than follow one — left as it is for the user; and `#11-inline-zero-edge-box-in-item-stream` is an **own** deferral of this program's own new behaviour, which the rule does not speak to. Found by Codex on #515 |
| A94 | R40-F1 | `#11-inline-item-boundary-soft-wrap` as a slot, on the ground that `place_item`'s guard is unchanged by this program | the guard is unchanged and the **input** is not: PR-1b adds the marker's advance to `current_inline` before the next item is tested (§5.1 M3), so `<p style="width:W">a<span style="padding-left:20px">bbbb</span></p>` flushes before `bbbb` at every `W` in `[measure_width("a") + measure_width("bbbb"), … + 20)` — widths whose line is correctly unbroken on `154bac3f`, since `find_break_opportunities` is UAX #14 over one string (`crates/text/elidex-linebreak/src/lib.rs:26`) and css-text-3 §5.5 grants no opportunity at an inline box boundary. Newly visible by **A93**, so it is §8's **item-boundary opportunity prereq**, in `main` before PR-1b, with §6 cell **15e** pinning `line_count == 1` across the PR; the slot stays registered for the engine-wide residue. Found by Codex on #515 |
| A95 | R40-F3 | `#11-pseudo-generation-on-replaced-originating-element` as a slot, on the ground that the missing replacedness test is entirely pre-existing | the test is pre-existing and its **consequence** is not: `img::before { content: ""; padding: 10px }` generates a pseudo css-pseudo-4 §4.1 suppresses, and today it is inert because the collect branch pushes a `Text` only for non-empty content (`inline/collect.rs:260-272`), while PR-1a brackets that branch with a marker pair (§5.1 M1, §8 requirement 7), PR-1d commits the line and PR-1c the `LayoutBox`. Newly reachable by **A93**, so it is §8's **replaced-origin pseudo prereq**, in `main` before PR-1a, with §6 cell **6j** pinning that no pseudo and so no marker exists; its classification input is the predicate prereq's replacedness half, already ordered ahead of PR-1a. Found by Codex on #515 |
| A96 | R40-F4 | ordering `#11-used-direction-text-orientation-upright` before a PR of this program, as the other three were | measured, the defect is author-visible today with no inline box in the markup, so **A93**'s rule does not reach it: `LogicalEdges::from_physical` swaps the inline edges in a vertical mode on `is_inline_reversed()` (`crates/core/elidex-plugin/src/logical.rs:204-210`), and a block box already maps its padding and border through a context built from the **computed** `direction` (`crates/layout/elidex-layout-block/src/block/mod.rs:158`, `:187-188`). The program adds one more caller of a shape nine production sites already have (`git grep -n 'WritingModeContext::new(' 154bac3f -- 'crates/**/*.rs'`) and no case an author could not already reach. It stays a **slot**, with the coverage limit written at its §9 registration: the vertical cells 12c and 12f take the default `ltr`, the `rtl` cells 12b and 12e are horizontal, and rev 72 adds no cell here. Found by Codex on #515 |
| A97 | enumeration attestation of revs 70–71; user decision, 2026-09-26 | **A92 itself** — the rev-71 reopening of M4's flush-time emit rule, and the premise it rested on: that in cell 17g's `<pre><span style="padding-left:10px"><span>\nx</span></span></pre>` the outer box's line-1 cursor span is **empty** | measured at `154bac3f`, and the premise is false. `is_collapsible_space` is `matches!(c, ' ' \| '\t')` (`inline/whitespace.rs:162-164`), so a preserved `"\n"` is never trimmed and `trimmed_width == full_width`; `place_item` runs **before** the forced flush (`pack/mod.rs:591-600`, then `if *break_after == Some(BreakOpportunity::Mandatory)` at `:602` and `force_break()` at `:603`), so that run's advance is already in `current_inline`; and `find_break_opportunities` filters only the end-of-text break (`crates/text/elidex-linebreak/src/lib.rs:31-33`), so `"\nx"` yields `(1, Mandatory)`. Line 1's outer span is therefore the `"\n"` segment's width, and "empty" required `measure_text("\n") == 0` — a font metric **A92 never measured**. It is **non-zero** in every `TEST_FAMILIES` font installed here: Arial **12.0** at `font-size: 16px`, Helvetica **4.445**, Hiragino Sans **5.328** (measured by the attestation against the same crates, `rustybuzz 0.20` / `fontdb 0.23`, and the path takes no filter — `buffer.push_str(text)` at `crates/text/elidex-shaping/src/shaping.rs:121`, summed into `total_advance` and returned as the width at `measurement.rs:56`/`:66`). Arial's number is corroborated from the font file itself, independently of the engine: glyph 0's `hmtx` advance is **1536** at **2048** upem = 12.0 px at 16 px (`/System/Library/Fonts/Supplemental/Arial.ttf`, `head` + `hmtx` parsed directly). ⚠ **Not always `.notdef`**: Helvetica's glyph-0 advance is 10.14 px at 16 px while `measure_text("\n")` is 4.445 there, so that font maps the LF elsewhere — the load-bearing fact is *non-zero*, not *which glyph*. **So `len == 2` was already reachable under the frozen rule**: the amendment bought nothing, the rule is restored, the freeze's reopening count goes back to two, and cell 17g keeps `len == 2` with line 1's rect starting at the outer's content-start — its `padding-left`, which the cell's neighbouring sentence already said. ⚠⚠ **A frozen column was reopened on an unmeasured premise**, and the premise was one measurement away throughout. ⚠ **The amendment had also been spliced *inside* that column** rather than recorded beside it: rev 71 broke M4's table row into three physical lines (the row text, the amendment paragraph, the OQ-1 paragraph plus the row's whole remainder) and, in the splice, inserted "at a soft wrap" into the frozen emit sentence and dropped its "would duplicate" clause. The withdrawal restores M4's row **byte-for-byte to its rev-70 text** (`git show 60c79448:<memo>`, the single `| **M4**` line), so the row is one line again and the frozen wording is back; **A92 is committed and stands as written**, withdrawn here. Two of its other supports go with it: the rule cannot turn on **break kind** — a soft wrap **inside** a box splits the box too (the attestation's counterexample, a zero-width U+200B inside the inner span), so the only discriminator the case supports is **where the box is**, which is what OQ-1's answer already turns on; and A92 quoted css-break-3 §2 without its second half — "Breaking inline content into lines is another form of fragmentation, and similarly creates box fragments when it breaks inline boxes across line boxes. **However, inline breaking is not covered here; see [CSS2]/[CSS3TEXT].**" (`body css-break-3 fragmentation-model`) — a referral away from that section rather than a warrant inside it, now carried whole in §3's new css-break-3 §2 row. Found by an enumeration attestation of revs 70–71 |
| A98 | enumeration attestation of revs 70–71 | **A88**'s published sweep output — "`grep -n 'ahead of PR-1c\|before PR-1c\|PR-1c depends on it' <memo>` returned seven lines, of which four were stale … and three are history" | seven at no head. The command is **line-oriented** (`/usr/bin/grep -c`, one count per line however many phrases match it), and against the file at each head it returns **12** at `81528113` (rev 69, the head rev 70's sweep was run against), **8** at `60c79448` (rev 70, the revision A88 records) and **8** at `0bade5b7` (rev 71). What the total cannot tell anyone is how many lines were *edited*: an edited line that still carries a matching phrase stays in the count, so "four stale, three history" is not derivable from it either — and rev 71 returned to two further sites (**A89**). **A88 is committed and stands**; the reconciliation is here, and from this row on every published figure names the head it was measured at and whether it counts lines or occurrences. Found by an enumeration attestation of revs 70–71 |
| A99 | enumeration attestation of revs 70–71 | **A89**'s published figures — "returned 28 lines at the head this row was written against … 14 in ledger rows (history), 8 stating the new order, 4 about other PRs' own ordering" | the pair reconciles at no frame. Measured with A89's own command, line-oriented: **28** at `81528113` (rev 69), **29** at `60c79448` (rev 70, A89's base) and **30** at `0bade5b7` (rev 71, where A89 was written) — so 28 is two heads behind the row that publishes it. Of those lines the ones that are ledger rows (`\| A` at the start) are **7 / 8 / 9** at the three heads, and occurrence-level counts (`grep -o … \| wc -l`) are **32 / 35 / 39** with **9 / 12 / 15** inside ledger rows: **14 appears in neither convention at any of the three heads**. The breakdown was internally consistent (14 + 8 + 4 + 2 = 28) and externally unmoored, which is how it survived a round. **A89 is committed and stands**; corrected here, with the head and the convention named. Found by an enumeration attestation of revs 70–71 |
| A100 | enumeration attestation of revs 70–71 | eight smaller defects, grouped because each is one sentence and none changes a decision | **(a)** cell **3c**'s "every other M8 cell — 25, 25b, 25c, 25d, 25e — carries padding or margin and no border" omitted rev 71's **25f**, which carries none of the three; the conclusion survives and the enumeration is replaced by the property with its command. **(b)** cell **25e**'s "25, 25b, 25c and 3c all carry text" omitted 25f, which does; re-stated as the property. **(c)** §8's PR-1b block called the end-of-line white-space prereq "the first of the three to land" while §9 orders the reconciler against no other prereq, so it may land first; the ordinal is dropped and the sentence says only that it lands before PR-1b. **(d)** §8's min-content preamble counted instance **6** among the defects "seen", where its own body and **A90** make it a guard on the PR's base; the guard is now stated apart from the present defects. **(e)** `pack/boxes.rs:56-81` and `:65-81` stopped **mid-literal** — the `LayoutBox` literal is `:80-87` and the write is `dom.set_layout_box` at `:88` — and `:703-715` was the second range in use for `place_item`'s push, whose statement ends at `:714`; every site now ends at the statement that makes its claim true. **(f)** bare `boxes.rs:N` was ambiguous between `inline/pack/boxes.rs` (129 lines at `154bac3f`) and `elidex-plugin/src/layout_types/boxes.rs`; every bare cite outside committed ledger rows **and outside §5.1's frozen Decision columns** is now file-qualified, as is `inline/tests/mod.rs:31`; **M4**'s own are inside that column and are left for the user — `python3 -c "import re;print(sorted(set(re.findall(r'\`boxes.rs:[0-9-]+', [x for x in open(MEMO) if x.startswith('| **M4**')][0]))))"` lists them — and one of them, `:80-81`, is also a range stopping at the literal's `content` field rather than at the `dom.set_layout_box` write the row itself cites at `:88`. **(g)** §3 had no row for **css-break-3 §2** or for **css-sizing-3 §2.1 / §2.2** although cells 17g, 25d, 25e and §8's instances rest on them; three rows added (M=53), each with the sentence it relies on quoted whole, and **A84**'s "§2.2 presupposes a negative contribution" corrected where cell 25d restates it — §2.2's floor is between the two contributions, not against zero (A84 is committed and stands). **(i)** the completeness sweep this row's (a) and (b) call for — every sentence, de-wrapped, carrying a completeness word and a run of three or more cell ids, committed ledger rows excluded — returns **one** live site, §5.3's "Cells 6b, 6i, 6g and 6h are the exceptions", which rev 72's own cell 6j had just made false; it now states the property ("pin *new* item-stream behaviour") instead of exclusivity. ⚠ **A second blind spot, measured rather than assumed**: `plan-xcheck` flags a plain-text reference to a cell §6 does not define, and misses the **same** reference in bold (planted both ways: the plain form returned `[CELL] … referenced but §6 does not define it`, the bold form returned 0 contradictions). Sweeping the whole memo for bold id-shaped tokens absent from §6 returns only `24f`, whose label the memo records as withdrawn (R10), and both hits are history. The gap belongs to the **tooling PR** (§9). **(h)** §8 instance 3 quoted css-text-3 §5.5 without its opening exception ("Except where explicitly defined otherwise (e.g. for `line-break: anywhere` or `overflow-wrap: anywhere`)"), which its fixture satisfies by setting neither property; quoted whole. ⚠ **One item is not taken**: §5.1 **M4**'s own "which edge **survives** a break" repeats a term this memo already retired once (§3's css-break-3 §5.4 row records the retirement — "this row wrote 'surviving edge' until R5"), but it sits inside a **frozen Decision column**, so it is flagged here and left for the user; the two copies outside that column (§9's bidi bullet and cell 17g) now say *broken* edge as §5.4 does. Found by an enumeration attestation of revs 70–71 |
| A101 | R40-F2; user decision, 2026-09-26 (option (f)) | a **gate** on the bidi loss — anything making a PR of this program wait on it — and the **predicate** the memo stated that loss on: `direction == rtl`, in the words "an RTL paragraph with a padded inline" (§5.3, §7, §9) | **no gate, and the predicate is the renderer's own.** Measured at `154bac3f`: the branch fires on `direction == Direction::Rtl \|\| <any `Text` run holds RTL characters>` (`crates/core/elidex-render/src/builder/inline_flow.rs:140-143`), so an **LTR** paragraph with two Hebrew runs reorders — the crate's own `converged_bidi_reorders_runs` is that shape, a block whose `ComputedStyle` leaves `direction` at its default with text `"אאב"` (`builder/tests/inline_flow/bidi.rs:18-70`) — while `bidi_visual_order` returns identity for a single run that is not itself RTL (`builder/bidi.rs:29-31`, discarded at `:153`), which is why the fixture offered at R40, `<p dir="rtl">a<span style="padding-left:10px">x</span></p>`, is on the **identity** path and paints the gap: right class, wrong example. The reorder branch repaints from `min(inline_start)` by shaped width (`:169-173`), dropping every baked between-run offset — after PR-1b that includes M3's advance. And the identity path is **not** the unaffected one: `InlineFlowRun::Text.inline_start` is the absolute **physical x** (`crates/core/elidex-ecs/src/components/inline_flow.rs:35-42`) while the packer's cursor runs left-to-right whatever the direction, so an `rtl` box's edges land mirrored — `<p dir="rtl"><span style="padding-left:10px">שלום</span></p>` paints `[שלום][10px]` where css-writing-modes-4 §2.4.5.2 puts the left padding on "the left-most fragment of the last line box in which the box appears" (`body css-writing-modes-4 bidi-fragment-boxes`). **Recorded as one divergence over the whole program**, stated once at §9 on both halves and owned by `#11-bidi-full-uba-fidelity`, with three cells in cell 17c's registered-divergence idiom: **12b** gains a painted assertion for the mirrored identity half, and new cells **12h** (LTR IFC) and **12i** (RTL IFC) assert that the painted glyph positions are unchanged by the `padding-left` — elidex's value asserted, the conformant value named, the flip conditions named (the branch carrying per-run offsets through; the IFC cursor mirroring for `rtl`), and neither flip in any PR of this program. All three read the display list through `elidex-shell`'s pipeline, keeping §5.2's `elidex-render` row a negative one (**A73**). Found by Codex on #515, decided by the user |
...
| A115 | R45-F1 | cell **25g**'s ⚠ tail as written at rev 76 — the two runs' advances summed called a value "not a statement about shaping across the boundary", with `measure_width("bc")` shaped whole handed to `#11-intra-word-shaping-across-line-break` as its question | wrong twice over, and the spec settles it in the cell's favour. **(a)** That slot's subject is a word split by a **line break** — css-text-3 §5.5's intra-word clause, "the characters must still be shaped … as if the word were still whole" across a break — and 25g's fixture has no line break at all, so the slot cannot own it; the memo's own sibling-distinction note already says as much ("`#11-intra-word-shaping-across-line-break` is about a word split by a **line break** … this one is about a boundary **within** one line"). **(b)** Read whole, css-text-3 §7.3 has **three** normative sentences (`body css-text-3 boundary-shaping`; §1.4 counts them), and the **first** applies: "Text shaping must be broken at inline box boundaries when any of the following are true for any box whose boundary separates the two typographic character units", first bullet "Any of margin/border/padding separating the two typographic character units in the inline axis is non-zero". The fixture's two 100px paddings separate `b` and `c` in the inline axis, so the break in shaping is **required** and the summed advances are the **conformant** value — not an interim one. The second sentence ("must not be broken … when there is no effective change in formatting") is the one `#11-shaping-break-at-unchanged-inline-boundary` owns, and a non-zero padding is an effective change, so that slot is not implicated either. **Swept for the same mis-attribution**, line-oriented and de-wrapped, at `4767086a`: `grep -n 'intra-word-shaping-across-line-break' <memo>` returns **8** lines and the de-wrapped scan the same **8** occurrences; every one read in full, and only 25g's cited the slot for a fixture without a line break — the rest are the slot's own registration (§5.3), §3's css-text-3 §5.5 row, the sibling-distinction note, §9's non-coverage bullet, §10's action row and two history rows. ⚠ **Provenance**: the hand-off was written at rev 76 to the review brief that approved it, and the brief's own framing carried it — the memo's §1.4 had the three sentences quoted since R20, so the applicable sentence was available in the document at the time. Found by Codex on #515 |
| A116 | R45-F2 (the session's first P1); measured, then decided without a freeze amendment, 2026-09-26 | PR-1b's advance as unable to commit a line wider than its containing width, and §1.3's dichotomy — splitting at opportunities inside the box, "never at its edges", with overflow as the exception for a box with no internal opportunity — as exhaustive | measured at `154bac3f`, the advance **can** commit an over-width line: `place_item`'s guard tests content only (`pack/mod.rs:690-692`), the marker path adds its advance through `note_line_occupancy` (`:695`) with no wrap check (§5.1 M3), and `finish()` commits an occupied line with no width test (`:786-790`) — so where nothing follows the `InlineBoxEnd`, the inline-end edge pushes the committed line past `W`. **Outcome, stated in §5.3's PR-1b bullet**: the line takes the **earlier licensed opportunity** and the box moves whole to the next line. Grounds quoted there: css-text-3 §5.5's single bullet, "before the first **or after the last character** of a box … at its margin edge", which covers both directions — so the end edge needs no clause of its own and rests on the same sentence as OQ-1's start-edge answer — and **css-text-3** §3's `normal`, lines "may wrap at allowed soft wrap opportunities … **in order to minimize inline-axis overflow**". §1.3 now carries the third case the dichotomy omitted: a break **outside** the box at a licensed opportunity, which is neither splitting at an edge nor overflow, and which the memo already adopted for the start edge (cell 17c, OQ-1). **No amendment to M3 was needed, and that is the decisive measurement**: M3 forbids a wrap check *on the marker path* on the ground that "a box boundary is not a break opportunity", while this outcome breaks at an earlier **licensed** opportunity — the marker path stays checkless, no boundary becomes an opportunity, and M3 as written is consistent with the outcome. M3 also has no subordination clause (`governs` occurs zero times in the row, unlike M4's "the outcome governs"), so had an amendment been needed it would have gone to the user. **Mechanism facts, recorded as facts**: the only shape that breaks no existing cell scopes the pending edges to boxes closing before the next licensed opportunity, which consumes the canonical opportunity API the item-boundary prereq owns (**A110**) and is already ordered before PR-1b — no new ordering; the cheaper running-sum-of-all-open-boxes shape is over-conservative and breaks cells **17**, **17d(b)** and **17f** (on `width:80px` with `padding:10px`, it flushes before `bbb` and moves the whole box to line N+1, destroying the two-fragment geometry those three read off one geometry); a rollback would make `place_item`'s five-write sequence reversible. The choice is PR-1b's plan-review's. **§6 cell 15g** pins the outcome as a **flip** (`line_count` 1 → 2), and it is deliberately not in §8's flip partition, which is PR-1d's. ⚠ **Not A19 re-raised**: **A19** (R6-b) withdrew the near-identical `aaaa ` + box + `b` fixture because css-text-3 §5.5's bullet is undetermined for a **characterless** box — "an empty decorated inline has no characters, so which side of it the break falls on is undetermined" — and this fixture's span **has** a character, so the bullet is determinate and the case is new. ⚠ Round 26's CRIT on this same guard settled its **rung** (an order test at-or-above `Content`, cell 15d discriminating it) and says nothing about when the guard runs, so the timing question was untouched until now. Found by Codex on #515, measured this round, decided without reopening the freeze |
| A117 | enumeration attestation of revs 72–77 | eight defects, **all eight authored inside revs 72–77 and five inside the revision that was fixing the previous batch**, grouped because none changes a decision | **(D1)** a **fabricated verbatim quotation**: §8's replaced-origin pseudo block quoted css-pseudo-4 §4.1 as "this means that **generated content is** suppressed when their parent …" where the spec reads "the `::before` and `::after` **pseudo-elements are** suppressed when their parent …" (`body css-pseudo-4 generated-content`) — the rewrite also left "their parent" dangling, and §3's row already carried the correct sentence, so the memo held two inconsistent "verbatim" copies of one sentence. Corrected to the spec's words. **Every spec quotation added in revs 72–78 was then pulled and diffed against `webref body`, not read**: **21** quotations checked against ten fetched section bodies, **1** differed — this one. ⚠ Three further "differences" were the **checker's** fault and are recorded because the shape recurs: `unicodedata.normalize('NFKC', …)` rewrites `…` to `...`, so splitting an elided quote on `…` silently produced one unsplittable part and reported a miss; the fix is to split after normalising. **(D2)** §9's "they carry no §10 row" is false of `#11-bidi-full-uba-fidelity` since rev 73 added its enrichment row; check 8 is one-directional, so the tool asserts no absence — the sentence now says the absence is a **seed** and gives the intersection that is checkable. **(D3)** **A107** contradicts itself — "the block now names four" beside "All four figures are gone rather than corrected" — and §8 reads "It owes four dependencies". Measured at `4767086a`: of the four figures, **three were removed** (the min-content block's "like its five siblings", the topology's "after **three** of them" and "The remaining three are **pending**") and **one was corrected** (§8's PR-1b "owes three" → "owes four", which is the authoritative site and says the count lives nowhere else). A107 is committed and stands; this is its correction. **(D4)** the published command `git grep -n 'generate_pseudo_entity(' 154bac3f -- crates/css/elidex-style/src/walk.rs` returns **four** lines, not five: `:14` is the `use` import and matches only without the trailing `(`. The **figure** was changed, not the command, and the import is now named as not a call site. **(D5)** the freeze's cell-add history carried a ledger-id parenthetical that had drifted twice (it stopped at A112 while rev 77's cell is A113's and rev 78's A116's); the list is **removed** and the property stated — each cell's own row names its id. **(D6)** **A100(g)**'s "(M=53)" is a derivable total in prose with no head and went stale at rev 73, when the checker began printing 54; A100 is committed, so it is corrected here rather than edited, and the figure belongs to `plan-xcheck`'s output per **A87** and **A98**. **(D7)** the front matter said the prereq PRs are "counted there" in §8, which provides blocks and no count; it now says one block each and no count anywhere. **(D8)** two doubled words straddling line breaks ("and / and", "the / the"), both invisible to a line-oriented grep; a **de-wrapped** sweep over the whole memo found those two and nothing else — five apparent hits are the `webref css <module> <name>` command form and one is a grammatical "that that". **Hygiene taken with them**: the bare coordinates after a `builder/bidi.rs` path in cell 12b and §9 now name `builder/inline_flow.rs`, whose lines they are; cell 12h no longer claims the crate's `converged_bidi_reorders_runs` has runs with differing entities (both carry `entity: div`, `:42` and `:47`) and separates its own fixture's entities from the test's reordering shape; cell 12i's "through `direction == Rtl` alone" now says that holds only in the `||`'s evaluation order, since `reorder_by_levels` is identity for two level-2 runs; §9's bidi half no longer says "a box whose **own** direction is `rtl`" flatly but names the **two** directions — elidex's own-direction mapping against css-writing-modes-4 §2.4.5.2's "the parent's `direction`" and css-break-3 §5.4's "the parent element's inline progression direction" — and books the mismatch as a second divergence on the same slot; §3's css-break-3 §2 row's Purpose now claims the **term** rather than the per-line rule its ⚠ refers away, and names the section's third normative sentence (bidi and display-type splitting) rather than leaving it unquoted and unaccounted; and the live bare `§N` that mean a spec section are qualified — one of them, a bare `§5.4` in **A100**, is committed and stays flagged, and it collides with this memo's own §5.4. ⚠ **Not touched**: the frozen M4 column's eight bare coordinates, flagged as A100(f) leaves them. Found by an enumeration attestation of revs 72–77 |
| A118 | the same attestation; two tooling findings and one premise correction | the brief's framing of the delta as **A86–A114**, and `plan-xcheck`'s coverage as reaching every ledger-integrity property | **(a)** the delta is **A93–A114** — A86–A92 predate rev 72 — so **A92**, the row rev 72 withdraws, is a *previously attested* row whose committed wording is outside this delta's authored surface; the withdrawal (A97) and this batch stand, but nothing in revs 72–78 authored A92's text. **(b)** a **second checker blind spot, planted and measured**: a duplicate ledger id goes undetected — a planted second `A113` in place of `A114` returned **0 contradictions** — alongside the known blindness to a **bolded** `cell **N**` reference. The memo's own duplicate-id check is the shell one-liner the battery runs (`grep -o '^| A[0-9]* ' <memo> | sort | uniq -d`), which is not in the tool, so a revision that skips the battery loses it. **Both blind spots are added to the tooling task's scope** (§9): a duplicate-id check and a bold-aware cell-reference scan. ⚠ The pattern behind both is the one this memo keeps recording — the checker's populations are what it parses, and a property nobody parsed is a property nobody checks. Found by an enumeration attestation of revs 72–77 |
...
unit whose inline-axis edges the line cannot hold (§5.3's PR-1b bullet; rev 81 widened it from rev
...
character is a space and its `InlineBoxEnd` sits at the opportunity's own offset (§6 cell 15g(h);
rev 81, ledger **A123**).
...
| CSS Break 3 §2 Fragmentation Model and Terminology | *box fragment* | the **term**, and that each fragment carries its own edges — not the per-line rule, which this section refers away | verbatim: "Breaking inline content into lines is another form of fragmentation, and similarly creates box fragments when it breaks inline boxes across line boxes. **However, inline breaking is not covered here; see [CSS2]/[CSS3TEXT].**" and "Each fragment has its own share of the box's border, padding, and margin" (`body css-break-3 fragmentation-model`) — M4: a forced break inside a box gives it a fragment per line (cell 17g), the rects **PR-1c**'s and the line **PR-1d**'s. ⚠ The second sentence refers inline breaking *away* from this section, which is why the Purpose above claims the term and not the rule; the per-line rule is css-inline-3 §2.1's row, and which side of a fragment is the broken edge is the **css-break-3 §5.4** row's (not this memo's own §5.4). ⚠ The section's **third** normative sentence is not relied on here and is named rather than quoted: a box can also be fragmented "due to bidi reordering of text … or higher-level display type box splitting", which is the decoration-splits slot's and the block-in-inline-split slot's territory (§5.3 names both, and this row relies on neither sentence) | ✓ | yes |
| CSS Sizing 3 §2.1 Auto Box Sizes | *max-content inline size* / *min-content inline size* | the two definitions the passes realise | verbatim: "Usually the narrowest inline size it could take while fitting around its contents if none of the soft wrap opportunities within the box were taken" and "Typically, the inline size that would fit around its contents if all soft wrap opportunities within the box were taken" (`body css-sizing-3 auto-box-sizes`) — M8 states both passes' edge terms (**PR-1b**), while the segmentation that realises "all" and "none" is §8's *Min-content prereq PR* in `main` before PR-1b (its instances 2–5 and 7) | ✗ (pre-existing: neither pass segments as the definitions require — §8) | yes |
| CSS Sizing 3 §2.2 Intrinsic Size Contributions | intrinsic size contributions | the outer size, and the floor between the two contributions | verbatim: "Intrinsic size contributions are based on the outer size of the box; for this purpose auto margins are treated as zero" and "If the ideal max-content contribution would be smaller than the min-content contribution (e.g. due to the use of negative margins) the effective max-content contribution is floored by the min-content contribution" (`body css-sizing-3 contributions`) — M8's margin term and cell 25d's two arms — **PR-1b**. ⚠ The floor is stated between the two contributions, not against zero | ✓ | yes |
...
| CSS Text 3 §4.1.2 Phase II: Trimming and Positioning | step 4 — its `normal`/`nowrap`/`pre-line`, `pre-wrap` and `break-spaces` bullets, and `pre`, which it does not list | `pre-wrap`: "the UA must (unconditionally) hang this sequence, unless the sequence is followed by a forced line break, in which case it must conditionally hang the sequence instead. It may also visually collapse the character advance widths of any that would otherwise overflow."; `break-spaces`: spaces "cannot hang nor have their advance width collapsed"; §3's informative table: `pre`'s end-of-line spaces "Preserve" | the step's first bullet is the umbrella's **for `normal` only**, by the same rule as step 3 — a sequence that survives step 3 (other space separators) **hangs**, M3 stating it by reference and no cell pinning it (ledger **A82**, scoped at **A88**); the same bullet's `nowrap` and `pre-line` outcomes are the **end-of-line white-space prereq**'s, like every value other than `normal`; the `pre-wrap` and `pre` outcomes are the **end-of-line white-space prereq**'s (§8; ledger **A67**), and `break-spaces` is vacuous, the parser rejecting it (`declaration/tests/values.rs:161`) | ✓ | yes |
| CSS Text 3 §8.2 Hanging Glyphs | the sentences this memo relies on — a **seed, not an inventory**: the section's own enumeration is `body css-text-3 hanging` (and its `hanging-punctuation` sub-section is §8.2.1's), and this column claims no completeness (ledger **A88**): a hanging glyph "is not considered when measuring the line's contents for fit, alignment, or justification" and "is also not taken into account when computing intrinsic sizes (min-content size and max-content size), and any sizes derived thereof" — the second being what §8's end-of-line white-space prereq block reaches; that non-zero inline-axis borders or padding block the hang; and that the glyph stays enclosed in its parent inline box as a negative margin, "the line … otherwise laid out as usual". Paragraphs this memo is **known** to reach elsewhere and not through this row: the conditional-hang rule (§4.1.2 step 4's `pre-wrap` bullet, the end-of-line white-space prereq's instance 2) | "Non-zero inline-axis borders or padding between a hangable glyph and the edge of the line prevent the glyph from hanging. For example, a period at the end of an inline box with end padding does not hang at the end edge of a line."; "A hanging glyph is still enclosed inside its parent inline box and still participates in text justification: its character advance is just not measured … Effectively, the hanging glyph character advance is re-interpreted as an additional negative margin on the affected edge of its parent inline box; the line is otherwise laid out as usual." (`body css-text-3 hanging`) | the **end-of-line white-space prereq** and **PR-1b**, in their own plans (ledger **A67**): which of a marker's edges block a hang, and how a hanging space sits in its box, are theirs; this umbrella names the section and stops | ✓ | yes |
...
| CSS Writing Modes 4 §2.4.5.2 Box Model of Reordering-induced Box Fragments | fragment edges in visual order | the start-most fragment carries the start edge, the end-most the end edge | verbatim: "For each line box, UAs must take the fragments of each inline box and assign the margins, borders and padding in visual order (not logical order)" and, for the `rtl` case, "the left-most fragment of the last line box in which the box appears has the left margin, left border and left padding" (`body css-writing-modes-4 bidi-fragment-boxes`) — **`#11-bidi-full-uba-fidelity`** (§9): the renderer's reorder branch drops the program's inline-axis edges, and on the identity path an `rtl` box's land on the mirrored physical side; the three cells §9 names assert elidex's values and name these | ✗ (registered divergence, §9 — ledger A101) | yes |
...
* **`pack/mod.rs:445-446` says `getClientRects()` "returns one border-box fragment per line …"** —
  ...
  attributed to `pack/boxes.rs:90-91`, where the shorter form matches the source — ⚠ **`:90-91`, not the
...
| `crates/layout/elidex-layout-block/src/inline/collect.rs` | Emits `InlineItem`, incl. the marker pair, with the payload M1 specifies, using the `parent_style` already in scope. Gains `containing_inline_size` on `collect_inline_items` (`:136`) / `collect_inline_items_inner` (`:194`), **and the IFC root's writing mode**, which M1's payload carries: `root_horizontal` (computed at `:150`, taken at `:211`) is a **`bool`** — `parent_style.writing_mode.is_horizontal()` — and cannot tell `vertical-rl` from `vertical-lr` or a sideways mode apart, while `collect_inline_items_inner` sees only the current ancestor's `parent_style`, so the root value has to reach the recursion. Whether that is a widening of `root_horizontal` to the `WritingMode` itself or a sibling parameter is **PR-1b's plan's** (ledger **A75**, owner corrected at **A81**); this row permits either and forbids neither. ⚠ The case is author-reachable exactly because css-writing-modes-4 §3.2 blockification is unimplemented (`#11-writing-mode-inline-blockification`; §6 cell 12f). Every `collect_inline_items` caller — enumerated by `grep -rn 'collect_inline_items(' crates/` minus the definition — takes the new argument: `inline/mod.rs:154` (has the value), `inline/measure.rs:22` and `:51` (the intrinsic passes, which pass `0.0` — css-sizing-3 §5.2.1 rule 4, decided at §3's row for it; cell 25b), and the test helper `inline/tests/mod.rs:17`. |
...
* **PR-1a — item stream. Behaviour-neutral.** Owns coupling 1×2 (both its rows): M1 + M2. Marker
  ...
  current behaviour, which for the first gate is a **requirement on its predicate and not a fact
  about today's**: `items.is_empty()` (`inline/mod.rs:161`) is false from PR-1a on for §4.2's
  Shape B, whose stream is an adjacent marker pair and nothing else, so PR-1a's test **ignores
  markers** — a marker-only stream still takes the early return — and **PR-1d flips it** (cell
  12g; ledger **A76**). The predicate's shape is PR-1a's; what the umbrella pins is that the two
  paths are not swapped under it, because they differ where no line count can see: the early
  return clears stale `InlineFlow` **unconditionally** (`inline/mod.rs:162`) while the tail
  reconciler's clear is `!env.is_probe`-gated (`:637-639`). The `any_font`
  ...
  until PR-1d. **Characterization tests for §6 cells 1, 2, 5, 6b, 6c, 6f, 6d, 6e, 6i, 6g, 6h, 6j, 7–12, 12d and 12g
  land here asserting today's behaviour** — the cells that pin *line suppression*, which is
  observable with `entity`-only markers. ⚠ **Cells 6b, 6i, 6g and 6h pin *new* item-stream behaviour rather than characterizing
  today's**: the marker variants do not exist on `154bac3f`, so all four pin *new*
  item-stream behaviour (6g and 6h the pseudo-element routing M1 records, non-empty and empty
  `content`; 6i that the predicate reads the box's own style and not its subtree). ⚠ **Cell 6j is an
  exception of a different kind** — a guard on PR-1a's **base**, the way the min-content prereq's
  own base is guarded (ledger A90): the replaced-origin pseudo prereq (§8; ledger **A95**) has landed by then,
  so there is no pseudo on the `<img>` to characterize, while on `154bac3f` there is one, inert.
  PR-1a stays
...
* **PR-1b — inline-axis advance.** Owns couplings 7×2, 7×(intrinsic), 1×7, 4×7 and 5×7: M3 + M8, **plus M5's
  ...
  ⚠⚠ **Outcome — a line whose content fits but whose edges do not takes the earlier licensed
  opportunity** (rev 78; R45-F2; ledger **A116**; generalised from the inline-end edge at rev 81,
  ledger **A123**). Where the inline-axis edges inside the unit being placed — the unit the next
  ⚠⚠ below defines — make the line unable to hold it, the break goes to a **licensed opportunity
  earlier in the line**, rather than the line committing over-width. css-text-3 §5 — "When wrapping is enabled (see
  ...
  * **Opportunity after the box's last character** — the same §5.5 bullet's other half: the box is
    ...
    never the opportunity the trigger sends the break to. ⚠ **It still needs an arm** (rev 81;
    **A123** — rev 80 wrote "it adds no cell"): when the box's last character is a space, the
    `InlineBoxEnd` sits **at** that opportunity's offset, and the edge belongs to the unit **before**
    it; an evaluation assigning it to the following unit keeps the box on a line that cannot hold
    its edge. §6 cell **15g(h)**.
  ...
  line-break-details`). That is why each 15g arm leaves **exactly one** licensed opportunity that
  avoids overflow on each line it asserts: an arm with an opportunity both outside and inside the
  box would pin the packer's greedy choice, which the spec leaves open, and not the outcome. §1.3 states the three positions beside
  ...
  ⚠⚠ **What the break test fits, stated once: the unit up to the next licensed opportunity, edges
  included — not a separate end-edge lookback** (rev 81; R49-F2, R49-F3; ledger **A123**). Once the
  item-boundary opportunity prereq, in `main` before PR-1b, withdraws the packer's two unlicensed
  grants (§8), a break can land only at a licensed opportunity, so what a content item is tested
  against is the whole **unit** from that item to the next licensed opportunity; and the licensed
  opportunities are the ones that prereq's canonical API reports (ledger **A110**) —
  `white-space`-filtered (its second grant, **A112**), atomic boundaries included (**A37**, with
  **A120**'s NBSP term), a forced break ending the unit and the end of the content ending the last.
  PR-1b's whole contribution to that test is that **every inline-axis edge a marker inside the unit
  advances is part of the unit's width** — inline-start and inline-end, any number of boxes, any
  nesting — since M3 puts those advances on the line in item order and nothing else does. There is
  **no second opportunity source**: a lookback that reads UAX #14 over raw text to find the earlier
  ...
  would reopen. Rev 78's "pending inline-end edge" — and A116's "end edges of boxes closing before
  the next licensed opportunity" — is the end-edge half of this and nothing more; the start-edge
  half, a box **opening** inside the unit after the tested item, was unstated until rev 81. The
  unit's axes are enumerated **once**, in the table after §6 cell 15g, each row to the arm that pins
  it or the reason no arm is needed; how the packer evaluates the unit (a lookahead over it, or
  another shape) stays PR-1b's plan-review's, on the prereq's API.
  ...
  canonical opportunity API the **item-boundary opportunity prereq** owns (ledger **A110**) — and that
  prereq is *already* ordered in `main` before PR-1b, so the outcome needs no new ordering. The
  cheaper shape, a running sum of **every** open box's end edge added to `place_item`'s guard
  (`pack/mod.rs:690`), is **over-conservative** and measured breaks three of PR-1c's cells: on the
  wrapping-box markup `<p style="width:80px">aaa <span style="padding:10px">bbb ccc</span></p>` it
  flushes before `bbb`, moving the whole box to line N+1 and destroying the two-fragment geometry the
  ...
  terminator made explicit; PR-1b's plan-review measures it. ⚠ **The fact names end edges only**
  (rev 81; **A123**): by the ⚠⚠ above, the unit's width also carries the inline-start edge of a box
  opening inside it, and a shape counting closing boxes alone fails §6 cell 15g(d) — and 15g(h) as
  well if it reads "closing before the opportunity" by text offset, the `InlineBoxEnd` there sitting
  **at** the opportunity's offset.
  ...
  *payload* fact
  ...
  ahead of **PR-1c** rather than after it; PR-1c's bullet states the asymmetry. ⚠ Rev 70's
  ordering sweep wrote "ahead of PR-1b" here (ledger **A89**) — an impossible ordering for PR-1b's
  own bullet, and a phrase-level replacement of a `PR-1c` the argument needs: what the zero edges
  establish is that PR-1b may precede **PR-1c**, not that it may precede itself. (The property is
  ...
  ⚠ states (six arms change the count and two do not, rev 81; §8 calls the changes PR-1b's flips), and every other width-constrained PR-1b cell pins its count
...
* **PR-1d — existence.** Owns couplings 1×3, 1×5, 1×4, 2×4, 2×5, 3×4 and 3×5: M5's *flip* + M6 + M7. The clause-3
  ...
  cells 18, 19, 20, 21, 22, 23, 23b, 24, 24b, 24c, 24d, 24e and 24g land here, plus the flip set §8 states.
...
⚠⚠ **Slot or ordering — the rule this memo has been using, written down** (rev 72; ledger **A93**):
a pre-existing defect that **this program makes newly reachable, or newly visible to an author**
is **ordered before the PR that makes it so**; a defect this program leaves exactly as it found it
stays a **slot**. Its precedents are the three prerequisites already carved on it — the reconciler
one because PR-1b moves `LayoutBox.content` while the box-assigner skips an existing box (A80), the
min-content one because "deferring either intrinsic half past PR-1b ships an inconsistency between
layout and intrinsic sizing" in as many words, and the end-of-line white-space one because
decoration is what makes that domain visible (A66) — and the memo's own **pre-existing** test is
its other half: "`origin/main` already fails it with no marker involved" decides *class*, this rule
decides *order*. It cuts both ways, and rev 72 records one refusal beside two promotions (A96). The
population it is applied over is a command, not a list — and the filter is **§10 membership**,
which is mechanical (`plan-xcheck`'s check 8 pairs a slot with the row that acts on it; the §9
*Why / Trigger / Re-eval* triple lives in nested bullets no one-liner counts reliably): `grep -o '\`#11-[a-z0-9-]*\`' <memo> | sort
-u` intersected with the slots §10's action rows name, which is the pairing check 8 enforces.
⚠ **The exception, added at rev 73 with its first instance** (user decision 2026-09-26; ledger
**A101**, **A102**): a pre-existing defect whose repair **is another lane's whole deferred program**
is recorded as an **accepted, cell-pinned divergence** rather than ordered, even where this program
makes it newly reachable. What separates it from a promotion is the owner and the size of the repair,
not the reach — the item-boundary opportunity (**A94**) is a bounded API inside this program's own
crates, while carrying per-run offsets through the renderer's bidi reorder is
`#11-bidi-full-uba-fidelity`'s program, an existing slot of another lane. Its first instance is the
bidi divergence §9 states once, pinned by the three §6 cells that section names; **A24** disposed that way before
the rule existed, which is the inconsistency between A24 and A94 the exception removes.
* **`#11-inline-box-decoration-splits`** (new slot, **own** deferral): css-inline-3 §2.1's
  fragments **as edge-bearing boxes** — across line boxes and as bidi fragments within one — and
  **not** the split of a box's content across line boxes at a licensed opportunity or a forced
  break, which the packer already performs and §6 cells 17, 17g and 15g(b) assert on the channels
  their PRs own (rev 81; ledger **A123**, closing the gap A122 opened when §3's css-inline-3 §2.1
  row was narrowed to "the facets §5.3's slot bullet lists" while this bullet still named the
  whole split); and **the whole of css-break-3 §5.4** — both `slice` and `clone`, and with them the
  **per-fragment `getClientRects` channel** (cssom-view-1 §6 step 3), in both the shapes §3's two
  ...
  longer holds: the box repair now lands in a **prerequisite PR** ahead of PR-1b (A80)
  ...
  **css-text-3 §5.5's adjacent-soft-wrap rule** (a break next to a decorated boundary lands at the
  ...
  continuation line's — the box getting a metric on a line css-text-3 §5.5 does not put it on, and
  on which this memo's own M4 says it has no fragment. §6 cell 24g pins elidex's value;
...
* **`#11-inline-item-boundary-soft-wrap`** — **ordered in `main` before PR-1b** since rev 72
  (**pre-existing** class, and the slot stays registered for the engine-wide residue; ledger
  **A94**; §8's *Item-boundary opportunity prereq PR*): PR-1b's advance widens the set of widths at
  which the defect fires, which is the criterion above. The gap itself: `place_item`
...
prereq** (R17's carve, registered here at rev 53; §8), the **end-of-line white-space prereq**
(the user's 2026-09-23 carve, registered here at rev 63; §8) and rev 72's two — the
**item-boundary opportunity** and **replaced-origin pseudo** prereqs (§8; ledger **A94**,
**A95**) — take the same treatment for the same reason, and open none here.
...
`min-content prereq` (R17's), `end-of-line white-space prereq` (rev 63's) and rev 72's `item-boundary opportunity prereq` and `replaced-origin pseudo prereq` are the names it still does not reach, which is the
...
tagged to a prereq its alternation does not name routes to nowhere by the checker's own reckoning (`:568` admits only
...
6e. **A block-level element reached through an inline gets no marker** (M1, the outer-display
    ...
    slot's.
6j. **A pseudo on a replaced originating element gets no markers, because it gets no box** (M1;
    R40) — `img::before { content: ""; padding: 10px }` on `<p><img src=x></p>`: **no pseudo entity
    exists**, so M1's emit test never sees one and the item stream carries no marker pair; the line
    is whatever the `<img>` makes it. css-pseudo-4 §4.1 suppresses generated content "when their
    parent, the originating element, is replaced", and the suppression is the **replaced-origin
    pseudo prereq**'s, in `main` before PR-1a (§8; ledger **A95**). **Rejects** the state PR-1a
    would otherwise reach: today the pseudo exists and is inert because its `content: ""` pushes no
    `Text` (`inline/collect.rs:260-272`), and PR-1a's markers would make it a box — a line at PR-1d
    and a `LayoutBox` at PR-1c for a box the spec forbids. ⚠ It does **not** flip at PR-1d: with no
    entity there is nothing to keep a line, before or after. ⚠ Distinct from cells **6g**/**6h**,
    whose pseudos have a non-replaced originating element and **do** get markers.
...
12g. **The emptiness gate ignores markers, and a probe is what shows it** (M1; PR-1a; R33) —
    §4.2's Shape B, `<p><span style="padding:1px"></span></p>`, laid out with **`is_probe: true`**
    after an `InlineFlow` has been seeded on a candidate key: the component is **gone** afterwards,
    exactly as it is today, because the stream counts as empty and `inline/mod.rs:161`'s early
    return clears it unconditionally (`:162`). **Rejects** leaving `items.is_empty()` as it
    stands: from PR-1a the marker pair makes it false, the pass runs to the tail whose
    `clear_inline_flows` is `!env.is_probe`-gated (`:637-639` — that call lives in
    `inline/reconcile.rs` since #508; the two early returns, `:162` and `:201`, clear
    unconditionally), and the seeded flow **survives** a
    probe — a state leak no line count can see, since `line_count` is 0 down either path (cell 12
    asserts that count and cannot separate them). The harness already builds the env by literal
    (`is_probe: false` at `inline/tests/baseline.rs:19` and its siblings), so the cell sets `true`
    and reads the component back. ⚠ PR-1d flips the gate with the rest of the existence change;
    cell 12 carries that flip, and this cell is what keeps the flip from arriving three PRs early.
...
3c. **`border` alone, in the advance and in both intrinsic sizes** (M3, M8; R32) — the one
    ...
    and the existence flip, 13c asserts the separate `LayoutBox.border` **field** (PR-1c),
    and no cell M8 routes carries a `border` at all: the fixtures that declare one are what
    `grep -n 'border:' <memo>` returns, and the only ones inside an M8 cell are this cell's own two
    markups (13c's is the `LayoutBox.border` **field**, PR-1c's, and 13d's is paint) — the rest
    carry `padding`, `margin`, or no edge whatever (25f). ⚠ Rev 71's 25f is why the enumeration
    that stood here ("25, 25b, 25c, 25d, 25e ... carries padding or margin") was false while its
    conclusion held (ledger **A100**). Two markups of its own, because the two
...
12b. **`direction: rtl`** (M1): `<p dir="rtl"><span style="padding-left:10px">x</span></p>` —
    ...
    identity line painting each run at its own baked position. That loss is the reorder branch's own
    ...
    **`#11-bidi-full-uba-fidelity`** (§9), where cells **12h** and **12i** now pin it.
    ⚠⚠ **This cell keeps its layout assertion and gains one *painted* assertion, in cell 17c's
    registered-divergence idiom** (R40-F2, user decision 2026-09-26; ledger **A101**). This markup is
    on the **identity** path, so the assertion holds today: the concatenated `Text` runs are `"x"`,
    one bidi run at level **2** — even, so `is_rtl` is false — `bidi_visual_order`'s fast path returns
    identity (`builder/bidi.rs:29-31`) and `builder/inline_flow.rs:153` discards it, leaving that
    file's logical branch to paint the run at its baked `inline_start` (`inline_flow.rs:191-196`). **Asserted — elidex's value**: the glyph's painted x
    is that baked `inline_start`, and the 10px is the **trailing** part of the line's inline advance,
    so it lies physically to the glyph's **right**; `InlineFlowRun::Text.inline_start` is an absolute
    **physical x** (`crates/core/elidex-ecs/src/components/inline_flow.rs:35-42`) and the packer's
    cursor runs left-to-right whatever the box's direction — which is the whole of the divergence:
    the **side mapping** is right (`padding-left` *is* the inline-end edge here, css-writing-modes-4
    §6.2's own-used-direction rule, cell 12e's subject), and what is wrong is **where that edge is
    laid down**. Read through `elidex-shell`'s pipeline
    (`crates/shell/elidex-shell/src/tests.rs:41`) off `DisplayItem::Text`'s `glyphs`
    (`crates/core/elidex-render/src/display_list.rs:190-200`, `GlyphEntry.position` at `:12-19`), the
    crate row §5.2 allows for a painted assertion. **The conformant value, named and not asserted**:
    css-writing-modes-4 §6.2 puts the inline-end side of an `rtl` box on its **line-left**, so the
    10px belongs physically **left** of the glyph (§6.4's table maps it the same way, "based on the
    used direction and writing-mode"). ⚠ **§2.4.5.2 is not the ground here**: this box has one
    fragment, so its "assign … in visual order" reduces to "the box's own two edges", and the
    parent-direction clause it carries selects *which fragment* takes which edge — a question that
    arises only once the box is broken (rev 79; ledger **A119**). **Registered divergence**,
    `#11-bidi-full-uba-fidelity` (§9): it flips when that program mirrors the IFC cursor for an `rtl`
    line, which no PR of this program does. Ledger **A24**, **A101**.
12h. **A line the renderer reorders paints the program's gap away — LTR IFC** (M3; R40-F2, user
    decision 2026-09-26) — `<p>אא<span style="padding-left:10px">ב</span></p>`, two `Text` runs with
    **different** entities — the `<p>`'s text and the span's, which is what puts an edge between them
    at all. The *reordering* half of the shape is the renderer's own
    `converged_bidi_reorders_runs` (`crates/core/elidex-render/src/builder/tests/inline_flow/bidi.rs:18-70`:
    a block leaving `direction` at its default, text `"אאב"`), whose two runs both carry the
    block's entity (`entity: div` at `:42` and `:47`) and which therefore shows the reorder without an
    edge. Run through `elidex-shell`'s pipeline
    (`crates/shell/elidex-shell/src/tests.rs:41`) and read off `DisplayItem::Text`'s `glyphs`
    (`crates/core/elidex-render/src/display_list.rs:190-200`, `GlyphEntry.position` at `:12-19`).
    **Asserted — elidex's value**: the painted glyph positions are **the same as for the same markup
    with the `padding-left` removed**. `text_has_rtl` puts the line on the reorder branch
    (`builder/inline_flow.rs:140-143`), and that branch repaints from `min(inline_start)` advancing by
    shaped width (`:169-173`), so M3's 10px never reaches the screen. **The conformant value, named
    and not asserted**: the span has one fragment, so the edge is its own — css-writing-modes-4 §6.2
    puts an `ltr` box's inline-start on its line-left — and css-writing-modes-4 §2.4.5.2's "assign
    the margins, borders and padding in **visual** order (not logical order)" is what places that
    edge against the **reordered** run, so `"ב"` should paint 10px inward of the line's visual start
    with `"אא"` after it. **Registered divergence**,
    `#11-bidi-full-uba-fidelity` (§9), in cell 17c's idiom: it flips when that program carries
    layout's per-run offsets through the reorder — the branch's own comment says so at `:159-168` —
    and no PR of this program moves it. ⚠ **The assertion is an invariance**, true before and after
    PR-1b, which is the point: PR-1b opens the gap and the paint does not change. ⚠ The span's
    `padding-left` is its inline-**start** edge here (the IFC is `ltr`), so the dropped advance is
    `InlineBoxStart`'s; cell **12i** is the same loss through `InlineBoxEnd`.
12i. **A line the renderer reorders paints the program's gap away — RTL IFC** (M3; R40-F2, user
    decision 2026-09-26) — `<p dir="rtl">אא<span style="padding-left:10px">ב</span></p>`, same two
    runs, read the same way — through `elidex-shell`'s pipeline
    (`crates/shell/elidex-shell/src/tests.rs:41`) off `DisplayItem::Text`'s `glyphs`.
    **Asserted — elidex's value**: as in 12h, the painted glyph
    positions are unchanged by the `padding-left`; here the line reaches the branch through
    `direction == Direction::Rtl` alone (`builder/inline_flow.rs:140`) and the runs still swap, so
    the first disjunct alone is what admits it — and only in the `||`'s evaluation order, since
    `reorder_by_levels` is identity for two level-2 runs, so no *reordered* line satisfies that
    disjunct without also holding RTL characters. What the loss depends on is neither arm. **What differs from 12h is the layout
    routing, not the paint**: under `rtl` the `padding-left` is the span's inline-**end** edge, so the
    advance rides `InlineBoxEnd`, and the reorder branch drops it just the same — the loss is
    indifferent to which marker carries the edge. **The conformant value, named and not asserted**:
    under `rtl` the same `padding-left` is the box's inline-**end** side (css-writing-modes-4 §6.2),
    and §2.4.5.2's visual-order assignment again places it against the reordered run — so the
    conformant paint coincides with 12h's while the logical side does not, and the pair is what shows
    that. ⚠ Neither cell rests on §2.4.5.2's **parent-direction** clause: both boxes are unfragmented,
    and that clause selects which *fragment* takes which edge (rev 79; ledger **A119**). **Registered divergence**,
    `#11-bidi-full-uba-fidelity` (§9); same flip condition as 12h.
...
15. **The boundary is not a wrap opportunity — the advance** (§1.3):
    ...
    `measure_width` (`inline/tests/mod.rs:31`), as the crate's justify tests do. Today there is no `b`
...
15e. **The advance does not move a line break to an item boundary** (M3; R40) —
    `<p style="width:W">a<span style="padding-left:20px">bbbb</span></p>` with `W` in
    `[measure_width("a") + measure_width("bbbb"), measure_width("a") + measure_width("bbbb") + 20)`,
    the test computing both bounds from the harness's `measure_width` (`inline/tests/mod.rs:31`) as cell
    15c does: `line_count` is **1**, before and after PR-1b, because css-text-3 §5.5
    gives an opportunity neither between adjacent letters nor at the inline box boundary. **Rejects**
    the guard as it stands: `place_item` tests `current_inline + trimmed_width` (`pack/mod.rs:690`)
    against a cursor the marker has advanced by 20, so it flushes before `bbbb` and the line count
    becomes 2 — a break at an item boundary, which is the **item-boundary opportunity prereq**'s
    defect, in `main` before PR-1b (§8; ledger **A94**). ⚠ **The assertion is an invariant across
    the PR, not a change**: the count is 1 on `154bac3f` for this `W` and 1 after, and the cell exists
    because PR-1b is what would otherwise move it. (The count each cell pins is stated per cell, §5.3;
...
15f. **A run whose `white-space` forbids wrapping must not break inside it** (M3; R43-F2) —
    ...
    the packer measures each alone (`pack/mod.rs:544`, `measure_segment_widths`), and at `"b"` the `:690`
    ...
    **item boundary** the oracle never returns; here the offset is inside one run and the oracle
    ...
    ⚠⚠ **Second markup, because the licence and not the offset is this cell's subject** (rev 79;
    ledger **A120**): `<p style="width:W"><span style="white-space:nowrap;padding-left:20px">a&#160;<span
    style="display:inline-block;width:0"></span>b</span></p>` with `W` in
    `[measure_width("a\u{A0}") + measure_width("b"), measure_width("a\u{A0}") + measure_width("b") + 20)`
    — the **sum of the two runs' own advances**, which is what the packer accumulates, rather than
    `measure_width("a\u{A0}b")`, because the engine measures each `StyledRun` alone and the atomic
    stands between them (cell 25f states its relation the same way). ⚠ Whether a conforming engine
    must also break **shaping** at that atomic is css-text-3 §7.3's question and not this cell's —
    the atomic here carries no edge, so §7.3's first bullet does not decide it — on the same
    footing as the first markup — `line_count` is **1**, before and after PR-1b. The atomic's boundary beside a no-break
    space is the **one** opportunity css-text-3 §5.5 licenses by name ("For Web-compatibility there is
    a soft wrap opportunity before and after each replaced element or other atomic inline, even when
    adjacent to a character that would normally suppress them, including U+00A0 NO-BREAK SPACE"), and
    §8's break invariant unions it in **manually** rather than reading it off the oracle (ledger
    **A37**) — so the `white-space` filter rev 76 added to the oracle's offsets left this term
    unconditional, and `nowrap` suppresses it too. **Rejects** an implementation that filters only the
    oracle's offsets: it flushes at the atomic boundary and the subset check still passes, because the
...
15g. **The inline-axis edges inside a unit send the break to an earlier licensed opportunity rather
    ...
    ledger **A116**; arm (b) R47-F1, rev 80 — ledger **A122**; arms (c)–(h) R49-F2/F3, rev 81 —
    ledger **A123**). **Eight arms in nine markups** — (e) carries two — one markup for each row of the matrix after
    this cell that names an arm, each
    carrying **exactly one** licensed opportunity that avoids overflow on each line it asserts, so
    that none pins the choice among several, which css-text-3 §5.5 leaves undefined ("CSS does not
    ...
    cursor advanced by the previous ones' **full** widths (`:695`) — so `measure_width` of a
    ...
    it**, in arms (a)–(d), (g) and (h): the edges reach `current_inline` through the marker path
    with no check (§5.1 M3), the guard tests content only (`:690`), and `finish()` commits whatever is
    on the line (`:786-790`, no width test), so the line overruns `W`; arms (e) and (f) are the
    matrix's **invariance** rows, where the advance alone is correct and the arm rejects a wrong
    **unit** instead. ⚠ **No committed-width assertion**: PR-1b's channels are `lines.len()`, the
    ...
    **trimmed**, the line-final space's disposition being the end-of-line white-space prereq's (§8)
    ...
    what grounds them**: every second line here is produced by edges, so at the arm's `W` the
    tagless text does not wrap at all; what makes each break genuine is §8's suite-level break
    invariant, which the realised offsets satisfy — after the space in `"a "` in (a)–(d) and (h),
    the atomic boundary in (g), the preserved break in (f), none in (e) — and (h)'s second break is
    the licensed offset after the span's own trailing space.
    ...
    one text run, the `<p>`'s, reading `"a"`; line 2 holds one text run, the span's, `"b"`, at
    ...
    `W` and `S` as in (a). **Asserted**: `line_count == 2`; line 1 holds one text run, **the
    span's**, reading `"a"`; line 2 holds one text run, the span's, `"b"`, at `inline_start` 0. The
    only licensed opportunity is the space **inside** the span, so the span is split there
    (css-inline-3 §2.1): its first fragment, holding `a`, stays on line 1, and only the suffix after
    the opportunity — `b` and the inline-end edge — moves, the edge on the **last** fragment because
    `slice` inserts no padding at a break (css-break-3 §5.4); the window's lower bound is what lets
    line 2 hold `b` plus that edge. ⚠ **What discriminates the two wrong answers**: the overflow
    answer gives `line_count == 1`; a "moves whole" answer — rev 78's universal — has no opportunity
    before the box to take, so it either overflows the same way or empties line 1 and puts `a` on
    line 2; the assertion that line 1 carries a span run reading `"a"` fails under both. ⚠ **Not
    cell 15f's complement by accident**: 15f's first markup puts the same text `a b` in a decorated
    span under `white-space: nowrap` (its edge on the start side), where the space is **not**
    licensed and the count stays 1; here it is licensed under `normal`, and the count goes to 2.
    ⚠ Whether the span's **rects** follow — one per line, the end edge on line 2's — is M4's and
    ...
    (c) **Two end edges closing together** (R49-F2) — `<p style="width:W">a <span
    style="padding-right:10px"><span style="padding-right:10px">b</span></span></p>`, `W` in
    `[max(S + 10, M(b) + 20), S + 20)` for `S = M(a␠) + M(b)`, non-empty since `M(b) < S`.
    **Asserted**: `line_count == 2`; line 1 holds one text run, the `<p>`'s, reading `"a"`; line 2
    holds one text run, the **inner** span's, `"b"`, at `inline_start` 0. The unit after the space is
    `b` and **both** closing edges. **The lower bound `S + 10` is the discriminating one**: an
    evaluation that counts only the nearest `InlineBoxEnd` sees `S + 10 ≤ W`, keeps `b` on line 1 and
    then overruns by the outer edge — `line_count == 1` — while the unit's width, `S + 20`, does not
    fit. Below `S + 10` both would break and the arm would not separate them.
    (d) **A start edge opening inside the unit, and an end edge closing mid-unit** — `<p
    style="width:W">a b<span style="padding-left:10px;padding-right:10px">c</span>d</p>`, `W` in
    `[max(T + 10, U + 20), T + 20)` for `T = M(a␠) + M(b) + M(c) + M(d)` and
    `U = M(b) + M(c) + M(d)`, non-empty since `U < T`. **Asserted**: `line_count == 2`; line 1 holds
    one text run, the `<p>`'s, reading `"a"`; line 2 holds three text runs — the `<p>`'s `"b"` at
    `inline_start` 0, the span's `"c"` at `M(b) + 10`, the `<p>`'s `"d"` at `M(b) + 10 + M(c) + 10`.
    `b`, `c` and `d` are **one** unit — css-text-3 §5.5: "Out-of-flow boxes and inline box
    boundaries do not introduce a forced line break or soft wrap opportunity in the flow" — and both
    of the span's edges lie inside it, **after** the tested item `b`. **What each rejected shape
    produces**: one counting only boxes that **close** in the unit (A116's measured fact read
    literally) sees `T + 10 ≤ W`, keeps `b`, and overruns at `d` — `line_count == 1`; one counting
    only **opening** boxes does the same; the advance alone overruns by 20 — `line_count == 1`; and
    `154bac3f`'s per-item guard, which the item-boundary prereq withdraws, flushes before `d` at an
    **unlicensed** boundary — `line_count == 2` with `"a b"` and `"c"` on line 1, which the line-1
    assertion rejects and §8's suite invariant reports. The run positions on line 2 pin that both
    edges travel with the unit.
    ...
    allow wrapping" (cell 15f quotes the `nowrap` and `pre` definitions whole), so no licensed opportunity
    ...
    and passes arms (a)–(d). An **invariance** row: 1 on `154bac3f`, 1 after PR-1b. ⚠ Its
    ...
    (e)(ii) **The same under `pre`** (rev 81; closes the matrix's row 6 rather than leaving it
    declared) — `<p style="width:W"><span style="white-space:pre;padding-right:20px">a b</span></p>`,
    ...
    row: 1 on `154bac3f`, 1 after PR-1b.
    (f) **A forced break ends the unit** — `<p style="width:W;white-space:pre-line">a <span
    style="padding-right:20px">b<i style="font-size:0">\nA</i>c</span></p>`, `W` in
    `[max(S, M(c) + 20), S + 20)` for `S = M(a␠) + M(b)`, with `M(c) < S` asserted as the
    precondition that makes it non-empty (Arial: `8.0 < 22.24`; Hiragino Sans: `9.456 < 25.408`,
    measured at `154bac3f`). **Asserted**: `line_count == 2`; the span's `"b"` run on line 1 and its
    `"c"` run on line 2 at `inline_start` 0. The preserved `"\n"` is a forced line break (§5.5,
    "Preserved segment breaks … must be treated as forced line breaks"), so the unit holding `b`
    ends there and the span's inline-end edge, which closes after `c`, belongs to line 2's unit.
    The `<i>`'s `font-size: 0` makes the break's advance 0 by style — cell 17h's device, whose
    frame facts this reuses (`find_break_opportunities("\nA") == [(1, Mandatory)]`, measured) — so
    no assertion reads a font's metric for LF (ledger **A106**). **Rejects** a unit that runs past a
    forced break to the next soft opportunity or the end of the content: at `b` it counts the 20px,
    `M(a␠) + M(b) + M(c) + 20 > W`, breaks after `"a "` — `line_count == 3`, `b` on line 2. An
    **invariance** row: 2 on `154bac3f`, 2 after PR-1b.
    (g) **The opportunity is an atomic boundary** — `<p style="width:W"><span
    style="display:inline-block;width:10px"></span><span style="padding-right:20px">b</span></p>`,
    `W` in `[M(b) + 20, M(b) + 30)` — the general `[max(S, M(b) + 20), S + 20)` with
    `S = 10 + M(b)`, the atomic's margin-box advance plus `b`'s. **Asserted**: `line_count == 2`;
    line 1 holds one member, the inline-block's `AtomicBox`; line 2 holds one text run, the span's,
    `"b"`, at `inline_start` 0. The boundary after the atomic is licensed by css-text-3 §5.5's
    atomic bullet, which the canonical API reproduces by encoding the atomic as U+FFFC
    (`find_break_opportunities("\u{FFFC}b") == [(3, Allowed)]`, measured; **A37**). **Rejects** an
    evaluation whose opportunities come from the concatenated **text** alone: it finds none before
    `b` and overflows — `line_count == 1`. ⚠ The realised break coincides with the span's start
    boundary, so an implementation treating that **boundary** as an opportunity also passes here;
    arm (b) and cell 15c are what reject that one.
    (h) **An end edge at the offset of an opportunity belongs to the unit before it** — `<p
    style="width:W">a <span style="padding-right:20px">b </span>c</p>`, `W` in
    `[max(M(a␠) + M(b␠) + M(c), M(b) + 20), min(M(a␠) + M(b) + 20, M(b␠) + 20 + M(c)))`, with the
    window's non-emptiness asserted as the precondition — it holds iff `M(b␠) + M(c) < M(b) + 20` and
    `M(a␠) < 20` — (Arial `[34.69, 41.34)`, Hiragino Sans `[40.19, 45.39)`, measured through
    `elidex-shaping`'s `measure_text` at `154bac3f`).
    **Asserted**: `line_count == 3`; line 1 holds one text run, the `<p>`'s, reading `"a"`; line 2
    one text run, the span's, reading `"b"`, at `inline_start` 0; line 3 one text run, the `<p>`'s,
    `"c"`, at `inline_start` 0. The opportunity after `"b "` lies **after the box's last
    character**, so css-text-3 §5.5's "the break occurs immediately before/after the box (at its
    margin edge)" puts that break after the box, the edge **before** it: the `InlineBoxEnd` sits at the opportunity's offset and belongs
    to the unit ending there. That unit — `b` plus 20, the space removed at the line end (M3's
    outcome, cell 16) — overruns line 1, so the break retreats to the space in `"a "`; on line 2 the
    same unit fits, and `c` cannot join it (the cursor's `M(b␠) + 20 + M(c)` exceeds `W`), so line 2
    breaks at the opportunity after the box. Each line has one choice that avoids overflow, so the
    arm pins no prioritization. **Rejects** an evaluation that assigns a marker at an opportunity's
    offset to the **following** unit: at `b` it sees `M(a␠) + M(b) ≤ W`, keeps `b` on line 1 with
    the 20px after it, and breaks before `c` — `line_count == 2`, line 1 over `W`. A116's measured
    fact, read by text offsets, is that evaluation: the span closes **at**, not before, the
    opportunity. `154bac3f` lays it out on one line.
    ⚠ **Flip or invariance, per arm, stated here rather than inherited** (**A121**): `line_count` is
    **1 → 2** in (a), (b), (c), (d) and (g), **1 → 3** in (h), and **unchanged** in (e), both markups
    (1), and (f) (2) — on `154bac3f` no edge is in effect and every window's lower bound keeps today's text on
    today's lines. ⚠ **It is not in §8's flip partition**, which is **PR-1d's** existence flip over
    PR-1a's characterization cells; these flips are PR-1b's own — the edges first exist there — and
    §8's PR-1b block names them as such. ⚠ **The start-edge case at a unit's *head*** — a box whose
    start edge immediately follows the opportunity — **is not an arm**: M3 advances that edge on the
    line before the break, cell 17c's registered deviation (`#11-inline-box-decoration-splits`), so
    the guard already sees it and no unit evaluation is involved; (d) is the start edge **inside**
    a unit, which is the one the evaluation must count.

*The unit's edge matrix, enumerated once* (rev 81; ledger **A123**; §5.3's PR-1b ⚠⚠ defines the
unit). One row per combination that changes which answer is correct or which wrong shape can pass;
a row with no arm says why none is needed.

| # | Unit shape | Correct outcome | Pinned by / why no arm |
|---|---|---|---|
| 1 | one end edge closing at the unit's end; opportunity before the box, in-run, `normal` | break at it; the box moves whole | 15g(a) |
| 2 | one end edge; opportunity inside the box | the box splits there; only the suffix and the end edge move | 15g(b) |
| 3 | several end edges closing together (nesting) | the unit carries their sum | 15g(c) |
| 4 | a start edge opening inside the unit after the tested item; an end edge closing mid-unit with text after it | both are in the unit's width | 15g(d) |
| 5 | the only candidate offset lies in a `nowrap` run (end edge) | no licensed opportunity: one over-wide line | 15g(e); the start-edge twin, reached by the guard, is 15f's first markup |
| 6 | the same under `pre` | as row 5 | 15g(e)(ii) — `nowrap` and `pre` are one predicate in the item-boundary prereq's filter (§8's invariant names both), and the second markup is what an opportunity source filtering only one of them fails |
| 7 | a forced break before the box closes | the unit ends at the break; the edge is the next line's | 15g(f) |
| 8 | the opportunity is an atomic boundary | break there | 15g(g); the NBSP-adjacent atomic under `nowrap` is 15f's second markup |
| 9 | an end edge at the offset of an opportunity after the box's last character | the edge is in the unit **before** the opportunity | 15g(h) |
| 10 | no licensed opportunity anywhere on the line (`normal`) | overflow; a box boundary is never an opportunity | 15c (PR-1c, both edges on a characterless box), 15e (start edge, the guard), 15 |
| 11 | a start edge at a unit's head | on the line before the break under M3 — a registered deviation, not an evaluation question | 17c (PR-1c, accepted divergence) |
| 12 | a box closing **after** the next licensed opportunity | its end edge is not in this unit | 17, 17d(b), 17f — A116's over-conservative running sum fails them |
| 13 | a characterless box beside an opportunity | undetermined by css-text-3 §5.5 | no arm — ledger **A19** |
| 14 | more than one licensed opportunity avoids overflow | unspecified — "CSS does not define prioritization of soft wrap opportunities" | no arm, deliberately: every arm above has one choice per line |
| 15 | `direction: rtl` / `writing-mode: vertical-rl` | the unit is logical; M1 supplies the logical sides | no arm of this cell: 12b, 12c, 12e, 12f pin the side mapping the unit reads |
| 16 | a forced break in the **intrinsic** passes | each edge on its own forced line's candidate | 25h |
...
15d. **The guard is an order test, not an equality** (M3's ⚠, the reader half of M7's rung) —
    ...
    **`20 + measure_width("bbb") ≤ W < measure_width("bbb ") + measure_width("ccc")`**, the shape
    cell 15c states its width constraint in, and `measure_width` the harness's (`inline/tests/mod.rs:31`).
    ...
    **trimmed** one, the second segment's guard arithmetic is `M("bbb ") + M("ccc") > W` today and
    ...
    `line_count` is 1 and PR-1b's is 2**, which is not what this cell is for. Dropping the `20 +`
    pins `W` **below** today's own wrap point, so the count is **2 before and after PR-1b**, while
...
16. **`text-align`: a marker does not end the trailing space's line-finality** (M3's outcome,
    ...
    harness's `measure_width` (`inline/tests/mod.rs:31`), as the crate's justify tests do.
...
25. **Shrink-to-fit, both intrinsic sizes** (M8): a `float: left` / `display: inline-block`
    ...
    (ledger **A47**).
25f. **An atomic's boundary keeps two texts in separate min-content candidates** (M8; R38) —
    `<div style="display:inline-block">a<span style="display:inline-block;width:0"></span>b</div>`,
    `width` left `auto`: the **min-content** inline size is
    `max(measure_width("a"), measure_width("b"))`, the test computing both from the harness's
    `measure_width` (`inline/tests/mod.rs:31`) and comparing, **not** `measure_width("ab")`. css-text-3
    §5.5 gives a soft wrap opportunity before and after every atomic inline whatever its width, and
    §8's suite-level break invariant encodes exactly that by mapping each atomic item to U+FFFC
    before calling `find_break_opportunities` (ledger **A37**). **Rejects** the joiner this
    program's min-content prerequisite adds, if it joins across every item boundary rather than
    only where the oracle has no opportunity: that producer reports `measure_width("ab")` and the
    box then never narrows to its own widest candidate. ⚠ Today's function passes by accident — it
    walks each `InlineItem::Text` alone — so the cell is a **guard on the prereq's base**, like
    cell 25, and it is that prereq's instance **6** (§8; ledger **A90**).
25e. **Marker-only content still contributes its edges to both intrinsic sizes** (M8; R35) —
    `<div style="display:inline-block"><span style="padding:10px"></span></div>`, `width` left
    `auto`: both intrinsic sizes are **20px**, the same quantity M3's markers advance the line by,
    so shrink-to-fit gives the box a width its own line fits. **Rejects** an implementation that
    attaches the edge term only where it commits a **text** segment: it returns **0** here for
    min-content (and for max-content, there being no text to add to) while PR-1b's line still
    advances 20px, so the box comes out narrower than its content — and it passes every other
    intrinsic cell, since each of them either carries text or, in 25d's marker-only arm, is
    negative and floors to 0 either way (ledger **A79**, **A100**). ⚠ This is §4.2's **Shape B**, the shape
    this program introduces to the item stream, so no pre-PR-1a characterization covers it; it is
    asserted on the min-content prereq's accumulator like cell 25 (§8; ledger **A58**).
25d. **Signed margins in intrinsic sizing: the used width floors at zero, and the margin is not
    dropped** (M8; R32, re-grounded at rev 69 — ledger **A84**) — two markups.
    * **the floor** — `<div style="display:inline-block"><span style="margin:-10px"></span></div>`,
      `width` left `auto`: the span's inline-axis edge sum is `−20`, and the **used width** the box
      takes is **0**, so no negative containing width reaches layout. The consumer is
      `shrink_to_fit_width` (`elidex-layout/src/intrinsic/mod.rs:134`), reached from
      `elidex-layout/src/layout/mod.rs:57` — **not** `elidex-layout-block`'s
      `positioned/constraints.rs:318` homonym, which reads max-content alone. Ground: `min-width`
      is `auto` here, which "resolves to a used value of 0" (css-sizing-3 §3.2), and the width
      algorithm is re-run against it — CSS 2 §10.4 step 3, "If the resulting width is smaller than
      `min-width`, the rules above are applied again, but this time using the value of `min-width`
      as the computed value for `width`" (`body CSS2 min-max-widths`); css-sizing-3 §3.1.2 defines
      the property and restates no algorithm, so the operative sentence is CSS 2's.
      ⚠ **What this arm does *not* assert, and why**: the two intrinsic sizes the passes report on
      this markup. They are not undecidable — css-sizing-3 §2.2 puts the margins inside the
      contribution ("Intrinsic size contributions are based on the outer size of the box; for this
      purpose auto margins are treated as zero"), so M8's arithmetic gives `−20`. ⚠ What css-sizing-3 §2.2 does
      **not** do is license a **negative** contribution: its own floor is between the two
      contributions — "If the ideal max-content contribution would be smaller than the min-content
      contribution (e.g. due to the use of negative margins) the effective max-content contribution
      is floored by the min-content contribution" — and not against zero, so **A84**'s "presupposes
      a negative contribution" over-reads it (committed; corrected here, ledger **A100**) — but **where the floor is applied is PR-1b's**, and a pass-level value asserted here
      would decide that: a producer flooring inside `max_content_inline_size` and one flooring at
      the used width both satisfy the spec, and this arm exists to pin what the spec fixes, which
      is the used width. **Rejects** propagating `−20` into
      the used width; how the floor is applied is PR-1b's.
    * **the discriminator** — the arm below, which is what separates the intended implementation
      from one that drops margins from M8 and from the per-marker clamp **A72** rejects; the floor
      arm alone cannot, all three reporting the same thing on it.
    ⚠ **The floor arm alone does not discriminate** (R33): on it an implementation that drops
    margins from M8 entirely, and the per-marker clamp **A72** rejects, both also give a used
    width of 0. This markup puts the signed sum where the floor cannot reach it —
    `<div style="display:inline-block"><span style="margin-left:-10px">verylongword</span></div>`,
    with `measure_width("verylongword") > 10` asserted as the precondition: both intrinsic sizes
    are `measure_width("verylongword") − 10`, computed from the harness's `measure_width`
    (`inline/tests/mod.rs:31`) rather than stated as a literal. **Dropping margins from M8** reports
    `measure_width("verylongword")`, and so does **the per-marker clamp** (it clamps `−10` to 0),
    so the arm separates the intended implementation from both — which the first markup cannot.
25c. **Where a decorated inline's two edges land when its content has several min-content
    segments** (M8; second markup added at rev 75, ledger **A108**) — **two markups.**
    The first — `<div style="display:inline-block"><span
    ...
    test computing the three from the harness's `measure_width` (`inline/tests/mod.rs:31`) and comparing
    ...
    (`max(measure_width(w) + 20)`). ⚠⚠ **What that arm cannot reject, and the second arm added at
    rev 75 for it** (ledger **A108**): under its own precondition the maximum is
    `measure_width("verylongword")`, a **middle** segment carrying no edge, so a producer that
    attaches the edges to a single segment's contribution and **drops them from the multi-segment
    path** reports the same number and passes — and cells 25 (one segment) and 25e (marker-only)
    cannot see it either, because in both the box's two edges are on the only segment there is.
    **Second markup, whose maximum is an edge-bearing boundary candidate**:
    `<div style="display:inline-block"><span
    style="padding-left:100px;padding-right:100px">aaaa bb c</span></div>`, `width` left `auto`:
    the **min-content** inline size is `100 + measure_width("aaaa")`, with
    `measure_width("aaaa") > measure_width("bb")` and `measure_width("aaaa") > measure_width("c")`
    asserted as the preconditions — computed from the harness's `measure_width`
    (`inline/tests/mod.rs:31`), not stated as literals — which make `aaaa` both the widest segment
    and the winning candidate. **What each rejected reading reports here**: edges dropped from the
    multi-segment path gives `max(measure_width("aaaa"), measure_width("bb"), measure_width("c"))`
    = `measure_width("aaaa")`, short by the whole `100`; both edges on the widest segment gives
    `measure_width("aaaa") + 200`; the pair on every segment gives
    `max(measure_width(w) + 200)` = `measure_width("aaaa") + 200` as well — over by `100` in both
    cases. ⚠ The two placement readings coincide on this markup, as they do on the first, so the
    two arms are complements: the first separates the conformant value from **placing** the edges
...
25g. **One min-content candidate carries both text advances and both internal edges** (M8; R43-F1)
    ...
    `InlineItem::Text` alone, gives `max(measure_width("b"), measure_width("c"))`. ⚠ **The sum is what css-text-3 §7.3
    *requires* here, not an interim value pending someone else's decision** (rev 78; ledger
    **A115**). §7.3's **first** normative sentence — "Text shaping must be broken at inline box
    boundaries when any of the following are true for any box whose boundary separates the two
    typographic character units" — takes as its first bullet "Any of margin/border/padding
    separating the two typographic character units in the inline axis is non-zero" (`body
    css-text-3 boundary-shaping`), and the two 100px paddings between `b` and `c` are exactly that:
    shaping **must** break there, so the two runs' own advances summed is the conformant value and
    `measure_width("bc")` would be the wrong one. ⚠ **Neither shaping slot is implicated**:
    `#11-intra-word-shaping-across-line-break`'s subject is a word split by a **line break**, and
    this fixture has none; `#11-shaping-break-at-unchanged-inline-boundary`'s is a boundary the
    engine breaks where css-text-3 §7.3's **second** sentence forbids it — "when there is no effective change
    in formatting" — which a non-zero padding is not. Rev 76 handed the value to the first of those
    two (**A115**). ⚠ It sits on the min-content prereq's cross-item accumulator like cells 25 and 25c, and
    it is that PR's **instance 2** (§8; ledger **A111**) — the one instance no cell asserted.
25h. **Across a forced break, each edge lands on its own forced line's candidate** (M8; R49-F1,
    rev 81 — ledger **A123**) — **two markups**, mirror images:
    (a) `<div style="display:inline-block"><span style="white-space:pre;padding-left:10px;padding-right:10px">a<i
    style="font-size:0">\nA</i>verylongword</span></div>` and
    (b) `<div style="display:inline-block"><span style="white-space:pre;padding-left:10px;padding-right:10px">verylongword<i
    style="font-size:0">\nA</i>a</span></div>`, `width` left `auto` in both. **Asserted**: in each
    arm the **max-content** and the **min-content** inline size are both
    `M(verylongword) + 10` — (a)'s `max(10 + M(a), M(verylongword) + 10)`, (b)'s
    `max(10 + M(verylongword), M(a) + 10)` — computed from the harness's `measure_width`
    (`inline/tests/mod.rs:31`, `M(x)` abbreviating `measure_width("x")`), with
    `M(verylongword) > M(a) + 20` asserted as the precondition (Arial `95.16 > 28.90`, Hiragino Sans
    `111.79 > 29.47`, measured at `154bac3f`). **Grounds**: css-sizing-3 §2.1 takes the max-content
    inline size "if none of the soft wrap opportunities within the box were taken" and the
    min-content one "if all soft wrap opportunities within the box were taken" (`body css-sizing-3
    auto-box-sizes`); under `pre` there are none — css-text-3 §3, "Lines only break at forced line
    breaks" — so both are the widest **forced** line, the preserved `"\n"` being a forced line
    break (§5.5). css-inline-3 §2.1 splits a box that "contains a forced line break" into fragments,
    and css-break-3 §5.4's `slice` inserts "no border and no padding" at the break, so the
    inline-start edge is on the first forced line and the inline-end edge on the last. The `<i>`'s
    `font-size: 0` gives the break a zero advance by style — cell 17h's device and its measured frame
    (`find_break_opportunities("\nA") == [(1, Mandatory)]`), and its zero-size `A` — so no value
    here reads a font's metric for LF (ledger **A106**). **What each rejected producer reports**
    (both arms, `V = M(verylongword)`): both edges added to the overall maximum (R49-F1's shape) —
    `V + 20`; the forced lines summed, today's `max_content_inline_size` (instance 7) —
    `M(a) + V + 20`; both edges on the first line — (a) `V`, (b) `V + 20`; both on the last — (a)
    `V + 20`, (b) `V`; edges dropped — `V`; the start edge dropped — (a) `V + 10`, (b) `V`; the end
    edge dropped — (a) `V`, (b) `V + 10`. Every rejected producer fails at least one arm, and the
    two dropped-one-edge producers are why there are two: each passes the arm whose widest line
    carries only the edge it keeps. ⚠ **Both intrinsic sizes, because shrink-to-fit reads the pair**
    (cell 25's reason). ⚠ It sits on the min-content prereq's accumulator — that PR owns both
    passes' segmentation, a forced break included (its **instance 7**, ledger **A91**) — and its edge
    terms are PR-1b's (**A58**), like cells 25, 25c and 25g. ⚠ **M8's frozen sentence is not
    amended by this cell**: "each marker adds its inline-axis edge sum to the running total" holds
    per forced line once instance 7 gives the pass one running total per line; its tail, "matching
    that pass's existing `total +=` shape", is the clause **A91** already flags as going stale when
    that prereq lands, and it stays flagged, for the user, rather than amended here.
...
6. **The emit/existence split's geometric half** (M1/M5) — the box a block-axis-only marker
   ...
   `assign_inline_layout_boxes` iterates `entity_bounds` (`pack/boxes.rs:56`) without asking whether the
...
13d. **A `position:relative` decorated inline's *painted rect* moves, and this is the one cell
    ...
    **Run through the real producer** (R32): the markup goes through `elidex-shell`'s pipeline —
    HTML + CSS to a display list — two tests of that suite, not one site:
    `build_pipeline_interactive_returns_all_fields` (`crates/shell/elidex-shell/src/tests.rs:41`)
    drives the pipeline, and `background_color_in_pipeline` (`:119-132`) matches `DisplayItem::SolidRect` off the other entry point, `build_pipeline` — so the `LayoutBox` the paint reads is the one
    **PR-1c's M4 produced**, not one the test set. Expected, as a **relation** over that box:
    ...
    `border-width` — **five** `SolidRect`s *for the span* where today's pipeline emits **one** for
    it (the display list also carries the line's text runs, which this count does not include),
    the one being the background at the bare content box, the 2px `border-style: solid` painting
    nothing because the inline's `LayoutBox` carries `EdgeSizes::default()`. ⚠ **A hand-set box cannot carry this cell** (R32):
...
17. **A decorated inline whose content wraps** —
    ...
    after PR-1b** — the count this cell pins, stated here rather than inherited (rev 79; **A121**). ⚠ **The wrap is genuine under the
...
10c. **Nested negative edges: the outer box's rect encloses the inner's border box on the inline
    ...
    Write `X` for the cursor after `a` (the harness's `measure_width("a")`, `inline/tests/mod.rs:31`). The
...
17c. **A box opened at a soft wrap has no rect on the line before it** (M4; rev 60, ledger
    ...
    **registered deviation** §3's css-text-3 §5.5 *adjacent soft wrap opportunity* row marks ✗
    ...
    shape, as a relation the test computes with the harness's `measure_width` (`inline/tests/mod.rs:31`):
    ...
    spec-correct (the box has no fragment there — css-text-3 §5.5 puts the whole box on line N+1);
    (ii) **one rect on line N+1**, starting at line
    ...
    point that **enters the hull** and widens the rect. This fixture carries no `white-space`, so
    it is `normal`, and the space is a **collapsible** one, where the outcome is this memo's and is
    **removal** (M3; cell 16; freeze discipline 5's main rule): line N's trailing space
    contributes nothing, and the stale point is
    `measure_width("aaaa") + 5`. The **precondition the test asserts**, with the harness's
    `measure_width` like the two relations above, is therefore
    `measure_width("aaaa") + 5 > measure_width("cccc")`. So the cell asserts the extent,
    not the orientation. `line_count` is **2 before and after PR-1b**: the 5px start edge does
    not move the break — the count this cell pins, stated here rather than inherited (rev 79; **A121**). ⚠ **The second line is a genuine
...
17d. **`getClientRects`, both channels, one asserting a divergence** (M4's ⚠). Two markups:
    ...
    `InlineClientRects` (`pack/boxes.rs:102`'s `len() > 1` guard), so `getClientRects` takes the
    ...
    **content** spans, i.e. edges missing. The width condition, as a relation the test computes
    with the harness's `measure_width` (`inline/tests/mod.rs:31`) rather than from literals — this is the
    ...
    **two** `line_rects` — ⚠ an earlier drafting stated the fit test with the *untrimmed* width,
    ...
    box opens exactly at that **soft** wrap, line 1 contributes no rect — correctly, css-text-3
    §5.5 putting the break at the box's margin edge — `line_rects.len() == 1`, and `getClientRects` takes
    ...
    stale-`InlineClientRects` arm*, pinning the `else` of `pack/boxes.rs:102`'s `len() > 1` branch, and
...
17g. **A forced break inside a box whose own content has no run across it** (M4; A55's
    ...
    css-inline-3 §2.1 splits the box into a fragment per line (css-break-3 §2 *box fragment*); the run that
...
24d. **A decoration-only line with *two* glyphless boxes: neither the height nor the baseline
    ...
    `max(line-height)` over the two spans, and the IFC's `first_baseline` is the **first** box's
    strut ascent — the `font-size:100px` span's, not the 10px one's (R35; ledger **A78**) — which
    is what the engine's own rule gives: `first_baseline` is written only `if
    self.first_baseline.is_none()` (`inline/pack/mod.rs:575`, and the atomic path's second writer
    at `:621` takes the same guard), the field is initialised once per IFC (`:187`) and no reset
    clears it per line, so the promote is **first-wins and IFC-wide**; M7's per-line tentative is
    recorded under the same idiom, the first `InlineBoxStart` on the line setting it and a later
    one leaving it. ⚠ **The pin is interim**: it fixes what this program ships, not what
    css-inline-3 §5.3 asks for — the composition below is facet (e) of
    `#11-inline-root-inline-box` (§5.3), and that slot supersedes this value when it lands. The
    cell's own subject is the relation, not the number: where css-inline-3 §5.3 gives each box `A′ = A + L/2` / `D′ = D + L/2` and
...
24e. **An open box's metrics on a *continuation* line** (M4's flush replay, M6's value, M7's
    ...
    **`1 + measure_width("aaa") ≤ W < measure_width("aaa ") + measure_width("bbb")`**, cell 15d's
    ...
    it pins `W` below today's own wrap point, so `line_count` is **2 before and after PR-1b** — the
    count this cell pins, stated here rather than inherited from a universal (rev 79; **A121**). The window is
...
24g. **A box that opens at a soft wrap gives its height to the preceding line too — pinned as
    accepted** (M6/M3; R37; ledger **A86**), in the shape cell 23 uses: cell 17c's markup with an
    explicit height on the span, `<p style="width:60px">aaaa <span
    style="padding:5px;line-height:100px">cccc</span></p>`, on the same relations 17c states
    (`measure_width("aaaa ") + 5 ≤ 60 < measure_width("aaaa ") + 5 + measure_width("cccc")`, so the
    start marker stays on line N and `cccc` opens line N+1). **Asserted — elidex's value**: after
    PR-1d **both** lines take the span's 100px, line N because M6 applies `block_advance` where the
    marker is consumed and line N+1 because M4's hook replays it for the still-open box. The
    conformant value is line N at the `<p>`'s inherited line height and only line N+1 at 100px —
    css-text-3 §5.5 puts the break at the box's margin edge, so the box has no fragment on line N
    (M4's outcome says so in its own words) — and it is **routed to
    `#11-inline-box-decoration-splits`** (§5.3), the slot that already owns the start-edge advance
    this follows from: one deviation, two observables. **Rejects** a producer that drops the
    replay: line N+1 would then take the inherited height, which is neither elidex's value nor the
    spec's, and is the defect R7-a withdrew the flush replay's baseline half against (M6, M4).
    ⚠ **No oracle before PR-1d**: the marker's `block_advance` is `0.0` in PR-1b (M3), so line N is
    untouched until the flip, which is why this is a PR-1d cell and not 17c's arm — 17c is PR-1c
    and its assertions are geometric.
    ⚠ **No other cell reaches the shape**, checked: 24e's outer box opens on line 1 **with
    content** on it, so its metric belongs there; 23, 23b, 24, 24b, 24c and 24d are single-line;
    17c itself asserts no height.
...
in `…/advance.rs`, PR-1c's in `…/geometry.rs`, PR-1d's in `…/existence.rs` — except where the
property stated below sends a cell to another crate — a new
...
⚠ **Every cell is wholly in that module from R4 on, with one bounded exception — and since rev 75
the exception is a *property*, not a list of cells** (R11; ledger **A109**): **a cell whose
assertion reads a display list is hosted by `elidex-shell`'s suite**
(`crates/shell/elidex-shell/src/tests.rs:41`, `:127`), the one host that runs the layout producer
*and* reads the display list; every other cell is wholly in its PR's module. The population is a
command rather than an enumeration that goes stale — `grep -n 'DisplayItem' <memo>` returns every
cell that reads one, because a cell of this kind names the variant it reads instead of pointing at
a sibling's (12i did point, until rev 75 made it name; ledger **A109**) — and a later painted cell
needs no edit here. ⚠ **This paragraph said "13d and only 13d" until rev 75**, three
revisions after rev 73 added the bidi cells, which is the class R41-F1 and R41-F3 also belong to:
a cell added without its placement following (ledger **A105**, **A107**). The three excluded hosts
are unchanged: `elidex-layout-block` can observe no display list, a layout-side cell could only
re-assert cell 13c's edges, and an `elidex-render` cell would have to hand-populate the very
`LayoutBox` PR-1c produces, which is the defect ledger **A73** closed (**A74** swept this
paragraph, which had kept the old assignment and would have sent an implementer back to the
`consumes_relpos_inline_subflow_with_gap` harness). Cell 13 used to carry a third, *painted*
assertion that only `elidex-render` could observe, and this paragraph carried the exception for it;
that assertion is withdrawn (freeze amendment, `#11-inline-decoration-paint-path`).
§5.2's `elidex-render` row is negative for every cell, and 13d adds no pass or member in either
crate, which is the line ledger **A6** draws.
...
* `entity_bounds` / `getClientRects` / `getBoundingClientRect` — **split by LINE count, which is
  ...
  entity per line (`pack/mod.rs:479-487`) and `pack/boxes.rs:102` gates on `line_rects.len() > 1`; there
  ...
  * **More than one line**: `getClientRects` answers from stored **content** spans — edges missing,
    *under*-inflated (cell 17d(b)). `getBoundingClientRect` does not read that list at all
    ...
    min/max **union** over lines (`pack/boxes.rs:65-88`, the write being `dom.set_layout_box` at
    `:88`) expanded on all four sides. ⚠ Expanding a union
...
* **Painted output — unchanged by PR-1c for a *static* inline (R4 withdraws the claim that it was
  ...
  and `assign_inline_layout_boxes` iterates `entity_bounds` (`pack/boxes.rs:56`) — so the box
  ...
  ⚠⚠ **That holds on the renderer's identity-order path, and the identity path is *not* the
  unaffected one** (R7-b, corrected at rev 73 — ledger **A103**): every site here saying the glyphs
  after the box move is speaking of the identity path, where each run does paint at its baked
  `inline_start` — but a box whose **own** `direction` is `rtl` has its edges land on the mirrored
  physical side there (§6 cell 12b), because `InlineFlowRun::Text.inline_start` is a physical x and
  the packer's cursor runs left-to-right whatever the direction. On a line the renderer **reorders** —
  which is `direction == Rtl` **or** any `Text` run holding RTL characters,
  `builder/inline_flow.rs:140-143`, so an **LTR** paragraph with two Hebrew runs is one — the branch
  discards every run's baked `inline_start`, repaints from `min(inline_start)` by shaped widths and the
  gap the markers open between two runs is not painted at all (cells **12h**, **12i**).
  ...
  intersection goes there — **§9 states it once, on the renderer's own predicate, and this bullet is a
  pointer** — cells 12b, 12h and 12i carry the pins, and the sites this scope reaches keep a pointer
  rather than a copy. Ledger **A24**, **A101**, **A103**.
...
* **Every reader of an inline `LayoutBox`.** **Three** distinct changes, in three different PRs:
  1. ⚠ **Position — PR-1b, and the re-slice's shippability argument does not cover it.** M3's
     ...
     (`pack/mod.rs:694`, `:706`) and that reaches `LayoutBox.content` (`pack/boxes.rs:80-88`, the
     literal's `content` field at `:81` and the write at `:88`). So
     ...
     painted — the bullet above; what moves in painted output **at PR-1b** is the glyphs after it, and
     for a box whose own `direction` is `rtl` the advance also lands the box's edge on the mirrored
     physical side, with no glyph after it to move — §9's single statement, pointer not copy.
     Ledger A8, narrowed at R11: a `position:relative` box's rect **is** painted, and **both PRs move
     it** — PR-1b because that box's `LayoutBox.content` is written from `place_item`'s rect for
     the run its own entity carries (`pack/mod.rs:706-714`, `pack/boxes.rs:56-88`) and PR-1b moves
     that rect with the cursor, PR-1c because M4 then inflates the painted `border_box()` by the
     real edges, which is what cell 13d measures. "PR-1c — not PR-1b — moves it" stood until rev
     70; A33's probe varied the **edges** only and never the content position, so its complement
     was unmeasured (ledger **A88**). Ledger A33.)
  ...
  3. ⚠ **Presence, and it lands in TWO stages, in two different PRs.** Absence is *observable* at
     ...
     `assign_inline_layout_boxes` iterates `entity_bounds` (`pack/boxes.rs:56`).
     ...
     * **PR-1d (M5's flip) — on lines that did not exist.** Flipping the predicate moves whole
     ...
     `assign_inline_layout_boxes` takes `bounds.block_end - bounds.block_start` (`pack/boxes.rs:77`).
  ...
  * ⚠ **`elidex-plugin/src/layout_types/` is where the accessors are *defined*, and this list
    ...
    `builder/paint/mod.rs` at 5 — and 3 for `layout_types/boxes.rs`. Neither is a reader of a decorated inline's box: `layout_types/boxes.rs:141`/`:148` are the
...
**PR-1a** (item stream, behaviour-neutral; cells 1, 2, 5, 6b, 6c, 6d, 6e, 6f, 6g, 6h, 6i, 6j, 7–12, 12d and **12g**): both marker variants reach `pack/items.rs`; every
...
every non-`Text` variant, so it cannot observe a marker; cells 1, 2, 5, 6b, 6c, 6f, 6d, 6e, 6i, 6g, 6h, 6j, 7–12, 12d and 12g land
here — as characterization tests except 6b, 6i, 6g and 6h, which pin new item-stream behaviour, and
6j, which guards PR-1a's base (§5.3); zero layout-output change. **Dead-field rule**: fields
...
changes across this PR in six of its arms and is invariant in two, each arm stating which (rev 80, ledger **A122**;
rev 81, ledger **A123**) — and it is *not* in the flip partition below, which is PR-1d's
...
PR because PR-1b's marker advance widens the set of widths at which `place_item` flushes at an item
boundary, which §6 cell 15e pins, and because its concatenation API is what the min-content
joining consumes), and
the **reconciler prereq** beside them (ledger **A80**: PR-1b moves `LayoutBox.content`'s position
...
ledger **A58**), and cell 16 asserts step 3's removal (`white-space: normal`), which the **end-of-line white-space** prereq, landing before PR-1b, makes conformant.
...
atomic boundary whose adjacent character is U+00A0, under the same `white-space` filter** (rev 79;
ledger **A120**: the manually unioned term is A37's, and `nowrap` and `pre` suppress *that*
opportunity as much as an in-run one, so it is licensed only where the surrounding regime admits a
soft wrap — without the filter an implementation that flushes at exactly that boundary under
`nowrap` passes the subset check) ∪ any forced break; a shared
...
through the cursor, never through the field. ⚠ **Re-derived at rev 69** (ledger **A81**), because
**13d** is no longer a hand-set-box cell: since A73 it runs the producer through `elidex-shell`
and its painted border segments take their thickness from `LayoutBox.border`, so the **border**
term now has two independent cells — 13c's field assertion and 13d's paint — and the end-to-end
clause below (`border:5px solid` through `elidex-shell`) is a third. What the re-derivation does
**not** change is the conclusion this paragraph exists for: the **margin** term is still 13c's
alone, cell **3** being PR-1b and observing the *cursor* rather than a produced box, so an
implementation that dropped `LayoutBox.margin`, or routed margin into `padding`, still satisfies
every other listed cell. 13c closes it with three pairwise-distinct
...
entity that already carries a `LayoutBox` (`pack/boxes.rs:62-64`) and **no production site removes
...
(`pack/boxes.rs:62-64`), so without this the new edges freeze at first layout — and R11's cell 13d is
...
positions while the background and border are painted at the **first-layout** box. ⚠ **Created at
PR-1b, widened here** (A80, A88): a `position:relative` inline's rect **is** painted today, so the
divergence begins with PR-1b's content move and PR-1c adds the edges to it — "created here" stood
until rev 70 and was false of the positioned arm. Which is
...
**prerequisite PR** that discharges `#11-inline-relayout-box-staleness`, and **PR-1b** depends on it (A80), PR-1c inheriting that.
...
the **Reconciler prereq PR** paragraph below. What PR-1c's DoD owes is the dependency, which PR-1b already owes too (rev 68; ledger **A80**):
neither is cut from `origin/main` until that PR has landed, and no cell of PR-1c asserts a
second-pass geometry. Found by Codex on #515; the prerequisite carve is R15's, its scope correction R16's.
...
`elidex-plugin`'s `layout_types/boxes.rs:206-207`, `content_rect_local`'s body, which the paragraph below
...
**PR-1d** (existence): cells 18, 19, 20, 21, 22, 23, 23b, 24, 24b, 24c, 24d, 24e and 24g, plus the flip set — **cells 1, 2, 7, 8, 10, 11, 12, 12d and 12g flip; 5, 6b, 6c, 6f, 6d, 6e, 6i, 6g, 6h, 6j and 9 do not**; this is the one normative statement of it — every §7
...
memo decides (§9's memo-split booking), plus the two measured checker blind spots rev 78 books
here — a **duplicate-ledger-id** check (a planted second `A113` returns 0 contradictions today, and
the shell one-liner the battery runs is the only thing that catches it) and a **bold-aware**
cell-reference scan (`cell **N**` is invisible where `cell N` is caught) — and nothing else** — the
...
the predicate prereq is approval-independent like its siblings — an earlier drafting called
...
1. **Stays.** An entity the IFC lays out in both passes keeps the first pass's geometry — `:62-64`
   ...
   measured scale (the probe `elidex-render`'s relpos harness hosted, cell 13d itself running
   through `elidex-shell` since A73): the box the edges give is
...
3. **Transfers — either way, with the entity still in `entity_bounds`.** `place_item` pushes a
   rect for every placed item whose entity is not the IFC's own (`inline/pack/mod.rs:703-714`),
...
Ordered **in `main` before PR-1b** (rev 68; ledger **A80**), and so before PR-1c, against nothing
else here — PR-1b already moves `LayoutBox.content`'s position for every decorated inline with
content while `assign_inline_layout_boxes` skips an entity that already has a box
(`pack/boxes.rs:62-64`), so a restyled `position:relative` inline's glyphs move and its painted
background does not; PR-1c widens the same desynchronisation to the box's edges. It read **before
PR-1c** until rev 68. Found by Codex on #515 — the carve
is R15's, its scope correction R16's, the transfer directions R17's and R18's. No `PR-1x` letter;
it is a prerequisite, spelled like its siblings.
...
(`InlineBlock` / `InlineFlex` / `InlineGrid` / `InlineTable`) with `width:auto`. The measured
instances are enumerated below and **no total is restated** (ledger **A87**'s rule). The
segmentation is one defect seen in several places — across items (2), inside one run (3, 4), in the
other pass (5) and at a forced break (7); 1 is the edge term PR-1b adds on top of it; and **6 is
not a present defect at all** — today's function gets an atomic's boundary right by accident, so
that instance is a **guard on this PR's own base** against the joining losing it, which is what
§6 cell 25f pins (ledger **A58**, **A64**, **A68**, **A90**, **A91**, **A100**).
...
3. **NBSP splits a segment layout keeps whole.** U+00A0 has the `White_Space` property, so
   ...
   requires: "Except where explicitly defined otherwise (e.g. for `line-break: anywhere` or
   `overflow-wrap: anywhere`) line breaking behavior defined for the WJ, ZW, GL, and ZWJ Unicode
   line breaking classes must be honored" (U+00A0 is GL, and this fixture sets neither of the two
   properties the exception names). The function reports `max(|a|, |b|)` for a segment
...
5. **max-content keeps a trailing space layout removes.** `max_content_inline_size` sums
   ...
   prereq makes conformant. A max-content inline size is, in css-sizing-3 §2.1's words, "**Usually** the narrowest inline
   size it could take while fitting around its contents if none of the soft wrap opportunities
   within the box were taken", so on this markup its end is that position: `<span>abc </span>` in an `auto`-width
   inline-block gives a used width one space wider than the line both prerequisites produce.

6. **An atomic's boundaries are soft wrap opportunities, and a joiner across items must not
   swallow them.** css-text-3 §5.5 puts an opportunity on each side of an atomic ("For
   Web-compatibility there is a soft wrap opportunity before and after each replaced element or
   other atomic inline"), which is why §8's suite-level break invariant encodes every atomic item
   as **U+FFFC** before calling `find_break_opportunities` — the licence and its one carve-out
   (an adjacent U+00A0) are stated there, and ledger **A37** carries the measurement. So for
   `a<span style="display:inline-block;width:0"></span>b` in an `auto`-width inline-block the
   min-content contribution is `max(|a|, |b|)` and **not** `|ab|`: the atomic supplies the
   boundary whatever its width, so a zero-width one still separates the candidates. Today's
   function gets this right by accident — it walks each `InlineItem::Text` alone and never joins —
   which is exactly why the joining this PR adds can lose it, and why the defect belongs here
   rather than to `#11-inline-atomic-intrinsic-contribution`, whose subject is the atomic's own
   **contribution** (zero, and unchanged by this instance). §6 cell **25f** pins it.
...
from under it, which is the mismatch the carve exists to remove — **and after the item-boundary
opportunity prereq** (rev 75; ledger **A110**), whose canonical `elidex-linebreak` API over the
concatenated item stream is what this PR's joining consumes: landing first, this PR would build
that construction locally and the other would then duplicate it or rewrite a prerequisite already
called done. Against nothing else here: the
...
segmentation and trimming of both intrinsic passes** (ledger **A64**, **A68**, **A90**, **A91**). Cross-item joining and the
...
⚠ A further part — `InlineItem::Atomic`
contributing zero — is **not** an instance of this PR's defect (which is every instance but the first): `measure.rs`'s own doc states
it of `min_content_inline_size` (`:13-14`) and `max_content_inline_size` (`:42-43`) alike, so it is
neither min-content-specific nor decoration-specific, and it is none of §8's measured
instances — including instance 6, whose subject is the atomic's **boundary** and not its
contribution (ledger **A90**). It is **registered**, not dismissed (Codex R27; ledger **A61**): `#11-inline-atomic-intrinsic-contribution` (§5.3, §10),
...
7. **A forced break is a segment boundary max-content does not take.** css-sizing-3 §2.1 suppresses
   the **soft** wrap opportunities only — the max-content inline size is what the box would take
   "if none of the **soft wrap** opportunities within the box were taken" — so a preserved newline
   still breaks the line and the contribution is the widest of the forced lines, not their sum.
   `max_content_inline_size` sums `measure_text(&run.text)` over every `InlineItem::Text`
   (`inline/measure.rs:52-58`), and a `white-space: pre` run reaches it with its `"\n"` inside,
   never split: the pass has no notion of a forced break at all, the split being `pack/items.rs`'s
   at layout time. So `<span style="white-space:pre;padding:10px">a\nverylongword</span>` measures
   as one run where layout lays out two lines. **Why here and not M8**: the accumulator is this
   PR's — M8 states the edge term "matching that pass's existing `total +=` shape", and the shape
   is what this instance changes, so the term lands on whatever accumulator this PR builds, as
   instance 1 already does for min-content. ⚠ **M8's sentence therefore goes stale when this PR
   lands** and is flagged rather than amended here: M8's Decision is a frozen column (ledger
   **A91**).
...
letters nor at an inline box boundary ("Out-of-flow boxes and inline box boundaries do not
introduce a forced line break or soft wrap opportunity in the flow"). Measured, at `154bac3f`:
`find_break_opportunities` is UAX #14 over **one string** (`crates/text/elidex-linebreak/src/lib.rs:26`)
and the packer holds no cross-item state, so `<p style="width:W">a<span>bbbb</span></p>` already
flushes between `a` and `bbbb` for any `W < measure_width("a") + measure_width("bbbb")`.
**Pre-existing** on the memo's own test, and **ordered in `main` before PR-1b** on the disposition
rule §5.3 states (ledger **A93**, **A94**): PR-1b's marker advance is added to `current_inline`
before the next item is tested (§5.1 M3), so with `padding-left:20px` on the span the same guard
fires for every `W` in
`[measure_width("a") + measure_width("bbbb"), measure_width("a") + measure_width("bbbb") + 20)` —
...
ledger **A37**) — and its own block hands the *joining mechanism* to its plan-review while naming
item boundaries as **this** slot's defect and not its oracle's, so the API is assigned here and
nowhere else (§5.3's slot bullet: "the fix is an API there over the concatenated paragraph … that
the packer consumes"). Unordered against the end-of-line white-space and reconciler prereqs, which
share no surface with it. The slot stays registered for the
residue this program does not reach. No `PR-1x` letter, like its siblings.

**Replaced-origin pseudo prereq PR** — **the gap**: `generate_pseudo_entity`
(`crates/css/elidex-style/src/pseudo.rs:27-81` at `154bac3f`) carries no replacedness test and its
call sites add only a `display` test — `git grep -n 'generate_pseudo_entity(' 154bac3f -- crates/css/elidex-style/src/walk.rs`
gives **four** lines, `:246`, `:255`, `:379` and `:388` (the `use` import at `:14` matches only
without the trailing `(`, and is not a call site), all four guarded by
`if …display != Display::None` at `:245` and `:377-378`, so `img::before { content: ""; padding:
10px }` generates a pseudo css-pseudo-4 §4.1 suppresses ("Also as with regular child elements, the
`::before` and `::after` pseudo-elements are suppressed when their parent, the originating element,
is **replaced**"; `body css-pseudo-4 generated-content`, and §3's row carries the same sentence). Today the pseudo is inert: the collect branch pushes a `Text` only under
`!tc.0.is_empty()` (`inline/collect.rs:260-272`, the pseudo branch, whose `continue` at `:271` is
...
⚠ **The reconciler and min-content prereqs are both ordered in `main` before PR-1b** — the
reconciler against no other prereq, the min-content one after the end-of-line white-space prereq
(ledger **A69**) — the min-content one because PR-1b's line advance has no intrinsic size
that fits it without it, and the reconciler one because **PR-1b already desynchronises a
positioned inline's painted chrome** (rev 68; ledger **A80**): PR-1b moves the position of
`LayoutBox.content` for every decorated inline with content, `assign_inline_layout_boxes` skips an
entity that already has a `LayoutBox` (`pack/boxes.rs:62-64`), and a `position:relative` inline's
background is painted from `border_box()` today (cell 13d's measurement), so after a restyle the
glyphs move and the background stays at the first-layout box. It was ordered before **PR-1c**
alone until rev 68, on the reading that PR-1c creates that desynchronisation; PR-1c widens it to
the box's edges, PR-1b creates it. Each touch set is likewise its own
...
it was cut from `origin/main` after #508 landed). **Branch topology**:
...
against PR-1b: the **end-of-line white-space** and **item-boundary opportunity** PRs both before
the **min-content PR** — the first because it decides the trimming rule that PR applies (ledger
**A69**), the second because it owns the concatenation API that PR's joining consumes (ledger
**A110**) — with the two of them unordered against each other and the **reconciler PR** beside all
three (ledger **A80**), so each may land any time before PR-1b and after whatever it follows. ⚠ Whether the predicate prereq is ordered against the others depended on its touch set,
...
* ⚠ **This memo's own length, and why the split is booked rather than done.** It is far past 1000
  ...
  (the four shipping PRs, every pending prereq and the tooling PR, one review each — §8 names them and no total is carried here, ledger **A87**), which
...
* **`#11-inline-relayout-box-staleness`** (`inline/pack/boxes.rs:96`) — pre-existing, and the
  ...
  **with no `ComputedStyle` mutation at all**. In both cases, on relayout `assign_inline_layout_boxes` `continue`s for any entity that
  already carries a `LayoutBox` (`pack/boxes.rs:62-64`) while nothing anywhere removes one. So the new
  ...
  **The write is repaired before the first PR that moves the content position** — rev 68 moves that from PR-1c to PR-1b (ledger **A80**) — **and R13/R15 corrected *how* twice.** The
...
⚠⚠ **R15: this slot is
  therefore not narrowed but *discharged*, as a prerequisite PR ahead of PR-1b (A80).** Extending the
...
* **`#11-css2-spec-label-normalisation`** — this memo cites css-inline-3 for the model, so its
  ...
  §10.4, §10.8.1 and §16.6.1** — the list is the output of the command below and **carries no
  total** (ledger **A87**; §10.4 entered with rev 69's cell 25d, §10.2 with rev 59's A59, whose list still read nine — the recurrence recorded next; re-derived by the command below). ⚠ **Seven until rev 34, and the eighth was invisible to the very command
  ...
  **CSS 2 §9.5** entered in that revision's own commit, in §3's css-inline-3 §2.1 row, where the
  ...
  the four untitled cites, CSS 2 §9.5 (quoted as a cross-reference, no title),
  CSS 2 §16.6.1, which §1.6 cites with no title, CSS 2 §10.2 (rev 59, quoted) and CSS 2 §10.4
  (rev 69, quoted at cell 25d). ⚠ **Two untitled, not one** (the R22-era record; three from rev 59): the front matter
...
* **`#11-bidi-full-uba-fidelity` — the program's inline-axis edges do not survive bidi, and they
  fail in two different ways** (R7-b, extended at R40-F2 by user decision 2026-09-26; **pre-existing**
  class, an **existing** slot of another program, so no deferral of this one's). **This is the single
  site; everywhere else keeps a pointer.** Stated on the predicate the renderer actually uses, not on
  `direction`: it reorders when `direction == Direction::Rtl || <any `Text` run holds RTL characters>`
  (`crates/core/elidex-render/src/builder/inline_flow.rs:140-143`), and `bidi_visual_order` returns
  identity when the concatenated text is a single run that is not itself RTL (`builder/bidi.rs:29-31`),
  which `builder/inline_flow.rs:153` then discards — every bare coordinate below is that file's. So
  the defect has two halves:
  — **Any line the renderer reorders** — two or more runs whose L2 order differs from the logical one,
  in **either** IFC direction — repaints from `min(inline_start)` (`:169-173`) advancing by each run's
  shaped width, so **every** baked between-run offset is dropped and the program's inline-axis edges
  collapse to the line end. The **LTR** case is not hypothetical: an LTR paragraph with two Hebrew runs
  reorders, which is exactly the shape of the crate's own `converged_bidi_reorders_runs`
  (`builder/tests/inline_flow/bidi.rs:18-70`, a block whose `ComputedStyle` leaves `direction` at its
  default and whose text is `"אאב"`). §6 cells **12h** (LTR IFC) and **12i** (RTL IFC) pin it.
  — **Any box on a line the renderer does *not* reorder, where the direction that decides its sides
  is `rtl`, has its edges mirrored.** ⚠⚠ **What is mirrored is *where the advance lands physically*, not which logical side the edge maps
  to** (rev 79; ledger **A119**), and rev 78 mis-stated this. The side mapping of an **unfragmented**
  box is css-writing-modes-4 §6.2's and §6.4's, by the box's **own used direction** — §6.2:
  "inline-start … For boxes with a used `direction` value of `ltr`, this means the line-left side.
  For boxes with a used `direction` value of `rtl`, this means the line-right side", with the Note
  that the inline sides "depend not only on the writing-mode property but also the direction
  property"; §6.4's table is "based on the used direction and writing-mode" (`body
  css-writing-modes-4 logical-directions`, `logical-to-physical`). elidex does that correctly, on
  the box's own `direction`, and §6 cell 12e is the cell that pins it — the only residue there is
  *used* vs *computed*, which `#11-used-direction-text-orientation-upright` owns (§9). **The
  parent-direction rules are about fragment edge *selection* and reach a box only once it is
  broken**: css-writing-modes-4 §2.4.5.2 tells a UA to "take the **fragments** of each inline box
  and assign the margins, borders and padding in visual order", and css-break-3 §5.4 fixes "which
  side of a **fragment** is considered the broken edge" (§3's row carries that sentence in full).
  ⚠ **So cell 12e's markup is *not* a divergence of this slot** — rev 78 booked it as one, from the
  attestation's hygiene list by way of the brief, and it is **withdrawn here**: an unfragmented box's
  sides are its own direction's by §6.2, so treating the parent's direction as governing would
  replace correct mapping with incorrect mapping. What remains, and what rev 73 measured, is the
  **physical placement** below. `InlineFlowRun::Text.inline_start` holds the absolute **physical x**
  (`crates/core/elidex-ecs/src/components/inline_flow.rs:35-42`) and the packer's cursor runs
  physically left-to-right whatever the box's direction, while M1 assigns the physical edges to logical
  sides — so `<p dir="rtl"><span style="padding-left:10px">שלום</span></p>` paints `[שלום][10px]` where
  conformance requires `[10px][שלום]` — by **§6.2**, since the box's own used `direction` is `rtl`
  its inline-end side is its **line-left**, and `padding-left` is that side's edge here, so the 10px
  belongs left of the glyph; §6.4's table maps it the same way. ⚠ **§2.4.5.2 is the rule for a box
  that is *broken*** — "the right-most fragment of the first line box in which the box appears has
  the right padding … and the left-most fragment of the last line box … has the left margin, left
  border and left padding" (`body css-writing-modes-4 bidi-fragment-boxes`) — and this box has one
  fragment, so it selects nothing here. ⚠ Ledger **A101** grounds this example on §2.4.5.2; that row
  is committed and **A119** is its correction. §6 cell **12b** pins it. ⚠ **The fixture offered for this half at R40 does not
  reach the reorder branch**: in `<p dir="rtl">a<span style="padding-left:10px">x</span></p>` the
  concatenated `Text` runs are `"ax"`, one bidi run at level **2** — even, so `is_rtl` is false — the
  fast path returns identity and the gap **is** painted. Right class, wrong example.
  ...
  **reach**, not the defect — and (4), added with the decision, the repair **is** another lane's whole
  deferred program, which §5.3's disposition rule now names as its standing exception rather than
  treating this as a one-off (ledger **A101**, **A102**). **No gate**: nothing in PR-1a–PR-1d waits on
  this, and the cells assert what elidex paints rather than what conformance wants. §6 cells **12b**
  (the mirrored identity half), **12h** and **12i** (the reorder half) carry the pins; each flips only
  when that program acts — the reorder half when the branch carries layout's per-run offsets through
  (its own comment, `:159-168`), the identity half when the IFC cursor mirrors for an `rtl` line.
  Ledger **A24**, **A101**, **A103**. ⚠ Not `#11-inline-box-decoration-splits`: that slot owns the **fragment
  attribution** a bidi split needs (one `DOMRect` per fragment, and which side of each is the
  **broken** edge — css-break-3 §5.4's own term, §3's row), a
...
* **`#11-inline-spec-cite-misattribution`** (new slot, **pre-existing** class): the wrong-section
  ...
  restatement of cssom-view-1 §6 step 3 at `pack/boxes.rs:90-91` and **five** further sites — defined by
  the command, not this list: `grep -rn 'fragment per line' crates/` → 6 hits / 3 files on
  `154bac3f` (`pack/boxes.rs:91`; `pack/mod.rs:218`, `:343`, `:443`, `:446`; `tests/inline_flow/align.rs:105`)
...
* **The canonical *inline box* predicate, with the `client*` guard as its first consumer — carved
  ...
  **Ordering**: in `main` before **PR-1a**, because M1's emit test is one of its two consumers
  ...
  the other prereqs, not a sibling topology, not a cold-gate width. All three follow from the
...
* **The enumeration-completeness audit (R20) — the standard it used, what it found, and the six
  ...
  * **css-pseudo-4 §4.1's suppression clause** →
    **`#11-pseudo-generation-on-replaced-originating-element`** — **ordered in `main` before
    PR-1a** since rev 72 (ledger **A95**; §8's *Replaced-origin pseudo prereq PR*), because PR-1a
    is what makes the inert pseudo reachable, and **pre-existing** class still.
  ...
  * **css-writing-modes-4 §6.4's Note** → **`#11-used-direction-text-orientation-upright`** (new,
    ...
    *Not ordered before a PR, measured* (rev 72; ledger **A96**): §5.3's rule orders a defect this
    program makes newly reachable or newly author-visible, and this one is already both without a
    marker. `LogicalEdges::from_physical` takes inline-start from `edges.bottom` in a vertical mode
    whenever `is_inline_reversed()` (`crates/core/elidex-plugin/src/logical.rs:204-210`), and a
    **block** box reaches it with the computed `direction` today:
    `crates/layout/elidex-layout-block/src/block/mod.rs:158` builds the context and `:187-188` map
    padding and border through it, so `<div style="writing-mode:vertical-rl;
    text-orientation:upright; direction:rtl; padding-top:20px">` has its own inline edges swapped
    where §6.4's Note reads the table at `ltr`, with no inline box in the markup. The program adds
    another caller of the same shape and no case the author could not already see —
    `git grep -n 'WritingModeContext::new(' 154bac3f -- 'crates/**/*.rs'` returns eleven sites,
    nine passing a style's `direction` and two literal `Ltr` inside `#[test]` fns. ⚠ **No §6 cell
    discriminates it, and rev 72 adds none**: the vertical cells **12c** and **12f** carry no `dir`
    and no `direction`, so `is_inline_reversed()` is false and the used and computed values
    coincide; the `rtl` cells **12b** and **12e** are `horizontal-tb`, where §6.4's Note does not
    fire. *Trigger*: the next change to `resolve_writing_mode_properties`
...
* **Gaps this memo *states* and never gives a token — the class behind R15's two findings, swept
  ...
  *opens* carries a §5.3-or-§9 definition and a §10 row, which `plan-xcheck.py`'s checks 4, 8 and
  ...
  `#11-preflight-css-module-labels` are named by this memo and owned by other lanes, and check 8
  exempts them by construction, firing only where a slot's full definition sits beside the token.
  ⚠ **The absence is a seed, not a measurement, and one of the three has since gained a row**
  (rev 78; ledger **A117**): rev 73 added a §10 **enrichment** row for `#11-bidi-full-uba-fidelity`
  (the bidi divergence this program registers against another lane's slot, A101), so "they carry no
  §10 row" is false of it from that revision on. Check 8 runs one-directionally — defined here ⇒
  owed a row — so nothing asserts the converse and the claim cannot be checked by the tool; what is
  checkable is the row set itself, `grep -n '#11-bidi-full-uba-fidelity' <memo>` intersected with
  §10's rows. The defect is narrower and invisible to all three — a gap
...
| Enrich `#11-bidi-full-uba-fidelity`'s SoT entry with the divergence this program registers against it (§9, stated on the renderer's own reorder predicate, both halves: the reorder branch dropping every baked between-run offset, and an `rtl` box's edges landing on the mirrored physical side on the identity path), and with the three §6 cells that assert elidex's values there. ⚠ **An enrichment of another lane's existing slot, not an open and not an own deferral** — no PR of this program waits on it (ledger **A101**, **A102**) | approval PR |
| Enrich `#11-inline-relayout-box-staleness`'s SoT entry with M4's write-path statement (§9), and note that `#11-inline-box-decoration-splits`'s work is first-layout-only until this slot lands — an **ordering** note, not a blocking dependency (§5.3). ⚠ R12/R13/R15: the slot is **discharged by a prerequisite PR** ahead of PR-1c (its own `layout_generation` remedy being inert off the paged path), so this row's scope is the splits facet alone (⚠ **ahead of PR-1b** since rev 68, A80; this row read "ahead of PR-1c" until rev 71) | PR-1c |
