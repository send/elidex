...
was **57** between them. ⚠ The figure and the parenthetical both stood at R8 until R10, which is
...
**⚠ Amended nine times, by the external channel this freeze deliberately keeps and by the
...
cell it had been refused with; ⚠ the count read "seven" and the parenthetical stopped at R8 until
...
(R9) and `#11-inline-open-box-strut-on-continuation-line` (R10) and the
dead arm's own disposition are pre-existing class, so none of them enters a per-PR count. ⚠ **The
last three were added at R10 and the first two of them were missing before it**: each round that
