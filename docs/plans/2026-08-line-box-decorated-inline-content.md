...
25b. **A percentage edge under shrink-to-fit contributes zero to both intrinsic sizes** (M8;
    ...
    **max-content** inline sizes each **equal** those of the same box with the span's
    ...
    content width**, the section's closing bullet (the box's size "is not re-resolved", so "the
    contents might thus overflow"): the content overflows the box by that amount. This cell
...
16b, 25 and 25b. **It owes one dependency**: it is not cut from `origin/main` until the min-content
