...
instruction.** Two prereq PRs move code before PR-1a: the seam-3 split relocates a block of `inline/mod.rs`, and
the dead-arm deletion removes a surface spanning **both** `pack/mod.rs` and `inline/mod.rs` (§8
names it; it is not one contiguous range, so no per-file delta is stated here — every earlier
attempt to state one was wrong). The memo does **not** re-anchor after
each: its coordinates exist to prove claims about the code as it stands today. **Each PR's own
plan-memo re-anchors against its actual base**, and §8's DoDs name behaviours and call sites wherever they can — a DoD that still carries a coordinate carries a `154bac3f` one and inherits this note. Round 9 found 27 distinct `pack/mod.rs` coordinates in this class; re-anchoring them
here would be a measurement invalidated by its own landing
([[feedback_document-landing-invalidates-its-own-measurements]]).

**Revision 11.** (The round-by-round CRIT series lives in
`project_line-box-decorated-inline-content.md`, not here — a count stated in this file is
invalidated by this file's own next commit.) The turn came at revision 6,
...
* CSS 2's **line-box strut** — the box CSS 2 §10.8.1 describes when it says each line box starts
  ...
  imaginary box a strut — **is the root inline box** in the current module — a
...
directly above).
⚠ **Reading note**: "§5.3" is overloaded — this memo's §5.3 (Program slicing), `css-inline-3` §5.3,
and the nonexistent "CSS Box Model L3 §5.3" of §3.1's bogus-cite record. Where the module is not
written out, read it from the sentence: §2's invariants 4 and 5 and pair 1×4 mean css-inline-3's;
§5.2, §5.3 and §8's "bogus §5.3 cites" mean the fabricated one; everything else means this memo's.

**Split decision**: K=8 ⇒ SPLIT-DEFAULT. The plan **is** split into three shipping PRs, each behaviour-scoped, with one owning PR per
coupling (§2, §5.3); the breadth verdict and the
...
|---|---|---|---|
| **M1** | What enters the item stream, carrying what? | Two `InlineItem` variants, `InlineBoxStart` / `InlineBoxEnd`, emitted around the recursion at `inline/collect.rs:291` for every inline with **at least one non-zero edge on any side**. Payload: `entity` + the **three physical `EdgeSizes`** `resolve_box_model(&style, containing_inline_size)` returns (`helpers.rs:116`) + the `WritingModeContext` they were resolved under (`WritingModeContext::new(root.writing_mode, root.direction)`, `logical.rs:27`) built from **the decorated inline's own `style`** (`inline/collect.rs:217`) —
`WritingModeContext::new(style.writing_mode, style.direction)`. Grounds, settled after three
revisions circled it:
* **The axis is the IFC's by construction, so its source cannot matter.** css-writing-modes-4:
  a box whose `writing-mode` differs from its containing block establishes an independent
  formatting context, and "if the box would otherwise become an in-flow box with a computed
  display of `inline`, **its display computes instead to `inline-block`**". A decorated *inline
  box* therefore always shares the IFC's writing mode; a `<span style="writing-mode:vertical-rl">`
  is an atomic, which M1 emits no marker for.
* **The direction is the box's own.** css-writing-modes-4 §6.4 gives the abstract-to-physical
  mappings "based on the **used** `direction` and `writing-mode`" — of the box whose sides are
  being mapped. `direction` *can* differ on an inline box without forcing an independent context,
  so this is the one component that varies, and §6.2/§6.4 say it is the box's own.
* **Why the two competing readings fail.** css-writing-modes-4 §2.1's "`direction` has no effect
  on bidi reordering … when `unicode-bidi` is `normal`" is about reordering *content*, not about
  mapping a box's own sides — it does not license taking the direction from elsewhere.
  css-break-3 §5.4's "the **parent element's** inline progression direction … neither the
  element's own nor its containing block's" is scoped to *which side of a fragment is the broken
  edge*, and its own example is an element that "breaks across two lines" — an unfragmented box
  has no broken edge, and §3 routes that whole section to `#11-inline-box-decoration-splits`.
⚠ **Revision history, so this stops recurring**: rev 6 used the box's own style with no stated
ground; rev 8 over-corrected to the IFC root; rev 11 over-corrected again to the parent on
css-break-3 §5.4, and *retired rev 8's §2.1 citation as "wrong twice" — a correction that was
itself false*: the proposition is verbatim in §2.1, and cell 12b's only `[dir]` element is the
`<p>`, so its `<span>` really is `unicode-bidi: normal`. The expression is back to rev 6's; only
now is it grounded. Only the physical edge *values* come from the element's own `ComputedStyle`. (Revision 8 grounded the choice on "`direction` has no bidi effect when `unicode-bidi: normal`", cited to css-writing-modes-4 §2.1 — wrong twice: that proposition is in **§2.2**, and it does not hold for cell 12b's `[dir]` markup, which HTML §15.3.5's UA sheet gives `unicode-bidi: isolate`.) Logical facts are *derived* at the point of use via `LogicalEdges::from_physical` (`logical.rs:186`), applied to each set separately: their inline-start/inline-end components summed across the three sets give M3's advance and M4's content offset (one quantity, computed once and carried on the marker as `inline_start_total` / `inline_end_total`); the same components tested *per set* give M5's predicate. `helpers.rs`'s `inline_pb` (`:148`) is **not** reusable here — it covers padding + border only, and the advance must include margin. Mirrored in `PackItem` (`inline/pack/items.rs:18`); never a `FlowMember`. | **Three sets, not one**: `LayoutBox` has independent `padding`/`border`/`margin` (`crates/core/elidex-plugin/src/layout_types/boxes.rs:88-91`) consumed separately by `padding_box`/`border_box`/`margin_box` (`:134-150`), and `resolve_box_model` already returns the triple — one `LogicalEdges` cannot fill three. **Physical, not logical, across the boundary**: `assign_inline_layout_boxes` (`inline/pack/boxes.rs:48`) writes *physical* fields and receives `is_vertical: bool` with **no `direction`**, so a logical payload would need a return trip whose context is not present there. Carrying physical + the ctx keeps one conversion direction and no reconstruction. Emit on **any** side because M4 must fill all four for a block-axis-only inline to paint; the *existence* test is inline-axis-only (M5) — see §6 cell 6 for the pair. `resolve_box_model` is mandatory: `sanitize_padding` (`:96`) resolves percentages against 0, `sanitize_border`/`sanitize_edge_values` (`:102`/`:48`) clamp non-negative. Basis = logical width = inline size (css-box-3 §3.1/§4.1; css-writing-modes-4 §6.1), `resolve_padding`'s documented contract (`:57-62`). |
...
| the bogus-cite sweep (`helpers.rs`, `lib.rs`, `positioned/layout.rs`, `block/mod.rs`, `block/children/helpers.rs`) | PR-1a's one-commit class fix across all seven sites; §3.1 enumerates them. Listed as its own row because five of the files it edits are not otherwise this program's. |
| `crates/layout/elidex-layout-block/src/inline/collect.rs` | Emits `InlineItem`, incl. the marker pair, with the payload M1 specifies, using the `parent_style` already in scope. Gains **one** new parameter: `containing_inline_size` on `collect_inline_items` (`:136`) / `collect_inline_items_inner` (`:194`). `root_horizontal` (`:211`) is unchanged. ⚠ `collect_inline_items` has **four** callers (verified 2026-08-02 via `grep -rn 'collect_inline_items(' crates/` minus the definition): `inline/mod.rs:154` (has the value), `inline/measure.rs:22` and `:51` (`min_content_inline_size`/`max_content_inline_size`, where a containing inline size is definitionally unavailable — see §9), and the test helper `inline/tests/mod.rs:17`. |
...
| `crates/layout/elidex-layout-block/src/inline/tests/mod.rs` | The test helper: a `collect_inline_items` caller (`:17`, one of four), an exhaustive `match item` (`:20-23`) that gains marker arms in PR-1a, the `mod decorated_inline;` declaration, and PR-1c's `setup_inline_test` change for cell 12d. |
...
Three shipping PRs, each **behaviour-scoped**, with one owning PR per coupling (§2), plus two booked
...
* **PR-1a — item stream (invariant 2). Behaviour-neutral.** M1 + M2. Marker variants, all
  consumers updated — **the same set §8's PR-1a DoD enumerates**, stated there once rather
  than in two lists that drifted for two rounds — `containing_inline_size` threaded, all seven bogus §5.3 cites fixed (§3.1). The packer gets a **no-op**
  ...
  **exhaustiveness arm** at `:192-199` returning `false` — behaviour-neutral, since a marker is
  neither `Text` nor `Atomic`. The **outer condition** at `:200` is untouched until PR-1c. **Characterization tests for §6 cells 1–12, 3b, 12b, 12c, 12d and 12e land here asserting
...
* **PR-1c — existence (invariants 1, 4, 5).** M5's *flip* (the predicate itself ships in PR-1b with its M3 consumers) + M6 + M7. The clause-3 predicate per §1.2,
  ...
  (M6);** everything else is already in place. §6 cells 18–24 land here, plus the PR-1c flip set §8 states. Closes
...
* **`#11-inline-box-decoration-splits`** (new slot, **own** deferral): css-inline-3 §2.1 box
  ...
  honour — §3's §5.5 row whose Step cell reads "adjacent soft wrap opportunity" and §6's note after cell 15 route here); and the `group_key` /
...
Own deferrals **per PR** (the policy's unit): PR-1b opens 2 (`#11-inline-box-decoration-splits`, `#11-inline-min-content-box-edges`), the seam-3 prereq opens 1 (`#11-plan-memo-consistency-checkers-on-ci`), PR-1a and PR-1c open none. `#11-inline-root-inline-box` and `#11-inline-nonpersist-arm-dead` are pre-existing class. All within ≤3.
...
**PR-1a characterization (assert today's behaviour; PR-1c's flip set is stated once, in §8's PR-1c DoD; this header does not restate it):**
...
12d. **The `any_font` early-out with a decorated inline** — text plus a decorated inline in an IFC
    ...
    ⚠ **Harness note**: `setup_inline_test` (`inline/tests/mod.rs:54`) returns `None` when the
    test families are unavailable and every caller early-returns, so the suite currently *skips*
    rather than *exercises* the no-font path. This cell needs a harness that can force
    `any_font == false` deterministically; without it the cell is unconstructible, which is the
    defect class round 5 named. PR-1c's DoD carries the harness change, not just the cell.
...
* **Every reader of an inline `LayoutBox`'s edges.** M4 flips `padding`/`border`/`margin` on inline
  ...
  constant now read data. Enumerated by `grep -rn "border_box()\|padding_box()\|margin_box()" crates/` — re-run it rather than trusting this list, which has twice been short — and dispositioned by family:
  ...
  * `elidex-layout-multicol`, `-table`, `-flex`, `-grid`, `block/mod.rs`, `block/children/stack.rs`,
  ...
  None is wrong-after-M4; the point is that PR-1b's DoD must reach past paint, which §8 now says.
...
* `clear_inline_flows` probe gating — a decoration-only IFC stops taking the early return at
  `inline/mod.rs:161` (unconditional clear) and starts taking the full path (`:637` — a pre-move coordinate; that line lives in the seam-3 module
  after the prereq PR). **PR-1c**, per §5.3.
...
docstring citations to their §3 rows; cells 1–12, 3b, 12b, 12c, 12d and 12e land as characterization tests; zero behaviour
...
silencing it.
**PR-1b** (geometry): cells 13, 14, 14b, 15, 15b, 16, 16b, 17, 17b, 17c, 17d and 25; **§7's full `LayoutBox`-edge reader list dispositioned, not just paint** (a `getBoundingClientRect` cell included); no double-counted edges on
...
**PR-1c** (existence): cells 18–24, plus the flip set — **cells 1–4, 3b, 7, 8, 10, 11, 12 and 12d flip; 5, 6, 9, 12b, 12c and 12e do not**; this is the one normative statement of it — every §7 consumer checked; the ⚠ caveat
at `inline/pack/mod.rs:110` retires, naming the root-inline-box divergence and pointing at
`#11-inline-root-inline-box`; **`setup_inline_test` (`inline/tests/mod.rs:54`) gains a way to force `any_font == false` deterministically**, without which cell 12d is unconstructible; M7's promotion carries docstring
...
moves to its own module, **byte-identical modulo the extracted signature**. The block reads a set of enclosing-fn bindings that become parameters — that is the **only**
permitted edit. The set is **not enumerated here**: revision 9 listed six and round 9 found the
list both wrong (`persisted_keys` is bound *inside* the block) and short (`unoffset_origins`,
`column_is_whole`, `skip`, `candidate_keys`, plus `env`/`dom`/`content_origin`/`parent_entity`/`children`).
The PR's own plan-memo enumerates it against its actual base. Proof obligation: a diff of the moved
body against the same block extracted from that base differs only in those bindings — and the
seam-3 PR must land **before** the dead-arm PR, or its range shifts under it. ⚠ Revision 8 wrote "no line of moved code edited", which the capture set makes
...
explain the two-path model. Removing only the arm would leave the same dead code half-alive, which
is what CLAUDE.md's rule forbids;
...
* **`#11-inline-fragmented-fn-decomposition` — trigger fires, and this program honours it.** The
  ...
  **Its own gate**: `/elidex-plan-review`, like every PR here. ⚠ Revision 6–8 waived it as "a
  ...
  judgment, and §10 routes non-mechanical ledger actions to this PR, so the premise was false even on its own
  terms.
...
* **`#11-inline-relayout-box-staleness`** (`inline/pack/boxes.rs:96`) — pre-existing, but
  **PR-1b widens its observable surface**: today an inline's stale `LayoutBox` is
  ...
  the latter). PR-1b's widening is a note on this slot. (`#11-inline-align-clientrects-nonpersist-path` was ledger-marked to fold into terminal-Z C-3/C-4 alongside it; the dead-arm prereq **closes** it instead, so that pairing no longer holds and its SoT entry needs the same correction.)
* **`#11-layoutbox-absence-unreachable`** (#488, registered in the SoT) — no longer relevant, and §10 carries the disposition row: M5 keeps no entity's box
...
* **`#11-css2-spec-label-normalisation`** — this memo now cites css-inline-3 for the model, so
  its remaining CSS 2 cites are **§8.3, §9.4.2, §9.4.3, §10.8.1 and §16.6.1** (`grep -o 'CSS 2 §[0-9.]*' <memo> | sort -u` → five distinct sections; the §16.6.1 one is §1.6's note on what css-text-3 §4.1.1 superseded, not a live grounding — but the grep returns five, so the inventory says five). Adjacent `CSS 2.1 §` lines in
...
* **The two plan checkers are not on any loop.** `plan-sweep.py` / `plan-xcheck.py` live in
  ...
  is the habit that failed six rounds with a tool attached. **Disposition**: they are **not** generic yet — `plan-xcheck.py` hard-codes `PR-1[abc]`, a
  memo-specific superseded-range dict and this umbrella's own slot name — so generalising them is
  part of the work, not a precondition. They belong in `elidex-plan-review`'s Step 1.5 or the
  ungated `trip-wires` job, as **its own PR**, not folded into this umbrella. New slot
  `#11-plan-memo-consistency-checkers-on-ci` (own deferral). ⚠ Until it lands, **this memo's own
  remaining rounds depend on running them by hand** — the habit the slot exists to end. Trigger:
  **this memo's next plan-review round**, not a PR landing (revision 8's trigger fired after the
  loop it protects). Re-eval: 2026-11-01.
...
| Register **and** close `#11-inline-fragmented-fn-decomposition` in one row — it is registered as closed-on-landing, not registered then closed — **as a partial close**, naming the seams §9 measures as still open, in the successor slot `#11-inline-fragmented-fn-seams-1-2` (**pre-existing** class — the seams predate this umbrella; Why: the prereq PR discharges seam 3 only; trigger: the **first change after this umbrella** touching the residue — PR-1a's own `:161`/`:190` touches are excluded, since they are what surfaced the seams and are covered by this program; re-eval 2026-11-01) — the prereq PR discharges only seam 3. | seam-3 prereq PR |
