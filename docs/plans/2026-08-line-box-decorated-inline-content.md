...
the Codex row below); a Step 4.5 gate on an author
...
divergence; cell 6g is new), so by the rule above the count is **0** again and **rounds 22–23**
are required before approval. #515 stays a **draft** until then and is reviewed under
...
CSS Sizing 3, CSS Writing Modes 4, CSS Display 3, CSS Backgrounds 3, CSSOM View 1), M=36 entries (`Split decision` below restates K; both are recomputed). Both figures are recomputed
...
* **PR-1a — item stream. Behaviour-neutral.** Owns couplings 2×(all) and 1×2: M1 + M2. Marker
  ...
  until PR-1d. **Characterization tests for §6 cells 1, 2, 5, 6b, 6c, 6f, 6d, 6e, 6g, 7–12 and 12d land here
  asserting today's behaviour** — the cells that pin *line suppression*, which is observable with
  `entity`-only markers. ⚠ **Cells 6b and 6g are the exceptions and are not characterization
  cells**: the marker variants do not exist on `154bac3f`, so both pin *new* item-stream behaviour
  (6g the pseudo-element routing M1 records). PR-1a stays
  behaviour-neutral in the sense that matters — no layout output moves — but the item stream is
  deliberately not neutral, and 6b and 6g are what assert that, since `grep -rn padding
...
* **`#11-inline-zero-edge-box-in-item-stream`** (new slot, **own** deferral — PR-1a's): M1 emits
  ...
  not deliver. Why deferred: a marker for every inline box grants — through M4's unconditional
  ...
  presence change over every empty `<span>`/`<a>`/`<b>` in every document, a presence-axis program
  of its own under the edge-dense rule. Cells **6b** (all edges zero ⇒ no marker), **14**'s
  contrast and **21** and §7's "a box with all edges zero gains nothing" are written against the
  conjunct and **flip** when this slot lands (14c is *not* one of them — a decorated empty inline
  ...
  payload). Trigger: `#11-inline-root-inline-box` work
  ...
  #515 (an earlier drafting folded it into the root-inline-box slot, a mechanism mismatch).
* **`#11-inline-item-boundary-soft-wrap`** (new slot, **pre-existing** class): `place_item`
  ...
  `b` lands), and the marker adds no item boundary the runs did not already have. Trigger: any line-breaking correctness work, a
...
6g. **A decorated pseudo-element inline box gets markers** (M1's routing special-case; Codex on
    ...
    it is an inline box, and `InlineBoxStart`/`InlineBoxEnd` bracket its single `Text` item
    (`collect.rs:265`'s branch, re-routed by PR-1a). Observed through the PR-1a harness and
    **behaviour-neutral** here (the markers are inert until PR-1b). Paired with 6c–6e it pins that
    ...
    `elidex-layout-block` has no `elidex-style` dependency, so the pseudo is built with the crate's
    idiom (`inline/tests/inline_flow/persist.rs:134-143` at `22de3078`) — a hand-inserted entity
    carrying `ComputedStyle` (padding **and** `display: Display::Inline` set explicitly),
    `PseudoElementMarker` and `TextContent`; that the cascade produces that style
    (`elidex-style/src/pseudo.rs:53-55` defaults `display` to `Inline`, `build_computed_style`
    carries padding) is a **stipulation**, as 6c's `ImageData` is. Consequence for **PR-1c**: its
...
15. **The boundary is not a wrap opportunity** (§1.3):
    ...
    not move to the next line at its own edge, and the cursor after it is **40px further along**
    than today (the PR-1b `line_count` change §5.3 states). ⚠ What this cell does **not** assert:
    where `b` lands. css-text-3 §5.5 gives this markup **no** soft wrap opportunity (letters, no
    spaces; the boundary adds none), so per spec `b` overflows — but today's packer wraps at
    **every item boundary** without a break-opportunity test (`place_item`'s per-item flush,
    `pack/mod.rs:658` at `22de3078`), so `b` wraps today and would wrap 40px earlier after PR-1b.
    That is a pre-existing §5.5 divergence this program does not own and must not pin as
    expected — `#11-inline-item-boundary-soft-wrap` (§5.3); an earlier drafting asserted "`b`
    **does** wrap" (Codex on #515).
...
to their §3 rows; **two test-harness additions, without which cells 12d, 6b and 6g are
...
every non-`Text` variant, so it cannot observe a marker; cells 1, 2, 5, 6b, 6c, 6f, 6d, 6e, 6g, 7–12 and 12d land as
...
**PR-1d** (existence): cells 18, 19, 20, 21, 22, 23, 24 and 24b, plus the flip set — **cells 1, 2, 7, 8, 10, 11, 12 and 12d flip; 5, 6b, 6c, 6f, 6d, 6e, 6g and 9 do not**; this is the one normative statement of it — every §7
...
| Open `#11-inline-item-boundary-soft-wrap` (**pre-existing** class) with the Why / trigger / date in §5.3 (found by Codex on #515; cell 15 stops pinning the divergence) | PR-1b |
