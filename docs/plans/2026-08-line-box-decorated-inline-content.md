...
| A67 | user, 2026-09-23 (third decision on this topic); scoped plan-review of rev 63c | the umbrella stating the end-of-line white-space domain's outcomes at all — rev 63's three attempts: (1) "the space is trimmed or hung", which left the two undecided; (2) removal for collapsible values, `pre-wrap` hanging under a css-text-3 §8.2 border/padding block, with cells 16(a)–(h) and a PR-1c border-box cell; (3) the same, carved into a prereq while the umbrella kept the finer outcomes | each attempt was reviewed and each review found its defects **in that revision's own new text** — three consecutive self-introduced resets (1 CRIT / 15 IMP; 1 CRIT / 4 IMP; 1 CRIT / ~14 IMP). The generator is altitude, not care: the domain's outcomes cannot be authored without choosing a mechanism (what the trim strips, where the hang is applied, what the rect then spans), and the umbrella is the one level that must not choose one. **Decided: the umbrella keeps three things** — the outcome **by reference** (css-text-3 §4.1.2 steps 3 and 4, §5, §8.2), the **defect** as measured instances with no invariant, and the **ordering** (the prereq in `main` before PR-1b, not ordered against the min-content prereq) — and hands the rest to the end-of-line white-space prereq's and PR-1b's plan-memos and plan-reviews: the §8.2 blocking rule's detail, the cells that would assert it (arms (c), (e)–(h) and the PR-1c border-box cell, all withdrawn; cell 16 returns to the (a)/(b) contrast and names elidex's value at the edge a **registered divergence** in cell 17c's shape), M4's end-cursor sentence and the §2 3×7 clause (a **named risk** for PR-1c's review instead), and M8's and §8's "a §8.2-blocked space counts in the trimmed width". The prereq's instance 4 becomes a **statement of a pre-existing disagreement**, not an invariant: `split_whitespace` drops every `char::is_whitespace` separator (U+00A0, U+3000) where layout's trim strips `' ' | '\t'` alone (`inline/whitespace.rs:162`), and `max_content_inline_size` measures `run.text` whole (`inline/measure.rs:52-58`). **Recorded as a scoped exception to freeze discipline 5**, that domain and nothing else. Found by Codex on #515 and by the scoped reviews |
...
| `crates/layout/elidex-layout-block/src/inline/measure.rs` | `max_content_inline_size` (`:44`) and `min_content_inline_size`'s edge term — M8's contribution, both PR-1b's. `min_content_inline_size`'s **accumulator** — `max_word` at `:23`/`:31` — is **not this program's**: the min-content prereq PR gives it layout's segmentation — cross-item joining and the break oracle (§8; ledger **A64**) — in `main` before PR-1b, and PR-1b re-anchors against that base. The function itself (`:15-37`) PR-1b does touch, because its `collect_inline_items` call (`:22`) takes the new argument like every other caller. |
...
M8 adds **both** edge terms **in this PR**; the min-content prereq's segmentation — cross-item
joining and layout's break oracle (ledger **A64**) — is in `main` before it (§8's *Min-content prereq PR*; ledger **A47** — an earlier drafting deferred the
...
auto-box-sizes`; ledger **A64**). PR-1b widens the line by the edges, and shrink-to-fit is
`min(max_content, max(min_content, available))` (`elidex-layout/src/intrinsic/mod.rs:134`),
reached from `elidex-layout/src/layout/mod.rs:57` for an inline-level atomic display
(`InlineBlock` / `InlineFlex` / `InlineGrid` / `InlineTable`) with `width:auto`. Four measured
instances, owned apart (below; ledger **A58**, **A64**) — 2–4 are the one defect, the segmentation, seen
across items (2) and inside one run (3, 4); 1 is the edge term PR-1b adds on top of it.
...
⚠⚠ **Ownership is split, and it is settled** (R25; ledger **A58**). **This PR owns instances 2–4
— the segmentation** (ledger **A64**). Cross-item joining and the
...
4. **Intrinsic sizing and layout already disagree about which trailing spaces a segment
   excludes**, and this is **pre-existing, stated rather than owned**: `min_content_inline_size`
   segments with `split_whitespace()`, which drops every `char::is_whitespace` separator —
   U+00A0 and U+3000 included — where layout's trim strips `' ' | '\t'` alone, and
   `max_content_inline_size` measures `run.text` whole (`inline/measure.rs:52-58`), trailing space
   and all. Neither prereq is asked here to close it; each plan states what its change does to it.

**How it relates to the min-content prereq**: not ordered against it — the two touch the same
fact (which trailing spaces a segment's width excludes) from different sides, and each plan
carries what its change does to the other's (instance 4). **Ordered in `main` before PR-1b**,
...
* **`#11-layoutbox-absence-unreachable`** (#488, registered in the SoT) — no longer relevant, and
  ...
  signal is required.
* **Intrinsic sizing** — per M8: both edge terms are **in PR-1b**; min-content's segmentation —
  cross-item joining and layout's break oracle — is a **prerequisite PR in `main` before PR-1b** (§8; ledger **A58**), because that pass
