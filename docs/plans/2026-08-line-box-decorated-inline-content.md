...
are frozen. Implementation starts.** The ledger below is kept as the record of how the loop
ran; it no longer gates anything.
...
three cells; R3 changes three markups and adds none; R4 removes one cell's third assertion and adds
one cell). **Not frozen**
(still correctable without reopening anything): Grounds columns, citations, coordinates, §7–§10,
and the ledgers.

**⚠ Amended three times, by the external channel this freeze deliberately keeps** (Codex R2, R3
then R4 on #515, 2026-09-20 — revs 36, 37 and 38; the freeze declares #515 unblocked, not the reviewer silenced,
...
* **R2-F3** — M3's hang gate was written against M5's **whole-box** `has_inline_axis_edge`, which
  ...
  other and one of which differs between the gates. *Rejected position, recorded here rather than
  in the cell*: the symmetric fixture as a gate test.
* **R2-F1 / R2-F2 / R2-F4** — one root: elidex has **no per-box layout-bounds model** (css-inline-3
  ...
  §5.3 model with four enumerated facets (§5.3, §9, §10) — **five from R4** — M6's and M7's disclosures are corrected
...
* **R4-b (P1) — §6 cell 13's third assertion asserted a paint no code path produces.** It claimed
  ...
  a render pass bolted onto a layout program. ⚠ **The fix is a sweep, not that cell**: every site
  claiming a painted or user-visible delta was re-checked — §1.2's clause list and §1.1's framing
  are spec statements and stand; §4.3, §5.1 M1, §5.2's `elidex-render` row and its
  box-decoration-break row, §5.3's PR-1b and PR-1c bullets (including the PR-1b-before-PR-1c
  *ordering* argument, which was made **on paint** and is remade on the readers that exist), §6
  cells 6, 6g and 13b, §6's test-placement note, §7's "Painted output" bullet, its numbered change
  1, its reader-family entry and its closing sentence, and §8's allowlist paragraph, PR-1b DoD and
  PR-1c DoD all carried the same claim and are corrected. *Rejected positions*: adding an
  `elidex-render` pass or member to this program; and deleting cell 13(c) alone, leaving the same
  claim standing at thirteen other sites.
  ...
  what render paints from, and (ii) the box's own rect at the readers that take
...
* **R4-a — rev 36's own "exact for a decoration-only line (one box, composition trivial)" is
  ...
  and is pinned by new cell **24d**. *Rejected position, recorded here rather than in the cell*:
  R4's own proposed markup, `font-size:100px;line-height:10px` beside
  `font-size:10px;line-height:100px` — measured against the test font it does **not** demonstrate
  what it was offered for, since that second box's `A′` **and** `D′` (53.467 / 46.533) both exceed
  the first's (39.668 / −29.668), so one box dominates, the composition is 100 and M6's
  `max(line-height)` already gives 100. Holding both line-heights at 10px is what puts the maximal
  ascent on one box and the maximal descent on the other (39.668 / 1.533, composition 41.201
  against a `max` of 10), and that is the markup cell 24d carries.
* **R4-c — the `:200` gate's lift is inline-axis-only, and the residual is pre-existing.** Decided
  by measurement at `22de3078` rather than by argument: `line_count` for
  `<p><span style="padding-top:10px;font-family:no-such-family">x</span></p>` is **0**, for the
  same markup with an inline-axis edge **0**, with no edge at all **0**, and with a resolvable font
  **1**. The gate is blind to edges, so it is the measurability guard M5 calls it; PR-1d's escape
  raises the inline-axis arm to 1 and leaves the others untouched, making no case worse than today.
  Disposition therefore **slot + pin**, not a wider lift: `#11-inline-fontless-measurability-gate`
  (§5.3, §10), pinned as a second assertion on §6 cell 12d. *Rejected position*: widening the
  escape to clause 1 here — the `:200` return also runs `clear_inline_flows` and
  `remove_one::<ColumnFlowSlice>`, so it would commit an IFC no run can measure.
...
their subjects are unchanged. R4 **does** touch the pass's surface and is flagged rather than
folded in: cell 13 loses an assertion, cell 24d is new (so M7's row now has a second cell beside
24c), and cell 12d gains one — no M-row *Decision* column changes, M7's correction being in its
Grounds.
...
CSS Sizing 3, CSS Writing Modes 4, CSS Display 3, CSS Pseudo-Elements 4, CSS Content 3, CSS Backgrounds 3, CSSOM View 1), M=39 entries (`Split decision` below restates K; both are recomputed). Both figures are recomputed
...
the following content is not displaced, which moves painted glyphs; and the span's own box is
...
* **PR-1b — inline-axis advance.** Owns couplings 7×2 and 7×(intrinsic): M3 + M8, **plus M5's
  ...
  ahead of PR-1c rather than after it; PR-1c's bullet states the asymmetry. ⚠ **This was stated
  about paint until R4** — "paint draws exactly the area it draws today" — which is true of a
  static inline only because it draws nothing at all (§5.2's `elidex-render` row); the property the
  ordering argument needs is the one about extent, so it is stated as that.
...
* **PR-1d — existence.** Owns couplings 1×3, 1×5 and 1×4: M5's *flip* + M6 + M7. The clause-3
  ...
  cells 18, 19, 20, 21, 22, 23, 23b, 24, 24b, 24c and 24d land here, plus the flip set §8 states.
...
* **`#11-inline-root-inline-box`** (new slot, **pre-existing** class): ⚠ **the whole css-inline-3
  ...
  its inline-level boxes". ⚠ Edge inflation is **not** part of the default: §2.2 step 2 and §5.3
  ...
  * **(b) the committed rect's block extent**: `commit_aligned_entity_rects` writes
    ...
    (`inline/pack/mod.rs:493`), where §5.3 composes the line *from* each box's bounds and never
  ...
  * **(e) composition on a *decoration-only* line carrying more than one box** (R4): css-text-3
    §5.5 gives an inline box boundary no soft wrap opportunity, so two glyphless decorated boxes
    share a line and §5.3 gives each a strut of its own; M6's scalar `max(block_advance)` and M7's
    single tentative `Option<f32>` between them carry one box's line-height and one box's ascent,
    which is not the aligned composition of two. M7's Grounds carries the measured instance —
    M7, cell 24d.
...
6g. **A decorated pseudo-element inline box gets markers** (M1's routing special-case; Codex on
    ...
    **behaviour-neutral** here (the markers are inert until PR-1b). Paired with 6c–6e it pins that
    ...
    background and border, which is what this clause said until R4**: an inline pseudo is emitted
    by `elidex-render/src/builder/inline.rs:205-213`, which pushes a `StyledTextSegment` from its
    `TextContent` and `continue`s — it never reaches `walk`, so no chrome is drawn for it either
    (`#11-inline-decoration-paint-path`).
...
16. **`text-align`, and *which side* of the box follows the space** (css-text-3 §4.1.2) — a
    ...
    * **(b) an edge after the space** — the same markup with `padding-right:10px`: the box's 10px
    ...
    disjunction — true for this box at *both* markers — clears it: (a)'s two readings differ by
    `(measure_width("abc ") − measure_width("abc")) / 2`, and so do (a) and (b). Expectations are
...
13. `<p>a<span style="padding:10px">text</span>b</p>` — the common case §4.3 is about. Two
    ...
    `inline_start` (`elidex-render/src/builder/inline_flow.rs`) and the advance moves it.
    (a) is what the `LayoutBox`-fed CSSOM readback then answers from —
    `clientWidth`/`clientHeight`, `getClientRects` and `getBoundingClientRect`, with
    `clientTop`/`clientLeft` held at zero by the predicate prereq (§9). A third assertion, on the
    *painted* background and border rects, stood here until R4 and is withdrawn: nothing emits
    either for a **static** inline, so PR-1c can neither satisfy nor violate it —
    `#11-inline-decoration-paint-path` (§5.3) owns that gap, and the freeze amendment records the
    withdrawal.
13b. **The committed rect takes the *line's* block extent, not the box's — pinned as accepted**
    ...
    style="padding:2px;line-height:10px">x</span></p>` — the span's `LayoutBox.content` is
    **40px** tall and its `border_box()` **44px**, where css-inline-3 §5.3 calculates the box's
    layout bounds "with respect to its **own** text metrics" and gives **10px** — so the border
    area cssom-view-1 §6 asks for is 14px, not 44px. (⚠ The 4px is the *padding*, not a §5.3
    inflation: §5.3 inflates layout bounds by the edges only when `line-fit-edge` ≠ its initial
    `leading`, as §3's own row records; the cell asserts the two quantities separately.)
    `commit_aligned_entity_rects` writes `block_size: line_height` — the *line's*
    height — for **every** merged entity (`inline/pack/mod.rs:493`), and §5.3 composes the line
    *from* each box's bounds rather than assigning the line's extent back to them; every
    `clientHeight` / `getClientRects` / hit-test / a11y-bounds reader of that rect inherits the 40.
    (The list named background-and-border until R4; that is not a reader of a static inline's box
    — §5.2's `elidex-render` row.)
    ⚠ **Pre-existing, which is why it is pinned and not fixed**: the write is reached through
    `place_item`'s push under `entity != parent_entity` (`:706`), so the 40px content extent is
    ...
    ⚠ **What discriminates**: the two candidate models are 30px apart on the same markup, and the
    cell fails in both directions — if per-box bounds ever land it goes 40 → 10, and a cell
    written on a span whose `line-height` equalled its line's would pass under either.
...
23b. **The vertical facet of the same divergence, pinned as accepted** (M6; Codex R2-F2):
    ...
    boxes with content") and css-inline-3 §5.3 computes, §5.3 applying in vertical writing modes
...
24c. **A glyphless decorated box beside differently-sized text: the strut does not compose —
    ...
    the cell asserts only the first: §5.3 makes a box's layout bounds `A′ + D′ = line-height`
...
24d. **A decoration-only line with *two* glyphless boxes: neither the height nor the baseline
    ...
    convention, `inline/tests/mod.rs:31` at `22de3078`): the line's height is `max(line-height) = 10`, and
    the IFC's `first_baseline` is **one** box's strut ascent, where css-inline-3 §5.3 gives each
    box `A′ = A + L/2` / `D′ = D + L/2` and css-inline-3 §2.2 step 3 sizes the line to enclose
    both — the first box supplying the maximal `A′` and the second the maximal `D′`, so the
    composition is neither box's and exceeds the scalar `max`. Facet (e) of
    `#11-inline-root-inline-box` (§5.3).
    ⚠ **Why two boxes and not one**: on a single-box decoration-only line the scalar `max` and
    the single tentative *are* the composition, which is why M7's Grounds claimed exactness there
    until R4. The cell exists to hold that claim withdrawn, and it discriminates only because the
...
* **Every reader of an inline `LayoutBox`.** **Three** distinct changes, in three different PRs:
  1. ⚠ **Position — PR-1b, and the re-slice's shippability argument does not cover it.** M3's
     ...
     after the box. §5.3 and §8 say PR-1b changes no `border_box()` *extent* — true, and not the
     same claim. PR-1b's DoD therefore carries its own reader disposition, `elidex-render`'s
     existing `border_box()`-reading paint tests included. ⚠ **The list named "every painted rect"
     until R4**: the box's *own* rect is never painted (the bullet above), while the glyphs after
     it are — the correction narrows what moves, it does not remove the painted channel.
  ...
  **PR-1c reconciles against it rather than re-deriving**: the grep below is the discovery method,
  ...
  clause named cell 13's paint assertions until R4; the cell has none, and the `elidex-render`
  files are untouched by every PR of this program.) ⚠ Two wires, and only one is blind here: **wire #1** (the allowlist) *does* see
...
(M3, M5), so a grep for the whole-box name at either boundary gate is the R2-F3 defect back;
M8 adds the max-content contribution and **not** the min-content one. ⚠ **The box's own geometry is
...
this PR can precede PR-1c (it read "no painted rect changes extent" until R4; §5.3's bullet carries
why that phrasing was vacuous for a static inline). `pack/mod.rs:726`'s fabricated citation is rewritten here (§3.1) — the one cite
...
chrome — the clause claimed "`walk.rs` paints on `ComputedStyle` + `LayoutBox`" until R4, which is
the block path; what a pseudo does carry today, through its own `place_item` run, is the
`LayoutBox` the CSSOM / hit-test / a11y readers take — cell 13's delta on a second class, no new
reader) — and §7's allowlist reconciliation re-run
...
**PR-1d** (existence): cells 18, 19, 20, 21, 22, 23, 23b, 24, 24b, 24c and 24d, plus the flip set — **cells 1, 2, 7, 8, 10, 11, 12 and 12d flip; 5, 6b, 6c, 6f, 6d, 6e, 6g, 6h and 9 do not**; this is the one normative statement of it — every §7
