...
instruction.** Two prereq PRs move code before PR-1a — the seam-3 split relocates
`inline/mod.rs:413-639`, and the dead-arm deletion removes 29 lines at `pack/mod.rs:393-421`,
shifting every `pack/mod.rs` coordinate above `:421` by −29. The memo does **not** re-anchor after
each: its coordinates exist to prove claims about the code as it stands today. **Each PR's own
plan-memo re-anchors against its actual base**, and §8's DoDs name behaviours and call sites, not
line numbers. Round 9 found 27 distinct `pack/mod.rs` coordinates in this class; re-anchoring them
...
**PR-1a characterization (assert today's behaviour; PR-1c flips 1–4, 3b, 7, 8, 10, 11, 12 and 12d — cells 5, 6, 9, 12b and 12c are non-flipping):**
...
12e. **Depth ≥ 2 direction divergence** (M1): `<p dir="rtl"><span dir="ltr"><em style="padding-left:10px">x</em></span></p>`
    — the `em`'s parent is the `ltr` span, so `padding-left` is its inline-**start** edge, even
    though the IFC root is `rtl`. Pins that the parent's direction governs (css-break-3 §5.4), not
    the root's; cell 12b cannot discriminate the two because at depth 1 they coincide.
...
* **Every reader of an inline `LayoutBox`'s edges.** M4 flips `padding`/`border`/`margin` on inline
  ...
  constant now read data. Enumerated 2026-08-02 by `grep -rn "border_box()\|padding_box()\|margin_box()" crates/`
  and dispositioned:
  ...
  * `elidex-js` `intersection_observer.rs` / `resize_observer.rs` — observed box sizes. Correct,
    and JS-observable.
  * `elidex-ecs/src/dom/geometry.rs`, `elidex-layout-multicol`, `-table`, `block/children/stack.rs`,
    `inline/atomic.rs` — block/atomic paths, unreached for a non-replaced inline.
...
silencing it.
**PR-1b** (geometry): cells 13, 14, 14b, 15, 15b, 16, 16b, 17, 17b, 17c and 25; **§7's full `LayoutBox`-edge reader list dispositioned, not just paint** (a `getBoundingClientRect` cell included); no double-counted edges on
