...
3b. **Cancelling pair, the sum/disjunction contrast on the advance side** —
    `<span style="margin-left:-10px;padding-left:10px">`: the inline-start components sum to zero
...
14b. **The gate's negative arm** — `<p>a<span style="padding-top:1px"></span>b</p>`: a marker
    ...
    (cell 16's contrast, and M3's `hang: None` path), **`b` is not displaced — the cursor advance
    is zero** because M3's sum is over inline-axis components only, and the line stays phantom.
...
17f. **The multi-line `getBoundingClientRect`, pinned as accepted** (§7; cssom-view-1 §6
    ...
    **union of its per-line content spans expanded by the padding on all four sides**, which is
    ...
    `box-decoration-break: slice` (initial) CSS 2 §9.4.2 gives the split no visual effect and
    css-break-3 §5.4 says which side of each fragment that is, so a fragment owning an extreme of
    the union may carry no edge there while elidex applies the full one. Block-axis edges are on
    every fragment — a line break is an inline-axis break — so the block axis agrees. elidex is
    therefore never *smaller* than the spec's box and strictly larger whenever a broken edge owns
    an extreme. The cell asserts elidex's value, in the shape cells 23 and 24 use; the comparison
...
21. **Co-resident entity on a newly-existing line** — css-inline-3 §2.3's "and its in-flow
    ...
    too (M5) — **which is that entity's first-ever `LayoutBox` if it owned no other line**. This
    is PR-1d's half of the presence change §7 states; PR-1c's half (cell 14c) reaches only lines
...
24b. **The cancelling pair keeps its line** (M5's disjunction, the half cell 3b cannot assert) —
    ...
    quantities, so the line is **not** phantom and a `LineBox` is emitted. Constructible only here,
...
* **Every reader of an inline `LayoutBox`.** Two distinct changes, and the second had been missed:
  ...
  2. ⚠ **Presence, and it lands in TWO stages, in two different PRs.** Absence is *observable* at
     ...
     * **PR-1d (M5's flip) — on lines that did not exist.** Flipping the predicate moves whole
       ...
       all before the flip, so M4 alone never reaches them — and every co-resident entity, decorated
       or not, that owned no other line. Cell 21 for the co-resident, and the flip of cells 7 and 8
       for the box itself.
     ...
     §2.1 the first broadcast must deliver an initial 0×0 entry" (`resize.rs:245-252`, the same at
     `intersection/mod.rs:298-300`) — so both observers already fire for that span today. And change
     detection compares the **content** size only (`resize.rs:257`), while M4's rect is a content
     span by construction and resize-observer-1 §3.3 says a "non-replaced inline Element will always
  ...
  **PR-1c reconciles against it rather than re-deriving**: the grep below is the discovery method,
  ...
  write-chokepoint ban, which "cannot see `let lb = LayoutBox{…}; insert_one(e, lb)`". And because
...
no-op. They arrive in **PR-1b** with M3 and M5 — M4 needs no payload field, deriving the
`LayoutBox` edges from the entity's own `ComputedStyle` at the box-assign site (§5.1 M4).
...
citation work (§9). `note_line_occupancy`, `pack/inline_box.rs` and M8's contribution carry
docstring citations to their §3 rows.
...
with its delta recorded; no double-counted edges on either side; the `InlineClientRects` write path
...
* **`#11-inline-fragmented-fn-decomposition` — trigger fires, and this program honours it.** The
  ...
  **Cold gate** ([[feedback_split-on-touch-prereq-workflow]]): re-run `gh pr list --state open` and
  ...
  `gh pr list` cannot see them. Re-run on `658cc302`: of the six open PRs, **none touches
  `crates/layout/elidex-layout-block/` at all**, and of every local branch not merged into
  `origin/main` only `feat/m4-1.5-2-plugin-arch-anim` and `layout-text-height-split` touch that
  crate — both in `block/tests/` or `inline/tests/text_height*`, **neither in `inline/mod.rs`**.
  ⚠ A round-16 finding held that widening the touch set to `elidex-render`/`elidex-dom-api` would
  surface `domform-submittable-category`. **Measured and refuted**: that branch's 36-file diff
  contains no `crates/layout/`, no `elidex-render` and no `elidex-dom-api` path, so it collides
  with neither the prereqs' touch set nor the feature PRs'. Recorded rather than dropped, because a
  finding accepted without re-derivation is the failure this memo's front matter exists to prevent.
...
* **`#11-css2-spec-label-normalisation`** — this memo cites css-inline-3 for the model, so its
  ...
  <memo> | sort -u`; each section↔title pair verified with `webref heading CSS2 <n>`). Adjacent `CSS 2.1 §`
