...
12g. **The emptiness gate ignores markers, and a probe is what shows it** (M1; PR-1a; R33) —
    ...
    `clear_inline_flows` is `!env.is_probe`-gated (`:637-639`), and the seeded flow **survives** a
...
25d. **An edge-only box with negative margins does not push an intrinsic size below zero**
    ...
    is PR-1b's.
    ⚠ **The zero case alone does not discriminate, so the cell carries a second markup** (R33):
    on the edge-only fixture an implementation that drops margins from M8 entirely, and the
    per-marker clamp **A72** rejects, both also report 0. The second markup puts the signed sum
    where the outer floor cannot reach it —
...
⚠ **The reconciler and min-content prereqs are ordered against none of the other prereqs and both
in `main` before PR-1b** — the min-content one because PR-1b's line advance has no intrinsic size
