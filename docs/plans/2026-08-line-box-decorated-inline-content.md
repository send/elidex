...
6d. **A decorated `display: contents` element gets no marker** (M1, same predicate, different
    conjunct) — `<p>a<span style="display:contents;padding:10px">x</span>b</p>`: css-display-3 §A
    says such an element generates **no box at all**, so it has no outer display type and cannot be
    an inline box. ⚠ It reaches the arm because `collect.rs` has no `Contents` branch and `:277`
...
* **The canonical *inline box* predicate, with the `client*` guard as its first consumer — carved
  ...
  **or** … establishes a new formatting context", so **where** the one canonical answer to "is this
  ...
  css-inline-3 §2.3 clause 3 does not reach. **Two requirements, therefore, and §8 states them**:
  the replacedness half is **complete** over what `get_intrinsic_size` covers, and the composed
  predicate is **callable from `elidex-layout-block`**, because M1 is a consumer. *How* — the home,
