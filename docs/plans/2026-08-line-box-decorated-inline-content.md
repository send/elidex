...
**seven** above is unaffected: seven titled pairs, four untitled cites (CSS 2 §10.2 from rev 59, §10.4 from rev 69), eleven sections.
...
behaviour** of §6's cells — 49 at rev 34, **58** after the amendments recorded next, **62** at rev 60, **63** at rev 65, **65** at rev 66, **66** at rev 67 and **67** at rev 68 by `plan-xcheck.py` (R22's 10c, R26's 25b, rev 60's 10d and 17g, rev 65's 25c, rev 66's 3c and 25d, rev 67's 12g, rev 68's 25e — ledger A54, A59, A60, A70, A71, A72, A76, A79); rev 63 adds none (R2 adds
...
25d. **Signed margins in intrinsic sizing: the used width floors at zero, and the margin is not
    ...
    * **the floor** — `<div style="display:inline-block"><span style="margin:-10px"></span></div>`,
      ...
      this markup. The spec floors the **used width**, not the contributions — css-sizing-3 §2.2
      presupposes that a contribution can be negative, which is why it floors the max-content
      contribution by the min-content one "(e.g. due to the use of negative margins)" — so a value
      for either pass here would be the umbrella inventing one. **Rejects** propagating `−20` into
...
13d. **A `position:relative` decorated inline's *painted rect* moves, and this is the one cell
    ...
    **Run through the real producer** (R32): the markup goes through `elidex-shell`'s pipeline —
    ...
    drives the pipeline, and `background_color_in_pipeline` (`:118-130`) matches `DisplayItem::SolidRect` off the other entry point, `build_pipeline` — so the `LayoutBox` the paint reads is the one
...
17c. **A box opened at a soft wrap has no rect on the line before it** (M4; rev 60, ledger
    ...
    **registered deviation** §3's css-text-3 §5.5 *adjacent soft wrap opportunity* row marks ✗
    ...
    it is `normal`, where the outcome is this memo's and is **removal** (M3; cell 16; freeze
    discipline 5's main rule): line N's trailing space contributes nothing, and the stale point is
...
ledger **A58**), and cell 16 asserts step 3's removal (`white-space: normal`), which the **end-of-line white-space** prereq — the first of the two — makes conformant.
...
**PR-1d** (existence): cells 18, 19, 20, 21, 22, 23, 23b, 24, 24b, 24c, 24d and 24e, plus the flip set — **cells 1, 2, 7, 8, 10, 11, 12, 12d and 12g flip; 5, 6b, 6c, 6f, 6d, 6e, 6i, 6g, 6h and 9 do not**; this is the one normative statement of it — every §7
...
* **`#11-css2-spec-label-normalisation`** — this memo cites css-inline-3 for the model, so its
  ...
  §10.4, §10.8.1 and §16.6.1** — eleven (§10.4 entered with rev 69's cell 25d; §10.2 with rev 59's A59, whose list still read nine — the recurrence recorded next; re-derived by the command below). ⚠ **Seven until rev 34, and the eighth was invisible to the very command
