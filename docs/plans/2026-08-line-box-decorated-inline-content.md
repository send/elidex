...
**seven** above is unaffected: seven titled pairs, three untitled cites (§10.2 from rev 59), ten sections.
...
behaviour** of §6's cells — 49 at rev 34, **58** after the amendments recorded next, **62** at rev 60 by `plan-xcheck.py` (R22's 10c, R26's 25b, rev 60's 10d and 17g — ledger A54, A59, A60) (R2 adds
...
17c. **A box opened at a soft wrap has no rect on the line before it** (M4; rev 60, ledger
    ...
    **registered deviation** §3's css-text-3 §5.5 *adjacent soft wrap opportunity* row marks ✗
    ...
    with inline size `measure_width("cccc")`. The §5.5-conformant rect would start at the 5px
    ...
    point that **enters the hull** and widens the rect — provided
    `measure_width("aaaa ") + 5 > measure_width("cccc")`, a **precondition the test asserts** with
    the harness's `measure_width`, like the two relations above. So the cell asserts the extent,
...
asserts a `LayoutBox.border` or `LayoutBox.margin` field** — the population is that list, not a
second copy of it; 10c's and 10d's margins reach their results through the cursor, never through
the field (⚠ "carries padding and nothing else" stood here until rev 60, false since 10c) — while
