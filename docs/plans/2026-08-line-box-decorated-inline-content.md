...
| `crates/layout/elidex-layout-block/src/inline/measure.rs` | `max_content_inline_size` (`:44`) and `min_content_inline_size`'s edge term — M8's contribution, both PR-1b's. `min_content_inline_size`'s **accumulator** — `max_word` at `:23`/`:31` — is **not this program's**: the min-content prereq PR gives it cross-item joining, in `main` before PR-1b (§8), and PR-1b re-anchors against that base. The function itself (`:15-37`) PR-1b does touch, because its `collect_inline_items` call (`:22`) takes the new argument like every other caller. |
...
carries; this paragraph was re-derived then** — reads **seven** of this program's nine `opens`
statements, not nine (six until #518, which added `predicate prereq` to its alternation).
...
`predicate prereq` was unmatched by name until #518 — and `reconciler prereq` (R16's carve) and
`min-content prereq` (R17's) are the names it still does not reach, which is the
...
no row is as live a pair as a "1" against one — while `reconciler prereq` and
...
to a fourth or fifth prereq routes to nowhere by the checker's own reckoning (`:568` admits only
those three; `:584` is where the fourth tag fails — `4394af4c` coordinates). The precedent is already the predicate
...
prereq PR has landed, and cell 25 asserts both intrinsic sizes on that base — the cross-item
joining the prerequisite's, **both edge terms this PR's** (ledger **A58**).
...
M8 adds **both** edge terms **in this PR**; only min-content's cross-item joining is in `main`
before it (§8's *Min-content prereq PR*; ledger **A47** — an earlier drafting deferred the
...
instances, owned apart (below; ledger **A58**).
...
⚠⚠ **Ownership is split, and it is settled** (R25; ledger **A58**). **This PR owns instance 2
only.** Cross-item joining reads `InlineItem::Text` and no marker, so it is realisable at the
...
has layout and intrinsic sizing disagreeing — A47's ground, met — and §6 routes cell 25 to it.
...
* **`#11-layoutbox-absence-unreachable`** (#488, registered in the SoT) — no longer relevant, and
  ...
  signal is required.
* **Intrinsic sizing** — per M8: both edge terms are **in PR-1b**; min-content's cross-item
  joining is a **prerequisite PR in `main` before PR-1b** (§8; ledger **A58**), because that pass
  has no accumulator to attach edges to and PR-1b is not correct without it. ⚠ **It was slotted until
...
* **The enumeration-completeness audit (R20) — the standard it used, what it found, and the six
  ...
  **The remaining three take no token, each for a stated reason.** css-break-3 §5.4's
  ...
  passes thread — so it is an input to PR-1b's own plan-review, both edge terms being its
  (ledger **A58**), not a platform slot. css-content-3 §1's per-value box count: conformant today by an empty
