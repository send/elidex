...
IFC root with the Grounds bullet that assumed css-writing-modes-4 §3.2 is implemented here. ⚠ The
...
**⚠ Amended ten times, by the external channel this freeze deliberately keeps and by the
...
* **PR-1c — box geometry.** Owns coupling 3×7: M4. The open-box stack and its flush-time
  ...
  the count is unchanged and only the geometry moves, which is why a count-based measurement
  missed it. Cell **13d** asserts it; ledger **A8** (narrowed) and **A33**. The rest of the delta is
...
(R9), `#11-inline-open-box-strut-on-continuation-line` (R10) and
`#11-writing-mode-inline-blockification` (R11) and the
dead arm's own disposition are pre-existing class, so none of them enters a per-PR count. ⚠ **Three
of them were added at R10, two of those having been missing before it — and the last, R11's, was
added to this list in the same revision that opened it, which is the discipline the record below
asks for**: each round that
...
* **Painted output — unchanged by PR-1c for a *static* inline (R4 withdraws the claim that it was
  the program's largest delta); for a `position:relative` one the count is unchanged and the
  *geometry* changes, which R11 measured and R4's count could not see.** A static
...
`find_break_opportunities(<that fixture's concatenated text>)` ∪ **the two boundaries of every
atomic item** ∪ any forced break; a shared
...
`#11-inline-item-boundary-soft-wrap`. The invariant is therefore defined over the **item stream**,
not over one string. ⚠ **The term is deliberately *permissive* at the GL/WJ/ZWJ exception**: it
admits both boundaries of every atomic without asking what is adjacent, so it over-licenses the
sentence's second half. That is the safe direction — the invariant's job is to catch flushes at
**inline-box** boundaries, and a subset assertion is unsound only when it *rejects* a licensed
break. ⚠ **And U+FFFC alone does not encode the rule**, which is why the item stream rather than a
sentinel character is the subject: measured through `find_break_opportunities`,
`"a\u{FFFC}b"` yields opportunities on both sides of the object character, but `"a\u{A0}\u{FFFC}b"`
yields only the one *after* it and `"a\u{FFFC}\u{A0}b"` only the one *before* — U+FFFC is UAX #14
class CB and LB12/LB12a suppress the break adjacent to a GL character, exactly the case §5.5's
first sentence licenses. U+FFFC may still be used as an **encoding convenience** where the two
agree; it is not the definition. Whoever discharges `#11-inline-item-boundary-soft-wrap` by
