...
| `crates/core/elidex-plugin/src/layout_types/boxes.rs` | `InlineClientRects` (`:106-111`) — the cross-crate contract type. **Not changed by this program**: its docstring's "per-line client rects … single-line inlines use `LayoutBox.border_box()`" stays true, and PR-1b makes the fallback half *correct* rather than redefining the component. The multi-fragment redefinition belongs to `#11-inline-box-decoration-splits`, which owns the docstring edit too. |
| `crates/core/elidex-render/src/builder/slice.rs` + `walk.rs:296` + `block/mod.rs:369-372` | **Existing `box-decoration-break` implementations, and why this program does not extend them.** `walk.rs:296` reads `style.box_decoration_break`; `slice.rs` implements per-fragment slice geometry for **column** fragments; `block/mod.rs:369-372` handles `Slice` and `Cloned` for **block** fragments. Both attribute edges across a *block-axis* break, and a line break is an **inline-axis** break with the parent's inline progression direction deciding which side survives (css-break-3 §5.4) — a different axis and a different direction source, so neither helper generalises as written. `#11-inline-box-decoration-splits` owns the inline case and should decide there whether to generalise `break_edges` or add a sibling. |
...
* **PR-1a — item stream. Behaviour-neutral.** Owns couplings 2×(all) and 1×2: M1 + M2. Marker
  ...
  until PR-1c. **Characterization tests for §6 cells 1, 2, 5, 7–12 and 12d land here
...
* **PR-1b — geometry.** Owns couplings 7×2, 3×7 and 7×(intrinsic): M3 + M4 + M8, **plus M5's
  ...
  **This is the engine-wide correction**: it changes painted output for every decorated inline
  with content. §6 cells 3, 3b, 4, 6, 12b, 12c, 12e, 13, 14, 14b, 15, 15b, 16, 16b, 17, 17b, 17c, 17d and 25
  land here — cells 3, 3b, 4 and 6 among them because each asserts a *payload* fact (negative
...
* **`#11-inline-box-decoration-splits`** (new slot, **own** deferral): css-inline-3 §2.1 box
  ...
  **multi-fragment `getClientRects` channel** (cssom-view-1 §6 step 3): inflating a stored
  ...
  `slice_and_rebase_fragment`'s `retain` (`pack/fragment.rs:70`) destroys for a paged or multicol
  IFC, and it needs the **parent's** inline progression direction, which §5.4 specifies and M1's
  own-direction mapping deliberately does not supply. ⚠ It additionally needs
  `#11-inline-relayout-box-staleness` first: the only site that could inflate
  (`assign_inline_layout_boxes`) skips any entity that already has a `LayoutBox`, so the work is
  unreachable on every layout after the first. Also fragmentation across the marker pair;
...
force `any_font == false`**, without which cell 12d is unconstructible; cells 1, 2, 5, 7–12 and 12d land as characterization
tests; zero behaviour change. **Dead-field rule**: fields
...
**PR-1b** (geometry): cells 3, 3b, 4, 6, 12b, 12c, 12e, 13, 14, 14b, 15, 15b, 16, 16b, 17, 17b,
17c, 17d and 25 — ⚠ **all of them assert a first-layout property**, because `assign_inline_layout_boxes`
...
**PR-1c** (existence): cells 18–24, plus the flip set — **cells 1, 2, 7, 8, 10, 11, 12 and 12d flip; 5 and 9 do not**; this is the one normative statement of it — every §7
...
ship with the **seam-3 prereq PR**, the first of the five to open *and* to land, per
...
(deleting `:413-639` renumbers nothing below it, and its `pack/mod.rs` half is in a file seam 3
never touches), while **dead-arm-first forces the seam-3 PR to re-measure the one criterion in this
...
stacks on PR-1a and PR-1c on PR-1b, since each consumes the previous one's mechanism.
...
* **File growth**: measure with `wc -l` at each PR rather than against a number written here, which
  ...
  predicates and the persist-block touch — no new seam decision needed. `pack/inline_box.rs` is the one source file this program **authors**, so
...
| Register **and** close `#11-inline-fragmented-fn-decomposition` in one row — it is registered as closed-on-landing, not registered then closed — **as a partial close**, naming the seams §9 measures as still open, in the successor slot `#11-inline-fragmented-fn-seams-1-2` (**pre-existing** class — the seams predate this umbrella; Why: the prereq PR discharges seam 3 only; trigger: **either** the first change after **any** of this umbrella's five PRs that touches the residue — self-exempted for all five PRs — the source slot's touch disjunct fires on "the next change that touches `layout_inline_context_fragmented`'s body", and every such touch in this program is one the slot's own decomposition work *surfaced*, so counting them would make the successor fire on its own predecessor — **or** `inline/mod.rs` growing back toward 1000 lines, the source slot's second disjunct, which has not fired and which this program's own growth could trip; re-eval 2026-11-01). | seam-3 prereq PR |
