...
* **`#11-inline-box-decoration-splits`** (new slot, **own** deferral): css-inline-3 §2.1 box
  ...
  **per-fragment `getClientRects` channel** (cssom-view-1 §6 step 3), in both the shapes §3's two
  ...
  longer holds: R12 moved the `layout_generation` comparison into PR-1c, so PR-1c's edges *are*
...
`#11-intra-word-shaping-across-line-break` (R12) and the
dead arm's own disposition are pre-existing class, so none of them enters a per-PR count. ⚠ **Three
of them were added at R10, two of those having been missing before it — and the last two, R11's
and R12's, were added to this list in the same revision that opened them, which is the discipline
the record below asks for**: each round that
...
already prescribes — a `layout_generation` comparison (the component carries one, `boxes.rs:86`)
in place of the presence test — at a file M4 is editing anyway, and it preserves the skip's stated
purpose, since a box written by `layout_child` in the same pass already carries the current
generation. The slot keeps the residue it names (§9). Found by Codex on #515. **§7's full `LayoutBox`-edge reader list
...
* **`#11-inline-relayout-box-staleness`** (`inline/pack/boxes.rs:96`) — pre-existing, and the
  ...
  **PR-1c therefore takes the `layout_generation` comparison** (the component already carries one,
  `boxes.rs:86`) in place of the presence test, at a file M4 edits anyway and without disturbing
  the skip's stated purpose (a box `layout_child` wrote in the same pass already carries the
  current generation) — §8 states it as a PR-1c deliverable. **This slot keeps the residue**: every
