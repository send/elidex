...
| CSS Break 3 §5.4 Fragmented Borders and Backgrounds: the box-decoration-break property | `box-decoration-break: slice` (the initial value) | per-fragment edge attribution: the inline-start edge on the first fragment, the inline-end edge on the last, block-axis edges on every fragment | M4's `InlineClientRects` derivation — **PR-1b** | ✓ (for `slice`) | yes |
| CSS Break 3 §5.4 Fragmented Borders and Backgrounds: the box-decoration-break property | `box-decoration-break: clone` | edges repeated on every fragment; "no visual effect where the split occurs" (that phrasing is CSS 2 §9.4.2's, not this section's) | **`#11-inline-box-decoration-splits`** | ✗ (deliberate) | yes |
...
| CSSOM View 1 §6 Extensions to the Element Interface | `getClientRects()` step 3 | one `DOMRect` per box fragment "describing its **border area**" — padding + border, never margin | M4 derives it at `assign_inline_layout_boxes` from the stored content spans — **PR-1b** | ✓ | yes |
...
| CSS Writing Modes 4 §3.2 Block Flow Direction: the writing-mode property | box whose `writing-mode` differs from its **parent box** | an otherwise-`inline` box's display computes to `inline-block` | M1's emit test — such a box is an atomic and gets no marker; §6 cell 12c — **PR-1a** | ✓ | yes |
...
CSS Sizing 3, CSS Writing Modes 4, CSS Backgrounds 3, CSSOM View 1), M=32 entries. Both figures are recomputed
...
* **PR-1a — item stream. Behaviour-neutral.** Owns couplings 2×(all) and 1×2: M1 + M2. Marker
  ...
  until PR-1c. **Characterization tests for §6 cells 1–12, 3b and 12d land here
...
* **PR-1b — geometry.** Owns couplings 7×2, 3×7 and 7×(intrinsic): M3 + M4 + M8, **plus M5's
  ...
  **This is the engine-wide correction**: it changes painted output for every decorated inline
  with content. §6 cells 12b, 12c, 12e, 13, 14, 14b, 15, 15b, 16, 16b, 17, 17b, 17c, 17d, 17e and 25 land here.
...
* **`#11-inline-box-decoration-splits`** (new slot, **own** deferral): css-inline-3 §2.1 box
  splitting across line boxes and bidi fragments, and **css-break-3 §5.4's `box-decoration-break: clone`** (the "no visual effect where the split
  occurs" rule — that string is CSS 2 §9.4.2 verbatim and appears nowhere in css-inline-3 §2.x);
  **`slice`, the initial value, is PR-1b's**, because M4's border-area derivation cannot answer
  `getClientRects` for a wrapped box without attributing the start and end edges to the first and
  last fragments. Also fragmentation across the marker pair;
...
17d. **`getClientRects` agrees with `getBoundingClientRect`** — a decorated inline that wraps must
    return per-fragment **border-area** rects (padding + border, **not** margin, per cssom-view-1
    §6 step 3), matching what the single-line case returns through the `LayoutBox`. Constructible
    as `<p style="width:60px">aaa <span style="padding:10px">bbb ccc</span></p>`. Per css-break-3
    §5.4's initial `slice`, the **first** fragment carries the inline-start padding and the
    **last** the inline-end padding; both carry the block-axis padding. Asserts the derivation
    M4 performs at `assign_inline_layout_boxes`, not a stored form.
17e. **The stale-`InlineClientRects` arm** (M4's second ⚠) — an element that wraps and then, after
    a width change, fits on one line must **lose** its `InlineClientRects` component, so
    `getClientRects` falls back to the `LayoutBox` border box rather than answering from the
    previous layout's fragments. Pins the `else` of the `len() > 1` branch (`boxes.rs:102`).
...
force `any_font == false`**, without which cell 12d is unconstructible; cells 1–12, 3b and 12d land as
...
**PR-1b** (geometry): cells 12b, 12c, 12e, 13, 14, 14b, 15, 15b, 16, 16b, 17, 17b, 17c, 17d, 17e
and 25 — ⚠ **all of them assert a first-layout property**, because `assign_inline_layout_boxes`
skips any entity that already carries a `LayoutBox` (`boxes.rs:60-62`) and nothing anywhere removes
one (`grep -rn "remove_one::<LayoutBox>\|remove::<LayoutBox>" crates/` → no hits), so no geometry
...
css-text-3 §5.5 + §7.3 (§3.1); the `InlineClientRects` derivation and its removal arm carry their
cssom-view-1 §6 and css-break-3 §5.4 citations; `note_line_occupancy`, `pack/inline_box.rs` and
M8's contribution carry docstring citations to their §3 rows.

**PR-1c** (existence): cells 18–24, plus the flip set — **cells 1–4, 3b, 7, 8, 10, 11, 12 and 12d
flip; 5, 6 and 9 do not**; this is the one normative statement of it — every §7
...
**This memo and the two checkers** (`docs/plans/2026-08-line-box-decorated-inline-content.md`,
`.claude/tools/plan-sweep.py`, `.claude/tools/plan-xcheck.py` — the branch's entire tracked delta)
ship with the **seam-3 prereq PR**, the first of the five to open, per
...
**Ordering: the two prereq PRs are independent, and neither is ordered against the other.** The
dead arm is in `pack/mod.rs`; its `inline/mod.rs` half (`:239`, `:240-251`, `:322` and their
comments) lies entirely **above** seam 3's `:413-639` and touches nothing inside it
(`awk 'NR>=413 && NR<=639' inline/mod.rs | grep -c "persist_candidate\|flow_align"` → 0). So
landing the dead-arm PR first shifts seam 3's *coordinates* but not its *content* — and a shifted
coordinate is exactly what the front matter already requires each PR to re-measure ("Each PR's own
plan-memo re-anchors against its actual base"), so it cannot ground an ordering. Both branch from
`main` as siblings; whichever lands second rebases and re-measures, and the seam-3 PR's
byte-identical proof obligation is stated against its own base either way. PR-1a branches after
both.
...
* **File growth**: measure with `wc -l` at each PR rather than against a number written here, which
  ...
  `pack/mod.rs` are re-checked against the ~1000-line convention **at every PR**, on their
...
* **The two plan checkers are not on any loop.** `plan-sweep.py` / `plan-xcheck.py` live in
  ...
  recorded instead as a **tooling task in `project_line-box-decorated-inline-content.md`**, the file
  the next round's session opens first, which is the loop that actually carries it. Scope: put both
...
| Register **and** close `#11-inline-fragmented-fn-decomposition` in one row — it is registered as closed-on-landing, not registered then closed — **as a partial close**, naming the seams §9 measures as still open, in the successor slot `#11-inline-fragmented-fn-seams-1-2` (**pre-existing** class — the seams predate this umbrella; Why: the prereq PR discharges seam 3 only; trigger: **either** the first change after **any** of this umbrella's five PRs that touches the residue — every touch site §5.2's two `inline/mod.rs` rows list is this program's own and does not fire it, the prereq PRs included — **or** `inline/mod.rs` growing back toward 1000 lines, the source slot's second disjunct, which has not fired and which this program's own growth could trip; re-eval 2026-11-01). | seam-3 prereq PR |
...
| Enrich `#11-inline-relayout-box-staleness`'s SoT entry with the write-path statement §9 gives, **and split the joint "fold into terminal-Z C-3/C-4" parenthetical it shares with `#11-inline-align-clientrects-nonpersist-path`** — the dead-arm prereq closes that one, so the pairing no longer holds and the shared line is otherwise unowned | PR-1b |
...
| Record the successor program the close hands off to — `#11-inline-box-decoration-splits`, `#11-inline-min-content-box-edges` and `#11-inline-fragmented-fn-seams-1-2` all arm at PR-1c landing, so the Layout lane's next task is a choice among them, not an empty slot | PR-1c |
