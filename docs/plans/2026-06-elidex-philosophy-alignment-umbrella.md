...
> **A0 governs Program A specifics.** A0 (`docs/plans/2026-06-web-api-compat-split-design.md`)
> is the plan-reviewed detailed design for Program A; where its §5 PR list / ACs
> are more precise than the rows below, **A0 is authoritative** (the rows here are
> the high-level program view). This avoids the two tables drifting — refine scope
> in A0, not by editing both. (Codex R7.)
...
| **A2** | **Gate the whole Web Storage surface** — `Storage` (`localStorage`/`sessionStorage`) **+ `StorageEvent` + `window.onstorage` + worker-realm exposure + shell-delivered storage events** (A0 §5 is authoritative) — gate-only; async core `elidex.storage` OUT of scope (`#11-async-core-storage-cookiestore`) | `vm/host/storage.rs`, `window.rs`, `vm/globals.rs`, event-handler seam; opt. `elidex-api-storage-compat` (per A0 §4.1) | `document.rs` cookie code; async-core build | A1 | **PR-R** | **See A0 §5 A2 AC** — all storage paths (Storage + StorageEvent + onstorage + worker realm + event delivery) absent together in `BrowserCore`/`App`; `[Exposed=Window]` honored; opaque-origin slot re-evaluated; `BrowserCompat` byte-identical; tests green |
| **A3** | **Gate only `document.cookie`** behind the cookies-compat boundary; **derive `navigator.cookieEnabled`** from UA cookie *handling* (HTML §8.10.1.5 — **not** `document.cookie` exposure), property never gated; CookieStore core path OUT of scope | `vm/host/document.rs`, `vm/host/navigator.rs`; opt. `elidex-api-cookies-compat` (per A0 §4.1) | storage code; async-core build | A1 (parallel to A2) | **PR-R** | **See A0 §5 A3 AC** — `document.cookie` reachable only under `BrowserCompat`; `cookieEnabled` returns `true` when the UA handles cookies (CookieJar bound), independent of `document.cookie` exposure; cookie opaque-origin slot `#11-cookie-opaque-origin-securityerror`; cookie-file stale comments removed (full §1.5 sweep = F2 micro-PR); tests green |
