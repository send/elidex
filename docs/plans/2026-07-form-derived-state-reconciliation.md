...
- **IDL hand-mirrors** ("the second maintainer" the reconciler doc forbids): delete the **two INPUT** mirrors — `html_input_value.rs:206-211` (`default_value`) and `:278-280` (`default_checked`) — the chokepoint covers them once the arms exist. The **textarea** `default_value` mirror (`html_textarea_proto.rs:654-661`) **STAYS**: `textarea.defaultValue` is child-text-derived (§4.10.11 raw value), not attribute-derived, so the AttributeChange-only reconciler cannot reach it (tracked by slot `#11-textarea-defaultvalue-textcontent-reconciliation`).
...
| **0b — L1 reconciliation completeness** (prereq) | missing `checked`/`rows`/`cols` arms; delete the **two input** IDL hand-mirrors (the textarea mirror stays — textContent-derived, unreachable by the AttributeChange reconciler → slot `#11-textarea-defaultvalue-textcontent-reconciliation`). (**No `FCS.disabled` re-layer here** — see Slice 5.) | reset-restores-stale-checkedness; textarea.rows layout bug |
