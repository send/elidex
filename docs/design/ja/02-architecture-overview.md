...
| Rendererプロセス | HTML/CSS解析、DOM管理（ECS）、スタイル解決（並列）、レイアウト計算（並列）、ディスプレイリスト生成、JavaScript実行 | elidex-core + plugins、SpiderMonkey（Phase 1–3）→ elidex-js、wasmtime（Wasmランタイム）、rayon（並列処理） |
...
│   ├── elidex-js-spidermonkey/ # SpiderMonkeyブリッジ（Phase 1-3フォールバック）
