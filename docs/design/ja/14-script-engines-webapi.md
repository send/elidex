...
JSエンジンの構築はelidexで最大の単一コンポーネントである。Boaが作業中のブラウザを提供し続ける中、実装はステージごとに進行する：

| ステージ | 成果物 | elidex-browser使用 | elidex-app使用 |
| --- | --- | --- | --- |
| 1 | Parser + AST（ES2020+構文） | Boa | BoaまたはWasm |
| 2 | バイトコードコンパイラ + インタプリタ | Boa | elidex-js（自前） |
| 3 | インラインキャッシュ + hidden classes | 切替可能：Boaまたはelidex-js | elidex-js |
...
ScriptEngineトレイト抽象化により、Boaとelidex-jsはいつでも交換可能。エンジンは直接ECSアクセスではなくScriptSessionを受け取る：
...
enum ScriptBackend {
    Boa(BoaEngine),      // Phase 1-3: 成熟、フル互換
...
| --- | --- | --- |
| elidex-browser | Boa → elidex-js（フェーズ移行）。\<script\>タグを処理。 | wasmtime。JSからのWebAssembly.instantiate()とネイティブ.wasmモジュールを処理。 |
...
| P1 | Web Crypto API | elidex-api-crypto | セキュリティ基盤。RustCrypto でバッキング — sha1/sha2/hmac は実装済 (PR-1 HMAC vertical)、aes-gcm/p256/rsa は後続 vertical で追加。 |
