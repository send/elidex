...
| 成果物 | ステータス | 依存関係 |
| --- | --- | --- |
| クレート構成を持つCargoワークスペース | 未着手 | — |
| CIパイプライン（GitHub Actions） | 未着手 | — |
| elidex-crawlerサーベイ実施（875サイト） | クローラー準備完了 | — |
| サーベイ結果分析、compatルール優先度付け | 未着手 | クローラー結果 |
| プラグイントレイト定義（CssPropertyHandler, HtmlElementHandler, LayoutModel） | 未着手 | — |
| ECS DOMストレージプロトタイプ | 未着手 | — |
| アーキテクチャ決定記録（ADR）：セキュリティモデル、テキストパイプライン、writing-mode座標系 | 未着手 | — |
...
| 成果物 | ステータス | 依存関係 |
| --- | --- | --- |
| サイトリストを3,000〜5,000サイトに拡大 | 未着手 | Phase 0分析 |
| サイトごとにトップページ + 5〜10サブページをクロール | 未着手 | 拡張サイトリスト |
| 全ページをhtml5everで解析、エラーイベント記録・回復可能性分類 | 未着手 | クロールデータ |
| LLMトレーニング/評価用の壊れたHTMLコーパス構築 | 未着手 | エラー分類 |
| 判定ゲート：回復不能エラー率に基づくLLMフォールバックのGo/No-Go | 未着手 | 分析完了 |
