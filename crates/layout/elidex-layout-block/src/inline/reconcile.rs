...
///
/// The disposition is a **dichotomy carried by one flag**: a run either persists
/// (`persist_flow`, the `if` arms) or is the multicol mid-break carrier case (the
/// `else` arms — the caller's `do_carrier` is definitionally `!persist_flow`, see
/// its derivation in [`super::layout_inline_context_fragmented`]). One bit means
/// no both-true or neither state can exist, so the exclusivity the arms rely on
/// holds by construction rather than by a caller convention.
...
pub(super) fn reconcile_flows(
    ...
    for (group_key, group_lines) in flow_lines {
        ...
        if persist_flow {
            ...
            // Carrier (mid-break): this column's slice for this run-start group. multicol
    ...
    let carrier_has_payload = !carrier_groups.is_empty() || !carrier_atomics.is_empty();
    if !persist_flow && carrier_has_payload {
