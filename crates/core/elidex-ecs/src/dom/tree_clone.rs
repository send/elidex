...
//! | derived copy (src-parity) | `InlineStyle` | derived from `attrs("style")` via the canonical `elidex_css::parse_inline_style` at creation; all live mutation paths sync component→attribute (`sync_to_attribute`), so copying preserves parity without a re-parse — and re-deriving from the attribute would needlessly re-run the CSS parser per cloned element. Copied *together with* `Attributes`; CSSOM read handlers treat an absent `InlineStyle` as an empty declaration block, so the copy is load-bearing for `.style.*` reads on clones |
