...
//! | derived copy (src-parity) | `InlineStyle` | derived from `attrs("style")` at creation but NOT re-derivable at clone time: the boa `Mutation::SetInlineStyle` path mutates the component without syncing `attrs("style")` back, so the component can hold declarations the attribute doesn't — re-deriving would drop them (the dom-api CSSOM handlers do sync via `sync_to_attribute`; the asymmetry collapses once `#11-derived-component-attr-maintenance` lands and the boa path is deleted). Copied *together with* `Attributes`; CSSOM read handlers treat an absent `InlineStyle` as an empty declaration block, so the copy is load-bearing for `.style.*` reads on clones |
