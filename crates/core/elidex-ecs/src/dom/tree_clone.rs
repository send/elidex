...
//! | derived re-derive | `IframeData` | a pure function of `Attributes` (`IframeData::from_attributes`), so the clone re-derives it from the *cloned* attributes (presence preserved from src) — consistent by construction even when the source component is stale against its attributes (generic `setAttribute("src", …)` has no re-derivation pass until `#11-derived-component-attr-maintenance`; a verbatim copy would make the inserted clone load the stale URL) |
//! | derived copy (src-parity) | `InlineStyle` | lazily materialized from `attrs("style")` via the canonical `elidex_css::parse_inline_style` on first CSSOM access (not at parse); every CSSOM mutator syncs component→attribute (`sync_to_attribute`), so in that direction copying preserves parity without re-running the CSS parser per cloned element. The attribute→component direction reconciles via the `EcsDom::set_attribute` / `remove_attribute` chokepoint, which drops the cached `InlineStyle` on a generic `setAttribute("style", …)` / `removeAttribute("style")` so the next CSSOM read rehydrates from `attrs("style")` (this closed the `InlineStyle` half of slot `#11-derived-component-attr-maintenance`; the `IframeData` row above still covers the open half). Copied *together with* `Attributes`; CSSOM read handlers treat an absent `InlineStyle` as an empty declaration block, so the copy is load-bearing for `.style.*` reads on clones |
