//! Process sandboxing — security constraints applied to content *processes*
//! ([`SandboxPolicy`] / [`PlatformSandbox`]). Enforcement is implemented in
//! the `elidex-sandbox` crate using platform-specific mechanisms
//! (seccomp-bpf, sandbox-exec, restricted tokens).
//!
//! Distinct from the content sandbox ([`crate::sandbox`]): this is an OS
//! isolation boundary, not a per-browsing-context capability set. The two
//! share the word "sandbox" and nothing else.
