//! Content thread `BrowserToContent` message dispatch.
//!
//! Carved from `content/event_loop.rs` at the message-dispatch cohesion seam
//! (touch-time 1000-line split, Codex PR#469 R15): the big `handle_message`
//! `BrowserToContent` dispatcher + its `handle_message_public` re-dispatch wrapper,
//! the chrome Back/Forward traversal (`chrome_traverse` / `ChromeTraversal`), and the
//! two input-guard helpers that ONLY serve the dispatcher (`input_placement_stale` /
//! `apply_device_facts`). The sibling `content/event_loop.rs` keeps the
//! event-loop-turn machinery (`run_event_loop` / `pump_turn`). Behavior-neutral move
//! (no logic change).
...
use super::{dispatch_storage_event, event_handlers, ime, navigation, scroll, ContentState};
...
#[allow(clippy::too_many_lines)]
pub(super) fn handle_message(msg: BrowserToContent, state: &mut ContentState) -> bool {
    match msg {
