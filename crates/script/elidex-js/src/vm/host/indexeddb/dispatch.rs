//! IndexedDB EventTarget model + WHATWG DOM §2.9/§2.10 event dispatch
//! (slot `#11-indexed-db-vm` / D-20a).  Split out of `mod.rs` to keep that
//! module under the repo's ~1000-line convention.
//!
//! The `IDBRequest` / `IDBTransaction` / `IDBDatabase` non-Node EventTargets
//! keep listeners in an in-VM side store (the AbortSignal precedent), so this
//! is a parallel of the canonical ECS-backed dispatch — the full unification
//! across all non-node EventTargets is deferred to
//! `#11-eventtarget-dispatch-core` (documented at `native_idb_add_event_listener`).
...
use super::super::events::{
    set_event_slot_raw, EventInit, EVENT_SLOT_CURRENT_TARGET, EVENT_SLOT_TARGET,
};
use super::IdbListener;
...
pub(crate) fn native_idb_add_event_listener(
    ...
    let options = super::super::event_target::parse_listener_options(
        ctx,
...
pub(crate) fn native_idb_remove_event_listener(
    ...
    let event_type = ctx.to_string_val(args.first().copied().unwrap_or(JsValue::Undefined))?;
    // WebIDL `EventListener? callback` (matches the shared
    // `EventTarget.removeEventListener`, host/event_target.rs): null/undefined
    // are a silent no-op; a non-callable, non-null value is a conversion
    // failure that throws `TypeError` (silently dropping
    // `req.removeEventListener('x', 42)` would mask user bugs).
    let callback = match args.get(1).copied().unwrap_or(JsValue::Undefined) {
...
pub(crate) fn native_idb_dispatch_event(
    ...
    if ctx.vm.dispatched_events.contains(&event_id) {
        ...
        return Err(super::super::dom_exception::invalid_state_error(
            name_sid,
            "EventTarget",
            "dispatchEvent",
