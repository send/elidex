...
/// dispatch as a user-visible abort.
fn abort_signal(
    ctx: &mut NativeContext<'_>,
    signal_id: ObjectId,
    reason: JsValue,
...
// ---------------------------------------------------------------------------
// AbortSignal static factories (WHATWG §3.1.3)
// ---------------------------------------------------------------------------
...
fn native_abort_signal_static_abort(
    ctx: &mut NativeContext<'_>,
    _this: JsValue,
    args: &[JsValue],
) -> Result<JsValue, VmError> {
...
/// [`VmInner::pending_timeout_signals`]; on fire, the VM
/// synthesises an internal abort *without* invoking any JS
/// callback.
fn native_abort_signal_static_timeout(
    ctx: &mut NativeContext<'_>,
    _this: JsValue,
    args: &[JsValue],
) -> Result<JsValue, VmError> {
    ...
    // `pending_timeout_signals` has the entry); passing an already-
    // interned `Function.prototype`-rooted object avoids a fresh
    // native-function alloc, but the drain guard never reads it.
    // Use the signal itself as a stand-in — it's never callable, but
    // the drain path bails before invoking anything.
...
fn native_abort_signal_static_any(
    ctx: &mut NativeContext<'_>,
    _this: JsValue,
    args: &[JsValue],
) -> Result<JsValue, VmError> {
    ...
    // No input was already aborted — install a listener on each
    // that propagates the abort to `composite`.  We capture
    // `composite` via the AbortSignalState of each input: adding
    // the composite's ObjectId to a new `any_propagate_to` field
    // would be cleaner, but for PR5a we route through
    // `abort_listeners` with a synthetic ObjectId-addressable
    // native bridge — defer the refactor to PR5a2 alongside the
    // Event-ctor cleanup.
    //
    // Phase 2 simplification: push the composite's signal id into
    // each input's `abort_listeners` vec, wrapped through a
    // newly-minted native fn that, when called with the input
    // signal as `this`, triggers `abort_signal(ctx, composite,
    // signal.reason)`.  The native fn is a closure-free function
    // pointer + we carry the composite id via a per-VM scratch
    // table.  This is the narrow path the plan accepts for PR5a;
    // swapping to a proper dependency structure is PR5a2 work.
    //
    // For the simpler scope we skip the listener install entirely
    // when the input list is empty (no propagation source →
    // composite is a plain non-aborted signal) and leave a TODO
    // for the multi-input case.  Tests only exercise the empty +
    // already-aborted paths on PR5a.
