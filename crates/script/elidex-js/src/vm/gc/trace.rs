...
pub(super) fn trace_work_list(
    ...
    while let Some(obj_idx) = work.pop() {
        ...
        match &obj.kind {
            ...
            // `seq` integer + the `document` `Entity` — ZERO `ObjectId`/`JsValue` to
            ...
            // `resize`/`scroll` listeners live in `vm_event_listeners`, traced
            // via that map's own root). S5-2.
            #[cfg(feature = "engine")]
            ObjectKind::VisualViewport => {}
