...
pub(super) fn trace_work_list(
    ...
    while let Some(obj_idx) = work.pop() {
        ...
        match &obj.kind {
            ...
            #[cfg(feature = "engine")]
            ObjectKind::Client => {}
            // `ServiceWorkerRegistration` / `ServiceWorker` (SW §3.1/§3.2,
            // D-19 PR-3) — payload-free brands; the scope-recovery side-stores
            // (`sw_registration_states` / `service_worker_states`) hold only a
            // `StringId`, and the registry / worker data lives in
            // `sw_registrations`.  No inline `ObjectId`, so no fan-out.  Live
            // interned wrappers are marked by the `sw_registrations`
            // registry-walk loop in `gc/collect.rs`; the brand side-stores are
            // value-mark swept.
            #[cfg(feature = "engine")]
            ObjectKind::ServiceWorkerRegistration | ObjectKind::ServiceWorker => {}
