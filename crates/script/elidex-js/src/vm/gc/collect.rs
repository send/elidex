    ...
    pub(crate) fn collect_garbage(&mut self) {
        ...
        #[cfg(feature = "engine")]
        {
            ...
            if let Some(hd) = self.host_data.as_deref_mut() {
                ...
                // EventSource}`).  A listener-held non-CLOSED WS (pure
                // tier), or a listener-held OR buffer-window
                // (`has_queued_task`) non-CLOSED ES, is marked by
                ...
                // genuine orphan (no in-tier listener, and for ES no
                // queued task) or a CLOSED wrapper reaches here unmarked
                // — exactly the spec's "GC while open ⇒ closing handshake
                // / abort fetch" case.  No edit was needed in this block:
                // it keys purely on the mark bit.
                //
