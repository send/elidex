...
fn gc_heap_bounded_in_loop() {
    ...
    // 1000 loop iterations would push this well over 3000, so the
    // `< 2200` assertion remains a meaningful "GC actually ran"
    // signal while leaving headroom for future built-ins.  Bumped
    // from 1500 with D-9 `#11-events-modern-input`
    ...
    // prototype-slot link + the `$$elidexCEConstructorId` brand property
    // alongside the ~280-byte `Generator(Box<GeneratorState>)` enum
    // variant boxing churn that lands the `clippy::large_enum_variant`
    // gate. Empirical baseline ~3216 live; 3500 keeps loop headroom.
    assert!(
        live < 3500,
