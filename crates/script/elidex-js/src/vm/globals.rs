...
use super::natives::{
    native_array_is_array, native_console_error, native_console_log, native_console_warn,
    native_error_constructor, native_is_finite, native_is_nan, native_json_parse_stub,
    native_json_stringify_stub, native_math_abs, native_math_ceil, native_math_floor,
...
impl Vm {
    ...
    fn create_object_with_methods(
        ...
        for &(name, func) in methods {
            let fn_id = self.create_native_function(name, func);
            let key = self.inner.strings.intern(name);
    ...
    fn register_console(&mut self) {
        ...
        let log_key = self.inner.well_known.log;
        ...
        let error_key = self.inner.well_known.error;
        self.get_object_mut(console_id)
            .properties
        ...
        let warn_key = self.inner.well_known.warn;
        self.get_object_mut(console_id)
            .properties
