...
// --- S5-3a: the keepalive-predicate seam ------------------------------------
//
// `#11-eventtarget-listener-keepalive-rooting`. An MQL anchored ONLY by a
// `change` listener has its callback rooted (`listener_store`) but not its own
// wrapper; the seam (`gc::keepalive::keepalive_survivors`, run in
// `collect_garbage`) keeps it alive iff it has a LIVE `change` listener — the
// dispatch-time `vm_path_has_listener` predicate, so kept-alive ⇔ would-fire —
// document-scoped so a prior-document MQL is not rooted across a rebind.
...
fn same_document_listener_only_mql_survives_unbound_inter_batch_gc() {
    // A GC fired while UNBOUND (between two batches of the SAME document — the
    // BATCH-BIND model unbinds at every batch boundary) must NOT collect a
    // listener-only MQL: the document is unknown while unbound, so the gate
    // keeps the MQL, and it still delivers after rebinding the same document.
    // Collecting it here would silently lose the later `change` — the very bug
    // the seam fixes. (The cross-DOM guard runs on the next *bound* GC, when a
    // foreign document is known — `prior_document_listener_only_mql_*`.)
    let mut vm = Vm::new();
    vm.install_host_data(HostData::new());
    let mut session = SessionCore::new();
    let mut dom = EcsDom::new();
