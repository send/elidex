...
fn setup_vm_for_binding() -> (Vm, SessionCore, EcsDom, elidex_ecs::Entity) {
    let vm = Vm::new();
...
fn exported_function_can_be_called() {
    let (mut vm, mut session, mut dom, doc) = setup_vm_for_binding();
...
fn ctors_require_new_operator() {
    ...
    let (mut vm2, mut session, mut dom, doc) = setup_vm_for_binding();
