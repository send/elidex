//! Core VM interpreter / evaluator tests (extracted from the
//! inline body of [`super`] to keep `tests/mod.rs` under the
//! 1000-line convention).
//!
//! Covers literal eval, arithmetic, control flow, declarations,
//! built-in globals (`Math`, `Object`, `Array`, `JSON`),
//! `StringPool` / object-heap / globals plumbing, ES2020 spec
//! corners, TDZ, `instanceof` / `in`, `delete`, prefix/postfix
//! property mutation, spread, prototype-chain `for-in`, function
//! hoisting, and the `globalThis` ↔ globals-HashMap fallback.

use super::super::pools::StringPool;
use super::super::shape::ROOT_SHAPE;
