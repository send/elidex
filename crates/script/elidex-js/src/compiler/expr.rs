...
// ---------------------------------------------------------------------------
// yield* bytecode expansion (ES2020 §14.4.14)
// ---------------------------------------------------------------------------
...
fn compile_yield_star(
    ...
    assert!(
        u16::try_from(throw_handler).is_ok(),
        "yield* throw_handler {throw_handler} exceeds u16 range"
    );
    assert!(
        u16::try_from(finally_pc).is_ok(),
        "yield* finally_pc {finally_pc} exceeds u16 range"
    );
    let catch_bytes = (throw_handler as u16).to_le_bytes();
    fc.bytecode[handler_patch_pos as usize] = catch_bytes[0];
    fc.bytecode[(handler_patch_pos + 1) as usize] = catch_bytes[1];
    let finally_bytes = (finally_pc as u16).to_le_bytes();
