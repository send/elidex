...
pub struct DispatchEvent {
    ...
    ///
    /// WHATWG DOM §2.2: `composedPath()` returns an empty sequence when the
    /// event path is empty. elidex does NOT clear `composed_path` after dispatch
    /// (DOM §2.9 clears the path before unsetting the dispatch flag); instead
    /// `composed_path_for_js()` guards on this `dispatch_flag`, returning empty
    /// when it is unset, so the retained entries are never observable via the API.
    pub dispatch_flag: bool,
    /// Whether this event was generated by the user agent (true) or script (false).
    ///
    /// WHATWG DOM §2.2: `[LegacyUnforgeable]` attribute. UA-dispatched events
...
/// Return the `composedPath()` for JS, applying WHATWG DOM §2.2 filtering.
///
