...
///
/// Missing default = empty (returned as `None` for nullable getters),
/// invalid default = `"anonymous"` per HTML.  Note the IDL is
/// nullable, so callers compare result to `""` for "no value".
