...
pub(crate) fn convert_number_to_string(kind: FormControlKind, n: f64) -> Option<String> {
    // A non-finite number, or one outside the `i64` range (reachable from a
    // huge but finite `step`, e.g. `type=date step=1e20`), has no
    // representable date/time: casting it to `i64` would saturate to
    // `i64::MAX` and yield an unrelated value.  Reject it (→ no-op) before
    // the cast, mirroring the checked arithmetic on the parse side.
    // `i64::{MIN,MAX} as f64` round to ±2⁶³, so a strict `< 2⁶³` bound
    // excludes exactly the values whose cast would saturate.
