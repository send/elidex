...
// -- date/time stepUp/stepDown (HTML §4.10.5.1.7–.11) -----------------
//
// Serialization is string-exact (integer ms / month-count → civil →
// string), so these assert the literal output with no float-noise
// tolerance — the round-trip that the numeric states cannot give.

#[test]
fn apply_step_date_up_one_day() {
    let mut s = make_state(FormControlKind::Date, "2025-01-15", None);
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2025-01-16");
}

#[test]
fn apply_step_date_down_one_day() {
    let mut s = make_state(FormControlKind::Date, "2025-01-15", None);
    assert!(apply_step(&mut s, 1.0, -1.0).is_ok());
    assert_eq!(s.value(), "2025-01-14");
}

#[test]
fn apply_step_date_crosses_month_end() {
    let mut s = make_state(FormControlKind::Date, "2025-01-31", None);
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2025-02-01");
}

#[test]
fn apply_step_date_step_is_in_days() {
    // step="7" is seven days (scale 86,400,000), not seven ms.  `min`
    // anchors the step grid at the value so the aligned step lands
    // cleanly seven days later (were the scale wrong, +7 ms would
    // serialize back to the same date).
    let mut s = make_state_mm(
    ...
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2025-01-22");
}

#[test]
fn apply_step_date_clamps_up_to_min() {
    let mut s = make_state_mm(
        FormControlKind::Date,
        "2025-01-15",
        None,
    ...
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2025-01-20");
}

#[test]
fn apply_step_date_any_step_is_no_allowed_value_step() {
    let mut s = make_state(FormControlKind::Date, "2025-01-15", Some("any"));
    assert_eq!(
        apply_step(&mut s, 1.0, 1.0),
        Err(StepError::NoAllowedValueStep)
    );
...
#[test]
fn apply_step_month_year_boundary() {
    // Month is a month-count space (scale 1), no day component.
    let mut s = make_state(FormControlKind::Month, "2025-12", None);
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2026-01");
}

#[test]
fn apply_step_month_end_has_no_day_component() {
    // Stepping a month never produces a day overflow (unlike Date):
    // 2025-01 + 1 month is just 2025-02, no "2025-02-31" pathology.
    let mut s = make_state(FormControlKind::Month, "2025-01", None);
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2025-02");
}

#[test]
fn apply_step_week_up_one_week() {
    let mut s = make_state(FormControlKind::Week, "2025-W01", None);
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2025-W02");
}

#[test]
fn apply_step_week_crosses_week_year_boundary() {
    // 2020 has 53 weeks; W53 + 1 week is 2021-W01 (week-year ≠ cal-year).
    let mut s = make_state(FormControlKind::Week, "2020-W53", None);
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2021-W01");
}

#[test]
fn apply_step_week_empty_uses_default_step_base() {
    // Empty value → 0 ms, which is off the week grid (anchored at the
    // −259,200,000 ms default step base); stepUp snaps to the next
    // grid Monday = 1970-W02.
    let mut s = make_state(FormControlKind::Week, "", None);
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "1970-W02");
}

#[test]
fn apply_step_time_default_step_is_one_minute() {
    let mut s = make_state(FormControlKind::Time, "12:30", None);
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "12:31");
}

#[test]
fn apply_step_time_step_in_seconds() {
    // step="900" is 900 seconds (15 minutes).
    let mut s = make_state(FormControlKind::Time, "12:30", Some("900"));
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "12:45");
}
...
fn apply_step_datetime_local_default_step_is_one_minute() {
    ...
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2025-01-15T12:31");
}

#[test]
fn apply_step_date_huge_step_is_noop() {
    // Codex R5: a huge but finite step drives the stepped value outside the
    // i64 millisecond range; the serializer rejects it (no representable
    // date), so the step is a no-op rather than fabricating a saturated
    // date — and never panics.
    let mut s = make_state(FormControlKind::Date, "2025-01-15", Some("1e20"));
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2025-01-15");
}

#[test]
fn apply_step_date_below_year_one_is_noop() {
    // Stepping below 0001-01-01 has no valid date string (year > 0
    // required), so the step leaves the value unchanged rather than
    // writing a non-conforming "0000-12-31".
    let mut s = make_state(FormControlKind::Date, "0001-01-01", None);
    assert!(apply_step(&mut s, 1.0, -1.0).is_ok());
    assert_eq!(s.value(), "0001-01-01");
}

#[test]
fn apply_step_datetime_local_crosses_midnight() {
    let mut s = make_state(FormControlKind::DatetimeLocal, "2025-01-15T23:59", None);
    assert!(apply_step(&mut s, 1.0, 1.0).is_ok());
    assert_eq!(s.value(), "2025-01-16T00:00");
}
