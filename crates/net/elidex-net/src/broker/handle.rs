...
/// [`NetworkHandle::create_sibling_handle`] are called
/// directly from browser-thread paths (e.g. `App::open_new_tab`,
/// `sw_coordinator::register`) and the `new Worker()`
/// constructor in the JS host — none of those tolerate a
/// multi-second freeze of the event loop on a stalled broker
/// (Copilot R1).  An R1-era 5 s ceiling would have been a UX
/// regression on the very paths that drove the original race
/// fix.
///
...
const REGISTER_ACK_TIMEOUT: Duration = Duration::from_millis(500);
