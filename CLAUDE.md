...
- **ECS-native first**: elidex は **既存にない novel な ECS-native browser engine**。OO browser engine (Blink / Gecko / WebKit) の inheritance + virtual dispatch + observer registry pattern を ideal として直訳しない。ECS first principles (component / system / query / marker / event queue / version counter) から設計を導く。OO pattern を翻訳する時は idiom 対応表を明示 (例: observer registry → marker component + query, subscriber list → component query, class-owned state → ECS component on entity)。**ECS-native であること自体が elidex の design contribution**。
  - **Side-store→component 判定ルール**: per-entity 状態を entity-keyed の side-store/registry (HostData の `*_cache` / `*Registry` / `HashMap<entity, _>`) に持たせる時、その値が `Send + Sync` **かつ per-VM identity handle でない**なら **ECS component on the entity が正**（SameObject = component get、GC = 単一 query、despawn = 自動 cleanup）。boa `HostBridge` side-store を VM へ移植する時の直訳が典型的な罠（D-21 R3 で CRIT 化）。`Send+Sync` は hecs 適格条件であって「すべき」とは別 — side-store が正当な例外は2つ:
    - **(a) per-VM identity handle (一時的例外)**: VM `ObjectId` (JS wrapper / callback) を保持するもの。値は Send (`ObjectId(u32)`) だが意味が per-VM — `EcsDom` は VM 間で entity-index 空間を共有し rebind され得るので、component 化すると **cross-DOM aliasing** (前 DOM の wrapper を拾う、`vm_api.rs` `Vm::unbind` の cache-clear 参照: コメント `cross-DOM references and must be cleared on unbind`)。`world_id` discriminator (`#11-wrapper-cache-cross-dom-discriminator`) が解禁条件で、それまで per-VM HostData (unbind-clear) が正。world_id 後は component 化可 (`#11-wrapper-identity-component-migration`)。**⚠ SUPERSEDED (2026-06-30, PR #434 `docs/plans/2026-06-agent-scoped-ecsdom-world.md`)**: world_id discriminator は撤回。解禁条件は **agent-scoped `EcsDom` World** (1 agent = 1 World = 1 Vm) — その下では cross-DOM aliasing が構造上不能 (1 World → unique entity → hecs `generation` が staleness 処理) ゆえ migration は world_id 無しで安全。**ただし wrapper component は既存 `WrapperKey {owner, kind, subkey}` + `realm` の full keying 必須** (PR #434 §5 req 6/7): B1 では 1 Vm が複数 Window realm を持ち prototype chain が別、かつ 1 entity が Node/classList/dataset/style/Attr 等の別 wrapper kind を持つので、`Entity→ObjectId` や `(entity, realm)` だけだと衝突し SameObject を壊す。この passage の全面 rewrite は B1 実装時 (world_id 実コード削除と同時)。それまで「per-VM HostData (unbind-clear) が interim 正」は不変。
...
- **1000-line debt = touch-time split (defer しない)**: >1000行 file を触る際、real cohesion seam があれば feature 着手前に **standalone な prereq split** として分割する (feature PR に bundle しない — split は単独 PR / 単独 commit)。line-count の機械適用でなく **cohesion 判断** (real seam があれば割る / 一枚岩の cohesive unit・巨大 generated table・flat な case table は対象外)。**test file も source と同基準で対象** (大 test file は scenario 単位で seam が明確なことが多く prime candidate)。1000行 debt を defer で溜める運用をやめ、**触った時点で bounded 化**する。本 discipline は **touch-time の proactive な振る舞い** (any-size touch / source・test 問わず)。`.claude/skills/elidex-review/axes.md` Axis 5 (既 1000+ file への >50 LoC add を MIN flag、`crates/**/*.rs` scope) は **trigger の異なる review-time backstop** であって本 discipline 全体の検出器ではない (discipline=touch-time / Axis 5=substantive-growth、相補)。
...
- **Plan-memo umbrella checker**: `.claude/tools/plan-memo-umbrella-check.py <memo>` は plan memo の §5/§8 row-kind census (umbrella / kind-undetermined) と、それらを owner / ordering / acceptance の役割で名指しする prose/cell site を、memo がリンクする memo 群 (transitive) まで含めて機械列挙する (exit 0 = assertion clean / 1 = mechanical finding / 2 = schema miss = 未走査、clean ではない)。memo 実行は手動 (SCHEMAS が特定 memo 族の header に束縛されるため trip-wire に入れない) だが、その self-test + mutation proof (`--self-test --mutants`、memo 非依存) は `.claude/tools/plan-memo-umbrella-selftest-trip-wire.sh` として `scripts/trip-wires.sh` の `REQUIRED_WIRES` に登録済 (`python3` 必須)。lexer / population / assertion を触ったら control + mutant を同 commit で足す。
...
4 つ目の job `trip-wires` (`bash scripts/trip-wires.sh` = `mise run trip-wires` と同一 script) は **filter で gate しない** — `main` 宛 PR / `main` への push で常時実行。gate すると `.claude/tools/**` (wire 本体 + `layout-box-reader-allowlist.tsv`) と `scripts/**` (entry point) を filter に列挙する必要があり、allowlist gate の改竄経路自体が「誰かが維持すべき allowlist 項目」になる (`layout-box-reader-allowlist.tsv` だけを触る PR が、それを読む job を skip できてしまう)。wire は Rust toolchain 不要 (layering wire ~1s、`scripts/trip-wires.sh` 全体 **~13.7 s** (13.68/14.36/14.42 の 3 回、`/usr/bin/time -p`、**当該 commit の `git clone --local` 上・本 project の他プロセス無し**。⚠ **同一 session・同一 clone で base も測り直すこと** — R27 で 11.8 s と記録された parent は同じ条件で 12.23/12.89/13.21 だったので、差の ~0.4 s は host であって branch ではない。片方だけ引用しない。R12 時点は ~6 s で、差分は R27 が足した生成 corpus の growth sweep (self-test 1 回 + それを名指す mutant 5 回) と R29 の網羅的 id-scan corpus + 2 sweep。⚠ **条件が数字の一部**: 同じ script を混雑した worktree で測ると 26-27 s = 2.2 倍になり、その値が 1 ラウンドだけ「余裕 2.6 倍」として commit されていた。条件を書かない timing は再現しない; `29dc1eb7` では local 49 s / Codex runner 211 s で、1 mutant の call counter が別 module を見ていた欠陥が原因。job `timeout-minutes` は 5); self-test wire だけ `python3` を要し、これは `ubuntu-latest` に標準搭載で、無ければ skip でなく FAIL する) なので filter は何も買わない。理由の正典 = 同 job のコメント。
