...
- **Plan-memo checker**: VM-P4 umbrella memo (`docs/plans/2026-07-vm-p4-es-language-completeness.md`) を編集したら `python3 .claude/tools/plan-memo-umbrella-check.py <memo> [<sibling>...]` を回す。§5 の row-kind census (declaring field 読み) と 「owner を持たない row (umbrella / kind-undetermined) を owner・ordering・acceptance の主体として名指す prose」を列挙する。**exit 0 = mechanical assertion clean / 1 = mechanical finding あり / 2 = schema 不一致 = SKIP** (seed と naming site は gate しない)。`--self-test` が firing proof。`SCHEMAS` はこの memo の header 行に一致するので他 memo では 2 を返す — だから `trip-wires` には入れない (ungated job なので無関係 PR が全部赤くなる)。⚠ **決定を 1 つ変えた時の restatement sweep はこれではなく `plan-sweep.py`** (`layout-decorated-inline`) が正準。
