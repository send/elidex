...
4 つ目の job `trip-wires` (`bash scripts/trip-wires.sh` = `mise run trip-wires` と同一 script) は **filter で gate しない** — `main` 宛 PR / `main` への push で常時実行。gate すると `.claude/tools/**` (wire 本体 + `layout-box-reader-allowlist.tsv`) と `scripts/**` (entry point) を filter に列挙する必要があり、allowlist gate の改竄経路自体が「誰かが維持すべき allowlist 項目」になる (`layout-box-reader-allowlist.tsv` だけを触る PR が、それを読む job を skip できてしまう)。wire は Rust toolchain 不要 (layering wire は grep のみ、self-test wire は `ubuntu-latest` 標準の `python3` だけを要し、無ければ skip でなく FAIL する) なので filter は何も買わない。**理由・実行時間・`timeout-minutes` の余裕の正典は同 job のコメント 1 箇所**で、ここには写さない (写した結果、2 つの live な数字が食い違う状態を 1 ラウンド作った — 計測値は動くので、動くものは SoT に置き、ここは方針とポインタだけ)。⚠ その数値を更新する時は測定条件ごと更新する: 条件を書かない timing は再現しない。
