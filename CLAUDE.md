...
4 つ目の job `trip-wires` (`bash scripts/trip-wires.sh` = `mise run trip-wires` と同一 script) は **filter で gate しない** — `main` 宛 PR / `main` への push で常時実行。gate すると `.claude/tools/**` (wire 本体 + `layout-box-reader-allowlist.tsv`) と `scripts/**` (entry point) を filter に列挙する必要があり、allowlist gate の改竄経路自体が「誰かが維持すべき allowlist 項目」になる (`layout-box-reader-allowlist.tsv` だけを触る PR が、それを読む job を skip できてしまう)。wire は Rust toolchain 不要 (layering wire ~1s、`scripts/trip-wires.sh` 全体 **~11.8 s** (11.83/11.83/11.85 の 3 回、`/usr/bin/time -p`、**当該 commit の `git clone --local` 上・本 project の他プロセス無し**。R12 時点は ~6 s で、差分は R27 が足した生成 corpus の growth sweep = self-test 1 回 + それを名指す mutant 5 回。⚠ **条件が数字の一部**: 同じ script を混雑した worktree で測ると 26-27 s = 2.2 倍になり、その値が 1 ラウンドだけ「余裕 2.6 倍」として commit されていた。条件を書かない timing は再現しない; `29dc1eb7` では local 49 s / Codex runner 211 s で、1 mutant の call counter が別 module を見ていた欠陥が原因。job `timeout-minutes` は 5); self-test wire だけ `python3` を要し、これは `ubuntu-latest` に標準搭載で、無ければ skip でなく FAIL する) なので filter は何も買わない。理由の正典 = 同 job のコメント。
